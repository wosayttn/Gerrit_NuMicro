...
typedef struct
{
     ...
     * |        |          |01 = Basic mode: The descriptor table only has one task. When this task is finished, PDMAx_INTSTS[1] will be asserted.
     ...
     * |        |          |11 = Reserved.
     * |        |          |Note 1: Before filling new transfer task in the Descriptor Table, user must check PDMAx_INTSTS[1] to make sure the current task is complete.
     * |        |          |Note 2: User must fill the PDMAx_DSCTn_NEXT register firstly if opmode is set to 0x10(scatter-gather mode)
     * |        |          |When opmode is in scatter-gather mode, PDMA controller will get information from address written in PDMAx_DSCTn_NEXT register in any time, so the PDMAx_DSCTn_NEXT register must be ready before setting the opmode.
     ...
     * |        |          |If the TBINTDIS bit is 1, it will not generates TDIFn (PDMAx_TDSTS[1:0]) when PDMA controller finishes transfer task.
...
typedef struct
{
     ...
     * |Bits    |Field     |Descriptions
     * | :----: | :----:   | :---- |
     * |[1:0]   |CHENn     |PDMA Channel Enable Bits
     ...
     * |Bits    |Field     |Descriptions
     * | :----: | :----:   | :---- |
     * |[1:0]   |PAUSEn    |PDMA Channel n Transfer Pause Control (Write Only)
     * |        |          |User can set PAUSEn bit field to pause PDMA transfer
     * |        |          |When user sets PAUSEn bit, PDMA will pause the on-going transfer, then clear channel enable bit CHENn (PDMAx_CHCTL [n], n=0,1) and clear request active flag (PDMAx_TRGSTS[n:0], n=0,1)
     ...
     * |Bits    |Field     |Descriptions
     * | :----: | :----:   | :---- |
     * |[1:0]   |SWREQn    |PDMA Software Request (Write Only)
     ...
     * |Bits    |Field     |Descriptions
     * | :----: | :----:   | :---- |
     * |[1:0]  |REQSTSn   |PDMA Channel Request Status (Read Only)
     ...
     * |Bits    |Field     |Descriptions
     * | :----: | :----:   | :---- |
     * |[1:0]   |FPRISETn  |PDMA Fixed Priority Setting
     ...
     * |Bits    |Field     |Descriptions
     * | :----: | :----:   | :---- |
     * |[1:0]   |FPRICLRn  |PDMA Fixed Priority Clear Bits (Write Only)
     ...
     * |Bits    |Field     |Descriptions
     * | :----: | :----:   | :---- |
     * |[1:0]   |INTENn    |PDMA Interrupt Enable Bits
     ...
     * |        |          |This flag indicates that PDMA controller has waited peripheral request for a period defined by TOC1(PDMAx_TOC[31:16]).
     * |        |          |0 = No request time-out.
     * |        |          |1 = Peripheral request time-out.
     * |        |          |Note 1: Please disable time-out function before clearing this bit.
     * |        |          |Note 2: User can write 1 to clear this bit.
     ...
     * |Bits    |Field     |Descriptions
     * | :----: | :----:   | :---- |
     * |[1:0]   |ABTIFn    |PDMA Read/Write Target Abort Interrupt Status Flag
     ...
     * |Bits    |Field     |Descriptions
     * | :----: | :----:   | :---- |
     * |[1:0]   |ALIGNn    |Transfer Alignment Flag
     ...
     * |Bits    |Field     |Descriptions
     * | :----: | :----:   | :---- |
     * |[1:0]   |TXACTFn   |Transfer on Active Flag (Read Only)
     ...
     * |        |          |The calculation unit is based on TOUTPSC1 (PDMAx_TOUTPSC[6:4]) clock.
     * |        |          |Time-out period = (Period of time-out clock) * (16-bit TOCn), n=1.
     * @var PDMA_T::CHRST
     * Offset: 0x460  PDMA Channel Reset Register
     * ---------------------------------------------------------------------------------------------------
     * |Bits    |Field     |Descriptions
     * | :----: | :----:   | :---- |
     * |[1:0]   |CHnRST    |PDMA Channel n Reset Control
     * |        |          |User can set CHnRST bit field to reset PDMA transfer
     * |        |          |When user sets CHnRST bit, PDMA will stop the on-going transfer, then clear channel enable bit CHENn (PDMAx_CHCTL[n], n=0,1) and clear request active flag (PDMAx_TRGSTS[n:0], n=0,1)
     ...
     * |        |          |Note 2: If channel trigger source comes from peripheral, set the REQSRCn to 0 before setting the channel reset.
     ...
     * |        |          |11 = Channel connects to TMR0.
     * |        |          |12 = Channel connects to TMR1.
     * |        |          |13 = Channel connects to TMR2.
     * |        |          |14 = Channel connects to TMR3.
     * |        |          |15 = Channel connects to TMR4.
     * |        |          |16 = Channel connects to TMR5.
     * |        |          |17 = Channel connects to TMR6.
     * |        |          |18 = Channel connects to TMR7.
     * |        |          |19 = Channel connects to TMR8.
     * |        |          |20 = Channel connects to I2C0_TX.
     * |        |          |21 = Channel connects to I2C0_RX.
     * |        |          |22 = Channel connects to I2C1_TX.
     * |        |          |23 = Channel connects to I2C1_RX..
     * |        |          |24 = Channel connects to I2C2_TX.
     * |        |          |25 = Channel connects to I2C2_RX.
     * |        |          |26 = Channel connects to USCI0_TX.
     * |        |          |27 = Channel connects to USCI0_RX.
     * |        |          |28 = Channel connects to USCI1_TX.
     * |        |          |29 = Channel connects to USCI1_RX.
     * |        |          |30 = Channel connects to USCI2_TX.
     * |        |          |31 = Channel connects to USCI2_RX.
     * |        |          |32 = Channel connects to USCI3_TX.
     * |        |          |33 = Channel connects to USCI3_RX.
     * |        |          |34 = Channel connects to USCI4_TX.
     * |        |          |35 = Channel connects to USCI4_RX.
     * |        |          |36 = Channel connects to EINT0.
     * |        |          |37 = Channel connects to EINT1.
     * |        |          |38 = Channel connects to EINT2.
     * |        |          |39 = Channel connects to EINT3.
     * |        |          |40 = Channel connects to EINT4.
     * |        |          |41 = Channel connects to EINT5.
     * |        |          |42 = Channel connects to ADC0_RX.
     * |        |          |Others = Reserved.
     ...
     */
    DSCT_T        DSCT[2];               /*!< [0x0000 ~ 0x001c] Control Register of PDMA Channel 0 ~ 1                  */
    __I  uint32_t RESERVE0[56];
    __I  uint32_t CURSCAT[2];            /*!< [0x0100 ~ 0x0104] Current Scatter-gather Descriptor Table Address of PDMA Channel n */
    __I  uint32_t RESERVE1[190];
...
#define PDMA_CHCTL_CHENn_Msk             (0x3ul << PDMA_CHCTL_CHENn_Pos)                   /*!< PDMA_T::CHCTL: CHENn Mask              */

#define PDMA_PAUSE_PAUSEn_Pos            (0)                                               /*!< PDMA_T::PAUSE: PAUSEn Position         */
#define PDMA_PAUSE_PAUSEn_Msk            (0x3ul << PDMA_PAUSE_PAUSEn_Pos)                  /*!< PDMA_T::PAUSE: PAUSEn Mask             */

#define PDMA_SWREQ_SWREQn_Pos            (0)                                               /*!< PDMA_T::SWREQ: SWREQn Position         */
#define PDMA_SWREQ_SWREQn_Msk            (0x3ul << PDMA_SWREQ_SWREQn_Pos)                  /*!< PDMA_T::SWREQ: SWREQn Mask             */

#define PDMA_TRGSTS_REQSTSn_Pos          (0)                                               /*!< PDMA_T::TRGSTS: REQSTSn Position       */
#define PDMA_TRGSTS_REQSTSn_Msk          (0x3ul << PDMA_TRGSTS_REQSTSn_Pos)                /*!< PDMA_T::TRGSTS: REQSTSn Mask           */

#define PDMA_PRISET_FPRISETn_Pos         (0)                                               /*!< PDMA_T::PRISET: FPRISETn Position      */
#define PDMA_PRISET_FPRISETn_Msk         (0x3ul << PDMA_PRISET_FPRISETn_Pos)               /*!< PDMA_T::PRISET: FPRISETn Mask          */

#define PDMA_PRICLR_FPRICLRn_Pos         (0)                                               /*!< PDMA_T::PRICLR: FPRICLRn Position      */
#define PDMA_PRICLR_FPRICLRn_Msk         (0x3ul << PDMA_PRICLR_FPRICLRn_Pos)               /*!< PDMA_T::PRICLR: FPRICLRn Mask          */

#define PDMA_INTEN_INTENn_Pos            (0)                                               /*!< PDMA_T::INTEN: INTENn Position         */
#define PDMA_INTEN_INTENn_Msk            (0x3ul << PDMA_INTEN_INTENn_Pos)                  /*!< PDMA_T::INTEN: INTENn Mask             */
...
#define PDMA_ALIGN_ALIGNn_Msk            (0x3ul << PDMA_ALIGN_ALIGNn_Pos)                  /*!< PDMA_T::ALIGN: ALIGNn Mask             */

#define PDMA_TACTSTS_TXACTFn_Pos         (0)                                               /*!< PDMA_T::TACTSTS: TXACTFn Position      */
#define PDMA_TACTSTS_TXACTFn_Msk         (0x3ul << PDMA_TACTSTS_TXACTFn_Pos)               /*!< PDMA_T::TACTSTS: TXACTFn Mask          */
...
#define PDMA_CHRST_CHnRST_Msk            (0x3ul << PDMA_CHRST_CHnRST_Pos)                  /*!< PDMA_T::CHRST: CHnRST Mask             */
