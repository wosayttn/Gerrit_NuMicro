...
uint32_t UI2C_Open(UI2C_T *ui2c, uint32_t u32BusClock)
{
    ...
    ui2c->CTL = (ui2c->CTL & ~UI2C_CTL_FUNMODE_Msk) | (uint32_t)(4UL << (uint32_t)UI2C_CTL_FUNMODE_Pos);

    /* Data format configuration */
    /* 8 bit data length */
    ui2c->LINECTL &= ~UI2C_LINECTL_DWIDTH_Msk;
    ui2c->LINECTL |= (uint32_t)(8UL << (uint32_t)UI2C_LINECTL_DWIDTH_Pos);
...
uint32_t UI2C_WriteMultiBytes(UI2C_T *ui2c, uint8_t u8SlaveAddr, const uint8_t *data, uint32_t u32wLen)
{
    ...
    while (u8Xfering)
    {
        ...
        case UI2C_PROTSTS_STARIF_Msk:
            ...
            UI2C_SET_DATA(ui2c, (uint8_t)((uint32_t)u8SlaveAddr << 1U)); /* Write SLA+W to Register UI2C_TXDAT */
            u8Ctrl = (uint8_t)UI2C_CTL_PTRG;                                     /* Clear SI */
            break;
        ...
        case UI2C_PROTSTS_ACKIF_Msk:
            ...
            {
                u8Ctrl = (uint8_t)(UI2C_CTL_PTRG | UI2C_CTL_STO);                /* Clear SI and send STOP */
            }
...
uint32_t UI2C_ReadMultiBytes(UI2C_T *ui2c, uint8_t u8SlaveAddr, uint8_t *rdata, uint32_t u32rLen)
{
    ...
    while (u8Xfering)
    {
        ...
        case UI2C_PROTSTS_STARIF_Msk:
            UI2C_CLR_PROT_INT_FLAG(ui2c, UI2C_PROTSTS_STARIF_Msk);     /* Clear START INT Flag */
            UI2C_SET_DATA(ui2c, (uint8_t)(((uint32_t)u8SlaveAddr << 1U) | 0x01U)); /* Write SLA+R to Register UI2C_TXDAT */
            eEvent = MASTER_SEND_H_RD_ADDRESS;
            u8Ctrl = (uint8_t)UI2C_CTL_PTRG;
            break;
        ...
        case UI2C_PROTSTS_ACKIF_Msk:
            ...
            if (eEvent == MASTER_SEND_H_RD_ADDRESS)
            {
                ...
                {
                    u8Ctrl = (uint8_t)(UI2C_CTL_PTRG | UI2C_CTL_AA);
                }
...
uint32_t UI2C_ReadMultiBytesOneReg(UI2C_T *ui2c, uint8_t u8SlaveAddr, uint8_t u8DataAddr, uint8_t *rdata, uint32_t u32rLen)
{
    ...
    while (u8Xfering)
    {
        ...
        case UI2C_PROTSTS_STARIF_Msk:
            ...
            {
                UI2C_SET_DATA(ui2c, (uint8_t)(((uint32_t)u8SlaveAddr << 1U) | 0x01U)); /* Write SLA+R to Register TXDAT */
                eEvent = MASTER_SEND_H_RD_ADDRESS;
            }
            else
            {
        ...
        case UI2C_PROTSTS_ACKIF_Msk:
            ...
            {
                u8Ctrl = (uint8_t)(UI2C_CTL_PTRG | UI2C_CTL_STA);                /* Send repeat START signal */
