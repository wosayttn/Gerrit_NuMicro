...
{
    timer->TRGCTL = (timer->TRGCTL & ~(TIMER_TRGCTL_TRGBPWM_Msk  | TIMER_TRGCTL_TRGADC_Msk | TIMER_TRGCTL_TRGPDMA_Msk)) | u32Mask;
}
...
uint32_t TIMER_GetModuleClock(const TIMER_T *timer)
{
    uint32_t u32Src = 0UL;
    uint32_t u32Clk;
    const uint32_t au32Clk[] = {0UL, __LIRC, __LXT, __HXT, __HIRC};

    if (timer == TIMER0)
    {
        u32Src = (CLK->TMRSEL0 & CLK_TMRSEL0_TMR0SEL_Msk) >> CLK_TMRSEL0_TMR0SEL_Pos;
    }
    else if (timer == TIMER1)
    {
        u32Src = (CLK->TMRSEL0 & CLK_TMRSEL0_TMR1SEL_Msk) >> CLK_TMRSEL0_TMR1SEL_Pos;
    }
    else if (timer == TIMER2)
    {
        u32Src = (CLK->TMRSEL0 & CLK_TMRSEL0_TMR2SEL_Msk) >> CLK_TMRSEL0_TMR2SEL_Pos;
    }
    else if (timer == TIMER3)
    {
        u32Src = (CLK->TMRSEL0 & CLK_TMRSEL0_TMR3SEL_Msk) >> CLK_TMRSEL0_TMR3SEL_Pos;
    }
    else if (timer == TIMER4)
    {
        u32Src = (CLK->TMRSEL0 & CLK_TMRSEL0_TMR4SEL_Msk) >> CLK_TMRSEL0_TMR4SEL_Pos;
    }
    else if (timer == TIMER5)
    {
        u32Src = (CLK->TMRSEL0 & CLK_TMRSEL0_TMR5SEL_Msk) >> CLK_TMRSEL0_TMR5SEL_Pos;
    }
    else if (timer == TIMER6)
    {
        u32Src = (CLK->TMRSEL0 & CLK_TMRSEL0_TMR6SEL_Msk) >> CLK_TMRSEL0_TMR6SEL_Pos;
    }
    else if (timer == TIMER7)
    {
        u32Src = (CLK->TMRSEL0 & CLK_TMRSEL0_TMR7SEL_Msk) >> CLK_TMRSEL0_TMR7SEL_Pos;
    }
    else if (timer == TIMER8)
    {
        u32Src = (CLK->TMRSEL1 & CLK_TMRSEL1_TMR8SEL_Msk) >> CLK_TMRSEL1_TMR8SEL_Pos;
    }
    else
    {
        u32Src = 0UL;
