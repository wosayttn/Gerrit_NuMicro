...
int32_t Xmodem(uint32_t u32DestAddr)
{
    ...
    int32_t ch;
    uint32_t u32StarAddr, u32Data[2];
        ...
        else
        {
            if(((s_au8XmdBuf[1] + s_au8XmdBuf[2]) == 0xFF) && check(crc, &s_au8XmdBuf[3], bufsz))
            {
                if(s_au8XmdBuf[1] == packetno)
                {
                    ...
                    if(count > 0)
                    {
                        for(j = 0; j < (bufsz + 3) / 4; j+=2)
                        {
                            memcpy((uint8_t *)u32Data, &s_au8XmdBuf[3 + (j * 0x4)], 8);

                            u32StarAddr = u32DestAddr + (uint32_t)i32TransBytes;

                            i32Err = XMD_Write(u32StarAddr + ((uint32_t)j * 0x4), u32Data[0],u32Data[1]);
