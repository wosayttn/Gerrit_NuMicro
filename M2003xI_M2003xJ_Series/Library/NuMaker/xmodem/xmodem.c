...
*/
static int32_t XMD_Write(uint32_t u32Addr, uint32_t u32Data0, uint32_t u32Data1)
{
            ...
            if (((s_au8XmdBuf[1] + s_au8XmdBuf[2]) == 0xFFU) && CheckPacketIntegrity(bUseCrc, &s_au8XmdBuf[3], i32BufSize))
            {
                if (s_au8XmdBuf[1] == u8PacketNo)
                {
                    ...
                    if (i32RemainCount > 0)
                    {
                        uint32_t u32Data0;
                        uint32_t u32Data1;
                        for (i32BlkIdx = 0; i32BlkIdx < (i32BufSize / 8); i32BlkIdx++)
                        {
                              (void)memcpy((uint8_t *)&u32Data0,
                                  &s_au8XmdBuf[3 + (i32BlkIdx * 8)],
                                  sizeof(uint32_t));

                              (void)memcpy((uint8_t *)&u32Data1,
                                  &s_au8XmdBuf[3 + (i32BlkIdx * 8) + 4],
                                  sizeof(uint32_t));

                              i32Err = XMD_Write(
                                  (u32DestAddr + (uint32_t)i32TransBytes) +
                                  ((uint32_t)i32BlkIdx * 8U),
                                  u32Data0,
                                  u32Data1);
