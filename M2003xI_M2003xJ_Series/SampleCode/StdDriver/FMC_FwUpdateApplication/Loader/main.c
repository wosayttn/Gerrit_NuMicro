...
void UART_Init(UART_T* uart, uint32_t u32ClkSrc, uint32_t u32ClkDiv, uint32_t u32BaudRate)
{
    ...
    CLK->UARTSEL = (CLK->UARTSEL & (~CLK_UARTSEL_UART1SEL_Msk)) | (u32ClkSrc << CLK_UARTSEL_UART1SEL_Pos);
    CLK->UARTDIV = (CLK->UARTDIV & (~CLK_UARTDIV_UART1DIV_Msk)) | (u32ClkDiv << CLK_UARTDIV_UART1DIV_Pos);
...
{
    /* Start TIMER0  */
    CLK->TMRSEL0 = (CLK->TMRSEL0 & (~CLK_TMRSEL0_TMR0SEL_Msk)) | CLK_TMRSEL0_TMR0SEL_HXT;
    /* enable TIMER0 clock */
...
int main()
{
    ...
    do
    {
        ...
        if((s_u32ExecBank == 0) && (u32Loader0ChkSum != u32Loader1ChkSum))
        {
            ...
            {
                FMC_Write8Bytes(FMC_BANK_SIZE * (s_u32ExecBank ^ 1) + i, FMC_Read((FMC_BANK_SIZE * s_u32ExecBank) + i), FMC_Read((FMC_BANK_SIZE * s_u32ExecBank) + i + 4));
            }
