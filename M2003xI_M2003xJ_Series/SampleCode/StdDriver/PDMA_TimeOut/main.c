 ...
 * @file     main.c
 * @version  V3.00
 * @brief    Demonstrate PDMA0 channel 0 get/clear timeout flag with UART1.
 *
...
void SYS_Init(void)
{
    ...
    CLK_SetModuleClock(UART1_MODULE, CLK_UARTSEL_UART1SEL_HIRC, CLK_UARTDIV_UART1(1));
...
{
    /* Open PDMA0 Channel */
    PDMA_Open(PDMA0, 1 << 1); // Channel 1 for UART1 TX
    PDMA_Open(PDMA0, 1 << 0); // Channel 0 for UART1 RX
    // Select basic mode
    PDMA_SetTransferMode(PDMA0, 1, PDMA_UART1_TX, 0, 0);
    PDMA_SetTransferMode(PDMA0, 0, PDMA_UART1_RX, 0, 0);
    // Set data width and transfer count
    PDMA_SetTransferCnt(PDMA0, 1, PDMA_WIDTH_8, PDMA_TEST_LENGTH);
    PDMA_SetTransferCnt(PDMA0, 0, PDMA_WIDTH_8, PDMA_TEST_LENGTH + 1);
    //Set PDMA Transfer Address
    PDMA_SetTransferAddr(PDMA0, 1, ((uint32_t)(&g_u8Tx_Buffer[0])), PDMA_SAR_INC, UART1_BASE, PDMA_DAR_FIX);
    PDMA_SetTransferAddr(PDMA0, 0, UART1_BASE, PDMA_SAR_FIX, ((uint32_t)(&g_u8Rx_Buffer[0])), PDMA_DAR_INC);
    ...
    PDMA_EnableInt(PDMA0, 0, PDMA_INT_TIMEOUT);
...
void PDMA_IRQHandler(void)
{
    ...
    else if (u32Status & 0x100)     /* channel 0 timeout */
    {
        ...
        PDMA_SetTimeOut(PDMA0, 0, 0, PDMA_TIME);
    }
    else
...
void UART_PDMATest(void)
{
    ...
    */
    ...
    while(1)
    {
        ...
        printf("PDMA timeout test Pass, Please press any key to next test. \n\n");
        getchar();
    }
