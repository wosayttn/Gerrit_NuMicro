...
#define MATCH_ADDRESS1     0xC0
#define MATCH_ADDRESS2     0xA2
#define UNMATCH_ADDRESS1   0xB1
#define UNMATCH_ADDRESS2   0xD3
...
void SYS_Init(void)
{
    ...
    /* Set PB multi-function pins for UART1 TXD, RXD and RTS */
...
void RS485_HANDLE()
{
    ...
    if (UART_GET_INT_FLAG(UART1,UART_INTSTS_RLSINT_Msk ) && UART_GET_INT_FLAG(UART1, UART_INTSTS_RDAINT_Msk))       /* RLS INT & RDA INT */ //For RS485 Detect Address
    {
        if (UART_RS485_GET_ADDR_FLAG(UART1))   /* ADD_IF, RS485 mode */
        {
            ...
            if ((addr == MATCH_ADDRESS1) || (addr == MATCH_ADDRESS2))
            {
...
void RS485_9bitModeSlave()
{
    ...
    printf("| Only Address %2x and %2x,data can be received              |\n", MATCH_ADDRESS1, MATCH_ADDRESS2);
    printf("+-----------------------------------------------------------+\n");
    ...
    printf("|    Auto Address Match Operation Mode                      |\n");
    printf("+-----------------------------------------------------------+\n");
    printf("|Only Address %2x,data can be received                      |\n", MATCH_ADDRESS1);
    printf("+-----------------------------------------------------------+\n");

    /* Set RS485-AAD Mode and address match is 0xC0 */
    UART_SelectRS485Mode(UART1, UART_ALTCTL_RS485AAD_Msk | UART_ALTCTL_ADDRDEN_Msk | UART_ALTCTL_RS485AUD_Msk, MATCH_ADDRESS1);
#endif
