...
uint32_t Get_NominalBitRate(CANFD_T *psCanfd);
#if (OP_MODE == CANFD_OP_CAN_FD_MODE)
uint32_t Get_DataBitRate(CANFD_T *psCanfd);
#endif
...
/*  u32DataBitRate : data phase bit rate (CAN FD mode only; pass 0 in CAN mode)                          */
/*---------------------------------------------------------------------------------------------------------*/
void CANFD_MonitorMode_Init(uint32_t u32NormBitRate, uint32_t u32DataBitRate)
{
...
#else
    /* CAN mode: classic CAN, no data phase */
    (void)u32DataBitRate;
...
int32_t main(void)
{
...
#if (OP_MODE == CANFD_OP_CAN_FD_MODE)
    printf("|    ' CANFD_CANFD_TxRx ' or ' CANFD_CANFD_TxRxINT ' as the transmission     |\n");
#else
    printf("|    ' CANFD_CAN_TxRx ' or ' CANFD_CAN_TxRxINT ' as the transmission         |\n");
#endif
    printf("|    network.                                                                 |\n");
     ...
     *   CAN FD mode: nominal = 1 Mbps, data = 4 Mbps
     */
#if (OP_MODE == CANFD_OP_CAN_FD_MODE)
