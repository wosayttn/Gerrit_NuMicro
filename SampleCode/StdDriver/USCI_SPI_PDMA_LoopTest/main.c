...
#define USPI_PDMA_TRANSFER_TIMEOUT  SystemCoreClock
...
int32_t SpiLoopTest_WithPDMA(void);
...
int32_t SpiLoopTest_WithPDMA(void)
{
    uint32_t u32DataCount, u32TestCycle, u32Timeout;
    ...
    for (u32TestCycle = 0; u32TestCycle < 10000; u32TestCycle++)
    {
        ...
        u32Timeout = USPI_PDMA_TRANSFER_TIMEOUT;
        ...
        while (1)
        {
            ...
            if (u32Timeout-- == 0UL)
            {
                printf("\nUSCI_SPI PDMA transfer timed out.\n");
                i32Err = 1;
                break;
            }
        }
