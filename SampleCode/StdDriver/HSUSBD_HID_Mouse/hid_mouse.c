...
uint8_t volatile g_u8LPMSuspend = 0, g_u8LPMResume = 0;
...
#ifdef SUPPORT_LPM
            HSUSBD_ENABLE_BUS_INT(HSUSBD_BUSINTEN_RSTIEN_Msk | HSUSBD_BUSINTEN_RESUMEIEN_Msk | HSUSBD_BUSINTEN_VBUSDETIEN_Msk | HSUSBD_BUSINTEN_LPMTKNIEN_Msk);
#else
...
void HID_UpdateMouseData(void)
{
    ...
    if((GPIO_GET_IN_DATA(PB) & BIT1) == 0x0)
        u32Key = 1;
    else
        u32Key = 0;
