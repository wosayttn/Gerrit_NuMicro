...
#define OCFLH_SECTOR_SIZE           0x1000UL
...
/**
 * @brief Get the last DMM-accessible 4 KiB sector from the JEDEC density code.
 *
 * @param[in]  au8JedecId     Three-byte JEDEC ID read while the flash is in legacy SPI mode.
 * @param[out] pu32TrimAddr   Flash offset reserved for trim-pattern storage.
 *
 * @return SPIM_OK on success; SPIM_ERR_FAIL for an invalid capacity code or a
 *         capacity smaller than one erase sector.
 *
 * @note The JEDEC density code represents a $2^N$-byte flash capacity. DMM is
 *       limited to SPIM_DMM_SIZE, so larger flash devices use the last 4 KiB
 *       of the DMM-addressable range rather than an unmapped physical address.
 */
static int32_t SPIM_GetTrimSectorAddr(const uint8_t au8JedecId[3], uint32_t *pu32TrimAddr)
{
    uint32_t u32DMMAccessibleSize = 0UL;
    uint32_t u32DensityCode = 0UL;
    uint64_t u64FlashSize = 0ULL;

    if ((au8JedecId == NULL) || (pu32TrimAddr == NULL))
    {
        return SPIM_ERR_FAIL;
    }

    u32DensityCode = au8JedecId[2];

    if ((u32DensityCode < 12UL) || (u32DensityCode > 63UL))
    {
        return SPIM_ERR_FAIL;
    }

    u64FlashSize = 1ULL << u32DensityCode;
    u32DMMAccessibleSize = (u64FlashSize > (uint64_t)SPIM_DMM_SIZE) ?
                           SPIM_DMM_SIZE : (uint32_t)u64FlashSize;

    if (u32DMMAccessibleSize < OCFLH_SECTOR_SIZE)
    {
        return SPIM_ERR_FAIL;
    }

    *pu32TrimAddr = u32DMMAccessibleSize - OCFLH_SECTOR_SIZE;
 ...
 *          1. Using the reserved DMM sector to store a known trim pattern
 ...
 *
 * @note u32SrcAddr must remain reserved for temporary trim-pattern storage.
 */
void SPIM_TrimRxClkDlyNum(SPIM_T *spim, uint32_t u32SrcAddr,
                          SPIM_PHASE_T *psMTWrCMD, SPIM_PHASE_T *psMTRdCMD)
{
    ...
    /* Restore command-based access before the caller changes flash protocol. */
 ...
 *          2. Using the reserved DMM sector to store a known trim pattern
 ...
 */
void SPIM_TrimDLLDelayNum(SPIM_T *spim, uint32_t u32SrcAddr,
                          SPIM_PHASE_T *psMTWrCMD, SPIM_PHASE_T *psMTRdCMD)
{
    uint32_t u32DMMAddr = SPIM_GET_DMMADDR(spim);
    uint32_t u32Div = SPIM_GET_CLOCK_DIVIDER(spim);
    ...
    /* Configure DMA phases only after the flash accepts Octal DDR commands. */
    SPIM_DMADMM_InitPhase(spim, psMTRdCMD, SPIM_CTL0_OPMODE_PAGEREAD);
    SPIM_DMADMM_InitPhase(spim, psMTWrCMD, SPIM_CTL0_OPMODE_PAGEWRITE);
    ...
    /* Restore command-based access before issuing the OPI exit command. */
    SPIM_ExitDirectMapMode(spim);
...
int dma_read_write(uint32_t u32Is4ByteAddr, uint32_t u32RdCmd, uint32_t WrCmd, uint32_t u32DDREn)
{
    ...
    for (offset = 0; offset < FLASH_BLOCK_SIZE; offset += BUFFER_SIZE)
    {
        ...
        SPIM_DMA_Read(SPIM_PORT, TEST_BLOCK_ADDR + offset, u32Is4ByteAddr,
                      BUFFER_SIZE, g_buff, u32RdCmd, SPIM_OP_ENABLE);
...
int main()
{
    ...
    if (SPIM_GetTrimSectorAddr(idBuf, &u32TrimAddr) != SPIM_OK)
    {
        printf("Unsupported JEDEC density code 0x%02X for trim sector.\n", idBuf[2]);
        goto lexit;
    }

    printf("SPIM trim sector address=0x%08X\n", u32TrimAddr);
    ...
    SPIM_TrimRxClkDlyNum(SPIM_PORT, u32TrimAddr, &gsMtC2hWrCMD, &gsMtCBhRdCMD);
    ...
    /* Trim delay cycle. Adjust the sampling clock of received data to latch the correct data. */
    SPIM_TrimRxClkDlyNum(SPIM_PORT, u32TrimAddr, &gsMt8EhWrCMD, &gsMtCChRdCMD);
    ...
    /* Trim delay cycle. The trim routine enters OPI before setting DDR phases. */
    SPIM_TrimDLLDelayNum(SPIM_PORT, u32TrimAddr, &gsMt02hWrDDRCMD, &gsMt8BhRdDDRCMD);
