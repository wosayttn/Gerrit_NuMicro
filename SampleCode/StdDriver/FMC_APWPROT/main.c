...
int main()
{
    uint32_t i, j, u32ISPReadData, u32CPUReadData, u32Start, u32End, u32ISPFF, u32ApwprotBlockSize;
    int32_t i32ISPSTS = 0;
    ...
    /* Get APROM protect region */
    u32ApwprotBlockSize = FMC_GetApwprotSize();

    printf("\nAPROM protect region %dK Bytes\n",u32ApwprotBlockSize/1024);
		
    /* Set APROM testing range */
    u32Start = FMC_APROM_BASE + u32ApwprotBlockSize*3;
    u32End   = FMC_APROM_END;

    /* Clear all APROM protect settings */
    for(i = 0; i < 32; i++)
    {
        FMC_DISABLE_APWPROT(i);
    }
    ...
    for(i = 0; i < 32; i++)
    {
        printf("APPROEN%02d  [%s]  address is 0x%08x ~ 0x%08x\n", i, FMC_IS_APWPROT(i & 0x1F) ? "ENABLE ":"DISABLE", (uint32_t)(i*u32ApwprotBlockSize), (uint32_t)((i+1)*u32ApwprotBlockSize-1) );
		}
    ...
    while(1)
    {
        ...
        u32Start = i8GetCh*u32ApwprotBlockSize;
        u32End   = (i8GetCh+1)*u32ApwprotBlockSize;
    ...
    for(i = 0; i < 32; i++)
    {
        if(FMC_IS_APWPROT(i & 0x1F))
        {

            /* Set APROM protected address by selected region */
            u32Start = i*u32ApwprotBlockSize;
            u32End   = (i+1)*u32ApwprotBlockSize;

            printf("\n\nAPPROEN%02d Testing, address is 0x%08x ~ 0x%08x \n", i,  (uint32_t)(i*u32ApwprotBlockSize), (uint32_t)((i+1)*u32ApwprotBlockSize-1));
