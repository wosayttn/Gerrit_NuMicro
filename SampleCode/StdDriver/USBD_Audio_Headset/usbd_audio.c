...
void UAC_ClassRequest(void)
{
    ...
    /* Cache the volatile EP0 buffer address to avoid multiple volatile accesses in one statement */
    u32EpBufAddr = USBD_GET_EP_BUF_ADDR(EP0);
    ...
    if (buf[0] & 0x80)
    {
        ...
        switch (buf[1])
        {
            case UAC_GET_CUR:
            {
                ...
                {
                    u32Val = g_usbd_RecSampleRate;
                    M32(USBD_BUF_BASE + u32EpBufAddr) = u32Val;
                    /* Data stage */
                    USBD_SET_DATA1(EP0);
                    ...
                    switch (buf[3])
                    {
                        case MUTE_CONTROL:
                        {
                            if (REC_FEATURE_UNITID == buf[5])
                                M8(USBD_BUF_BASE + u32EpBufAddr) = g_usbd_RecMute;
                            else if (PLAY_FEATURE_UNITID == buf[5])
                                M8(USBD_BUF_BASE + u32EpBufAddr) = g_usbd_PlayMute;
                        ...
                        case VOLUME_CONTROL:
                        {
                            if (REC_FEATURE_UNITID == buf[5])
                            {
                                ...
                                if (buf[2] == 1)
                                {
                                    i16Val = g_usbd_RecVolumeL;
                                    M8(USBD_BUF_BASE + u32EpBufAddr) = (uint8_t)i16Val;
                                    M8(USBD_BUF_BASE + u32EpBufAddr + 1) = (uint8_t)(i16Val >> 8);
                                }
                                else
                                {
                                    i16Val = g_usbd_RecVolumeR;
                                    M8(USBD_BUF_BASE + u32EpBufAddr) = (uint8_t)i16Val;
                                    M8(USBD_BUF_BASE + u32EpBufAddr + 1) = (uint8_t)(i16Val >> 8);
                                }
                            }
                            else if (PLAY_FEATURE_UNITID == buf[5])
                            {
                                if (buf[2] == 1)
                                {
                                    i16Val = g_usbd_PlayVolumeL;
                                    M8(USBD_BUF_BASE + u32EpBufAddr) = (uint8_t)i16Val;
                                    M8(USBD_BUF_BASE + u32EpBufAddr + 1) = (uint8_t)(i16Val >> 8);
                                }
                                else
                                {
                                    i16Val = g_usbd_PlayVolumeR;
                                    M8(USBD_BUF_BASE + u32EpBufAddr) = (uint8_t)i16Val;
                                    M8(USBD_BUF_BASE + u32EpBufAddr + 1) = (uint8_t)(i16Val >> 8);
                                }
                            }
            ...
            case UAC_GET_MIN:
            {
                switch (buf[3])
                {
                    case VOLUME_CONTROL:
                    {
                        if (REC_FEATURE_UNITID == buf[5])
                        {
                            i16Val = g_usbd_RecMinVolume;
                            M8(USBD_BUF_BASE + u32EpBufAddr) = (uint8_t)i16Val;
                            M8(USBD_BUF_BASE + u32EpBufAddr + 1) = (uint8_t)(i16Val >> 8);
                        }
                        else if (PLAY_FEATURE_UNITID == buf[5])
                        {
                            i16Val = g_usbd_PlayMinVolume;
                            M8(USBD_BUF_BASE + u32EpBufAddr) = (uint8_t)i16Val;
                            M8(USBD_BUF_BASE + u32EpBufAddr + 1) = (uint8_t)(i16Val >> 8);
                        }
            ...
            case UAC_GET_MAX:
            {
                switch (buf[3])
                {
                    case VOLUME_CONTROL:
                    {
                        if (REC_FEATURE_UNITID == buf[5])
                        {
                            i16Val = g_usbd_RecMaxVolume;
                            M8(USBD_BUF_BASE + u32EpBufAddr) = (uint8_t)i16Val;
                            M8(USBD_BUF_BASE + u32EpBufAddr + 1) = (uint8_t)(i16Val >> 8);
                        }
                        else if (PLAY_FEATURE_UNITID == buf[5])
                        {
                            i16Val = g_usbd_PlayMaxVolume;
                            M8(USBD_BUF_BASE + u32EpBufAddr) = (uint8_t)i16Val;
                            M8(USBD_BUF_BASE + u32EpBufAddr + 1) = (uint8_t)(i16Val >> 8);
                        }
            ...
            case UAC_GET_RES:
            {
                switch (buf[3])
                {
                    case VOLUME_CONTROL:
                    {
                        if (REC_FEATURE_UNITID == buf[5])
                        {
                            i16Val = g_usbd_RecResVolume;
                            M8(USBD_BUF_BASE + u32EpBufAddr) = (uint8_t)i16Val;
                            M8(USBD_BUF_BASE + u32EpBufAddr + 1) = (uint8_t)(i16Val >> 8);
                        }
                        else if (PLAY_FEATURE_UNITID == buf[5])
                        {
                            i16Val = g_usbd_PlayResVolume;
                            M8(USBD_BUF_BASE + u32EpBufAddr) = (uint8_t)i16Val;
                            M8(USBD_BUF_BASE + u32EpBufAddr + 1) = (uint8_t)(i16Val >> 8);
                        }
...
void UAC_GetPlayData(uint8_t *psrc, uint32_t u32Samples)
{
    uint32_t i, u32len;
    uint32_t u32BuffLen;
    uint32_t u32BufPlayIdxTmp, u32PlayBufPosTmp;
    uint8_t u8AudioPlayingTmp, u8TxDataCntInBufferTmp;
    uint32_t *pBuff;
    uint32_t *pSrc;

    /* if buffer has enough data, play it!! */
    u8AudioPlayingTmp = u8AudioPlaying;
    u8TxDataCntInBufferTmp = u8TxDataCntInBuffer;

    if (!u8AudioPlayingTmp && (u8TxDataCntInBufferTmp >= (PDMA_TXBUFFER_CNT / 2 + 1)))
    {
    ...
    /* Ring buffer check */
    u32PlayBufPosTmp = u32PlayBufPos;
    u32BufPlayIdxTmp = u32BufPlayIdx;
    u32BuffLen = g_u32BuffLen;

    if ((u32PlayBufPosTmp + u32len) > u32BuffLen)
    {
        PcmPlayBuffLen[u32BufPlayIdxTmp] = u32PlayBufPosTmp;
        u32PlayBufPos = 0;
        u32BufPlayIdx ++;
    ...
    u32BufPlayIdxTmp = u32BufPlayIdx;
    u32PlayBufPosTmp = u32PlayBufPos;
    pBuff = &PcmPlayBuff[u32BufPlayIdxTmp][u32PlayBufPosTmp];
    pSrc = (uint32_t *) psrc;
...
void UAC_SendRecData(void)
{
    ...
    short *p16Src;
    uint32_t u32EpBufAddr;
    uint32_t u32BufRecIdxTmp;
    uint32_t u32ResampleRecIdx;

    /* Get the address in USB buffer */
    u32EpBufAddr = USBD_GET_EP_BUF_ADDR(EP2);
    u32BufRecIdxTmp = u32BufRecIdx;
    p16Buf = (short *)((uint32_t)USBD_BUF_BASE + u32EpBufAddr);
    p16Src = (short *)(&PcmRecBuff[u32BufRecIdxTmp][0]);

    /* when record buffer full, send data to host */
    if (u8PcmRxBufFull[u32BufRecIdxTmp])
    {
        /* Set empty flag */
        u8PcmRxBufFull[u32BufRecIdxTmp] = 0;

        /* Cache the volatile resample index to avoid multiple volatile accesses in one statement */
        u32ResampleRecIdx = g_resample_rec_s_idx;

        /* Audio data is resampled to 48MHz (Audio Codec is fixed to 48MHz) */
        /* Do Resample operation to data in USB buffer to SRAM buffer */
        if (REC_CHANNELS == 2)
        {
            Resamples(E_RS_REC_CH0, p16Src, REC_CHANNELS, srt[u32ResampleRecIdx].count, g_a16AudioTmpBuf0, u32ResampleRecIdx);
            Resamples(E_RS_REC_CH1, &p16Src[1], REC_CHANNELS, srt[u32ResampleRecIdx].count, &g_a16AudioTmpBuf0[1], u32ResampleRecIdx);
        }
        else
            Resamples(E_RS_REC_CH0, p16Src, REC_CHANNELS, srt[u32ResampleRecIdx].count, g_a16AudioTmpBuf0, u32ResampleRecIdx);
