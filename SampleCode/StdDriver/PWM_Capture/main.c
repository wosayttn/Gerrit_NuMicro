...
int32_t CalPeriodTime(PWM_T *PWM, uint32_t u32Ch)
{
    ...
    while (PWM_GetCaptureIntFlag(PWM, u32Ch) != (PWM_CAPTURE_INT_FALLING_LATCH << u32Ch))
    {
        if (--u32TimeOutCnt == 0)
        {
    ...
    while (u32i < 4)
    {
        ...
        while (PWM_GetCaptureIntFlag(PWM, u32Ch) != (PWM_CAPTURE_INT_RISING_LATCH << u32Ch))
        {
            if (--u32TimeOutCnt == 0)
            {
...
int main(void)
{
    ...
    while (1)
    {
        ...
        */
        ...
        PWM_ConfigCaptureChannel(PWM1, PWM_CAP_CH, 97, 0);
