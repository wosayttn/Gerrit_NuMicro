...
{
    uint32_t IrqStL, IrqSt, IntEn;
    uint32_t u32TimeOutCnt;

    IrqStL = HSUSBD->GINTSTS;
    IntEn = HSUSBD->GINTEN;
    IrqStL &= IntEn;    /* get enabled interrupt status */
    ...
    for (uint32_t ep = EPA; ep < HSUSBD_MAX_EP; ep++)
    {
        if (IrqStL & (HSUSBD_GINTSTS_EPAIF_Msk << ep))
        {
            IrqSt = HSUSBD->EP[ep].EPINTSTS;
            IntEn = HSUSBD->EP[ep].EPINTEN;
            IrqSt = IrqSt & IntEn;
            ...
            HSUSBD_CLR_EP_INT_FLAG(ep, IrqSt);
        }
    }
}
...
void UAC_DeviceDisable(uint32_t bIsPlay)
{
    if (bIsPlay)
    {
        ...
        {
            int8_t i8Tx = i8TxDataCntInBuffer;
            int8_t i8Rx = i8RxDataCntInBuffer;
...
{
    uint32_t u32len;
    uint32_t u32PlayPosLocal;
    uint32_t u32BufIdxLocal;
    uint32_t u32PacketSizeLocal;
    uint32_t u32BuffLenLocal;
    ...
    u32PlayPosLocal    = u32PlayBufPos;
    u32BufIdxLocal     = u32BufPlayIdx;
    u32PacketSizeLocal = u32PacketSize;
    u32BuffLenLocal    = u32BuffLen;

    if ((u32PlayPosLocal + u32PacketSizeLocal) > u32BuffLenLocal)
    {
        PcmPlayBuffLen[u32BufIdxLocal] = u32PlayPosLocal;
        DMA_TXDESC[u32BufIdxLocal].ctl = (DMA_TXDESC[u32BufIdxLocal].ctl & ~PDMA_DSCT_CTL_TXCNT_Msk) | ((u32PlayPosLocal - 1) << PDMA_DSCT_CTL_TXCNT_Pos);
        u32PlayPosLocal = 0;
        u32BufIdxLocal ++;

        /* change buffer index */
        if (u32BufIdxLocal >= PDMA_TXBUFFER_CNT)
            u32BufIdxLocal = 0;
    ...
    /* Write back updated buffer index */
    u32BufPlayIdx = u32BufIdxLocal;
    ...
    HSUSBD_SET_DMA_ADDR((uint32_t)&PcmPlayBuff[u32BufIdxLocal][u32PlayPosLocal]);
    HSUSBD_SET_DMA_LEN(u32PacketSizeLocal << 2);     /* byte count */
    g_hsusbd_DmaDone = 0;
    HSUSBD_ENABLE_DMA();

    u32PlayBufPos = u32PlayPosLocal + u32PacketSizeLocal;
}
...
{
    uint32_t u32RecPosLocal;
    uint32_t u32BufIdxLocal;
    uint32_t u32PacketSizeLocal;
    uint32_t u32RxBuffLenLocal;
    ...
    u32RecPosLocal     = u32RecBufPos;
    u32BufIdxLocal     = u32BufRecIdx;
    u32PacketSizeLocal = u32PacketSize;
    u32RxBuffLenLocal  = u32RxBuffLen;

    /* when record buffer full, send data to host */
    if (u8PcmRxBufFull[u32BufIdxLocal])
    {
        if ((u32RecPosLocal + u32PacketSizeLocal) > u32RxBuffLenLocal)
        {
            /* Set empty flag */
            u8PcmRxBufFull[u32BufIdxLocal] = 0;
            u32RecPosLocal = 0;
            /* Change to next PCM buffer */
            u32BufIdxLocal ++;

            if (u32BufIdxLocal >= PDMA_RXBUFFER_CNT)
                u32BufIdxLocal = 0;
        ...
        /* Write back updated buffer index */
        u32BufRecIdx = u32BufIdxLocal;

        HSUSBD_SET_DMA_READ(ISO_IN_EP_NUM);
        HSUSBD_ENABLE_BUS_INT(HSUSBD_BUSINTEN_DMADONEIEN_Msk | HSUSBD_BUSINTEN_SUSPENDIEN_Msk | HSUSBD_BUSINTEN_RSTIEN_Msk | HSUSBD_BUSINTEN_VBUSDETIEN_Msk);
        HSUSBD_SET_DMA_ADDR((uint32_t)&PcmRecBuff[u32BufIdxLocal][u32RecPosLocal]);
        HSUSBD_SET_DMA_LEN(u32PacketSizeLocal << 2);     /* byte count */
        g_hsusbd_DmaDone = 0;
        HSUSBD_ENABLE_DMA();
        u32RecBufPos = u32RecPosLocal + u32PacketSizeLocal;
    }
    else
    {
        i8RxDataCntInBuffer = 0;
