...
#define I2S_TRANSFER_TIMEOUT    ((int32_t)SystemCoreClock)
...
int32_t main(void)
{
    ...
    volatile int32_t i32TimeoutCount = I2S_TRANSFER_TIMEOUT;
    ...
    printf("      NOTE: This sample uses bounded waits for demo recovery only. Add application-specific timeout handling before production use.\n\n");
    ...
    i32TimeoutCount = I2S_TRANSFER_TIMEOUT;

    while (g_u8TransDone && (--i32TimeoutCount >= 0)) {}

    if (i32TimeoutCount < 0)
    {
        printf("I2S PDMA transfer timed out.\n");
        goto sample_exit;
    }
