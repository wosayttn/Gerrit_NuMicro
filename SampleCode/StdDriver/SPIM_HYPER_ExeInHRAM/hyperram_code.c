...
void HyperRAM_TrimDLLDelayNumber(SPIM_T *spim)
{
    ...
    uint8_t au8TrimPattern[HRAM_TRIM_SIZE] = {0};
    uint8_t au8VerfiyBuf[HRAM_TRIM_SIZE] = {0};
    ...
    {
        patternGenerators[u32PatIdx](au8TrimPattern, HRAM_TRIM_SIZE);
        SPIM_HYPER_DMAWrite(spim, u32SrcAddr, au8TrimPattern, HRAM_TRIM_SIZE);
        ...
        for (u32Retry = 0; u32Retry < u32ReTrimMaxCnt; u32Retry++)
        {
            for (u8RdDelay = 0; u8RdDelay < SPIM_HYPER_MAX_LATENCY; u8RdDelay++)
            {
                ...
                memset(au8VerfiyBuf, 0, HRAM_TRIM_SIZE);
                ...
                for (u32Pass = 0; u32Pass < DLL_TRIM_PASS_COUNT; u32Pass++)
                {
                    ...
                    for (u32i = 0; u32i + 8 <= HRAM_TRIM_SIZE;)
                    {
                        ...
                        SPIM_HYPER_DMARead(spim, u32SrcAddr + u32LoopAddr, &au8VerfiyBuf[u32i], 8);

                        if (memcmp(&au8TrimPattern[u32LoopAddr], &au8VerfiyBuf[u32i], 8) == 0)
                        {
    ...
    for (u32i = bestIdxStart; u32i < bestIdxStart + bestIdxLen; u32i++)
    {
        ...
        {
            if (VerifyFinalRead(spim, u32DMMAddr + u32SrcAddr, au8TrimPattern, HRAM_TRIM_SIZE))
            {
