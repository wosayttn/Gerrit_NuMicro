...
void VCOM_TransferData(void)
{
...
#endif
    {
        ...
        if (p->u32TxSize == 0)
        {
            ...
            if (p->u16ComRbytes)
            {
                ...
                for (i = 0; i < u32Len; i++)
                {
                    ...
                    p->au8RxBuf[i] = p->au8ComRbuf[u16Head];
                    u16Head++;
        ...
        /* Process the Bulk out data when bulk out data is ready. */
        {
            ...
            {
                for (i = 0; i < p->u32RxSize; i++)
                {
                    uint16_t u16Tail = p->u16ComTtail;

                    p->au8ComTbuf[u16Tail] = p->pu8RxBuf[i];
                    u16Tail++;
                ...
                __set_PRIMASK(1);
                {
                    ...
                    p->u16ComTbytes = (uint16_t)(u16Bytes + p->u32RxSize);
                }
                __set_PRIMASK(0);
        ...
        if (p->u16ComTbytes)
        {
            ...
            if ((tVCOM[u32chidx].psPeripheralInfo->UART_BASE->INTEN & UART_INTEN_THREIEN_Msk) == 0)
            {
                ...
                u8Data = p->au8ComTbuf[u16Head];
                tVCOM[u32chidx].psPeripheralInfo->UART_BASE->DAT = u8Data;
                u16Head++;
    ...
    */
    {
        VCOM_CONTROL_VAR_t *p4 = tVCOM[IDX_VCOM4].psCtrlVar;
    ...
    if (p4->u32TxSize == 0)
    {
        ...
        if (p4->u16ComTbytes)
        {
            ...
            for (i = 0; i < u32Len; i++)
            {
                uint16_t u16Head = p4->u16ComThead;

                p4->au8RxBuf[i] = p4->au8ComTbuf[u16Head];
                u16Head++;

                if (u16Head >= TXBUFSIZE)
                    u16Head = 0;

                p4->u16ComThead = u16Head;
            }

            __set_PRIMASK(1);
            {
                uint16_t u16Bytes = p4->u16ComTbytes;
                p4->u16ComTbytes = (uint16_t)(u16Bytes - u32Len);
            }
            __set_PRIMASK(0);

            p4->u32TxSize = u32Len;
            USBD_MemCopy((uint8_t *)(USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR(GET_BULK_IN_EP(IDX_VCOM4))), (uint8_t *)p4->au8RxBuf, u32Len);
    ...
    {
        int8_t   i8BulkOutReady = p4->i8BulkOutReady;
        uint32_t u32RxSize      = p4->u32RxSize;
        uint16_t u16ComTbytes   = p4->u16ComTbytes;

        if (i8BulkOutReady && (u32RxSize <= (uint32_t)(TXBUFSIZE - u16ComTbytes)))
        {
            for (i = 0; i < p4->u32RxSize; i++)
            {
                uint16_t u16Tail = p4->u16ComTtail;

                p4->au8ComTbuf[u16Tail] = p4->pu8RxBuf[i];
                u16Tail++;

                if (u16Tail >= TXBUFSIZE)
                    u16Tail = 0;

                p4->u16ComTtail = u16Tail;
            }

            __set_PRIMASK(1);
            {
                uint16_t u16Bytes = p4->u16ComTbytes;
                p4->u16ComTbytes = (uint16_t)(u16Bytes + p4->u32RxSize);
            }
            __set_PRIMASK(0);

            p4->u32RxSize = 0;
            p4->i8BulkOutReady = 0; /* Clear bulk out ready flag */
