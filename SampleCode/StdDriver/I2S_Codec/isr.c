...
//------------------------------------------------------------------------------
/**
 * @brief    I2S0 interrupt handler.
 *
 * @param    None
 *
 * @return   None
 *
 * @details  The I2S0 interrupt handler is used to handle the I2S0 TX and RX FIFO threshold interrupts.
 *           It reads data from the RX FIFO and writes data to the TX FIFO based on the current buffer position.
 *           If the buffer is full, it resets the buffer position to 0.
 *           If the buffer is empty, it writes zeros to the TX FIFO.
 */
