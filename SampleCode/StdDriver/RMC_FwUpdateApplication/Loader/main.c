...
#define DB_PROG_LEN        (4 * RMC_FLASH_PAGE_SIZE)
/* Loop count */
...
void UART_Init(UART_T* uart, uint32_t u32ClkSrc, uint32_t u32ClkDiv, uint32_t u32BaudRate)
{
    ...
    CLK->CLKSEL4 = (CLK->CLKSEL4 & (~CLK_CLKSEL4_UART1SEL_Msk)) | (u32ClkSrc << CLK_CLKSEL4_UART1SEL_Pos);
...
int main()
{
    ...
    do
    {
        ...
        printf("\n Firmware CRC in [0x%x] is [0x%x]\n", FW_CRC_BASE, RMC_Read(FW_CRC_BASE));
        if(RMC_Read(FW_CRC_BASE) == 0xFFFFFFFF)
        {

            RMC_Write(FW_CRC_BASE, u32App0ChkSum);
            printf("\n Update Firmware CRC in [0x%x] is [0x%x]\n", FW_CRC_BASE, RMC_Read(FW_CRC_BASE));
        }

        /* Write backup firmware CRC in backup CRC base for following checking */
        printf("\n Backup Firmware CRC in [0x%x] is [0x%x]\n", BACKUP_FW_CRC_BASE, RMC_Read(BACKUP_FW_CRC_BASE));
        if(RMC_Read(BACKUP_FW_CRC_BASE) == 0xFFFFFFFF)
        {

            RMC_Write(BACKUP_FW_CRC_BASE, u32App1ChkSum);
            printf("\n Update Firmware CRC in [0x%x] is [0x%x]\n", BACKUP_FW_CRC_BASE, RMC_Read(BACKUP_FW_CRC_BASE));
        }
        ...
        if((s_u32ExecBank == 0) && (u32Loader0ChkSum != u32Loader1ChkSum))
        {
            ...
            /* Erase loader region */
            for(i = LOADER_BASE; i < (LOADER_BASE + LOADER_SIZE); i += RMC_FLASH_PAGE_SIZE)
            {
                RMC_Erase(RMC_BANK_SIZE * (s_u32ExecBank ^ 1) + i);
            }
            /* Create loader in the other bank */
            for(i = LOADER_BASE; i < (LOADER_BASE + LOADER_SIZE); i += 4)
            {
                RMC_Write(RMC_BANK_SIZE * (s_u32ExecBank ^ 1) + i, RMC_Read((RMC_BANK_SIZE * s_u32ExecBank) + i));
            }
            printf(" Create Bank%d Loader completed! \n", (s_u32ExecBank ^ 1));
        }

#if CREATE_BANK1_APP
        if((s_u32ExecBank == 0) && ((RMC_CheckAllOne((RMC_APROM_BANK0_END + APP_BASE), APP_SIZE)) == READ_ALLONE_YES))
        {
            printf("\n Create BANK%d App... \n", s_u32ExecBank ^ 1);

            /* Erase app region */
            for(i = APP_BASE; i < (APP_BASE + APP_SIZE); i += RMC_FLASH_PAGE_SIZE)
            {
                RMC_Erase(RMC_BANK_SIZE * (s_u32ExecBank ^ 1) + i);
            }
        ...
        else
        {
            ...
            if(u8GetCh == 'y')
            {
                ...
                else
                {
                    ...
                    getchar();
                    /* Remap to Loader */
                    RMC_SetVectorPageAddr(LOADER_BASE);
                    ResetCPU();
                }
            }
            else
