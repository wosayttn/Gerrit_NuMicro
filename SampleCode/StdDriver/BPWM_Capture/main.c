...
int32_t CalPeriodTime(BPWM_T *BPWM, uint32_t u32Ch)
{
    ...
    while (BPWM_GetCaptureIntFlag(BPWM, u32Ch) != (BPWM_CAPTURE_INT_FALLING_LATCH << u32Ch))
    {
        if (--u32TimeOutCnt == 0)
        {
    ...
    while (u32i < 4)
    {
        ...
        while (BPWM_GetCaptureIntFlag(BPWM, u32Ch) != (BPWM_CAPTURE_INT_RISING_LATCH << u32Ch))
        {
            if (--u32TimeOutCnt == 0)
            {
...
int32_t main(void)
{
    ...
    while (1)
    {
        ...
        */
        ...
        BPWM_ConfigCaptureChannel(BPWM0, BPWM_CAP_CH, 97, 0);
