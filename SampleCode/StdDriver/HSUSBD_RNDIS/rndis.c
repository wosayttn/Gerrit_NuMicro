            ...
            if (ep == EPB)
            {
                ...
                if (len > 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            uint32_t u32UsbIdxAcc = _rtxq.usb_idx;
                            _rtxq.data_len[u32UsbIdxAcc] += len;
                        }
                    }

                    {
                        uint32_t u32UsbIdxChk  = _rtxq.usb_idx;
                        uint32_t u32DataLenChk = _rtxq.data_len[u32UsbIdxChk];
                        uint32_t u32MsgTypeChk = hdr->MessageType;
                        uint32_t u32MsgLenChk  = hdr->MessageLength;

                        if ((u32MsgTypeChk == REMOTE_NDIS_PACKET_MSG) &&
                                (u32DataLenChk >= u32MsgLenChk))
                        {
                            uint32_t u32UsbIdx     = u32UsbIdxChk;
                            uint32_t u32DataLen    = u32DataLenChk;
                            uint32_t u32MsgLen     = u32MsgLenChk;
                            uint32_t u32EthDoneIdx = _rtxq.eth_done_idx;

                            if (u32DataLen > u32MsgLen)
                                printf("Long tx rmsg!\n");

                            if (((u32UsbIdx + 1) % RQ_SZ) == u32EthDoneIdx)
                            {
                                // printf("! %d %d %d\n", _rtxq.usb_idx, _rtxq.eth_idx, _rtxq.eth_done_idx);   /* drop the current packet */
                                _rtxq.data_len[u32UsbIdx] = 0;
                            }
                            else
                            {
                                // printf("TX: %d\n", hdr->MessageLength);
                                u32UsbIdx = (u32UsbIdx + 1) % RQ_SZ;
                                _rtxq.usb_idx = u32UsbIdx;
                                _rtxq.data_len[u32UsbIdx] = 0;
                            }
                        }
                    }
...
void RNDIS_ProcessOutData(void)
{
    ...
    while (1)
    {
        uint32_t u32EthIdx    = _rtxq.eth_idx;
        uint32_t u32UsbIdxCur = _rtxq.usb_idx;

        if (u32EthIdx == u32UsbIdxCur)
            break;

        hdr = (RNDIS_MSG_HDR *)&_rtxq.data[u32EthIdx][0];
        ppkt = (uint8_t *)&_rtxq.data[u32EthIdx][hdr->DataOffset + 8];

        if (My_EMAC_SendPkt(ppkt, hdr->DataLength) == 1)
        {
            _rtxq.eth_idx = (u32EthIdx + 1) % RQ_SZ;
        }
        else
        {
            HSUSBD_ENABLE_EP_INT(EPB, 0);
