...
void EnterToPowerDown(uint32_t u32PDMode)
{
    ...
    if (u32PDMode == NPD0_MODE)
        CLK_SetPowerDownMode(CLK_PMUCTL_PDMSEL_NPD);   //Power down
    else if (u32PDMode == NPD1_MODE)
        CLK_SetPowerDownMode(CLK_PMUCTL_PDMSEL_LLPD);   //Power down
    else if (u32PDMode == NPD2_MODE)
        CLK_SetPowerDownMode(CLK_PMUCTL_PDMSEL_FWPD);   //Power down
    else if (u32PDMode == SPD0_MODE)
    {
        CLK_ENABLE_SPDACMP();
        CLK_SetPowerDownMode(CLK_PMUCTL_PDMSEL_SPD);   //Power down
    }
