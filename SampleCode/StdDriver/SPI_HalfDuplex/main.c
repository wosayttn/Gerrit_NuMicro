...
int32_t SPI_Init(void)
{
    ...
    if (SPI_Open(SPI0, SPI_SLAVE, SPI_MODE_0, 16, 0) == 0U)
    {
        return -1;
    }
...
int main(void)
{
    ...
    printf("NOTE: This sample uses bounded waits for demo recovery only. Add application-specific fault handling before production use.\n");
    ...
    while ((SPI0->STATUS & SPI_STATUS_TXRXRST_Msk) && (u32Timeout > 0U))
    {
        u32Timeout--;
    }
    ...
    if (u32Timeout == 0U)
    {
        printf("SPI half-duplex reset timed out.\n");
        goto sample_exit;
    }
    ...
    for (u16RxDataCount = 0; u16RxDataCount < TEST_COUNT; u16RxDataCount++)
    {
        ...
        while (USPI_GetIntFlag(USPI0, USPI_PROTSTS_TXENDIF_Msk) && (u32Timeout > 0U))
        {
            u32Timeout--;
        }
        ...
        if (u32Timeout == 0U)
        {
            printf("USPI TX end wait timed out.\n");
            goto sample_exit;
        }
        ...
        while (USPI_GetStatus(USPI0, USPI_PROTSTS_BUSY_Msk) && (u32Timeout > 0U))
        {
            u32Timeout--;
        }
        ...
        if (u32Timeout == 0U)
        {
            printf("USPI busy wait timed out.\n");
            goto sample_exit;
        }
