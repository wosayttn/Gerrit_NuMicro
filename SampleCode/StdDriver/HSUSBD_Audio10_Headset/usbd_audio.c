...
{
    uint32_t u32len, tmp, u32Samples = 0;
    short *pu16Buf;
    short *p16Src;
    uint32_t u32PlayIdxLocal;
    uint32_t u32BufIdxLocal;
    uint32_t u32BuffLenLocal;
    uint32_t u32PlaySIdxLocal;
    ...
    u32PlaySIdxLocal = g_resample_play_s_idx;

    /* Audio data is resampled to 48MHz (Audio Codec is fixed to 48MHz) */
    /* Do Resample operation to data in USB buffer to SRAM buffer */
    if (PLAY_CHANNELS == 2)
    {
        u32Samples  = Resamples(E_RS_PLAY_CH0, p16Src, PLAY_CHANNELS, tmp, pu16Buf, u32PlaySIdxLocal);

        u32Samples += Resamples(E_RS_PLAY_CH1, &p16Src[1], PLAY_CHANNELS, tmp, &pu16Buf[1], u32PlaySIdxLocal);
    }
    else
        u32Samples  = Resamples(E_RS_PLAY_CH0, p16Src, PLAY_CHANNELS, tmp, pu16Buf, u32PlaySIdxLocal);
    ...
    u32PlayIdxLocal = u32PlayBufPos;
    u32BufIdxLocal  = u32BufPlayIdx;
    u32BuffLenLocal = u32BuffLen;

    /* Ring buffer check */
    if ((u32PlayIdxLocal + tmp) > u32BuffLenLocal)
    {
        PcmPlayBuffLen[u32BufIdxLocal] = u32PlayIdxLocal;
        u32PlayIdxLocal = 0;
        u32BufIdxLocal ++;
    ...
    HSUSBD_MemCopy((uint8_t *)&PcmPlayBuff[u32BufIdxLocal][u32PlayIdxLocal], (uint8_t *)g_au32UsbTmpBuf, (tmp << 2));

    /* Write back updated indices */
    u32BufPlayIdx = u32BufIdxLocal;
    u32PlayBufPos = u32PlayIdxLocal + tmp;
    g_usbd_rxflag = 0;
}
...
void UAC_SendRecData(void)
{
    ...
    if (u8PcmRxBufFull[u32BufRecIdx])
    {
        ...
        uint32_t u32RecSIdxLocal = g_resample_rec_s_idx;

        /* Audio data is resampled to 48MHz (Audio Codec is fixed to 48MHz) */
        /* Do Resample operation to data in USB buffer to SRAM buffer */
        if (REC_CHANNELS == 2)
        {
            Resamples(E_RS_REC_CH0, p16Src, REC_CHANNELS, srt[u32RecSIdxLocal].count, g_a16AudioTmpBuf0, u32RecSIdxLocal);
            Resamples(E_RS_REC_CH1, &p16Src[1], REC_CHANNELS, srt[u32RecSIdxLocal].count, &g_a16AudioTmpBuf0[1], u32RecSIdxLocal);
        }
        else
            Resamples(E_RS_REC_CH0, p16Src, REC_CHANNELS, srt[u32RecSIdxLocal].count, g_a16AudioTmpBuf0, u32RecSIdxLocal);
