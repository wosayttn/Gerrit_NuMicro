...
#define USPI_TRANSFER_TIMEOUT  SystemCoreClock

#define SAMPLE_CODE_HOLD_FOREVER() do { \
        for (;;) { \
            __WFI(); \
        } \
    } while (0)
...
volatile uint32_t g_u32TransferError;

/*----------------------------------------------------------------------------------------------------------*/
/* Define Function Prototypes                                                                               */
/*----------------------------------------------------------------------------------------------------------*/
void SYS_Init(void);
int32_t USCI_SPI_Init(void);
...
int main()
{
    uint32_t u32TxDataCount, u32RxDataCount, u32LastRxDataCount, u32Timeout;
    ...
    u32LastRxDataCount = g_u32RxDataCount;
    u32Timeout = USPI_TRANSFER_TIMEOUT;

    while ((g_u32RxDataCount < TEST_COUNT) && (g_u32TransferError == 0UL))
    {
        if (u32LastRxDataCount != g_u32RxDataCount)
        {
            u32LastRxDataCount = g_u32RxDataCount;
            u32Timeout = USPI_TRANSFER_TIMEOUT;
        }
        ...
        {
            printf("USCI_SPI slave interrupt transfer timed out.\n");
            goto sample_exit;
        }
    }

    if (g_u32TransferError != 0UL)
    {
        printf("USCI_SPI slave interrupt transfer aborted due to unexpected RX data.\n");
        goto sample_exit;
    }
...
sample_exit:
    ...
    if ((g_u32TransferError == 0UL) && (g_u32RxDataCount == TEST_COUNT))
    {
...
int32_t USCI_SPI_Init(void)
{
    ...
    if (USPI_Open(USPI0, USPI_SLAVE, USPI_MODE_0, 16, 0) == 0UL)
    {
        return -1;
    }
