...
int main(void)
{
    ...
    PWM_ConfigOutputChannel(PWM1, 0, 300, 30);
    ...
    PDMA_SetTransferAddr(PDMA0, 0, (uint32_t)&g_au16Period[0], PDMA_SAR_INC, (uint32_t) & (PWM1->PERIOD[0]), PDMA_DAR_FIX);

    /* Select PDMA request source as PWM1_CH0_TX(PWM1 channel 0 accumulator interrupt) */
    PDMA_SetTransferMode(PDMA0, 0, PDMA_PWM1_CH0_TX, FALSE, 0);
