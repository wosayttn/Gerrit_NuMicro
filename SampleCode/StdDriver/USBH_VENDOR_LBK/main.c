...
void demo_interrupt_xfer(void)
{
    ...
    for (loop = 1; ; loop++)
    {
        ...
        if (get_ticks() - msg_tick >= 100)
        {
            int read_IntInCnt = s_i8IntInCnt;
            int read_IntOutCnt = s_i8IntOutCnt;
            printf("Interrupt transfer loop %d RX: %d, TX: %d    \r", loop, read_IntInCnt, read_IntOutCnt);
            msg_tick = get_ticks();
        }
    }
}
...
void demo_isochronous_xfer(void)
{
    ...
    for (loop = 1; ; loop++)
    {
        ...
        if (get_ticks() - msg_tick >= 100)
        {
            int read_IsoInCnt = s_i8IsoInCnt;
            int read_IsoOutCnt = s_i8IsoOutCnt;
            printf("Isochronous transfer loop %d RX: %d, TX: %d    \r", loop, read_IsoInCnt, read_IsoOutCnt);
            msg_tick = get_ticks();
        }
    }
}
