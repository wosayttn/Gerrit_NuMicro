...
//------------------------------------------------------------------------------
/**
 * @brief    PDMA0 interrupt handler.
 *
 * @details  The PDMA0 interrupt handler is used to handle the PDMA transfer done interrupts for both TX and RX channels.
 *           It checks the interrupt status, clears the corresponding flags, and updates the buffer indices for TX and RX.
 *           If the RX channel transfer is done, it sets a flag to indicate that data is ready to be copied from RX to TX buffer.
 */
...
/*---------------------------------------------------------------------------------------------------------*/
/**
 * @brief    Write 9-bit data to 7-bit address register of NAU8822 via I2C.
 *
 * @return   None
 *
 * @details  This function sends a 9-bit data to a 7-bit address register of the NAU8822 audio codec using the I2C interface.
 */
void I2C_WriteNAU8822(uint8_t u8Addr, uint16_t u16Data)
{
    I2C_START(I2C_PORT);
 ...
 */
t8_t I2C_WriteMultiByteforNAU88L25(uint8_t u8ChipAddr, uint16_t u16SubAddr, const uint8_t *p, uint32_t u32Len)
{
    (void)u32Len;
 ...
 *
 * @details  This function sends a 16-bit data value to a specified register
 *           of the NAU88L25 audio codec using the I2C interface.
 */
t8_t I2C_WriteNAU88L25(uint16_t u16Addr, uint16_t u16Dat)
{
    uint8_t u8TxData0[2];
