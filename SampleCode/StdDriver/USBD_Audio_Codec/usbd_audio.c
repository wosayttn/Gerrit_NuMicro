...
void EP3_Handler(void)
{
    ...
    uint32_t u32PayloadLen = USBD_GET_PAYLOAD_LEN(EP3);

    /* ISO OUT transfer ACK */
    UAC_GetPlayData((uint8_t *)((uint32_t)USBD_BUF_BASE + u32BufAddr), u32PayloadLen);
    ...
    if (buf[0] & 0x80)
    {
        ...
        switch (buf[1])
        {
            case UAC_GET_CUR:
            {
                if ((buf[0] & 0x0f) == 0x2)
                {
                    u32SampleRate = g_usbd_SampleRate;

                    M8(USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR(EP0)) = (uint8_t)u32SampleRate;
                    M8(USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR(EP0) + 1) = (uint8_t)(u32SampleRate >> 8);
                    M8(USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR(EP0) + 2) = (uint8_t)(u32SampleRate >> 16);
                    /* Data stage */
                    USBD_SET_DATA1(EP0);
...
void UAC_GetPlayData(uint8_t *pu8Src, uint32_t u32Samples)
{
    uint32_t i, u32len;
    uint32_t u32BuffLen;
    uint32_t u32BufPlayIdx, u32PlayBufPos;
    uint8_t u8AudioPlaying, u8TxDataCntInBuffer;
    uint32_t *pu32Buff;
    uint32_t *pu32Src;

    /* if buffer has enough data, play it!! */
    u8AudioPlaying = g_u8AudioPlaying;
    u8TxDataCntInBuffer = g_u8TxDataCntInBuffer;
    ...
    if ((s_u32PlayBufPos + u32len) > u32BuffLen)
    {
        s_u32PlayBufPos = 0;
        s_u32BufPlayIdx ++;
    ...
    u32BufPlayIdx = s_u32BufPlayIdx;
    u32PlayBufPos = s_u32PlayBufPos;
    pu32Buff = &g_au32PcmPlayBuff[u32BufPlayIdx][u32PlayBufPos];
...
void UAC_SendRecData(void)
{
    ...
    if (g_au8PcmRxBufFull[s_u32BufRecIdx])
    {
        ...
        u32BufAddr = USBD_GET_EP_BUF_ADDR(EP2);
        u32RxBuffLen = g_u32RxBuffLen;

        USBD_MemCopy((uint8_t *)((uint32_t)USBD_BUF_BASE + u32BufAddr), (void *)&g_au8PcmRecBuff[s_u32BufRecIdx][0], u32RxBuffLen);
