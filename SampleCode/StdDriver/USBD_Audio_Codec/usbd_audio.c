...
void EP3_Handler(void)
{
    ...
    UAC_GetPlayData((uint8_t *)((uint32_t)USBD_BUF_BASE + u32BufAddr), u32PayLen);
    ...
    if (buf[0] & 0x80)
    {
        ...
        switch (buf[1])
        {
            case UAC_GET_CUR:
            {
                if ((buf[0] & 0x0f) == 0x2)
                {
                    uint32_t u32Rate = g_usbd_SampleRate;
                    uint32_t u32EpBuf = USBD_GET_EP_BUF_ADDR(EP0);

                    M8(USBD_BUF_BASE + u32EpBuf)     = (uint8_t)u32Rate;
                    M8(USBD_BUF_BASE + u32EpBuf + 1) = (uint8_t)(u32Rate >> 8);
                    M8(USBD_BUF_BASE + u32EpBuf + 2) = (uint8_t)(u32Rate >> 16);
                    /* Data stage */
                    USBD_SET_DATA1(EP0);
...
void UAC_GetPlayData(uint8_t *pu8Src, uint32_t u32Samples)
{
    ...
    /* Ring buffer check */
    {
        uint32_t u32PlayBufPos = s_u32PlayBufPos;
        uint32_t u32BuffLen    = g_u32BuffLen;
    ...
    {
        uint32_t u32BufIdx  = s_u32BufPlayIdx;
        uint32_t u32BufPos  = s_u32PlayBufPos;

        pu32Buff = &g_au32PcmPlayBuff[u32BufIdx][u32BufPos];
    }
...
void UAC_SendRecData(void)
{
    ...
    {
        uint32_t u32BufIdx  = s_u32BufRecIdx;
        uint32_t u32EpBuf   = USBD_GET_EP_BUF_ADDR(EP2);
        uint32_t u32RxLen   = g_u32RxBuffLen;

        /* Set empty flag */
        g_au8PcmRxBufFull[u32BufIdx] = 0;

        USBD_MemCopy((uint8_t *)((uint32_t)USBD_BUF_BASE + u32EpBuf), (void *)&g_au8PcmRecBuff[u32BufIdx][0], u32RxLen);
        USBD_SET_PAYLOAD_LEN(EP2, u32RxLen);
