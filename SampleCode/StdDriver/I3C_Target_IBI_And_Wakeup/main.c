 ...
 * @file     main.c
 * @version  V3.00
 * @brief    Demonstrate how to issue an In-Band Interrupt request to an I3C Controller, and 
 *           Wake-up the MCU from Power-down mode after receiving read/write request from the Controller.
...
static void SystemEnterToPowerDonw(void)
{
    volatile uint32_t u32TimeOutCnt;
    
    /* Enter to Power-down mode */
    printf("\n[ System enter to Power-down mode ]\n\n");

    /* Check if all the debug messages are finished */
    u32TimeOutCnt = SystemCoreClock; /* About to 1 second time-out */
...
/**
  * @brief  Polling a specified char for IBI and Wake-up test.
  */
void ParseUartInputChar(I3C_DEVICE_T *dev)
{
    char        mode;
    ...
    if((DEBUG_PORT->FIFOSTS&UART_FIFOSTS_RXEMPTY_Msk) == 0U)
    {
        ...
        do {
            ...
            if(mode == 'i')
            {
                ...
                IBI_MDB = 0xA0;
                IBI_PL = 0xA4A3A2A1;
                printf("\nSend IBI MDB 0x%02x and Payload 0x%02x-0x%02x to Controller.\n", 
            ...
            if(mode == 'I')
            {
                ...
                IBI_MDB = 0xB1;
                IBI_PL = 0xB8B7B6B5;
                printf("\nSend IBI MDB 0x%02x and Payload 0x%02x-0x%02x-0x%02x-0x%02x to Controller.\n", 
                    IBI_MDB, ((IBI_PL>>0)&0xFF), ((IBI_PL>>8)&0xFF), ((IBI_PL>>16)&0xFF), ((IBI_PL>>24)&0xFF));
            }
            
            dev->target_index = 0; // Use main Target
            dev->ibi_type     = I3C_IBI_TYPE_SIR;
            dev->ibi_MDB      = IBI_MDB;
            dev->ibi_len      = IBI_PLLen;
            dev->ibi_payload  = IBI_PL;
...
int main(void)
{
    ...
    printf("\n\nCPU @ %d Hz\n", SystemCoreClock);
    printf("+--------------------------------------------------------------------+\n");
    printf("|    I3C Target In-Band Interrupt Request and Wake-up Sample Code    |\n");
    printf("+--------------------------------------------------------------------+\n");
    ...
    printf("    - An I3C Controller send one byte of data 0x50 to I3C Target\n");
    printf("    - User can hit [p/P] to enter power-down\n");
    printf("# How to wake-up the system ?\n");
    printf("    - An I3C Controller can perform any private write/read operation to I3C Target\n");
    printf("# How to trigger an IBI request ?\n");
    ...
    while(1)
    {
        if(g_I3CDev.intsts != 0)
        {
            ...
            if(g_I3CDev.intsts & I3C_INTSTS_IBI_UPDATED)                               
        ...
        /* Polling a specified char for IBI and Wake-up test */
        ParseUartInputChar(&g_I3CDev);

        /* Check if enter to Power-Down */
        if(u8InPWD)
        {            
            u8InPWD = 0;
            SystemEnterToPowerDonw();
...
void SYS_Init(void)
{
   ...
   /* Enable UART0 module clock */
    ...
    CLK_SetHCLK(CLK_CLKSEL0_HCLKSEL_HXT, CLK_CLKDIV0_HCLK(1UL));
