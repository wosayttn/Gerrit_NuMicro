...
#define SPI_TRANSFER_TIMEOUT     (SystemCoreClock)
...
int32_t SPI_Init(void)
{
    ...
    if (SPI_Open(SPI0, SPI_MASTER, SPI_MODE_0, 32, 2000000) == 0U)
    {
        return -1;
    }
...
{
    /* Check RX EMPTY flag */
    while ((SPI_GET_RX_FIFO_EMPTY_FLAG(SPI0) == 0) && (g_u32RxDataCount < TEST_COUNT))
    {
        /* Read RX FIFO */
    ...
    if (SPI_GetIntFlag(SPI0, SPI_FIFO_RXTO_INT_MASK))
    {
        ...
        while (((SPI0->STATUS & SPI_STATUS_RXEMPTY_Msk) == 0) && (g_u32RxDataCount < TEST_COUNT))
            g_au32DestinationData[g_u32RxDataCount++] = SPI_READ_RX(SPI0);
...
int main(void)
{
    ...
    printf("NOTE: This sample uses a bounded wait for demo recovery only. Add application-specific timeout handling before production use.\n");
    ...
    g_u32TxDataCount = 0;
    g_u32RxDataCount = 0;
    g_u32TransferError = 0U;
    NVIC_EnableIRQ(SPI0_IRQn);

    /* Wait for transfer done */
    u32Timeout = SPI_TRANSFER_TIMEOUT;

    while ((g_u32RxDataCount < TEST_COUNT) && (g_u32TransferError == 0U) && (u32Timeout > 0U))
    {
        u32Timeout--;
    }

    if (g_u32TransferError != 0U)
    {
        printf("SPI receive overflow detected.\n");
        goto sample_exit;
    }
