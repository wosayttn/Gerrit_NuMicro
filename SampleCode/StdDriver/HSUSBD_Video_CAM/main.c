...
int32_t main(void)
{
    ...
    while (1)
    {
        ...
        if (gAppConnected == 1)
        {
            ...
            {
                uint32_t u32XferBufAddr = s_u32XferBufAddr;
                uint32_t u32XferSize    = s_u32XferSize;

#if (NVT_DCACHE_ON == 1)
                // Invalidate D-Cache to ensure CPU reads the latest image data written by CCAP
                SCB_InvalidateDCache_by_Addr((void *)u32XferBufAddr, u32XferSize);
#endif
                UVC_SendImage(u32XferBufAddr, u32XferSize, uvcStatus.StillImage);
