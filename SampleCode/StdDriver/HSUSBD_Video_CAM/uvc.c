    ...
    if (gUsbCmd.bmRequestType & 0x80)
    {
        ...
        else if (gUsbCmd.wIndex == 0x0001)  //VideoStreaming interface(0x01)
        {
            if ((gUsbCmd.wValue == VS_PROBE_CONTROL) || (gUsbCmd.wValue == VS_COMMIT_CONTROL))
            {
                ...
                {
                    uint32_t u32FmtIdx   = uvcInfo.VSCmdCtlData.bFormatIndex;
                    uint32_t u32FrmIdx   = uvcInfo.VSCmdCtlData.bFrameIndex;
                    uint32_t u32CurFmt   = uvcInfo.u32FormatIndx;
                    uint32_t u32CurFrm   = uvcInfo.u32FrameIndx;

                    if ((u32FmtIdx != u32CurFmt) || (u32FrmIdx != u32CurFrm))
                    {
                        uvcInfo.u32FormatIndx = u32FmtIdx;
                        uvcStatus.FormatIndex = u32FmtIdx;
                        uvcInfo.u32FrameIndx  = u32FrmIdx;
                        uvcStatus.FrameIndex  = u32FrmIdx;
...
{
    gAltInterface = u32AltInterface;
    gAppConnected = u32AltInterface;
...
void UVC_IsoIn(uint32_t u32Addr, uint32_t u32Len)
{
    ...
    uint32_t i, header;
    uint8_t u8UVC_PD_Local;
    uint8_t u8UVC_FID_Local;
    uint8_t *ptr;
    int32_t ret;
    ...
    for (i = 0; i < u32Loop; i++)
    {
        ...
        u8UVC_PD_Local  = g_u8UVC_PD;
        u8UVC_FID_Local = g_u8UVC_FID;
        header = ((u8UVC_PD_Local | (u8UVC_FID_Local & 0x01)) << 8) | 0x02;
    ...
    if (u32Loop)
    {
        ...
        u8UVC_PD_Local  = g_u8UVC_PD;
        u8UVC_FID_Local = g_u8UVC_FID;
        header = ((u8UVC_PD_Local | UVC_PH_EndOfFrame | (u8UVC_FID_Local & 0x01)) << 8) | 0x02; //End Of Frame
