...
void UAC_GetPlayData(void)
{
    ...
    uint32_t u32PlayBufPosLocal;
    uint32_t u32BufPlayIdxLocal;
    ...
    u32PlayBufPosLocal = u32PlayBufPos;
    u32BufPlayIdxLocal = u32BufPlayIdx;
    u32BuffLenLocal    = u32BuffLen;

    /* Ring buffer check */
    if ((u32PlayBufPosLocal + tmp) > u32BuffLenLocal)
    {
        PcmPlayBuffLen[u32BufPlayIdxLocal] = u32PlayBufPosLocal;
        u32PlayBufPosLocal = 0;
        u32BufPlayIdxLocal ++;

        /* change buffer index */
        if (u32BufPlayIdxLocal >= PDMA_TXBUFFER_CNT)
            u32BufPlayIdxLocal = 0;

        /* increase data count in buffer */
        u8TxDataCntInBuffer ++;
    }

    /* Write back the updated index before starting DMA */
    u32PlayBufPos = u32PlayBufPosLocal;
    u32BufPlayIdx = u32BufPlayIdxLocal;
    ...
    HSUSBD_SET_DMA_ADDR((uint32_t)&PcmPlayBuff[u32BufPlayIdxLocal][u32PlayBufPosLocal]);
    ...
    u32PlayBufPos = u32PlayBufPosLocal + tmp;
    g_usbd_rxflag = 0;
}
