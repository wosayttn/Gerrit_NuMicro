 ...
 * pointer always stays inside g_au8TestPages, so erase/read/write can never
 * access memory out of bounds regardless of the page/base parameters used.
 */
static uint8_t *GetPageBufAddr(uint32_t u32Page)
{
    return g_au8TestPages + ((u32Page % TEST_PAGES) * PAGE_SIZE);
}
...
int32_t HID_CmdEraseSectors(CMD_T *psCmd)
{
    ...
    /* Erase sector-by-sector through the bounded mapping so the access can
       never fall outside g_au8TestPages, even if the host addresses a sector
...
int32_t HID_CmdReadPages(CMD_T *psCmd)
{
    ...
    if (u32Pages)
    {
        ...
        memcpy(g_au8PageBuff, GetPageBufAddr(0), sizeof(g_au8PageBuff));
...
void HID_GetOutReport(uint8_t *pu8EpBuf, uint32_t u32Size)
{
    ...
    if ((u8Cmd == HID_CMD_WRITE) && (u32PageCnt < u32Pages))
    {
        ...
        if (g_u32BytesInPageBuf >= PAGE_SIZE)
        {
            ...
            memcpy(GetPageBufAddr(u32PageCnt), g_au8PageBuff, sizeof(g_au8PageBuff));
    ...
    if (u8Cmd == HID_CMD_READ)
    {
        ...
        else
        {
            ...
            if (g_u32BytesInPageBuf == 0)
            {
                ...
                memcpy(g_au8PageBuff, GetPageBufAddr(u32PageCnt), sizeof(g_au8PageBuff));
