...
void VCOM_TransferData(void)
{
    ...
    if (g_u32TxSize0 == 0)
    {
        ...
        if (g_u16ComRbytes0)
        {
            ...
            for (i = 0; i < u32Len; i++)
            {
                uint16_t u16Head = g_u16ComRhead0;

                if (u16Head >= RXBUFSIZE)
                    u16Head = 0;

                g_au8RxBuf0[i] = g_au8ComRbuf0[u16Head];
                g_u16ComRhead0 = (uint16_t)(u16Head + 1);
            }

            __set_PRIMASK(1);
            {
                uint16_t u16Bytes = g_u16ComRbytes0;
                g_u16ComRbytes0 = (uint16_t)(u16Bytes - u32Len);
            }
            __set_PRIMASK(0);
    ...
    if (g_u32TxSize1 == 0)
    {
        ...
        if (g_u16ComRbytes1)
        {
            ...
            for (i = 0; i < u32Len; i++)
            {
                uint16_t u16Head = g_u16ComRhead1;

                if (u16Head >= RXBUFSIZE)
                    u16Head = 0;

                g_au8RxBuf1[i] = g_au8ComRbuf1[u16Head];
                g_u16ComRhead1 = (uint16_t)(u16Head + 1);
            }

            __set_PRIMASK(1);
            {
                uint16_t u16Bytes = g_u16ComRbytes1;
                g_u16ComRbytes1 = (uint16_t)(u16Bytes - u32Len);
            }
            __set_PRIMASK(0);
    ...
    {
        int8_t   i8BulkOutReady = g_i8BulkOutReady0;
        uint32_t u32RxSize      = g_u32RxSize0;
        uint16_t u16ComTbytes   = g_u16ComTbytes0;

    if (i8BulkOutReady && (u32RxSize <= (uint32_t)(TXBUFSIZE - u16ComTbytes)))
    {
        for (i = 0; i < g_u32RxSize0; i++)
        {
            uint16_t u16Tail = g_u16ComTtail0;

            g_au8ComTbuf0[u16Tail] = g_pu8RxBuf0[i];
            u16Tail++;

            if (u16Tail >= TXBUFSIZE)
                u16Tail = 0;

            g_u16ComTtail0 = u16Tail;
        }

        __set_PRIMASK(1);
        {
            uint16_t u16Bytes = g_u16ComTbytes0;
            g_u16ComTbytes0 = (uint16_t)(u16Bytes + g_u32RxSize0);
        }
        __set_PRIMASK(0);
    ...
    {
        int8_t   i8BulkOutReady = g_i8BulkOutReady1;
        uint32_t u32RxSize      = g_u32RxSize1;
        uint16_t u16ComTbytes   = g_u16ComTbytes1;

    if (i8BulkOutReady && (u32RxSize <= (uint32_t)(TXBUFSIZE - u16ComTbytes)))
    {
        for (i = 0; i < g_u32RxSize1; i++)
        {
            uint16_t u16Tail = g_u16ComTtail1;

            g_au8ComTbuf1[u16Tail] = g_pu8RxBuf1[i];
            u16Tail++;

            if (u16Tail >= TXBUFSIZE)
                u16Tail = 0;

            g_u16ComTtail1 = u16Tail;
        }

        __set_PRIMASK(1);
        {
            uint16_t u16Bytes = g_u16ComTbytes1;
            g_u16ComTbytes1 = (uint16_t)(u16Bytes + g_u32RxSize1);
        }
        __set_PRIMASK(0);
    ...
    if (g_u16ComTbytes0)
    {
        ...
        {
            uint16_t u16Head = g_u16ComThead0;
            uint8_t  u8Data;
            ...
            u8Data = g_au8ComTbuf0[u16Head];
            UART4->DAT = u8Data;
            g_u16ComThead0 = (uint16_t)(u16Head + 1);
    ...
    if (g_u16ComTbytes1)
    {
        ...
        {
            uint16_t u16Head = g_u16ComThead1;
            uint8_t  u8Data;
            ...
            u8Data = g_au8ComTbuf1[u16Head];
            UART1->DAT = u8Data;
            g_u16ComThead1 = (uint16_t)(u16Head + 1);
