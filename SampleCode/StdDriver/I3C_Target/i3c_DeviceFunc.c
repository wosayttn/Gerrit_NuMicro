...
int32_t I3C_FuncIBIReceived(I3C_DEVICE_T *dev)
{
    ...
    else
    {
        if(dev->ibi_type == I3C_IBI_TYPE_TIR)
        {
            ...
            printf("#IBI: Process In-Band Interrupt from Target-0x%02x and get MDB 0x%02x, %d-bytes data.\n\t", 
...
/**
  * @brief  Send Controller Role Request to Controller.
  */
