...
void DumpCurrentTargetInfo(I3C_DEVICE_T *dev);
...
/**
  ...
  */
void I3C_TargetRole(I3C_DEVICE_T *dev)
{
    volatile uint32_t i;
...
#endif
                I3C_SetDeviceAddr(dev->port, 0, I3C_DEVTYPE_I3C, I3C0_CTR_DA, 0x0);
                dev->target_da[0] = I3C0_CTR_DA;
                printf("\nValid Target's DA on the bus: \n\t");

                for (i = 0; i < dev->target_count; i++)
                {
                    printf("0x%02x, ", ((dev->port->DEVADR[i] >> I3C_DEVADR_DADR_Pos) & 0x7F));
                }
