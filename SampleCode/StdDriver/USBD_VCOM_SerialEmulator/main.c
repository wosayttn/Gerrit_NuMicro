...
void VCOM_TransferData(void)
{
    ...
    if (g_u32TxSize == 0)
    {
        ...
        if (g_u16ComRbytes)
        {
            ...
            for (i = 0; i < u32Len; i++)
            {
                uint16_t u16Head = g_u16ComRhead;

                if (u16Head >= RXBUFSIZE)
                    u16Head = 0;

                g_au8RxBuf[i] = g_au8ComRbuf[u16Head];
                g_u16ComRhead = (uint16_t)(u16Head + 1);
            }

            __set_PRIMASK(1);
            {
                uint16_t u16Bytes = g_u16ComRbytes;
                g_u16ComRbytes = (uint16_t)(u16Bytes - u32Len);
            }
            __set_PRIMASK(0);
    ...
    {
        int8_t   i8BulkOutReady = g_i8BulkOutReady;
        uint32_t u32RxSize      = g_u32RxSize;
        uint16_t u16ComTbytes   = g_u16ComTbytes;

    if (i8BulkOutReady && (u32RxSize <= (uint32_t)(TXBUFSIZE - u16ComTbytes)))
    {
        for (i = 0; i < g_u32RxSize; i++)
        {
            uint16_t u16Tail = g_u16ComTtail;

            g_au8ComTbuf[u16Tail] = g_pu8RxBuf[i];
            u16Tail++;

            if (u16Tail >= TXBUFSIZE)
                u16Tail = 0;

            g_u16ComTtail = u16Tail;
        }

        __set_PRIMASK(1);
        {
            uint16_t u16Bytes = g_u16ComTbytes;
            g_u16ComTbytes = (uint16_t)(u16Bytes + g_u32RxSize);
        }
        __set_PRIMASK(0);
    ...
    if (g_u16ComTbytes)
    {
        ...
        {
            uint16_t u16Head = g_u16ComThead;
            uint8_t  u8Data;
            ...
            u8Data = g_au8ComTbuf[u16Head];
            UART4->DAT = u8Data;
            g_u16ComThead = (uint16_t)(u16Head + 1);
