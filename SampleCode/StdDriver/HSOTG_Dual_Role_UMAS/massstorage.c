...
NVT_ITCM void HSUSBD_IRQHandler(void)
{
    ...
    {
        IntEn = HSUSBD->EP[EPA].EPINTEN;
        IrqSt = HSUSBD->EP[EPA].EPINTSTS & IntEn;
    ...
    {
        IntEn = HSUSBD->EP[EPB].EPINTEN;
        IrqSt = HSUSBD->EP[EPB].EPINTSTS & IntEn;
    ...
    {
        IntEn = HSUSBD->EP[EPC].EPINTEN;
        IrqSt = HSUSBD->EP[EPC].EPINTSTS & IntEn;
        HSUSBD_CLR_EP_INT_FLAG(EPC, IrqSt);
    }

    if (IrqStL & HSUSBD_GINTSTS_EPDIF_Msk)
    {
        IntEn = HSUSBD->EP[EPD].EPINTEN;
        IrqSt = HSUSBD->EP[EPD].EPINTSTS & IntEn;
        HSUSBD_CLR_EP_INT_FLAG(EPD, IrqSt);
    }

    if (IrqStL & HSUSBD_GINTSTS_EPEIF_Msk)
    {
        IntEn = HSUSBD->EP[EPE].EPINTEN;
        IrqSt = HSUSBD->EP[EPE].EPINTSTS & IntEn;
        HSUSBD_CLR_EP_INT_FLAG(EPE, IrqSt);
    }

    if (IrqStL & HSUSBD_GINTSTS_EPFIF_Msk)
    {
        IntEn = HSUSBD->EP[EPF].EPINTEN;
        IrqSt = HSUSBD->EP[EPF].EPINTSTS & IntEn;
        HSUSBD_CLR_EP_INT_FLAG(EPF, IrqSt);
    }

    if (IrqStL & HSUSBD_GINTSTS_EPGIF_Msk)
    {
        IntEn = HSUSBD->EP[EPG].EPINTEN;
        IrqSt = HSUSBD->EP[EPG].EPINTSTS & IntEn;
        HSUSBD_CLR_EP_INT_FLAG(EPG, IrqSt);
    }

    if (IrqStL & HSUSBD_GINTSTS_EPHIF_Msk)
    {
        IntEn = HSUSBD->EP[EPH].EPINTEN;
        IrqSt = HSUSBD->EP[EPH].EPINTSTS & IntEn;
        HSUSBD_CLR_EP_INT_FLAG(EPH, IrqSt);
    }

    if (IrqStL & HSUSBD_GINTSTS_EPIIF_Msk)
    {
        IntEn = HSUSBD->EP[EPI].EPINTEN;
        IrqSt = HSUSBD->EP[EPI].EPINTSTS & IntEn;
        HSUSBD_CLR_EP_INT_FLAG(EPI, IrqSt);
    }

    if (IrqStL & HSUSBD_GINTSTS_EPJIF_Msk)
    {
        IntEn = HSUSBD->EP[EPJ].EPINTEN;
        IrqSt = HSUSBD->EP[EPJ].EPINTSTS & IntEn;
        HSUSBD_CLR_EP_INT_FLAG(EPJ, IrqSt);
    }

    if (IrqStL & HSUSBD_GINTSTS_EPKIF_Msk)
    {
        IntEn = HSUSBD->EP[EPK].EPINTEN;
        IrqSt = HSUSBD->EP[EPK].EPINTSTS & IntEn;
        HSUSBD_CLR_EP_INT_FLAG(EPK, IrqSt);
    }

    if (IrqStL & HSUSBD_GINTSTS_EPLIF_Msk)
    {
        IntEn = HSUSBD->EP[EPL].EPINTEN;
        IrqSt = HSUSBD->EP[EPL].EPINTSTS & IntEn;
