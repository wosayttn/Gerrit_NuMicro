...
int main(void)
{
    uint32_t u32DataCount, u32Timeout;
    ...
    while (((g_u32RxDataCount < TEST_COUNT) || (g_u32TxDataCount < TEST_COUNT)) && (g_u32TransferError == 0UL))
    {
        if (u32Timeout-- == 0UL)
        {
            printf("USCI_SPI transfer timed out.\n");
            goto sample_exit;
        }
    }
    ...
    {
        printf("USCI_SPI transfer aborted due to unexpected RX state.\n");
        goto sample_exit;
    }
...
sample_exit:
    ...
    if ((g_u32TransferError == 0UL) && (g_u32RxDataCount == TEST_COUNT) && (g_u32TxDataCount == TEST_COUNT))
    {
...
int32_t USCI_SPI_Init(void)
{
    ...
    if (USPI_Open(USPI0, USPI_MASTER, USPI_MODE_0, 16, 2000000) == 0UL)
    {
        return -1;
    }
...
void USCI0_IRQHandler(void)
{
    ...
    uint32_t u32Timeout = USPI_TRANSFER_TIMEOUT;
