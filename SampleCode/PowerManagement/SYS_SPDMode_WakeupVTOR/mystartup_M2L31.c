...
void __Enter_SPD(void) {
    ...
    // Set the initial vector table address
	SYS->INIVTOR = 0x20000000;

    // Backup the stack pointer
	*psp_backup = __get_MSP();

    // Backup WakeUp address
    // Set the __SPD_Wakeup address at 0x20000004
	psp_backup++;
	*psp_backup = (uint32_t)__SPD_Wakeup;
