...
// Local Variables
NVT_NONCACHEABLE static            uint8_t       uart_rx_buf[UART_BUFFER_SIZE] __attribute__((aligned(4)));
NVT_NONCACHEABLE static            uint8_t       uart_tx_buf[UART_BUFFER_SIZE] __attribute__((aligned(4)));

static   volatile int32_t       uart_rx_cnt         =   0;
static   volatile int32_t       usb_tx_cnt          =   0;

static   void                  *cdc_acm_bridge_tid  =   0U;
static   CDC_LINE_CODING        cdc_acm_line_coding = { 0, 0U, 0U, 0U };
...
static void UART_Callback(uint32_t event)
{
    ...
    {
        // UART data timeout, recount buffer,and restart new reception
        uart_rx_cnt = ptrUART->GetRxCount();
        usb_tx_cnt = 0;
        ptrUART->Control(ARM_USART_ABORT_RECEIVE, 0);
        ptrUART->Receive(uart_rx_buf, UART_BUFFER_SIZE);
    }
    ...
    if (event & ARM_USART_EVENT_SEND_COMPLETE)
        printf("ARM_USART_EVENT_SEND_COMPLETE\n");

    if (event & ARM_USART_EVENT_RECEIVE_COMPLETE)
        printf("ARM_USART_EVENT_RECEIVE_COMPLETE\n");

    if (event & ARM_USART_EVENT_TRANSFER_COMPLETE)
        printf("ARM_USART_EVENT_TRANSFER_COMPLETE\n");

    if (event & ARM_USART_EVENT_TX_COMPLETE)
        printf("ARM_USART_EVENT_TX_COMPLETE\n");

    if (event & ARM_USART_EVENT_TX_UNDERFLOW)
        printf("ARM_USART_EVENT_TX_UNDERFLOW\n");

    if (event & ARM_USART_EVENT_RX_OVERFLOW)
        printf("ARM_USART_EVENT_RX_OVERFLOW\n");

    if (event & ARM_USART_EVENT_RX_TIMEOUT)
        printf("ARM_USART_EVENT_RX_TIMEOUT\n");

    if (event & ARM_USART_EVENT_RX_BREAK)
        printf("ARM_USART_EVENT_RX_BREAK\n");

    if (event & ARM_USART_EVENT_RX_FRAMING_ERROR)
        printf("ARM_USART_EVENT_RX_FRAMING_ERROR\n");

    if (event & ARM_USART_EVENT_RX_PARITY_ERROR)
        printf("ARM_USART_EVENT_RX_PARITY_ERROR\n");

    if (event & ARM_USART_EVENT_CTS)
        printf("ARM_USART_EVENT_CTS\n");

    if (event & ARM_USART_EVENT_DSR)
        printf("ARM_USART_EVENT_DSR\n");

    if (event & ARM_USART_EVENT_DCD)
        printf("ARM_USART_EVENT_DCD\n");

    if (event & ARM_USART_EVENT_RI)
        printf("ARM_USART_EVENT_RI\n");
...
void USBD_CDC0_ACM_Initialize(void)
{
    ...
    // If ARM_USART_EVENT_RX_TIMEOUT is needed, remove the comment marks on the two lines below
    //UART_SetTimeoutCnt(UART1, 255);
    //UART_BUS_IDLE_TIMEOUT_ENABLE(UART1);
