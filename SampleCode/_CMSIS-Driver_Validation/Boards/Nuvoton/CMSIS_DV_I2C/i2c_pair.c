...
{
    uint32_t i = 0, I2C_Event[2] = {event0, event1};

    for (i = 0; i < 2; i++)
    {
        uint32_t event = I2C_Event[i];
...
#if (I2C_VALIDATION_SET == 0) // I2C0/I2C1/I2C2
    i2c_pair_7bit(&Driver_I2C0, &Driver_I2C1);
    i2c_pair_GCmode(&Driver_I2C1, &Driver_I2C2);
    i2c_pair_10bit(&Driver_I2C2, &Driver_I2C0);
#elif (I2C_VALIDATION_SET == 1) // UI2C0/UI2C1

    i2c_pair_7bit(&Driver_I2C3, &Driver_I2C4);
    i2c_pair_GCmode(&Driver_I2C3, &Driver_I2C4);
    i2c_pair_10bit(&Driver_I2C3, &Driver_I2C4);
#elif (I2C_VALIDATION_SET == 2) // I3C0
    // I3C does not support 10-bit mode & GC mode.
    i2c_pair_7bit(&Driver_I2C5, &Driver_I2C1);
#endif
    return 0;
}
