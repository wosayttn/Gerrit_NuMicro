...
__attribute__((unused)) static uint32_t USART_Com_GetMdm(void);
...
__WEAK void USART_Server_Pins_Initialize(void)
{
    dcd_ri_mask = 0U;
}
__WEAK void USART_Server_Pins_Uninitialize(void) {}
__WEAK __attribute__((unused)) void USART_Server_Pin_DCD_SetState(uint32_t state);
__WEAK __attribute__((unused)) void USART_Server_Pin_RI_SetState(uint32_t state);
...
static int32_t USART_Cmd_GetCap(const char *cmd)
{
    ...
    for (i = 0; i < (int32_t)(sizeof(usart_baudrates) / sizeof(uint32_t)); i++)
    {
