...
{
    uint32_t u32EpSts   = USBD->EPSTS0 & USBD_EPSTS0_EPSTS3_Msk;
    VCOM_CONTROL_VAR_t *p = tVCOM[IDX_VCOM0].psCtrlVar;
    uint32_t u32Toggle  = p->u32OutToggle;
    ...
    {
        uint32_t u32PayLen  = USBD_GET_PAYLOAD_LEN(EP3);
        uint32_t u32BufAddr = USBD_GET_EP_BUF_ADDR(EP3);

        p->u32RxSize = u32PayLen;
        p->pu8RxBuf  = (uint8_t *)(USBD_BUF_BASE + u32BufAddr);
        p->u32OutToggle = u32EpSts;
...
{
    uint32_t u32EpSts   = USBD->EPSTS0 & USBD_EPSTS0_EPSTS5_Msk;
    VCOM_CONTROL_VAR_t *p = tVCOM[IDX_VCOM1].psCtrlVar;
    uint32_t u32Toggle  = p->u32OutToggle;
    ...
    {
        uint32_t u32PayLen  = USBD_GET_PAYLOAD_LEN(EP5);
        uint32_t u32BufAddr = USBD_GET_EP_BUF_ADDR(EP5);
...
{
    uint32_t u32EpSts   = USBD->EPSTS0 & USBD_EPSTS0_EPSTS7_Msk;
    VCOM_CONTROL_VAR_t *p = tVCOM[IDX_VCOM2].psCtrlVar;
    uint32_t u32Toggle  = p->u32OutToggle;
    ...
    {
        uint32_t u32PayLen  = USBD_GET_PAYLOAD_LEN(EP7);
        uint32_t u32BufAddr = USBD_GET_EP_BUF_ADDR(EP7);
...
{
    uint32_t u32EpSts   = USBD->EPSTS1 & USBD_EPSTS1_EPSTS9_Msk;
    VCOM_CONTROL_VAR_t *p = tVCOM[IDX_VCOM3].psCtrlVar;
    uint32_t u32Toggle  = p->u32OutToggle;
    ...
    {
        uint32_t u32PayLen  = USBD_GET_PAYLOAD_LEN(EP9);
        uint32_t u32BufAddr = USBD_GET_EP_BUF_ADDR(EP9);
...
{
    uint32_t u32EpSts   = USBD->EPSTS1 & USBD_EPSTS1_EPSTS11_Msk;
    VCOM_CONTROL_VAR_t *p = tVCOM[IDX_VCOM4].psCtrlVar;
    uint32_t u32Toggle  = p->u32OutToggle;
    ...
    {
        uint32_t u32PayLen  = USBD_GET_PAYLOAD_LEN(EP11);
        uint32_t u32BufAddr = USBD_GET_EP_BUF_ADDR(EP11);
...
{
    uint32_t u32EpSts   = USBD->EPSTS1 & USBD_EPSTS1_EPSTS13_Msk;
    VCOM_CONTROL_VAR_t *p = tVCOM[IDX_VCOM5].psCtrlVar;
    uint32_t u32Toggle  = p->u32OutToggle;
    ...
    {
        uint32_t u32PayLen  = USBD_GET_PAYLOAD_LEN(EP13);
        uint32_t u32BufAddr = USBD_GET_EP_BUF_ADDR(EP13);
...
{
    uint32_t u32EpSts   = USBD->EPSTS1 & USBD_EPSTS1_EPSTS15_Msk;
    VCOM_CONTROL_VAR_t *p = tVCOM[IDX_VCOM6].psCtrlVar;
    uint32_t u32Toggle  = p->u32OutToggle;
    ...
    {
        uint32_t u32PayLen  = USBD_GET_PAYLOAD_LEN(EP15);
        uint32_t u32BufAddr = USBD_GET_EP_BUF_ADDR(EP15);
