...
void VCOM_TransferData(void)
{
...
#endif
    {
        ...
        if (tVCOM[u32chidx].psCtrlVar->u32TxSize == 0)
        {
            ...
            if (tVCOM[u32chidx].psCtrlVar->u16ComRbytes)
            {
                ...
                if (i32Len > ENDPOINT_MAX_PKT_SIZE)
                    i32Len = ENDPOINT_MAX_PKT_SIZE;
...
#if (VCOM_CNT > MAX_PORT_UART)
    {
        ...
        if (tVCOM[IDX_VCOM5].psCtrlVar->u32TxSize == 0)
        {
            ...
            {
                i32Len = tVCOM[IDX_VCOM5].psCtrlVar->u16ComRbytes;
                ...
                __set_PRIMASK(1);
                tVCOM[IDX_VCOM5].psCtrlVar->u16ComRbytes -= i32Len;
                __set_PRIMASK(0);

                tVCOM[IDX_VCOM5].psCtrlVar->u32TxSize = i32Len;
                USBD_MemCopy((uint8_t *)(USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR(GET_BULK_IN_EP(IDX_VCOM5))), (uint8_t *)tVCOM[IDX_VCOM5].psCtrlVar->au8RxBuf, i32Len);
                USBD_SET_PAYLOAD_LEN(GET_BULK_IN_EP(IDX_VCOM5), i32Len);
            }
            else
            {
                /* Prepare a zero packet if previous packet size is ENDPOINT_MAX_PKT_SIZE and
                   no more data to send at this moment to note Host the transfer has been done */
                i32Len = USBD_GET_PAYLOAD_LEN(GET_BULK_IN_EP(IDX_VCOM5));
    ...
    */
    if (tVCOM[IDX_VCOM6].psCtrlVar->u32TxSize == 0)
    {
        ...
        {
            i32Len = tVCOM[IDX_VCOM6].psCtrlVar->u16ComTbytes;
            ...
            __set_PRIMASK(1);
            tVCOM[IDX_VCOM6].psCtrlVar->u16ComTbytes -= i32Len;
            __set_PRIMASK(0);

            tVCOM[IDX_VCOM6].psCtrlVar->u32TxSize = i32Len;
            USBD_MemCopy((uint8_t *)(USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR(GET_BULK_IN_EP(IDX_VCOM6))), (uint8_t *)tVCOM[IDX_VCOM6].psCtrlVar->au8RxBuf, i32Len);
            USBD_SET_PAYLOAD_LEN(GET_BULK_IN_EP(IDX_VCOM6), i32Len);
        }
        else
        {
            ...
            i32Len = USBD_GET_PAYLOAD_LEN(GET_BULK_IN_EP(IDX_VCOM6));
