 ...
 * @file     main.c
 * @version  V0.10
 * @brief    Demonstrate the implementation of virtual COM port device with 7 ports.
 *
...
#endif
#if (VCOM_CNT>=6)
    /* Set PA.9 and PA.10 multi-function pins for UUART0 TXD and RXD */
...
void UUART0_Init(void)
{
    ...
    /* Configure UUART0 and set UUART0 Baudrate */
...
void UART_IRQHandler(VCOM_CONTROL_BLOCK_t *tVCOM)
{
    ...
    uint8_t bInChar;
    UART_T *pUART = tVCOM->psPeripheralInfo->UART_BASE;
    u32IntStatus = pUART->INTSTS;
    ...
    if ((u32IntStatus & UART_INTSTS_RDAIF_Msk) || (u32IntStatus & UART_INTSTS_RXTOIF_Msk))
    {
        ...
        while ((pUART->FIFOSTS & UART_FIFOSTS_RXEMPTY_Msk) == 0)
        {
    ...
    if ((u32IntStatus & UART_INTSTS_THREIF_Msk))
    {
        ...
        if (tVCOM->psCtrlVar->u16ComTbytes
                && (pUART->INTEN & UART_INTEN_THREIEN_Msk))
...
void USCI0_IRQHandler(void)
{
    ...
    if (u32IntStatus & UUART_PROTSTS_RXENDIF_Msk)
    {
        ...
        while (!UUART_IS_RX_EMPTY(UUART0))
        {
            ...
            /* Check if buffer full */
            if (tVCOM[IDX_VCOM5].psCtrlVar->u16ComRbytes < RXBUFSIZE)
            {
                /* Enqueue the character */
                tVCOM[IDX_VCOM5].psCtrlVar->au8ComRbuf[tVCOM[IDX_VCOM5].psCtrlVar->u16ComRtail++] = bInChar;

                if (tVCOM[IDX_VCOM5].psCtrlVar->u16ComRtail >= RXBUFSIZE)
                    tVCOM[IDX_VCOM5].psCtrlVar->u16ComRtail = 0;

                tVCOM[IDX_VCOM5].psCtrlVar->u16ComRbytes++;
            }
            else
            {
                /* FIFO over run */
...
{
    uint32_t i, u32Len, u32chidx;
#if (VCOM_CNT <= MAX_PORT_UART) /* VCOM 0 ~ 4 using UART0 ~ 4 */

    for (u32chidx = 0; u32chidx < (VCOM_CNT); u32chidx++)
#else
    for (u32chidx = 0; u32chidx < MAX_PORT_UART; u32chidx++)
#endif
    {
        UART_T *pUart_base = (UART_T *)tVCOM[u32chidx].psPeripheralInfo->UART_BASE;
        ...
        if (tVCOM[u32chidx].psCtrlVar->u32TxSize == 0)
        {
            ...
            if (tVCOM[u32chidx].psCtrlVar->u16ComRbytes)
            {
                ...
                if (u32Len > ENDPOINT_MAX_PKT_SIZE)
                    u32Len = ENDPOINT_MAX_PKT_SIZE;
            ...
            {
                /* Prepare a zero packet if previous packet size is ENDPOINT_MAX_PKT_SIZE and
        ...
        if (tVCOM[u32chidx].psCtrlVar->i8BulkOutReady && (tVCOM[u32chidx].psCtrlVar->u32RxSize <= TXBUFSIZE - tVCOM[u32chidx].psCtrlVar->u16ComTbytes))
        {
            ...
            USBD_SET_PAYLOAD_LEN(GET_BULK_OUT_EP(u32chidx), ENDPOINT_MAX_PKT_SIZE);
        }
        ...
        if (tVCOM[u32chidx].psCtrlVar->u16ComTbytes)
        {
            ...
            if ((pUart_base->INTEN & UART_INTEN_THREIEN_Msk) == 0)
            {
                /* Send one bytes out */
                pUart_base->DAT = tVCOM[u32chidx].psCtrlVar->au8ComTbuf[tVCOM[u32chidx].psCtrlVar->u16ComThead++];
                ...
                pUart_base->INTEN |= UART_INTEN_THREIEN_Msk;
            }
        }

    }

#if (VCOM_CNT > MAX_PORT_UART)
    {
        /* for VCOM - 5 using USCI0 */
        if (tVCOM[IDX_VCOM5].psCtrlVar->u32TxSize == 0)
        {
            /* Check whether we have new COM Rx data to send to USB or not */
            if (tVCOM[IDX_VCOM5].psCtrlVar->u16ComRbytes)
            {
                u32Len = tVCOM[IDX_VCOM5].psCtrlVar->u16ComRbytes;
                ...
                for (i = 0; i < u32Len; i++)
                {
                    tVCOM[IDX_VCOM5].psCtrlVar->au8RxBuf[i] = tVCOM[IDX_VCOM5].psCtrlVar->au8ComRbuf[tVCOM[IDX_VCOM5].psCtrlVar->u16ComRhead++];

                    if (tVCOM[IDX_VCOM5].psCtrlVar->u16ComRhead >= RXBUFSIZE)
                        tVCOM[IDX_VCOM5].psCtrlVar->u16ComRhead = 0;
                }

                __set_PRIMASK(1);
                tVCOM[IDX_VCOM5].psCtrlVar->u16ComRbytes -= u32Len;
                __set_PRIMASK(0);

                tVCOM[IDX_VCOM5].psCtrlVar->u32TxSize = u32Len;
                USBD_MemCopy((uint8_t *)(USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR(GET_BULK_IN_EP(IDX_VCOM5))), (uint8_t *)tVCOM[IDX_VCOM5].psCtrlVar->au8RxBuf, u32Len);
                USBD_SET_PAYLOAD_LEN(GET_BULK_IN_EP(IDX_VCOM5), u32Len);
            }
            else
            {
                /* Prepare a zero packet if previous packet size is ENDPOINT_MAX_PKT_SIZE and
                   no more data to send at this moment to note Host the transfer has been done */
                u32Len = USBD_GET_PAYLOAD_LEN(GET_BULK_IN_EP(IDX_VCOM5));

                if (u32Len == ENDPOINT_MAX_PKT_SIZE)
                    USBD_SET_PAYLOAD_LEN(GET_BULK_IN_EP(IDX_VCOM5), 0);
            }
        }

        if (tVCOM[IDX_VCOM5].psCtrlVar->i8BulkOutReady && (tVCOM[IDX_VCOM5].psCtrlVar->u32RxSize <= TXBUFSIZE - tVCOM[IDX_VCOM5].psCtrlVar->u16ComTbytes))
        {
            for (i = 0; i < tVCOM[IDX_VCOM5].psCtrlVar->u32RxSize; i++)
            {
                tVCOM[IDX_VCOM5].psCtrlVar->au8ComTbuf[tVCOM[IDX_VCOM5].psCtrlVar->u16ComTtail++] = tVCOM[IDX_VCOM5].psCtrlVar->pu8RxBuf[i];

                if (tVCOM[IDX_VCOM5].psCtrlVar->u16ComTtail >= TXBUFSIZE)
                    tVCOM[IDX_VCOM5].psCtrlVar->u16ComTtail = 0;
            }

            //__set_PRIMASK(1); //not necessary in the same thread
            tVCOM[IDX_VCOM5].psCtrlVar->u16ComTbytes += tVCOM[IDX_VCOM5].psCtrlVar->u32RxSize;
            //__set_PRIMASK(0);

            tVCOM[IDX_VCOM5].psCtrlVar->u32RxSize = 0;
            tVCOM[IDX_VCOM5].psCtrlVar->i8BulkOutReady = 0; /* Clear bulk out ready flag */

            /* Ready to get next BULK out */
            USBD_SET_PAYLOAD_LEN(GET_BULK_OUT_EP(IDX_VCOM5), ENDPOINT_MAX_PKT_SIZE);
        }

        /* Check if data received from USB_OUT are not been transfered to USCI_UART(UUART)*/
        if (tVCOM[IDX_VCOM5].psCtrlVar->u16ComTbytes)
        {
            uint32_t size;
            UUART_T *pUart_base = (UUART_T *)tVCOM[IDX_VCOM5].psPeripheralInfo->UART_BASE;
            size = tVCOM[IDX_VCOM5].psCtrlVar->u16ComTbytes;

            while (size)
            {
                if (UUART_IS_TX_FULL(pUart_base)) break; /* Wait Tx is not full to transmit data */

                /* Send one bytes out */
                uint8_t bInChar = tVCOM[IDX_VCOM5].psCtrlVar->au8ComTbuf[tVCOM[IDX_VCOM5].psCtrlVar->u16ComThead++];
                UUART_WRITE(pUart_base, bInChar);

                if (tVCOM[IDX_VCOM5].psCtrlVar->u16ComThead >= TXBUFSIZE)
                    tVCOM[IDX_VCOM5].psCtrlVar->u16ComThead = 0;

                //__set_PRIMASK(1); //not necessary in the same thread
                tVCOM[IDX_VCOM5].psCtrlVar->u16ComTbytes--;
                //__set_PRIMASK(0);
                size--;
            }
        }
    ...
    */
    if (tVCOM[IDX_VCOM6].psCtrlVar->u32TxSize == 0)
    {
        /* Check whether we have new COM Rx data to send to USB or not */
        if (tVCOM[IDX_VCOM6].psCtrlVar->u16ComTbytes)
        {
            u32Len = tVCOM[IDX_VCOM6].psCtrlVar->u16ComTbytes;
            ...
            for (i = 0; i < u32Len; i++)
            {
                tVCOM[IDX_VCOM6].psCtrlVar->au8RxBuf[i] = tVCOM[IDX_VCOM6].psCtrlVar->au8ComTbuf[tVCOM[IDX_VCOM6].psCtrlVar->u16ComThead++];

                if (tVCOM[IDX_VCOM6].psCtrlVar->u16ComThead >= TXBUFSIZE)
                    tVCOM[IDX_VCOM6].psCtrlVar->u16ComThead = 0;
            }

            __set_PRIMASK(1);
            tVCOM[IDX_VCOM6].psCtrlVar->u16ComTbytes -= u32Len;
            __set_PRIMASK(0);

            tVCOM[IDX_VCOM6].psCtrlVar->u32TxSize = u32Len;
            USBD_MemCopy((uint8_t *)(USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR(GET_BULK_IN_EP(IDX_VCOM6))), (uint8_t *)tVCOM[IDX_VCOM6].psCtrlVar->au8RxBuf, u32Len);
            USBD_SET_PAYLOAD_LEN(GET_BULK_IN_EP(IDX_VCOM6), u32Len);
        }
        else
        {
            /* Prepare a zero packet if previous packet size is ENDPOINT_MAX_PKT_SIZE
            and no more data to send at this moment to note Host the transfer has been done */
            u32Len = USBD_GET_PAYLOAD_LEN(GET_BULK_IN_EP(IDX_VCOM6));

            if (u32Len == ENDPOINT_MAX_PKT_SIZE)
                USBD_SET_PAYLOAD_LEN(GET_BULK_IN_EP(IDX_VCOM6), 0);
        }
    }

    if (tVCOM[IDX_VCOM6].psCtrlVar->i8BulkOutReady && (tVCOM[IDX_VCOM6].psCtrlVar->u32RxSize <= TXBUFSIZE - tVCOM[IDX_VCOM6].psCtrlVar->u16ComTbytes))
    {
        for (i = 0; i < tVCOM[IDX_VCOM6].psCtrlVar->u32RxSize; i++)
        {
            tVCOM[IDX_VCOM6].psCtrlVar->au8ComTbuf[tVCOM[IDX_VCOM6].psCtrlVar->u16ComTtail++] = tVCOM[IDX_VCOM6].psCtrlVar->pu8RxBuf[i];

            if (tVCOM[IDX_VCOM6].psCtrlVar->u16ComTtail >= TXBUFSIZE)
                tVCOM[IDX_VCOM6].psCtrlVar->u16ComTtail = 0;
        }

        __set_PRIMASK(1);
        tVCOM[IDX_VCOM6].psCtrlVar->u16ComTbytes += tVCOM[IDX_VCOM6].psCtrlVar->u32RxSize;
        __set_PRIMASK(0);

        tVCOM[IDX_VCOM6].psCtrlVar->u32RxSize = 0;
        tVCOM[IDX_VCOM6].psCtrlVar->i8BulkOutReady = 0; /* Clear bulk out ready flag */

        /* Ready to get next BULK out */
        USBD_SET_PAYLOAD_LEN(GET_BULK_OUT_EP(IDX_VCOM6), ENDPOINT_MAX_PKT_SIZE);
    }
...
void PowerDown()
{
    ...
    while (!UART_IS_TX_EMPTY((UART_T *)tVCOM[IDX_VCOM0].psPeripheralInfo->UART_BASE));

#endif
#if (VCOM_CNT>=2)

    while (!UART_IS_TX_EMPTY((UART_T *)tVCOM[IDX_VCOM1].psPeripheralInfo->UART_BASE));

#endif
#if (VCOM_CNT>=3)

    while (!UART_IS_TX_EMPTY((UART_T *)tVCOM[IDX_VCOM2].psPeripheralInfo->UART_BASE));

#endif
#if (VCOM_CNT>=4)

    while (!UART_IS_TX_EMPTY((UART_T *)tVCOM[IDX_VCOM3].psPeripheralInfo->UART_BASE));

#endif
#if (VCOM_CNT>=5)

    while (!UART_IS_TX_EMPTY((UART_T *)tVCOM[IDX_VCOM4].psPeripheralInfo->UART_BASE));

#endif
#if (VCOM_CNT>=6)

    while (!UUART_IS_TX_EMPTY((UUART_T *)tVCOM[IDX_VCOM5].psPeripheralInfo->UART_BASE));
