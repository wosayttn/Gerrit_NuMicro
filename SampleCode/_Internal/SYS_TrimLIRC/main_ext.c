...
#define GET_LIRC_TRIM_VALUE           (inpw(0x40000110) & 0x3FF)
#define SET_LIRC_TRIM_VALUE(value)    (outpw(0x40000110, (inpw(0x40000110) & (~0x3FF)) | (value & 0x3FF)))
...
void TrimExtClk(uint32_t u32FreqInHz)
{
    uint32_t u32CapVal0 = 0, u32CapVal1 = 0, u32CapVal_Interval = 0;
    uint32_t u32TrimLoopIndex = 0, u32RCValue = 0;
    uint32_t u32Freq_PCLK0_DIV_LIRC = ((CLK_GetPCLK0Freq() * 10) / u32FreqInHz + 5) / 10;
    ...
    TIMER_SET_PRESCALE_VALUE(TIMER, 0x3);
    ...
    TIMER_EnableCaptureDebounce(TIMER);

    /* Select timer capture source from internal signal*/
    //TIMER_CaptureSelect(TIMER, TIMER_CAPTURE_FROM_EXTERNAL);
    ...
    SET_LIRC_TRIM_VALUE(0x3FF);//(u32RCValue - 0x30);
    ...
    while (1)
    {
        for (u32TrimLoopIndex = 0, u32CapVal_Interval = 0; u32TrimLoopIndex < TRIM_LOOP; u32TrimLoopIndex++)
        {
            ...
            if (u32CapVal1 > u32CapVal0)
                u32CapVal_Interval += (u32CapVal1 - u32CapVal0);
            else
                u32CapVal_Interval += ((0xFFFFFF - u32CapVal1) + u32CapVal0);
        }

        printf("adj: %d\n", (((u32CapVal_Interval * 10) / TRIM_LOOP + 5) / 10));
