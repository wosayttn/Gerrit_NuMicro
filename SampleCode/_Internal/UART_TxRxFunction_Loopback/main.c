...
void UART1_TEST_HANDLE(void)
{
    ...
    if (u32IntSts & UART_INTSTS_RDAINT_Msk)
    {
        ...
        while (!UART_GET_RX_EMPTY(UART1))
        {
            ...
            uint32_t u32RecLen    = g_u32RecLen;
            uint32_t u32RecOffset = g_u32RecOffset;
            uint8_t  u8Data       = (uint8_t)UART_READ(UART1);
            g_au8RecData[u32RecLen] = u8Data;

            if (u32RecLen == (BUFSIZE - 1 - u32RecOffset))
            {
                g_i32RecOK = TRUE;
                g_u32RecLen = 0;
            }
...
void UART_FunctionTest(void)
{
    ...
    */
    ...
    do
    {
        ...
        printf("+-------------------------------------------------------------------+\n");
        printf("|  (1)Loopback UART1(PB3)send data to UART1(PB2). RFITL: 1 byte     |\n");
        printf("|  (2)Loopback UART1(PB3)send data to UART1(PB2). RFITL: 4 bytes    |\n");
        printf("|  (3)Loopback UART1(PB3)send data to UART1(PB2). RFITL: 8 bytes    |\n");
        printf("|  (4)Loopback UART1(PB3)send data to UART1(PB2). RFITL:14 bytes    |\n");
        printf("|  (E)Exit                                                          |\n");
        printf("+-------------------------------------------------------------------+\n");
