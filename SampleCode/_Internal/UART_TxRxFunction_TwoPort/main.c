...
void UART2_TEST_HANDLE(void)
{
    ...
    if (u32IntSts & UART_INTSTS_RDAINT_Msk)
    {
        ...
        while (UART_IS_RX_READY(UART2))
        {
            /* Get the character from UART Buffer */
            uint32_t u32RecLen = g_u32RecLen;
            uint8_t  u8Data    = (uint8_t)UART_READ(UART2);
            g_au8RecData[u32RecLen] = u8Data;
...
void UART_FunctionTest(void)
{
    ...
    printf("|    UART1(PA3)send data to UART 2(PB0).                    |\n");
    printf("|    UART2(PB1)send data to UART 1(PA2).                    |\n");
    ...
    */
    ...
    do
    {
        ...
        printf("+------------------------------------------------------------+\n");
        printf("|    (1)UART1(PA3)send data to UART2(PB0).                   |\n");
        printf("|    (2)UART2(PB1)send data to UART1(PA2).                   |\n");
        printf("|    (E)Exit                                                 |\n");
        printf("+------------------------------------------------------------+\n");
