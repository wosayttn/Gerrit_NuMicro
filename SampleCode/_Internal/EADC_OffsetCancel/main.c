...
/*
 * 20200903 move this sample to internal folder
 *
 */
#if 1
...
#define EADC_ENABLE_OFFSETCANCEL(eadc,\
    ...
    M32(((uint32_t)eadc)+0x208) = (i16OffsetCancel & EADC_OFFSETCAL_OFFSETCANCEL_Msk);}
...
/**
  ...
  * @return None
  */
#define EADC_DISABLE_OFFSETCANCEL(eadc) {M32(((uint32_t)eadc)+0x208)=0;\
...
void EADC_FunctionTest(void)
{
    ...
    while (1)
    {
        ...
        if (u8Option == '1')
            printf("Conversion result of channel %u without offset: 0x%X (%d)\n\n", u32ChannelNum, i32ConversionData, i32ConversionData);
        else
            printf("Conversion result of channel %u with offset %d: 0x%X (%d)\n\n", u32ChannelNum, 10, i32ConversionData, i32ConversionData);
