...
void EP2_Handler(void)
{
    ...
    if (gu8IsBulkInReady)
    {
        ...
        {
            int32_t i32Len = gi32UsbdMessageLength;
            USBD_MemCopy((uint8_t *)(USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR(EP2)), pUsbMessageBuffer, i32Len);
...
void EP3_Handler(void)
{
    ...
    u32EpSts = USBD->EPSTS0 & USBD_EPSTS0_EPSTS3_Msk;
...
void EP6_Handler(void)
{
    ...
    u32EpSts = USBD->EPSTS0 & USBD_EPSTS0_EPSTS6_Msk;
...
void VCOM_TransferData(void)
{
    ...
    /* Process the Bulk out data when bulk out data is ready. */
    {
        ...
        {
            uint16_t u16ComTtail = g_u16ComTtail;
            uint8_t *pu8RxBuf    = (uint8_t *)g_pu8RxBuf;

            for (i = 0; i < u32RxSize; i++)
            {
                g_au8ComTbuf[u16ComTtail++] = pu8RxBuf[i];

                if (u16ComTtail >= TXBUFSIZE)
                    u16ComTtail = 0;
            }

            g_u16ComTtail = u16ComTtail;

            __set_PRIMASK(1);
            g_u16ComTbytes = (uint16_t)(u16ComTbytes + u32RxSize);
            __set_PRIMASK(0);
