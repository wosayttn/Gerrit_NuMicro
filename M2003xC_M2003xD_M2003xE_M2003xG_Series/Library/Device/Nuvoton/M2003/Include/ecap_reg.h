...
typedef struct
{
     ...
     * |        |          |Note: Please avoid setting this bit and CAP0CLREN to 1 at the same time
     * |        |          |If both are set to 1, Capture event 2 will perform only a reload operation and will not execute a clear.
     ...
     * |        |          |Note: Please avoid setting this bit and CAP1CLREN to 1 at the same time
     * |        |          |If both are set to 1, Capture event 2 will perform only a reload operation and will not execute a clear.
     ...
     * |        |          |Note: Please avoid setting this bit and CAP2CLREN to 1 at the same time
     * |        |          |If both are set to 1, Capture event 2 will perform only a reload operation and will not execute a clear.
     ...
     * |        |          |Note: Please avoid setting this bit and CAP0RLDEN to 1 at the same time
     * |        |          |If both are set to 1, Capture event 2 will perform only a reload operation and will not execute a clear.
     ...
     * |        |          |Note: Please avoid setting this bit and CAP1RLDEN to 1 at the same time
     * |        |          |If both are set to 1, Capture event 2 will perform only a reload operation and will not execute a clear.
     ...
     * |        |          |Note: Please avoid setting this bit and CAP2RLDEN to 1 at the same time
     * |        |          |If both are set to 1, Capture event 2 will perform only a reload operation and will not execute a clear.
     * |[24]    |CAPMSKEN0 |Capture Event0 Control Mask Enable
     * |        |          |0 = Capture counter behavior following the CAPTE0 event.
     * |        |          |1 = Capture counter behavior following the CAPTF0 flag status
     * |        |          |When the CAPTE0 event trig, if CAPTF0 is 1 and before the event CAPTF0 also is 1, the capture counter continues counting even CAP0RLDEN or CAP0CLREN is enabled
     * |        |          |if CAPTF0 is 0 to 1, the capture counter will be cleared or reloaded.
     * |        |          |This mask register is used to prevent capture counter from being mistakenly cleared or reloaded due to noise-triggered events.
     * |[25]    |CAPMSKEN1 |Capture Event1 Control Mask Enable
     * |        |          |0 = Capture counter behavior following the CAPTE1 event.
     * |        |          |1 = Capture counter behavior following the CAPTF1 flag status
     * |        |          |When the CAPTE1 event trig, if CAPTF1 is 1 and before the event CAPTF1 also is 1, the capture counter continues counting even CAP1RLDEN or CAP1CLREN is enabled
     * |        |          |if CAPTF1 is 0 to 1, the capture counter will be cleared or reloaded.
     * |        |          |This mask register is used to prevent capture counter from being mistakenly cleared or reloaded due to noise-triggered events.
     * |[26]    |CAPMSKEN2 |Capture Event2 Control Mask Enable
     * |        |          |0 = Capture counter behavior following the CAPTE2 event.
     * |        |          |1 = Capture counter behavior following the CAPTF2 flag status
     * |        |          |When the CAPTE2 event trig, if CAPTF2 is 1 and before the event CAPTF2 also is 1, the capture counter continues counting even CAP2RLDEN or CAP2CLREN is enabled
     * |        |          |if CAPTF2 is 0 to 1, the capture counter will be cleared or reloaded.
     * |        |          |This mask register is used to prevent capture counter from being mistakenly cleared or reloaded due to noise-triggered events.
     * @var ECAP_T::STATUS
     ...
     * |        |          |Note: This control register is only effective when associated with PWM0.
