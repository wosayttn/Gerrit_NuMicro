...
typedef struct
{
     ...
     * |        |          |When A/D conversion is completed, the converted data is loaded into ADDR0~13, ADDR29, ADDR30
     * |        |          |Software can enable this bit to generate a PDMA data transfer request.
     * |        |          |0 = PDMA data transfer Disabled.
     * |        |          |1 = PDMA data transfer in ADDR0~13, ADDR29, ADDR30 Enabled.
     ...
     * |        |          |1 = Conversion starts.
     * |        |          |Note: When ADST becomes from 1 to 0, ADC macro will reset to initial state.
     ...
     * |        |          |Set ADCHER[31:0] bits to enable the corresponding analog input channel 31 ~ 0.
     * |        |          |ADCHER[30:29], ADCHER[13:0] are designed.
     ...
     * |        |          |0 = Channel Disabled.
     * |        |          |1 = Channel Enabled.
     * |        |          |Note: If the internal channel for band-gap voltage (CHEN[29]) is active, the maximum sampling rate will be 50 KSPS.
     * @var ADC_T::ADCMPR[2]
     ...
     * |        |          |11110 = Temperture sensor voltage conversion result is selected to be compared.
     * |        |          |Others = Reserved.
     ...
     * |[24]    |ADCINIT   |ADC Initialization Busy Flag (Read Only)
     * |        |          |When ADEN (ADC_ADCR[0]) set to 1, the ADC need a stable time.
     * |        |          |The ADCINIT will keep high until ADC stable.
     * |        |          |The ADC initialization busy time is 32 ADC clock.
     * |        |          |Note: ADC can start converting after ADCINIT is 0.
     ...
     * |        |          |VALID[30, 29, 13:0] are the mirror of the VALID bits in ADDR30[17], ADDR29[17], ADDR13[17] ~ ADDR0[17].
     * |        |          |The other bits are reserved.
     * |        |          |Note: When ADC is in burst mode and any conversion result is valid, VALID[30, 29, 13:0] will be set to 1.
     * @var ADC_T::ADSR2
     ...
     * |        |          |OVERRUN[30, 29, 13:0] are the mirror of the OVERRUN bit in ADDR30[16], ADDR29[16], ADDR13[16] ~ ADDR0[16].
     * |        |          |The other bits are reserved.
     * |        |          |Note: When ADC is in burst mode and the FIFO is overrun, OVERRUN[30, 29, 13:0] will be set to 1.
     * @var ADC_T::ESMPCTL
     ...
     * |        |          |Note: Set this bit and wait for more than 2us before ADEN (ADC_ADCR[0]) is set.
     * @var ADC_T::ADPDMA
     ...
     * |        |          |Current PDMA transfer data could be the content of ADDR0 ~ ADDR13, ADDR29, and ADDR30 registers.
     */
...
#define ADC_ADSR0_ADCINIT_Pos           (24)                                        /*!< ADC_T::ADSR0: ADCINIT Position         */
#define ADC_ADSR0_ADCINIT_Msk           (0x1ul << ADC_ADSR0_ADCINIT_Pos)            /*!< ADC_T::ADSR0: ADCINIT Mask             */
