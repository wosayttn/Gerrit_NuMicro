...
This CMSIS product SBOM component covers only the `Core`, `Driver`, and `RTOS2` directories. `CoreValidation` is a validation suite in the Test Sample SBOM scope, and `Documentation` is excluded as documentation content rather than target-product runtime content.
...
- The tracked product subset contains Core, Driver, and RTOS2 API headers and templates. It excludes `CoreValidation` and `Documentation` from the Product SBOM scope.
