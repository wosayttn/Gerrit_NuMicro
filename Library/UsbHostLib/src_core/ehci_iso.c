    ...
    while (iso_ep != USBNULL)                  /* Search all activated iso endpoints         */
    {
        ...
        while (itd != USBNULL)                 /* traverse all iTDs of itd list              */
        {
            if (review_itd(itd))            /* inspect and reclaim iTD                    */
            {
                ...
                if (frnidx >= FL_SIZE)
                {
                    USB_error("Invalid frame index! 0x%x -> %d\n", (int)itd, frnidx);
                }
                else if (_PFList[frnidx] == ITD_HLNK_ITD(itd))
                {
...
static void  write_itd_micro_frame(UTR_T *utr, uint8_t fidx, iTD_T *itd, int mf)
{
    uint32_t   buff_addr;
...
/**
 * @brief Handle isochronous transfer for the given UTR (USB Transfer Request).
 *
 ...
 * <b>Justification:</b> Endpoint hardware-pipe storage and the USB host allocator use
 *                       void *; this transfer routine restores the ISO_EP_T ownership type.
 */
int ehci_iso_xfer(UTR_T *utr)
{
    ...
    int        i;
    int        itd_cnt;
    uint8_t    trans_mask;                  /* bit mask of used xfer in an iTD            */
    uint8_t    fidx;                        /* index to the 8 iso frames of UTR           */
    ...
    else
    {
        ...
        /* cppcheck-suppress misra-c2012-11.5 */
        iso_ep = usbh_alloc_mem((int)sizeof(*iso_ep));
    ...
    utr->iso_sf = (uint16_t)iso_ep->next_frame;
    ...
    while (itd != USBNULL)
    {
        if (fidx >= (uint8_t)IF_PER_UTR)    /* unlikely                                   */
        {
        ...
        {
            if (((uint32_t)trans_mask & ((uint32_t)0x1U << (uint32_t)i)) == 0U)
            {
            ...
            if (fidx == (uint8_t)IF_PER_UTR)/* is the last scheduled micro-frame?         */
            {
...
int ehci_quit_iso_xfer(UTR_T *utr, EP_INFO_T *ep)
{
    ...
    (void)usbh_free_mem(iso_ep, (int)sizeof(*iso_ep));/* free this iso_ep                 */
