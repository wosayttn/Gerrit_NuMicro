 ...
 * <b>Justification:</b> The source data is a USB descriptor byte stream; mapping it onto a
 *                       DESC_CONF_T descriptor struct avoids repetitive byte-by-byte field
 *                       assembly and mirrors the USB specification's descriptor layout.
 */
...
/**
 * @brief  Parse a single endpoint descriptor.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
 * <b>Justification:</b> The source data is a USB descriptor byte stream; mapping it onto a
 *                       DESC_EP_T descriptor struct avoids repetitive byte-by-byte field
 *                       assembly and mirrors the USB specification's descriptor layout.
 *                       bLength and remaining-buffer-length are checked before each access.
 */
static int  usbh_parse_endpoint(ALT_IFACE_T *alt, int ep_idx, uint8_t *desc_buff, int len)
{
    ...
    pksz = ((uint32_t)ep_desc->wMaxPacketSize & 0x07ffU);
    transaction_count = 1U + (((uint32_t)ep_desc->wMaxPacketSize >> 11U) & 3U);
    pksz = pksz * transaction_count;
...
/**
  ...
  * <b>Justification:</b> usbh_alloc_mem() is the USB Host common generic memory pool allocator;
  *                       it returns void* by design so it can back any object type. Assigning
  *                       its return value to IFACE_T* here is the established, project-wide
  *                       allocator usage pattern.
  *
  * @static_deviation
  * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
  * <b>Justification:</b> The source data is a USB descriptor byte stream; mapping it onto
  *                       DESC_IF_T / DESC_HDR_T descriptor structs avoids repetitive byte-by-byte
  *                       field assembly and mirrors the USB specification's descriptor layout.
  *                       bLength and remaining-buffer-length are checked before each cast/access.
  */
static int  usbh_parse_interface(UDEV_T *udev, uint8_t *desc_buff, int len)
{
    ...
    while ((len_tmp > 0) && (bDone == 0))
    {
        ...
        if (if_desc->bNumEndpoints > (uint8_t)MAX_EP_PER_IFACE)
        {
...
/**
 * @brief  Parse a full configuration descriptor and its nested interface descriptors.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
 * <b>Justification:</b> The source data is a USB descriptor byte stream; mapping it onto
 *                       DESC_CONF_T / DESC_HDR_T descriptor structs avoids repetitive
 *                       byte-by-byte field assembly and mirrors the USB specification's
 *                       descriptor layout. bLength and remaining-buffer-length are checked
 *                       before each cast/access.
 */
...
#endif
/**
 * @brief  Enumerate and connect a newly attached USB device.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> usbh_alloc_mem() is the USB Host common generic memory pool allocator;
 *                       it returns void* by design so it can back any object type. Converting
 *                       its return value to DESC_CONF_T* here is the established, project-wide
 *                       allocator usage pattern.
 */
...
/**
 * @brief  Reset and re-enumerate a USB device already attached to the bus.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
 * <b>Justification:</b> udev->cfd_buff is a raw byte buffer holding the device's previously
 *                       fetched configuration descriptor; mapping it onto a DESC_CONF_T struct
 *                       avoids repetitive byte-by-byte field assembly and mirrors the USB
 *                       specification's descriptor layout.
 */
...
/**
 * @brief  Handle removal of a USB device: notify class drivers and release resources.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
 * <b>Justification:</b> udev->cfd_buff is a raw byte buffer holding the device's previously
 ...
 * <b>Justification:</b> This wrapper is the single, centralized debug-output entry point used by
 *                       USB_debug()/USB_DBGMSG() across the whole USB Host stack, so that
 *                       &lt;stdarg.h&gt; usage stays confined to one function instead of being
 *                       scattered across every driver/parser file. It is used only for
 *                       debug/diagnostic output and never participates in USB enumeration,
 *                       transfer scheduling, descriptor parsing, or class-driver data paths.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 21.6<br>
 * <b>Justification:</b> vprintf() is required to forward the variadic format string; this usage
 *                       is confined to this single debug wrapper function in usb_core.c and does
 *                       not affect USB Host core functional correctness.
 */
