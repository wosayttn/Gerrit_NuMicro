/**************************************************************************//**
 * @file     usb_core.c
 * @version  V1.00
 * @brief    USB Host library core driver.
 *
 * SPDX-License-Identifier: Apache-2.0
 * @copyright (C) 2023 Nuvoton Technology Corp. All rights reserved.
*****************************************************************************/

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdarg.h>

#include "NuMicro.h"

#include "usb.h"
#include "hub.h"

/// @cond HIDDEN_SYMBOLS

USBH_T     *_ohci0;
USBH1_T    *_ohci1;
#ifdef ENABLE_EHCI
    HSUSBH_T   *_ehci;
#endif

static UDEV_DRV_T   *_drivers[MAX_UDEV_DRIVER];

static CONN_FUNC  *g_conn_func;
static CONN_FUNC  *g_disconn_func;

static int _ovc_active_high = 0;

/// @endcond HIDDEN_SYMBOLS
/**
  * @brief       Initialize USB Host controller and USB stack.
  *
  * @return      None.
  */
void  usbh_core_init(void)
{
#ifdef ENABLE_EHCI
    DISABLE_EHCI_IRQ();
#endif
#ifdef ENABLE_OHCI0
    DISABLE_OHCI0_IRQ();
#endif
#ifdef ENABLE_OHCI1
    DISABLE_OHCI1_IRQ();
#endif
#ifdef ENABLE_OHCI0//hydra...
    _ohci0 = USBH0;
#endif
#ifdef ENABLE_OHCI1//Synopsys...
    _ohci1 = USBH1;
#endif
#ifdef ENABLE_EHCI//Synopsys...
    _ehci = HSUSBH;
#endif

    (void)memset(_drivers, 0, sizeof(_drivers));

    g_conn_func = USBNULL;
    g_disconn_func = USBNULL;

    (void)usbh_hub_init();

    usbh_memory_init();
#ifdef ENABLE_OHCI0

    if (_ovc_active_high)
    {
        _ohci0->HcMiscControl &= ~USBH_HcMiscControl_OCAL_Msk; // Over-current active high
    }
    else
    {
        _ohci0->HcMiscControl |= USBH_HcMiscControl_OCAL_Msk; // Over-current active Low
    }

#endif

#if defined ENABLE_EHCI || defined ENABLE_OHCI1

    if (_ovc_active_high)
    {
        // Over-current active high
        HSOTG_SET_VBUS_OC_POL(HSOTG_VBUS_OC_VALID_HIGH);
        HSOTG_SET_VBUS_STS_POL(HSOTG_VBUS_ST_VALID_LOW);
    }
    else
    {
        // Over-current active Low
        HSOTG_SET_VBUS_OC_POL(HSOTG_VBUS_OC_VALID_LOW);
        HSOTG_SET_VBUS_STS_POL(HSOTG_VBUS_ST_VALID_HIGH);
    }

#endif

#ifdef ENABLE_OHCI0
    ohci0_driver.init();
    USB_debug("OHCI0 init done.\n");
    ENABLE_OHCI0_IRQ();
#endif

#ifdef ENABLE_OHCI1
    ohci1_driver.init();
    USB_debug("OHCI1 init done.\n");
    ENABLE_OHCI1_IRQ();
#endif

#ifdef ENABLE_EHCI
    ehci_driver.init();
    USB_debug("EHCI init done.\n");
    ENABLE_EHCI_IRQ();
#endif
}

/**
 * @brief       Initialize USB Host controller and USB stack.
 *
 * @param[in]   ovc_alv   Over-current active level setting
 *                        0: active low
 *                        1: active high
 * @return      None.
 */
void  usbh_core_init_ex(int ovc_alv)
{
    _ovc_active_high = ovc_alv;
    usbh_core_init();
}

/**
  * @brief    Install device connect and disconnect callback function.
  *
  * @param[in]  conn_func       Device connect callback function.
  * @param[in]  disconn_func    Device disconnect callback function.
  * @return     None.
  */
void usbh_install_conn_callback(CONN_FUNC *conn_func, CONN_FUNC *disconn_func)
{
    g_conn_func = conn_func;
    g_disconn_func = disconn_func;
}

static int  reset_device(UDEV_T *udev)
{
    if (udev->parent == USBNULL)
    {
        if (udev->hc_driver)
        {
            return udev->hc_driver->rthub_port_reset(udev->port_num - 1U);
        }
        else
        {
            return USBH_ERR_NOT_FOUND;
        }
    }
    else
    {
        return udev->parent->port_reset(udev->parent, udev->port_num);
    }
}

#ifdef ENABLE_EHCI
    static uint32_t ehci_UCMDR;
#endif

/**
  * @brief    Suspend USB Host Controller and devices
  * @return   None
  *
  */
void usbh_suspend(void)
{
#ifdef ENABLE_EHCI
    int   time_out = 10;   /* ms */
    uint32_t ehci_ucmdr;
    uint32_t ehci_ustsr;
#endif

#ifdef ENABLE_OHCI0
    uint32_t ohci0_HcControl_tamp = 0U;

    /* port is hydra */
    /* set port suspend if connected */
    if (_ohci0->HcRhPortStatus[0] & USBH_HcRhPortStatus_CCS_Msk)
    {
        _ohci0->HcRhPortStatus[0] = USBH_HcRhPortStatus_PSS_Msk;    /* set port suspend    */
    }

    /* enable Device Remote Wakeup */
    _ohci0->HcRhStatus |= USBH_HcRhStatus_DRWE_Msk;

    /* enable USBH RHSC interrupt for system wakeup */
    _ohci0->HcInterruptEnable =  USBH_HcInterruptEnable_RHSC_Msk | USBH_HcInterruptEnable_RD_Msk;

    /* set Host Controller enter suspend state */
    ohci0_HcControl_tamp = _ohci0->HcControl;
    _ohci0->HcControl = ((ohci0_HcControl_tamp & ~USBH_HcControl_HCFS_Msk) | (3U << USBH_HcControl_HCFS_Pos));
#endif

#ifdef ENABLE_OHCI1
    uint32_t ohci1_HcControl_tamp = 0U;

    /* port is Synopsys */
    /* set port suspend if deivce has been not connected */
    if ((_ohci1->HcRhPortStatus[0] & ~USBH_HcRhPortStatus_CCS_Msk) == 0U)
    {
        _ohci1->HcRhPortStatus[0] = USBH_HcRhPortStatus_PSS_Msk;    /* set port suspend    */
    }

    /* enable Device Remote Wakeup */
    _ohci1->HcRhStatus |= USBH_HcRhStatus_DRWE_Msk;

    /* enable USBH RHSC interrupt for system wakeup */
    _ohci1->HcInterruptEnable =  USBH_HcInterruptEnable_RHSC_Msk | USBH_HcInterruptEnable_RD_Msk;

    /* set Host Controller enter suspend state */
    ohci1_HcControl_tamp = _ohci1->HcControl;
    _ohci1->HcControl = ((ohci1_HcControl_tamp & ~USBH_HcControl_HCFS_Msk) | (3U << USBH_HcControl_HCFS_Pos));
#endif

#ifdef ENABLE_EHCI
    ehci_UCMDR = _ehci->UCMDR;

    if (_ehci->UPSCR[0] & HSUSBH_UPSCR_PE_Msk)
    {
        _ehci->UPSCR[0] |= HSUSBH_UPSCR_SUSPEND_Msk;
        delay_us(2000);         /* wait 2 ms */
    }

    _ehci->UCMDR &= ~(HSUSBH_UCMDR_PSEN_Msk | HSUSBH_UCMDR_ASEN_Msk | HSUSBH_UCMDR_RUN_Msk);

    while (time_out > 0)
    {
        ehci_ucmdr = _ehci->UCMDR;

        if ((ehci_ucmdr & HSUSBH_UCMDR_RUN_Msk) == 0U)
        {
            ehci_ustsr = _ehci->USTSR;

            if ((ehci_ustsr & HSUSBH_USTSR_HCHalted_Msk) != 0U)
            {
                break;
            }
        }

        delay_us(1000);         /* wait 1 ms */
        time_out --;
    }

    if (time_out == 0)
    {
        USB_error("usbh_suspend - RUN/HCHalted error!\n");
    }

    delay_us(100);
#endif
}

/**
  * @brief    Resume USB Host controller and devices
  * @return   None
  */
void usbh_resume(void)
{
#ifdef ENABLE_OHCI0
    uint32_t ohci0_HcControl_tamp = 0U;
    /*hydra*/
    ohci0_HcControl_tamp = _ohci0->HcControl;
    _ohci0->HcControl = (ohci0_HcControl_tamp & ~USBH_HcControl_HCFS_Msk) | (1U << USBH_HcControl_HCFS_Pos);

    if (_ohci0->HcRhPortStatus[0] & USBH_HcRhPortStatus_PSS_Msk)
    {
        _ohci0->HcRhPortStatus[0] = USBH_HcRhPortStatus_POCI_Msk;   /* clear suspend status */
    }

    delay_us(30000);                       /* wait at least 30ms for Host to resume device */

    /* enter operational state */
    ohci0_HcControl_tamp = _ohci0->HcControl;
    _ohci0->HcControl = (ohci0_HcControl_tamp & ~USBH_HcControl_HCFS_Msk) | (2U << USBH_HcControl_HCFS_Pos);
#endif

#ifdef ENABLE_EHCI

    _ehci->UCMDR = ehci_UCMDR;

    if (_ehci->UPSCR[0] & HSUSBH_UPSCR_PE_Msk)
    {
        _ehci->UPSCR[0] |= HSUSBH_UPSCR_FPR_Msk;
        delay_us(25000);                         /* keep resume signal for 25 ms */
        _ehci->UPSCR[0] &= ~HSUSBH_UPSCR_FPR_Msk;
    }

    delay_us(1000);
#endif

#ifdef ENABLE_OHCI1
    uint32_t ohci1_HcControl_tamp = 0U;
    /*Synopsys*/
    ohci1_HcControl_tamp = _ohci1->HcControl;
    _ohci1->HcControl = (ohci1_HcControl_tamp & ~USBH_HcControl_HCFS_Msk) | (1U << USBH_HcControl_HCFS_Pos);

    if (_ohci1->HcRhPortStatus[0] & USBH_HcRhPortStatus_PSS_Msk)
    {
        _ohci1->HcRhPortStatus[0] = USBH_HcRhPortStatus_POCI_Msk;   /* clear suspend status */
    }

    delay_us(30000);                       /* wait at least 30ms for Host to resume device */

    /* enter operational state */
    ohci1_HcControl_tamp = _ohci1->HcControl;
    _ohci1->HcControl = (ohci1_HcControl_tamp & ~USBH_HcControl_HCFS_Msk) | (2U << USBH_HcControl_HCFS_Pos);
#endif


}

/**
 * @brief  Deinitialize USB Host controllers.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 17.7<br>
 * <b>Justification:</b> The diagnostic printf return value has no recovery use during
 *                       host-controller shutdown.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 21.6<br>
 * <b>Justification:</b> printf provides optional controller-shutdown diagnostics for
 *                       application builds that supply a retargeted output channel.
 */
void usbh_core_deinit(void)
{
    int   time_out = 10;   /* ms */

#ifdef ENABLE_OHCI0
    uint32_t ohci0_HcControl_tamp = 0;
    /*hydra*/
    //disable ohci irq
    DISABLE_OHCI0_IRQ();
    //disable ohci mode.
    ohci0_HcControl_tamp = _ohci0->HcControl;
    _ohci0->HcControl = (ohci0_HcControl_tamp & ~USBH_HcControl_HCFS_Msk) | (3U << USBH_HcControl_HCFS_Pos);

    while (time_out > 0)
    {
        if ((_ohci0->HcControl & (3U << USBH_HcControl_HCFS_Pos)) == (3U << USBH_HcControl_HCFS_Pos))
        {
            if ((_ohci0->HcCommandStatus & USBH1_HcCommandStatus_HCR_Msk) == 0U)
            {
                /* cppcheck-suppress misra-c2012-17.7 */
                /* cppcheck-suppress misra-c2012-21.6 */
                printf("ohci0_deinit done\n");
                break;
            }
        }

        delay_us(1000);         /* wait 1 ms */
        time_out --;
    }

    if (time_out == 0)
    {
        USB_error("ohci0_deinit - HCFS error!\n");
    }

    delay_us(20000);                       /* wait at least 20ms */
#endif

#ifdef ENABLE_OHCI1
    time_out = 10; /* ms */
    uint32_t ohci1_HcControl_tamp = 0;
    /*Synopsys*/
    //disable ohci irq
    DISABLE_OHCI1_IRQ();
    //disable ohci mode.
    ohci1_HcControl_tamp = _ohci1->HcControl;
    _ohci1->HcControl = ((ohci1_HcControl_tamp & ~USBH_HcControl_HCFS_Msk) | (3U << USBH_HcControl_HCFS_Pos));

    while (time_out > 0)
    {
        if ((_ohci1->HcControl & (3U << USBH_HcControl_HCFS_Pos)) == (3U << USBH_HcControl_HCFS_Pos))
        {
            if ((_ohci1->HcCommandStatus & USBH1_HcCommandStatus_HCR_Msk) == 0U)
            {
                /* cppcheck-suppress misra-c2012-17.7 */
                /* cppcheck-suppress misra-c2012-21.6 */
                printf("ohci1_deinit done\n");
                break;
            }
        }

        delay_us(1000);         /* wait 1 ms */
        time_out --;
    }

    if (time_out == 0)
    {
        USB_error("ohci1_deinit - HCFS error!\n");
    }

    delay_us(20000);                       /* wait at least 20ms */
#endif

#ifdef ENABLE_EHCI
    time_out = 10; /* ms */
    /*Synopsys*/
    //disable ehci irq
    DISABLE_EHCI_IRQ();
    //disable ehci mode.
    _ehci->UCMDR &= ~(HSUSBH_UCMDR_PSEN_Msk | HSUSBH_UCMDR_ASEN_Msk | HSUSBH_UCMDR_RUN_Msk);

    while (time_out > 0)
    {
        uint32_t ehci_ucmdr = _ehci->UCMDR;

        if ((ehci_ucmdr & HSUSBH_UCMDR_RUN_Msk) == 0U)
        {
            uint32_t ehci_ustsr = _ehci->USTSR;

            if ((ehci_ustsr & HSUSBH_USTSR_HCHalted_Msk) != 0U)
            {
                /* cppcheck-suppress misra-c2012-17.7 */
                /* cppcheck-suppress misra-c2012-21.6 */
                printf("ehci_deinit done\n");
                break;
            }
        }

        delay_us(1000);         /* wait 1 ms */
        time_out --;
    }

    if (time_out == 0)
    {
        USB_error("ehci_deinit - RUN/HCHalted error!\n");
    }

    delay_us(1000);
#endif
}

/// @cond HIDDEN_SYMBOLS

/**
  * @brief    Register an USB device class or vendor driver to lightweight USB stack.
  *           All USB device drivers must be registered before usbh_core_init() called.
  * @param[in]  udrv   The USB device driver to be registered.
  * @return   Driver registration success or not.
  * @retval   0  Success
  * @retval   USBH_ERR_MEMORY_OUT   Registered drivers have reached MAX_UDEV_DRIVER limitation.
  */
int  usbh_register_driver(UDEV_DRV_T *udrv)
{
    int   i;

    for (i = 0; i < MAX_UDEV_DRIVER; i++)
    {
        if (_drivers[i] == udrv)
        {
            return 0;                  /* already registered, do nothing */
        }

        if (_drivers[i] == USBNULL)
        {
            _drivers[i] = udrv;        /* register this driver */
            return 0;
        }
    }

    return USBH_ERR_MEMORY_OUT;        /* reached MAX_UDEV_DRIVER limitation, aborted */
}

/**
  * @brief    Execute an USB request in control transfer. This function returns after the request
  *           was done or aborted.
  * @param[in]  udev            The target USB device.
  * @param[in]  bmRequestType   Characteristics of request
  * @param[in]  bRequest        Specific request
  * @param[in]  wValue          Word-sized field that varies according to request
  * @param[in]  wIndex          Word-sized field that varies according to request
  * @param[in]  wLength         Number of bytes to transfer if there is a Data stage
  * @param[in]  buff            Data buffer used in data stage
  * @param[in]  data_len        Length of data to be transmitted/received
  * @param[out] xfer_len        Transmitted/received length of data
  * @param[in]  timeout         Time-out limit (in 10ms - timer tick) of this transfer
  * @retval   0     Transfer success
  * @retval   < 0   Transfer failed. Refer to error code definitions.
  */
int usbh_ctrl_xfer(UDEV_T *udev, uint8_t bmRequestType, uint8_t bRequest, uint16_t wValue, uint16_t wIndex,
                   uint16_t wLength, uint8_t *buff, uint32_t *xfer_len, uint32_t timeout)
{
    UTR_T      *utr;
    uint32_t   t0;
    int        status;

    *xfer_len = 0;

    //if(check_device(udev))
    //    return USBH_ERR_INVALID_PARAM;

    utr = alloc_utr(udev);

    if (utr == USBNULL)
    {
        return USBH_ERR_MEMORY_OUT;
    }

    utr->setup.bmRequestType = bmRequestType;
    utr->setup.bRequest   = bRequest;
    utr->setup.wValue     = wValue;
    utr->setup.wIndex     = wIndex;
    utr->setup.wLength    = wLength;

    utr->buff = buff;
    utr->data_len = wLength;
    utr->bIsTransferDone = 0;
    status = udev->hc_driver->ctrl_xfer(utr);

    if (status < 0)
    {
        udev->ep0.hw_pipe = USBNULL;
        free_utr(utr);
        return status;
    }

    t0 = get_ticks();

    while (utr->bIsTransferDone == 0U)
    {
        if ((get_ticks() - t0) > timeout)
        {
            (void)usbh_quit_utr(utr);
            free_utr(utr);
            udev->ep0.hw_pipe = USBNULL;
            return USBH_ERR_TIMEOUT;
        }
    }

    status = utr->status;

    if (status == 0)
    {
        *xfer_len = utr->xfer_len;
    }

    free_utr(utr);

    return status;
}

/**
  * @brief    Execute a bulk transfer request. This function will return immediately after
  *           issued the bulk transfer. USB stack will later call back utr->func() once the bulk
  *           transfer was done or aborted.
  * @param[in]  utr    The bulk transfer request.
  * @retval   0     Transfer success
  * @retval   < 0   Failed. Refer to error code definitions.
  */
int usbh_bulk_xfer(UTR_T *utr)
{
    return utr->udev->hc_driver->bulk_xfer(utr);
}

/**
  * @brief    Execute an interrupt transfer request. This function will return immediately after
  *           issued the interrupt transfer. USB stack will later call back utr->func() once the
  *           interrupt transfer was done or aborted.
  * @param[in]  utr    The interrupt transfer request.
  * @retval   0     Transfer success
  * @retval   < 0   Failed. Refer to error code definitions.
  */
int usbh_int_xfer(UTR_T *utr)
{
    return utr->udev->hc_driver->int_xfer(utr);
}

/**
  * @brief    Execute an isochronous transfer request. This function will return immediately after
  *           issued the isochronous transfer. USB stack will later call back utr->func() once the
  *           isochronous transfer was done or aborted.
  * @param[in]  utr    The isochronous transfer request.
  * @retval   0     Transfer success
  * @retval   < 0   Failed. Refer to error code definitions.
  */
int usbh_iso_xfer(UTR_T *utr)
{
    if (utr->udev->hc_driver == USBNULL)
    {
        USB_debug("hc_driver is USBNULL!\n");
        return -1;
    }

    if (utr->udev->hc_driver->iso_xfer == USBNULL)
    {
        USB_debug("iso_xfer is USBNULL!\n");
        return -1;
    }

    return utr->udev->hc_driver->iso_xfer(utr);
}

/**
  * @brief    Force to quit an UTR transfer.
  * @param[in]  utr    The UTR transfer to be quit.
  * @retval   0     Transfer success
  * @retval   < 0   Failed. Refer to error code definitions.
  */
int usbh_quit_utr(UTR_T *utr)
{
    if ((utr == USBNULL) || (utr->udev == USBNULL))
    {
        return USBH_ERR_NOT_FOUND;
    }

    return utr->udev->hc_driver->quit_xfer(utr, USBNULL);
}

/**
  * @brief    Force to quit an endpoint transfer.
  * @param[in]  udev     The USB device.
  * @param[in]  ep       The endpoint to be quit.
  * @retval   0     Transfer success
  * @retval   < 0   Failed. Refer to error code definitions.
  */
int usbh_quit_xfer(UDEV_T *udev, EP_INFO_T *ep)
{
    return udev->hc_driver->quit_xfer(USBNULL, ep);
}
#if  defined(DUMP_DESCRIPTOR) && defined(ENABLE_DEBUG_MSG)
static void  dump_device_descriptor(DESC_DEV_T *desc)
{
    USB_debug("\n[Device Descriptor]\n");
    USB_debug("----------------------------------------------\n");
    USB_debug("  Length              = %2d\n",  desc->bLength);
    USB_debug("  DescriptorType      = 0x%02x\n", desc->bDescriptorType);
    USB_debug("  USB version         = %x.%02x\n",
              desc->bcdUSB >> 8, desc->bcdUSB & 0xffU);
    USB_debug("  Vendor:Product      = %04x:%04x\n",
              desc->idVendor, desc->idProduct);
    USB_debug("  MaxPacketSize0      = %d\n",   desc->bMaxPacketSize0);
    USB_debug("  NumConfigurations   = %d\n",   desc->bNumConfigurations);
    USB_debug("  Device version      = %x.%02x\n",
              desc->bcdDevice >> 8, desc->bcdDevice & 0xffU);
    USB_debug("  Device Class:SubClass:Protocol = %02x:%02x:%02x\n",
              desc->bDeviceClass, desc->bDeviceSubClass, desc->bDeviceProtocol);
}
#endif
void usbh_dump_interface_descriptor(DESC_IF_T *if_desc)
{
    USB_debug("\n    [Interface Descriptor]\n");
    USB_debug("    ----------------------------------------------\n");
    USB_debug("      Length              = %2d\n",  if_desc->bLength);
    USB_debug("      DescriptorType      = %02x\n", if_desc->bDescriptorType);
    USB_debug("      bInterfaceNumber    = %d\n", if_desc->bInterfaceNumber);
    USB_debug("      bAlternateSetting   = %d\n", if_desc->bAlternateSetting);
    USB_debug("      bNumEndpoints       = %d\n", if_desc->bNumEndpoints);
    USB_debug("      bInterfaceClass     = 0x%02x\n", if_desc->bInterfaceClass);
    USB_debug("      bInterfaceSubClass  = 0x%02x\n", if_desc->bInterfaceSubClass);
    USB_debug("      bInterfaceProtocol  = 0x%02x\n", if_desc->bInterfaceProtocol);
    USB_debug("      iInterface          = %d\n", if_desc->iInterface);
}

void usbh_dump_endpoint_descriptor(DESC_EP_T *ep_desc)
{
    USB_debug("\n        [Endpoint Descriptor]\n");
    USB_debug("        ----------------------------------------------\n");
    USB_debug("          Length              = %2d\n",  ep_desc->bLength);
    USB_debug("          DescriptorType      = %02x\n", ep_desc->bDescriptorType);
    USB_debug("          bEndpointAddress    = 0x%02x\n", ep_desc->bEndpointAddress);
    USB_debug("          bmAttributes        = 0x%02x\n", ep_desc->bmAttributes);
    USB_debug("          wMaxPacketSize      = %d\n", ep_desc->wMaxPacketSize);
    USB_debug("          bInterval           = %d\n", ep_desc->bInterval);
    USB_debug("          bRefresh            = %d\n", ep_desc->bRefresh);
    USB_debug("          bSynchAddress       = %d\n", ep_desc->bSynchAddress);
}
#if  defined(DUMP_DESCRIPTOR) && defined(ENABLE_DEBUG_MSG)
static void  dump_config_descriptor(DESC_CONF_T *desc)
{
    uint8_t     *bptr = (uint8_t *)desc;
    DESC_HDR_T  *hdr;
    int         tlen = desc->wTotalLength;

    while (tlen > 0)
    {
        switch (bptr[1])
        {
            case USB_DT_CONFIGURATION:
                USB_debug("\n[Configuration Descriptor]\n");
                USB_debug("----------------------------------------------\n");
                USB_debug("  Length              = %2d\n",  desc->bLength);
                USB_debug("  DescriptorType      = %02x\n", desc->bDescriptorType);
                USB_debug("  wTotalLength        = %2d\n", desc->wTotalLength);
                USB_debug("  bNumInterfaces      = %d\n", desc->bNumInterfaces);
                USB_debug("  bConfigurationValue = %d\n", desc->bConfigurationValue);
                USB_debug("  iConfiguration      = %d\n", desc->iConfiguration);
                USB_debug("  bmAttributes        = 0x%02x\n", desc->bmAttributes);
                USB_debug("  MaxPower            = %d\n", desc->MaxPower);
                break;

            case USB_DT_INTERFACE:
                (void)usbh_dump_interface_descriptor((DESC_IF_T *)bptr);
                break;

            case USB_DT_ENDPOINT:
                (void)usbh_dump_endpoint_descriptor((DESC_EP_T *)bptr);
                break;

            default:
                hdr = (DESC_HDR_T *)bptr;
                USB_debug("\n!![Unknown Descriptor]\n");
                USB_debug("----------------------------------------------\n");
                USB_debug("Length              = %2d\n",  hdr->bLength);
                USB_debug("DescriptorType      = %02x\n", hdr->bDescriptorType);
                break;
        }

        if (bptr[0] == 0U)
        {
            break;
        }

        tlen -= (int)bptr[0];
        bptr += bptr[0];
    }
}
#endif
/**
 *  @brief  Execute USB standard request SET ADDRESS.
 *  @retval   0  Success
 *  @retval   < 0   Failed. Refer to error code definitions.
 */
static int usbh_set_address(UDEV_T *udev)
{
    uint32_t  read_len;
    int       dev_num;
    uint8_t   dev_addr;
    int       ret;

    if (udev->dev_num != 0U)
    {
        return USBH_ERR_SET_DEV_ADDR;
    }

    dev_num = alloc_dev_address();
    dev_addr = (uint8_t)dev_num;

    /*------------------------------------------------------------------------------------*/
    /* Issue SET ADDRESS command to set device address                                    */
    /*------------------------------------------------------------------------------------*/
    ret = usbh_ctrl_xfer(udev, REQ_TYPE_OUT | REQ_TYPE_STD_DEV | REQ_TYPE_TO_DEV,
                         USB_REQ_SET_ADDRESS, (uint16_t)dev_addr, 0, 0,
                         USBNULL, &read_len, 100);

    if (ret < 0)
    {
        free_dev_address(dev_num);
        return ret;
    }

    udev->dev_num = dev_addr;

    return 0;
}

/**
 *  @brief  Execute USB standard request SET CONFIGURATION.
 *  @retval   0  Success
 *  @retval   < 0   Failed. Refer to error code definitions.
 */
int usbh_set_configuration(UDEV_T *udev, uint8_t conf_val)
{
    uint32_t  read_len;
    int       ret;

    /* Current configuration is the same. Do nothing. */
    if (udev->cur_conf == (int)conf_val)
    {
        return 0;
    }

    /* Set another configuration is currently not supported! */
    if (udev->cur_conf != -1)
    {
        return USBH_ERR_SET_CONFIG;
    }

    /*------------------------------------------------------------------------------------*/
    /* Issue SET CONFIGURATION command to select device configuration                     */
    /*------------------------------------------------------------------------------------*/
    ret = usbh_ctrl_xfer(udev, REQ_TYPE_OUT | REQ_TYPE_STD_DEV | REQ_TYPE_TO_DEV,
                         USB_REQ_SET_CONFIGURATION, conf_val, 0, 0,
                         USBNULL, &read_len, 300);

    if (ret < 0)
    {
        return ret;
    }

    udev->cur_conf = (int8_t)conf_val;

    return 0;
}

/**
 *  @brief    Select an alternate setting for the specified interface.
 *  @param[in]  iface        The interface
 *  @param[in]  alt_setting  Desired alternative setting
 *  @retval   0  Success
 *  @retval   < 0   Failed. Refer to error code definitions.
 */
int usbh_set_interface(IFACE_T *iface, uint16_t alt_setting)
{
    ALT_IFACE_T  *aif = USBNULL;

    uint8_t      alt_num;
    uint32_t     xfer_len;
    int          i;
    int          ret;

    if (alt_setting > 0xFFU)
    {
        return USBH_ERR_NOT_FOUND;
    }

    alt_num = (uint8_t)alt_setting;

    for (i = 0; i < (int)iface->num_alt; i++)
    {
        const DESC_IF_T    *ifd;
        ifd = iface->alt[i].ifd;

        if ((ifd != USBNULL) && (ifd->bAlternateSetting == alt_num))
        {
            aif = &iface->alt[i];
            break;
        }
    }

    if (aif == USBNULL)
    {
        return USBH_ERR_NOT_FOUND;          /* cannot find desired alternative setting    */
    }

    ret = usbh_ctrl_xfer(iface->udev, REQ_TYPE_OUT | REQ_TYPE_STD_DEV | REQ_TYPE_TO_IFACE,
                         USB_REQ_SET_INTERFACE, alt_setting, iface->if_num, 0,
                         USBNULL, &xfer_len, 100);

    if (ret == 0)
    {
        iface->aif = aif;                   /* change active alternative setting          */
    }

    return ret;
}

/**
 *  @brief  Get device descriptor from the USB device.
 *  @param[out] desc_buff  Data buffer to receive device descriptor data.
 *  @return   Success or not.
 *  @retval   0  Success
 *  @retval   Otherwise  Failed
 */
int usbh_get_device_descriptor(UDEV_T *udev, DESC_DEV_T *desc_buff)
{
    uint32_t  read_len;
    int       ret = 0;
    int       retry;
    int       timeout = 10;

    for (retry = 0; retry < 3; retry++)
    {
        ret = usbh_ctrl_xfer(udev, REQ_TYPE_IN | REQ_TYPE_STD_DEV | REQ_TYPE_TO_DEV,
                             USB_REQ_GET_DESCRIPTOR,
                             (uint16_t)(((uint32_t)USB_DT_STANDARD | (uint32_t)USB_DT_DEVICE) << 8U), 0, (uint16_t)sizeof(DESC_DEV_T),
                             (uint8_t *)desc_buff, &read_len, timeout);

        if (ret == 0)
        {
            return 0;
        }

        USB_debug("Get device descriptor failed - %d, retry!\n", ret);
    }

    return ret;
}

/**
 *  @brief Get configuration descriptor total length from the USB device.
 *
 *  @param[in] udev The target USB device.
 *  @param[in] conf_header Data buffer to receive configuration descriptor header.
 *  @return   Success or not.
 *  @retval   0  Success
 *  @retval   Otherwise  Failed
 */

int usbh_get_config_descripotr_total_length(UDEV_T *udev, DESC_CONF_T *conf_header)
{
    uint32_t  read_len;
    int       ret;

    /*------------------------------------------------------------------------------------*/
    /* Get Configuration Descriptor Header (9 bytes)                                      */
    /*------------------------------------------------------------------------------------*/
    ret = usbh_ctrl_xfer(udev, REQ_TYPE_IN | REQ_TYPE_STD_DEV | REQ_TYPE_TO_DEV,
                         USB_REQ_GET_DESCRIPTOR,
                         (uint16_t)(((uint32_t)USB_DT_STANDARD | (uint32_t)USB_DT_CONFIGURATION) << 8U), 0,
                         (uint16_t)sizeof(DESC_CONF_T), // read 9 bytes only.
                         (uint8_t *)conf_header, &read_len, 200);

    if (ret < 0)
    {
        USB_error("Get config descriptor header failed!\n");
        return ret;
    }

    return 0;
}

/// @endcond HIDDEN_SYMBOLS
/**
 *  @brief  Get configuration descriptor from the USB device.
 *  @param[out] desc_buff  Data buffer to receive configuration descriptor data.
 *  @param[in]  buff_len   Valid length of <desc_buff>
 *  @return   Success or not.
 *  @retval   0  Success
 *  @retval   Otherwise  Failed
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
 * <b>Justification:</b> The caller supplies a descriptor byte buffer whose leading bytes
 *                       are interpreted as the standard configuration descriptor header.
 */
int usbh_get_config_descriptor(UDEV_T *udev, uint8_t *desc_buff, int buff_len)
{
    uint32_t  read_len;
    /* cppcheck-suppress misra-c2012-11.3 */
    DESC_CONF_T  *conf = (DESC_CONF_T *)desc_buff;
    uint16_t  request_len;
    int       ret;

    /*------------------------------------------------------------------------------------*/
    /* Issue GET DESCRIPTOR command to get configuration descriptor                       */
    /*------------------------------------------------------------------------------------*/
    ret = usbh_ctrl_xfer(udev, REQ_TYPE_IN | REQ_TYPE_STD_DEV | REQ_TYPE_TO_DEV,
                         USB_REQ_GET_DESCRIPTOR,
                         (uint16_t)(((uint32_t)USB_DT_STANDARD | (uint32_t)USB_DT_CONFIGURATION) << 8U), 0, 9,
                         desc_buff, &read_len, 200);

    if (ret < 0)
    {
        return ret;
    }

    if ((int)conf->wTotalLength > buff_len)
    {
        USB_error("Device configuration %d length > %d!\n", conf->wTotalLength, buff_len);
        return USBH_ERR_DATA_OVERRUN;
    }

    request_len = conf->wTotalLength;
    read_len = (uint32_t)request_len;

    ret = usbh_ctrl_xfer(udev, REQ_TYPE_IN | REQ_TYPE_STD_DEV | REQ_TYPE_TO_DEV,
                         USB_REQ_GET_DESCRIPTOR,
                         (uint16_t)(((uint32_t)USB_DT_STANDARD | (uint32_t)USB_DT_CONFIGURATION) << 8U), 0, request_len,
                         desc_buff, &read_len, 200);

    if (ret < 0)
    {
        return ret;
    }

    return 0;
}
/// @cond HIDDEN_SYMBOLS
#if 0
/**
 *  @brief  Get string descriptor from the USB device.
  * @param[in]  udev       The target USB device.
 *  @param[in]  index      Index of string descriptor
 *  @param[out] desc_buff  Data buffer to receive the string descriptor data.
 *  @param[in]  buff_len   Valid length of <desc_buff>
 *  @return   Success or not.
 *  @retval   0  Success
 *  @retval   Otherwise  Failed
 */
static int usbh_get_string_descriptor(UDEV_T *udev, int index, uint8_t *desc_buff, int buff_len)
{
    uint32_t  read_len;
    int       ret;

    /*------------------------------------------------------------------------------------*/
    /* Issue GET DESCRIPTOR command to get configuration descriptor                       */
    /*------------------------------------------------------------------------------------*/
    ret = usbh_ctrl_xfer(udev, REQ_TYPE_IN | REQ_TYPE_STD_DEV | REQ_TYPE_TO_DEV,
                         USB_REQ_GET_DESCRIPTOR,
                         (uint16_t)((((uint32_t)USB_DT_STANDARD | (uint32_t)USB_DT_STRING) << 8U) | index), 0x0409, buff_len,
                         desc_buff, &read_len, 200);
    return ret;
}
#endif
/**
 *  @brief  Issue a standard request SET_FEATURE to clear USB device endpoint halt state.
 *  @param[in] ep_addr  Endpoint to be clear halt.
 *  @return   Success or not.
 *  @retval   0  Success
 *  @retval   Otherwise  Failed
 */
int usbh_clear_halt(UDEV_T *udev, uint16_t ep_addr)
{
    uint32_t  read_len;

    USB_debug("Clear endpoint 0x%x halt.\n", ep_addr);
    return usbh_ctrl_xfer(udev, REQ_TYPE_OUT | REQ_TYPE_STD_DEV | REQ_TYPE_TO_EP,
                          USB_REQ_CLEAR_FEATURE, 0, ep_addr, 0,
                          USBNULL, &read_len, 100);
}

/// @endcond HIDDEN_SYMBOLS
/**
 * @brief    Parse endpoint descriptor.
 * @param[in]  alt      The alternative interface.
 * @param[in]  ep_idx   Index of the endpoint within the alternative interface.
 * @param[in]  desc_buff  Pointer to the endpoint descriptor buffer.
 * @param[in]  len      Length of the descriptor buffer.
 *
 * @return   Parsed length of endpoint descriptor.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
 * <b>Justification:</b> Descriptor bytes are decoded as an endpoint descriptor after the
 *                       parser validates the descriptor length and type.
 */
static int  usbh_parse_endpoint(ALT_IFACE_T *alt, int ep_idx, uint8_t *desc_buff, int len)
{
    DESC_EP_T    *ep_desc = USBNULL;
    int          parsed_len = 0;
    unsigned int pksz;
    int          len_tmp = len;

    while (len_tmp > 0)
    {
        /* cppcheck-suppress misra-c2012-11.3 */
        ep_desc = (DESC_EP_T *)&desc_buff[parsed_len];

        if ((len_tmp < (int)ep_desc->bLength) || (ep_desc->bLength < 2U))
        {
            USB_error("ERR DESCRIPTOR EP LEN [0x%X %d]\n", ep_desc->bDescriptorType, ep_desc->bLength);
            return USBH_ERR_DESCRIPTOR;
        }

        if (ep_desc->bDescriptorType == USB_DT_ENDPOINT)
        {
            break;     /* endpoint descriptor found */
        }

        /* unrecognized descriptor */
        USB_vdebug("ignore descriptor 0x%X %d\n", ep_desc->bDescriptorType, ep_desc->bLength);
        parsed_len += (int)ep_desc->bLength;
        len_tmp -= (int)ep_desc->bLength;
    }

    if (ep_desc == USBNULL)
    {
        USB_error("ERR - no endpoint descriptor found!\n");
        return USBH_ERR_DESCRIPTOR;
    }

    USB_vdebug("Descriptor Found - Alt: %d, Endpoint 0x%x, remaining len: %d\n", alt->ifd->bAlternateSetting, ep_desc->bEndpointAddress, len_tmp);

    alt->ep[ep_idx].bEndpointAddress = ep_desc->bEndpointAddress;
    alt->ep[ep_idx].bmAttributes     = ep_desc->bmAttributes;
    alt->ep[ep_idx].bInterval        = ep_desc->bInterval;
    /* HS isochronous/interrupt endpoints use wMaxPacketSize bits 12:11 for additional transactions per microframe. */
    pksz = ep_desc->wMaxPacketSize;
    pksz = (pksz & 0x07FFU) * (1U + ((pksz >> 11U) & 3U));
    alt->ep[ep_idx].wMaxPacketSize   = (uint16_t)pksz;
    alt->ep[ep_idx].hw_pipe          = USBNULL;

    return parsed_len + (int)ep_desc->bLength;
}

/**
  * @brief    Parse interface descriptor.
  * @param[in]  udev    The USB device.
  * @param[in]  desc_buff   Descriptor buffer
  * @param[in]  len         Remaining length of descriptors in buffer.
  * @return   Driver registration success or not.
  * @retval   0  Success
  * @retval   USBH_ERR_MEMORY_OUT   Registered drivers have reached MAX_UDEV_DRIVER limitation.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
 * <b>Justification:</b> Descriptor byte storage is interpreted as the standard interface
 *                       and descriptor-header layouts only while the parser bounds checks it.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> The USB host allocator returns untyped storage that is initialized
 *                       as an interface object before the typed pointer is used.
  */
static int  usbh_parse_interface(UDEV_T *udev, uint8_t *desc_buff, int len)
{
    int         i;
    int         matched;
    int         parsed_len = 0;
    DESC_HDR_T  *hdr = USBNULL;
    DESC_IF_T   *if_desc;
    IFACE_T     *iface = USBNULL;
    int         ret = -1;
    int         len_tmp = len;
    int8_t      bDone = 0;
    int8_t      bError = 0;

    /* cppcheck-suppress misra-c2012-11.5 */
    iface = usbh_alloc_mem((int)sizeof(*iface)); /* create an interface                        */

    if (iface == USBNULL)
    {
        return USBH_ERR_MEMORY_OUT;
    }

    iface->udev = udev;
    iface->aif = &iface->alt[0];            /* Default active interface should be the
                                               first found alternative interface          */
    /* cppcheck-suppress misra-c2012-11.3 */
    iface->if_num = ((DESC_IF_T *)desc_buff)->bInterfaceNumber;

    while ((len_tmp > 0) && (bDone == 0))
    {
        /*--------------------------------------------------------------------------------*/
        /* Find the first/next interface descriptor                                       */
        /*--------------------------------------------------------------------------------*/
        /* cppcheck-suppress misra-c2012-11.3 */
        if_desc = (DESC_IF_T *)&desc_buff[parsed_len];

        if (if_desc->bDescriptorType != USB_DT_INTERFACE)
        {
            parsed_len += (int)if_desc->bLength;
            len_tmp -= (int)if_desc->bLength;
            continue;
        }

        if (if_desc->bInterfaceNumber != iface->if_num)
        {
            bDone = 1;                      /* parse_done                                 */
            continue;
        }

        if ((int)if_desc->bNumEndpoints > MAX_EP_PER_IFACE)
        {
            USB_error("IF EP LIMITE %d\n", if_desc->bNumEndpoints);
            ret = USBH_ERR_IF_EP_LIMIT;
            bError = 1;
            bDone = 1;
            continue;
        }

        /* Step over the interface descriptor */
        parsed_len += (int)if_desc->bLength;
        len_tmp -= (int)if_desc->bLength;
        USB_vdebug("Descriptor Found - Interface %d, Alt: %d, num_alt:%d, remaining len: %d\n", if_desc->bInterfaceNumber, if_desc->bAlternateSetting, iface->num_alt, len_tmp);

        /*--------------------------------------------------------------------------------*/
        /* Add to alternative interface list                                              */
        /*--------------------------------------------------------------------------------*/
        if (iface->num_alt >= (uint8_t)MAX_ALT_PER_IFACE)
        {
            ret = USBH_ERR_IF_ALT_LIMIT;
            bError = 1;
            bDone = 1;
            continue;
        }

        /*--------------------------------------------------------------------------------*/
        /* Find the next alternative interface or endpoint descriptor                     */
        /*--------------------------------------------------------------------------------*/
        while ((len_tmp > 0) && (bDone == 0))
        {
            /* cppcheck-suppress misra-c2012-11.3 */
            hdr = (DESC_HDR_T *)&desc_buff[parsed_len];

            if ((len_tmp < (int)hdr->bLength) || (hdr->bLength < 2U))
            {
                USB_error("ERR DESCRIPTOR IF LEN [0x%X %d]\n", hdr->bDescriptorType, hdr->bLength);
                ret = USBH_ERR_DESCRIPTOR;
                bError = 1;
                bDone = 1;
            }
            else if (hdr->bDescriptorType == USB_DT_CONFIGURATION)
            {
                bDone = 1;                  /* is other configuration, parsing completed  */
            }
            else if ((hdr->bDescriptorType == USB_DT_INTERFACE) || (hdr->bDescriptorType == USB_DT_ENDPOINT))
            {
                break;                      /* the first endpoint descriptor found        */
            }
            else
            {
                /* unrecognized descriptor */
                USB_vdebug("ignore descriptor 0x%X %d\n", hdr->bDescriptorType, hdr->bLength);
                parsed_len += (int)hdr->bLength;
                len_tmp -= (int)hdr->bLength;
            }
        }

        if (bDone != 0)
        {
            continue;
        }

        iface->alt[iface->num_alt].ifd = if_desc;
        iface->num_alt++;

        if (len_tmp == 0)
        {
            bDone = 1;                      /* parse_done                                 */
            continue;
        }

        if (hdr->bDescriptorType == USB_DT_INTERFACE)
        {
            continue;                       /* is the next interface descriptor           */
        }

        USB_vdebug("Finding %d endpoints of interface %d, alt %d...\n", if_desc->bNumEndpoints, if_desc->bInterfaceNumber, if_desc->bAlternateSetting);

        /* parsing all endpoint descriptors */
        for (i = 0; (i < (int)if_desc->bNumEndpoints) && (bDone == 0); i++)
        {
            ret = usbh_parse_endpoint(&iface->alt[iface->num_alt - 1U], i, &desc_buff[parsed_len], len_tmp);

            if (ret < 0)
            {
                bError = 1;
                bDone = 1;
            }
            else
            {
                parsed_len += ret;
                len_tmp -= ret;
                USB_vdebug("EP parse remaining %d\n", len_tmp);
            }
        }
    }

    /* error path */
    if (bError != 0)
    {
        (void)usbh_free_mem(iface, (int)sizeof(*iface));
        return ret;
    }

    /*
     *  Probing all registered USB device drivers to find a matched driver.
     */
    matched = 0;

    for (i = 0; i < MAX_UDEV_DRIVER; i++)
    {
        if ((_drivers[i] != USBNULL) && (_drivers[i]->probe(iface) == 0))
        {
            matched = 1;
            break;
        }
    }

    if ((matched != 0) && (i < MAX_UDEV_DRIVER))
    {
        iface->driver = _drivers[i];        /* have a driver now */
        iface->next = USBNULL;

        /* Added this interface to USB device interface list */
        if (udev->iface_list == USBNULL)
        {
            udev->iface_list = iface;
        }
        else
        {
            iface->next = udev->iface_list;
            udev->iface_list = iface;
        }
    }
    else
    {
        (void)usbh_free_mem(iface, (int)sizeof(*iface));
        iface = USBNULL;
    }

    return parsed_len;
}

/**
 * @brief Parse the USB configuration descriptor.
 * @param[in]  udev      The USB device.
 * @param[in]  desc_buff  Descriptor buffer
 *
 * @return   Success or not.
 * @retval   0  Success
 * @retval   USBH_ERR_DESCRIPTOR  Descriptor parsing error
 * @retval   Otherwise  Failed
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
 * <b>Justification:</b> Descriptor byte storage is interpreted as standard configuration
 *                       and descriptor-header layouts while this parser bounds checks it.
 */
static int  usbh_parse_configuration(UDEV_T *udev, uint8_t *desc_buff)
{
    /* cppcheck-suppress misra-c2012-11.3 */
    const DESC_CONF_T  *config = (const DESC_CONF_T *)desc_buff;
    int    i;
    int    len;
    int    parsed_len;

    len = (int)config->wTotalLength;

    parsed_len = (int)config->bLength;
    len -= (int)config->bLength;

    USB_vdebug("Parsing CONFIG =>\n");

    for (i = 0; i < (int)config->bNumInterfaces; i++)
    {
        /*
         *  find the next interface descriptor
         */
        while (len >= (int)sizeof(DESC_HDR_T))
        {
            DESC_HDR_T   *hdr;
            /* cppcheck-suppress misra-c2012-11.3 */
            hdr = (DESC_HDR_T *)&desc_buff[parsed_len];

            if (((int)hdr->bLength > len) || (hdr->bLength < 2U))
            {
                USB_error("ERR DESCRIPTOR CONFIG [%d]\n", hdr->bLength);
                return USBH_ERR_DESCRIPTOR;
            }

            if (hdr->bDescriptorType == USB_DT_INTERFACE)
            {
                break;
            }

            USB_debug("ignore descriptor 0x%X %d\n", hdr->bDescriptorType, hdr->bLength);

            parsed_len += (int)hdr->bLength;
            len -= (int)hdr->bLength;
        }

        int    ret;
        ret = usbh_parse_interface(udev, &desc_buff[parsed_len], len);

        if (ret < 0)
        {
            return ret;
        }

        parsed_len += ret;
        len -= ret;
        USB_vdebug("IFACE parse remaining %d\n", len);
    }

    if (len > 0)
    {
        USB_debug("ERR DESCRIPTOR CONFIG LEN %d\n", len);
        return USBH_ERR_DESCRIPTOR;
    }

    return len;
}
#if 0
static void print_usb_string(char *lead, uint8_t *str)
{
    int  len;
    int  i = 2;

    USB_debug("%s", lead);
    len = str[0];

    while (i < len)
    {
        USB_debug("%c", str[i]);
        i += 2;
    }

    USB_debug("\n");
}
#endif

/**
 * @brief Connect a USB device and retrieve its configuration descriptor.
 * @param[in]  udev  The USB device.
 *
 * @return   Success or not.
 * @retval   0  Success
 * @retval   < 0   Failed. Refer to error code definitions.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> The USB host allocator returns storage that is used as a standard
 *                       configuration descriptor after the descriptor transfer completes.
 */
int  connect_device(UDEV_T *udev)
{
    DESC_CONF_T  *conf;
    DESC_CONF_T  conf_header; // configer Header
    uint32_t     read_len;
    int          ret;
    uint16_t     conf_total_len = 0U;

    USB_debug("Connect device =>\n");

    delay_us(100 * 1000);                   /* initially, give 100 ms delay               */

    USB_debug("get device =>\n");
    (void)usbh_get_device_descriptor(udev, &udev->descriptor);

    (void)reset_device(udev);
    USB_debug("reset device =>\n");

    delay_us(100 * 1000);                   /* initially, give 100 ms delay               */

    ret = usbh_set_address(udev);

    if (ret < 0)
    {
        USB_debug("Set address command failed!!\n");
        return ret;
    }

    delay_us(100 * 1000);                   /* initially, give 100 ms delay               */

    USB_debug("New %s device address %d assigned.\n", (udev->speed == SPEED_HIGH) ? "high-speed" : ((udev->speed == SPEED_FULL) ? "full-speed" : "low-speed"), udev->dev_num);

    /* Get device descriptor again with new device address */
    ret = usbh_get_device_descriptor(udev, &udev->descriptor);

    if (ret < 0)
    {
        free_dev_address((int)udev->dev_num);
        return ret;
    }

#if  defined(DUMP_DESCRIPTOR) && defined(ENABLE_DEBUG_MSG)
    dump_device_descriptor(&udev->descriptor);
#endif

    if (udev->descriptor.bNumConfigurations != 1U)
    {
        USB_debug("Warning! This device has multiple configurations [%d]. \n", udev->descriptor.bNumConfigurations);
    }

    /*------------------------------------------------------------------------------------*/
    /*  Step 1: Get Configuration Descriptor Header (9 bytes) first                       */
    /*------------------------------------------------------------------------------------*/
    ret = usbh_get_config_descripotr_total_length(udev, &conf_header);

    if (ret < 0)
    {
        USB_debug("Get config descriptor header failed!\n");
        free_dev_address((int)udev->dev_num);
        return ret;
    }

    conf_total_len = conf_header.wTotalLength;
    USB_debug("Config Descriptor Total Length: %d\n", conf_total_len);

    /* cppcheck-suppress misra-c2012-11.5 */
    conf = (DESC_CONF_T *)usbh_alloc_mem((int)conf_total_len);

    if (conf == USBNULL)
    {
        free_dev_address((int)udev->dev_num);
        return USBH_ERR_MEMORY_OUT;
    }

    udev->cfd_buff = (uint8_t *)conf;

    /* Get configuration descriptor again with new device address */
    ret = usbh_get_config_descriptor(udev, (uint8_t *)conf, (int)conf_total_len);

    if (ret < 0)
    {
        free_dev_address((int)udev->dev_num);
        return ret;
    }

#if  defined(DUMP_DESCRIPTOR) && defined(ENABLE_DEBUG_MSG)
    dump_config_descriptor(conf);
#endif

#if 0  /* printf string descriptors, for debug only */
    str_buff = (uint8_t *)usbh_alloc_mem(MAX_DESC_BUFF_SIZE);

    if (udev->descriptor.iManufacturer != 0)
    {
        (void)usbh_get_string_descriptor(udev, udev->descriptor.iManufacturer, str_buff, MAX_DESC_BUFF_SIZE);
        print_usb_string("Manufactor: ", str_buff);
    }

    if (udev->descriptor.iProduct != 0)
    {
        (void)usbh_get_string_descriptor(udev, udev->descriptor.iProduct, str_buff, MAX_DESC_BUFF_SIZE);
        print_usb_string("Product: ", str_buff);
    }

    if (udev->descriptor.iSerialNumber != 0)
    {
        (void)usbh_get_string_descriptor(udev, udev->descriptor.iSerialNumber, str_buff, MAX_DESC_BUFF_SIZE);
        print_usb_string("Serial Number: ", str_buff);
    }

    (void)usbh_free_mem(str_buff, MAX_DESC_BUFF_SIZE);
#endif

    /* Always select the first configuration */
    ret = usbh_set_configuration(udev, conf->bConfigurationValue);

    if (ret < 0)
    {
        USB_debug("Set configuration %d failed!\n", conf->bConfigurationValue);
        free_dev_address((int)udev->dev_num);
        return ret;
    }

    /* Parse the configuration/interface/endpoint descriptors and find corresponding drivers. */
    ret = usbh_parse_configuration(udev, (uint8_t *)conf);

    if (ret < 0)
    {
        USB_debug("Parse configuration %d failed!\n", conf->bConfigurationValue);
        free_dev_address((int)udev->dev_num);
        return ret;
    }

    if (conf->bmAttributes & (1U << 5))
    {
        /* If this configuration supports remote wakeup, enable it.                           */
        if (usbh_ctrl_xfer(udev, REQ_TYPE_OUT | REQ_TYPE_STD_DEV | REQ_TYPE_TO_DEV,
                           USB_REQ_SET_FEATURE, 0x01, 0x0000, 0x0000,
                           USBNULL, &read_len, 300) < 0)
        {
            USB_debug("Device does not accept remote wakeup enable command.\n");
        }
    }

    if (g_conn_func)
    {
        g_conn_func(udev, 0);
    }

    return ret;
}

/**
 * @brief Reset the USB device.
 * @param[in]  udev  The USB device.
 *
 * @return   Success or not.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
 * <b>Justification:</b> The existing configuration byte buffer is interpreted as the
 *                       standard configuration descriptor owned by the USB device.
 */
int  usbh_reset_device(UDEV_T *udev)
{
    IFACE_T      *iface;
    DESC_CONF_T  *conf;
    uint32_t     read_len;
    uint16_t     conf_total_len;
    int          dev_num;
    int          ret;

    USB_debug("Reset device =>\n");

    (void)usbh_pooling_hubs();

    /*------------------------------------------------------------------------------------*/
    /*  Disconnect device                                                                 */
    /*------------------------------------------------------------------------------------*/

    if (g_disconn_func)
    {
        g_disconn_func(udev, 0);
    }

    (void)usbh_quit_xfer(udev, &(udev->ep0));    /* Quit control transfer if hw_pipe is not USBNULL.  */

    /* Notified all actived interface device driver  */
    iface = udev->iface_list;

    while (iface != USBNULL)
    {
        udev->iface_list = iface->next;
        iface->driver->disconnect(iface);
        (void)usbh_free_mem(iface, (int)sizeof(*iface));
        iface = udev->iface_list;
    }

    /*------------------------------------------------------------------------------------*/
    /*  Reset device                                                                      */
    /*------------------------------------------------------------------------------------*/
    USB_debug("Port device =>\n");
    (void)reset_device(udev);


    delay_us(100 * 1000);                   /* initially, give 100 ms delay               */


    /*------------------------------------------------------------------------------------*/
    /*  Set address (use current address)                                                 */
    /*------------------------------------------------------------------------------------*/
    USB_debug("Set Address =>\n");
    dev_num = (int)udev->dev_num;
    udev->dev_num = 0;
    /* Issue SET ADDRESS command to set the same device address                           */
    ret = usbh_ctrl_xfer(udev, REQ_TYPE_OUT | REQ_TYPE_STD_DEV | REQ_TYPE_TO_DEV,
                         USB_REQ_SET_ADDRESS, (uint16_t)dev_num, 0, 0,
                         USBNULL, &read_len, 100);
    udev->dev_num = (uint8_t)dev_num;

    if (ret < 0)
    {
        return ret;
    }

    delay_us(100 * 1000);                   /* after set address, give 100 ms delay       */

    /*------------------------------------------------------------------------------------*/
    /*  Get device descriptor                                                             */
    /*------------------------------------------------------------------------------------*/
    USB_debug("Get descriptor =>\n");
    /* Get device descriptor again with new device address */
    ret = usbh_get_device_descriptor(udev, &udev->descriptor);

    if (ret < 0)
    {
        return ret;
    }

    /*------------------------------------------------------------------------------------*/
    /*  Get configuration descriptor                                                      */
    /*------------------------------------------------------------------------------------*/

    /* cppcheck-suppress misra-c2012-11.3 */
    conf = (DESC_CONF_T *)udev->cfd_buff;   /* using the previously allocated buffer      */
    conf_total_len = conf->wTotalLength;

    /* Get configuration descriptor again with new device address */
    ret = usbh_get_config_descriptor(udev, (uint8_t *)conf, (int)conf_total_len);

    if (ret < 0)
    {
        return ret;
    }

    /* Always select the first configuration */
    ret = usbh_set_configuration(udev, (uint8_t)udev->cur_conf);

    if (ret < 0)
    {
        USB_debug("Set configuration %d failed!\n", udev->cur_conf);
        return ret;
    }

    /* Parse the configuration/interface/endpoint descriptors and find corresponding drivers. */
    ret = usbh_parse_configuration(udev, (uint8_t *)conf);

    if (ret < 0)
    {
        USB_debug("Parse configuration %d failed!\n", conf->bConfigurationValue);
        return ret;
    }

    /* Enable remote wakeup                                                                   */
    if (usbh_ctrl_xfer(udev, REQ_TYPE_OUT | REQ_TYPE_STD_DEV | REQ_TYPE_TO_DEV,
                       USB_REQ_SET_FEATURE, 0x01, 0x0000, 0x0000,
                       USBNULL, &read_len, 300) < 0)
    {
        USB_debug("Device does not accept remote wakeup enable command.\n");
    }

    if (g_conn_func)
    {
        (void)g_conn_func(udev, 0);
    }

    return ret;
}

/**
 * @brief Disconnect the USB device and release its resources.
 *
 * @param[in]  udev  The USB device.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
 * <b>Justification:</b> The configuration byte buffer begins with the standard
 *                       configuration descriptor header used to obtain its allocated size.
 */
void disconnect_device(UDEV_T *udev)
{
    IFACE_T      *iface;

    USB_debug("disconnect device...\n");

    if (g_disconn_func)
    {
        g_disconn_func(udev, 0);
    }

    (void)usbh_quit_xfer(udev, &(udev->ep0));    /* Quit control transfer if hw_pipe is not USBNULL.  */

    /*------------------------------------------------------------------------------------*/
    /* FCall driver's disconnect first to stop application layer transfer requests        */
    /*------------------------------------------------------------------------------------*/
    iface = udev->iface_list;

    while (iface != USBNULL)
    {
        IFACE_T *iface_next = iface->next;

        if ((iface->driver != USBNULL) && (iface->driver->disconnect != USBNULL))
        {
            iface->driver->disconnect(iface);  // stop application layer first (set flag_streaming = 0)
        }

        iface = iface_next;
    }

    /*------------------------------------------------------------------------------------*/
    /* release Interface source                                                             */
    /*------------------------------------------------------------------------------------*/
    iface = udev->iface_list;

    while (iface != USBNULL)
    {
        udev->iface_list = iface->next;
        (void)usbh_free_mem(iface, (int)sizeof(*iface));
        iface = udev->iface_list;
    }

    if (udev->cfd_buff != USBNULL)
    {
        /* cppcheck-suppress misra-c2012-11.3 */
        const DESC_CONF_T *conf = (const DESC_CONF_T *)udev->cfd_buff;
        uint16_t total_len = conf->wTotalLength;
        (void)usbh_free_mem(udev->cfd_buff, (int)total_len);
        udev->cfd_buff = USBNULL;
    }

    /* remove device from global device list */
    free_dev_address((int)udev->dev_num);
    free_device(udev);

    (void)usbh_memory_used();
}
/// @cond HIDDEN_SYMBOLS

#if 0
static int  check_device(UDEV_T *udev)
{
    UDEV_T  *d;

    if (udev == USBNULL)
    {
        return USBH_ERR_INVALID_PARAM;
    }

    //if((udev->hc_driver != &ohci_driver) && (udev->hc_driver != &ehci_driver))
    //    return USBH_ERR_INVALID_PARAM;

    d = g_udev_list;

    while (d)
    {
        if (d == udev)
        {
            return USBH_OK;
        }

        d = d->next;
    }

    return USBH_ERR_INVALID_PARAM;
}
#endif

EP_INFO_T *usbh_iface_find_ep(IFACE_T *iface, uint8_t ep_addr, uint8_t dir_type)
{
    ALT_IFACE_T  *aif = iface->aif;
    int     i;

    if (ep_addr == 0U)      /* find the first EP matched with specified direction and type */
    {
        for (i = 0; i < (int)aif->ifd->bNumEndpoints; i++)
        {
            if (((aif->ep[i].bEndpointAddress & EP_ADDR_DIR_MASK) == (dir_type & EP_ADDR_DIR_MASK)) &&
                    ((aif->ep[i].bmAttributes & EP_ATTR_TT_MASK) == (dir_type & EP_ATTR_TT_MASK)))
            {
                return &aif->ep[i];
            }
        }
    }
    else                   /* find the EP with specified endpoint address                 */
    {
        for (i = 0; i < (int)aif->ifd->bNumEndpoints; i++)
        {
            if (aif->ep[i].bEndpointAddress == ep_addr)
            {
                return &aif->ep[i];
            }
        }
    }

    return USBNULL;
}

void  usbh_dump_buff_bytes(uint8_t *buff, int nSize)
{
    int     nIdx;
    int     i;
    int     nSizeTmp = nSize;

    nIdx = 0;

    while (nSizeTmp > 0)
    {
        USB_debug("0x%04X  ", nIdx);

        for (i = 0; i < 16; i++)
        {
            USB_debug("%02x ", buff[nIdx + i]);
        }

        USB_debug("  ");

        for (i = 0; i < 16; i++)
        {
            if ((buff[nIdx + i] >= (uint8_t)0x20) && (buff[nIdx + i] < (uint8_t)127))
            {
                USB_debug("%c", buff[nIdx + i]);
            }
            else
            {
                USB_debug(".");
            }

            nSizeTmp--;
        }

        nIdx += 16;
        USB_debug("\n");
    }

    USB_debug("\n");
}

void usbh_dump_iface(IFACE_T *iface)
{
    USB_debug("\n  [IFACE info] (0x%x)\n", (int)iface);
    USB_debug("  ----------------------------------------------\n");
    USB_debug("  udev       = 0x%x\n", (int)iface->udev);
    USB_debug("  if_num     = %d\n",   iface->if_num);
    USB_debug("  driver     = 0x%x\n", (int)iface->driver);
    USB_debug("  next       = 0x%x\n", (int)iface->next);
    (void)usbh_dump_interface_descriptor(iface->aif->ifd);
}

void usbh_dump_ep_info(EP_INFO_T *ep)
{
    USB_debug("\n  [Endpoint Info] (0x%x)\n", (int)ep);
    USB_debug("  ----------------------------------------------\n");
    USB_debug("  bEndpointAddress    = 0x%02x\n", ep->bEndpointAddress);
    USB_debug("  bmAttributes        = 0x%02x\n", ep->bmAttributes);
    USB_debug("  bInterval           = %d\n",     ep->bInterval);
    USB_debug("  wMaxPacketSize      = %d\n",     ep->wMaxPacketSize);
    USB_debug("  hw_pipe             = 0x%x\n", (int)ep->hw_pipe);
}

/// @endcond HIDDEN_SYMBOLS

/**
 * @brief  Default debug output function for USB Host Library.
 *         Routes output through standard printf (retargeted to UART by application).
 *         This wrapper isolates the library from direct stdio dependency (MISRA C:2012 Rule 21.6).
 * @param[in] fmt  Format string (printf-compatible)
 * @return  Number of characters printed
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 17.1<br>
 * <b>Justification:</b> This isolated compatibility wrapper uses stdarg to forward formatted
 *                       diagnostic output to the application-provided printf retarget.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 21.6<br>
 * <b>Justification:</b> vprintf is confined to the USB host diagnostic wrapper and relies
 *                       on the application's configured output retarget.
 */
int usbh_printf(const char *fmt, ...)
{
    /* cppcheck-suppress misra-c2012-17.1 */
    va_list args;
    int     ret;

    /* cppcheck-suppress misra-c2012-17.1 */
    va_start(args, fmt);
    /* cppcheck-suppress misra-c2012-21.6 */
    ret = vprintf(fmt, args);
    /* cppcheck-suppress misra-c2012-17.1 */
    va_end(args);

    return ret;
}
