...
#else
    #error "FL_SIZE must be 256U, 512U, or 1024U"
#endif
...
static int  ehci_reinit(void)
{
    ...
    while (((_ehci->UCMDR & HSUSBH_UCMDR_HCRST_Msk) != 0U) && (timeout > 0))
    {
        delay_us(1000);
        timeout -= 1000;
    }
...
static void  write_qtd_bptr(qTD_T *qtd, uint32_t buff_addr, uint32_t xfer_len)
{
    int       i;
...
/**
 * @brief Handle control transfer for the given UTR (USB Transfer Request).
 *
 * @param[in,out] utr Pointer to the UTR structure containing transfer details.
 * @return 0 on success, or an error code on failure.
 * @retval USBH_ERR_BUFF_OVERRUN if the buffer overrun occurs.
 * @retval USBH_ERR_EHCI_QH_BUSY if the endpoint hardware-pipe context is busy.
 * @retval USBH_ERR_MEMORY_OUT if memory allocation for qTDs fails.
 ...
 *                       and restored as the EHCI queue-head type owned by this driver.
 */
static int ehci_ctrl_xfer(UTR_T *utr)
{
    ...
    if (utr->data_len > 0U)
    {
        ...
        if ((utr->setup.bmRequestType & 0x80U) == 0U)
        {
...
/**
 * @brief Handle bulk transfer for the given UTR (USB Transfer Request).
 *
 * @param[in,out] utr Pointer to the UTR structure containing transfer details.
 * @return 0 on success, or an error code on failure.
 * @retval USBH_ERR_EHCI_QH_BUSY if the endpoint hardware-pipe context is busy.
...
static int ehci_bulk_xfer(UTR_T *utr)
{
    ...
    while (data_len > 0U)
    {
        ...
        write_qtd_bptr(qtd, (uint32_t)&buff[buff_offset], xfer_len);
...
/**
 * @brief Handle interrupt transfer for the given UTR (USB Transfer Request).
 *
 * @param[in,out] utr Pointer to the UTR structure containing transfer details.
 * @return 0 on success, or an error code on failure.
 * @retval USBH_ERR_MEMORY_OUT if memory allocation for qTDs fails.
 ...
 * @return 0 on success, or an error code on failure.
 * @retval USBH_ERR_EHCI_QH_BUSY if the endpoint hardware-pipe context is busy.
 * @retval USBH_ERR_NOT_FOUND if the endpoint or UTR is not found.
 * @retval Other error codes as defined in the USB host library.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
...
static int ehci_quit_xfer(UTR_T *utr, EP_INFO_T *ep)
{
    QH_T       *qh;
    uint32_t t0;
    uint32_t iaad_pending;
    uint32_t iaa_pending;
    ...
    iaad_pending = _ehci->UCMDR & HSUSBH_UCMDR_IAAD_Msk;
    iaa_pending = _ehci->USTSR & HSUSBH_USTSR_IAA_Msk;

    while ((iaad_pending != 0U) || (iaa_pending != 0U))
    {
        if ((get_ticks() - t0) > 2U)
        {
            USB_error("%s - IAAD lost!! \n", __func__);
            (void)ehci_reinit();
            _ehci->UCMDR |= HSUSBH_UCMDR_IAAD_Msk;
            break;
        }
...
static int ehci_rh_port_reset(int port)
{
    ...
    for (retry = 0; retry < PORT_RESET_RETRY; retry++)
    {
        ...
        while (((get_ticks() - t0) < (reset_delay_ticks + 1U)) && (port_ready == 0))
        {
            uint32_t port_status;

            port_status = _ehci->UPSCR[port];

            if ((port_status & HSUSBH_UPSCR_CCS_Msk) == 0U)
            {
                port_ready = 1;
            }
            else
            {
                if ((port_status & (HSUSBH_UPSCR_CCS_Msk | HSUSBH_UPSCR_PE_Msk)) == (HSUSBH_UPSCR_CCS_Msk | HSUSBH_UPSCR_PE_Msk))
                {
                    port_ready = 1;
                }
            }
