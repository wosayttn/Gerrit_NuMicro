...
static int  clear_hub_feature(HUB_DEV_T *hub, int feature_selector)
{
    ...
    return usbh_ctrl_xfer(udev, REQ_TYPE_OUT | REQ_TYPE_CLASS_DEV | REQ_TYPE_TO_DEV,
                          USB_REQ_CLEAR_FEATURE, (uint16_t)feature_selector, 0, 0,
                          USBNULL, &read_len, 200);
}
...
static int  get_hub_status(HUB_DEV_T *hub, uint16_t *wHubStatus, uint16_t *wHubChange)
{
    ...
    *wHubStatus = (uint16_t)(((uint16_t)buff[1] << 8) | buff[0]);
    *wHubChange = (uint16_t)(((uint16_t)buff[3] << 8) | buff[2]);
    return 0;
}
...
static int  set_port_feature(HUB_DEV_T *hub, int feature_selector, int port)
{
    ...
    return usbh_ctrl_xfer(udev, REQ_TYPE_OUT | REQ_TYPE_CLASS_DEV | REQ_TYPE_TO_OTHER,
                          USB_REQ_SET_FEATURE, (uint16_t)feature_selector, (uint16_t)port, 0,
                          USBNULL, &read_len, 200);
}
...
static int  clear_port_feature(HUB_DEV_T *hub, int feature_selector, int port)
{
    ...
    return usbh_ctrl_xfer(udev, REQ_TYPE_OUT | REQ_TYPE_CLASS_DEV | REQ_TYPE_TO_OTHER,
                          USB_REQ_CLEAR_FEATURE, (uint16_t)feature_selector, (uint16_t)port, 0,
                          USBNULL, &read_len, 200);
}
...
static int  get_port_status(HUB_DEV_T *hub, int port, uint16_t *wPortStatus, uint16_t *wPortChange)
{
    ...
    ret =  usbh_ctrl_xfer(udev, REQ_TYPE_IN | REQ_TYPE_CLASS_DEV | REQ_TYPE_TO_OTHER,
                          USB_REQ_GET_STATUS, 0, (uint16_t)port, 4,
                          buff, &read_len, 200);
    ...
    *wPortStatus = (uint16_t)(((uint16_t)buff[1] << 8) | buff[0]);
    *wPortChange = (uint16_t)(((uint16_t)buff[3] << 8) | buff[2]);
    return 0;
}

/// @endcond HIDDEN_SYMBOLS
/**
 * @brief  Hub status change interrupt-in transfer complete callback.
 *
 * @static_deviation
 ...
 *                       to HUB_DEV_T here is an architectural requirement. The object registered
...
static int hub_probe(IFACE_T *iface)
{
    ...
    else
    {
        ...
        hub->bPwrOn2PwrGood = desc_hub.bPwrOn2PwrGood * 2U;
    ...
    {
#if defined(ENABLE_HUB_DBGMSG)
        ret = set_port_feature(hub, FS_PORT_POWER, (int)i);
...
#else
        (void)set_port_feature(hub, FS_PORT_POWER, (int)i);  /* ignore error for power enabling failure, just try best to enable it. */
#endif
    }

    {
        int power_ready_delay;

        power_ready_delay = ((int)hub->bPwrOn2PwrGood * 1000) + 100000;
        delay_us(power_ready_delay);   /* delay to wait hub power ready     */
    }
...
static int  hub_polling(void)
{
    ...
    for (i = 0; i < MAX_HUB_DEVICE; i++)
    {
        if ((g_hub_dev[i].iface != USBNULL) && (g_hub_dev[i].sc_bitmap != 0U))
        {
            /*
            ...
            {
                if (hub->sc_bitmap & ((uint16_t)1U << (uint32_t)port))
                {
