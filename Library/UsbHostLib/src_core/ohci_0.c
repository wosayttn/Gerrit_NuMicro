...
#define get_ohci_interval_shift      _ohci0_get_ohci_interval_shift
...
#define RH_PORT_STATUS_COUNT    (6U)
...
/*
 *  get_ohci_interval() always returns one of {1,2,4,8,16,32} (a power of two).
 *  This helper returns the equivalent right-shift count (0..5) instead, so a
 *  caller can replace "x / get_ohci_interval(bInterval)" with
 *  "x >> get_ohci_interval_shift(bInterval)" and avoid a runtime division.
 */
static uint32_t get_ohci_interval_shift(const int interval)
{
    int    i;
    uint32_t  interval_tmp = (uint32_t)interval;
    ...
    return 5U;                               /* for interval >= 32, i.e. shift by 5 (32)    */
}
...
static int  ohci_init(void)
{
    ...
    if (_ohci->HcRhDescriptorA & USBH_HcRhDescriptorA_PSM_Msk)
    {
        ...
        if (portcnt > RH_PORT_STATUS_COUNT)
        {
            portcnt = RH_PORT_STATUS_COUNT;
        }
...
/**
 * @brief Quit the current transfer via UTR or hardware endpoint.
 * @param[in,out] utr Pointer to the UTR structure containing transfer details.
 * @param[in,out] ep Pointer to the endpoint information structure.
 *
 * @return 0 on success, or an error code on failure.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> The endpoint hardware-pipe context is intentionally stored as void *
 *                       and restored as the OHCI endpoint-descriptor type owned by this driver.
 */
...
/// @endcond HIDDEN_SYMBOLS
/**
 * @brief Prepare and start an OHCI control transfer.
 * @param[in,out] utr Pointer to the UTR structure containing control transfer details.
 *
 * @return 0 on success, or an error code on failure.
 *
 * @static_deviation
...
static int ohci_rh_port_reset(int port)
{
    ...
    for (retry = 0; (retry < PORT_RESET_RETRY) && (bResetDone == 0); retry++)
    {
        ...
        while ((get_ticks() - t0) < ((uint32_t)((uint32_t)reset_time / 10U) + 1U))
        {
             ...
             */
            port_status = _ohci->HcRhPortStatus[port];

            if (((port_status & USBH_HcRhPortStatus_CCS_Msk) == 0U) ||
                    ((port_status & (USBH_HcRhPortStatus_PES_Msk | USBH_HcRhPortStatus_CCS_Msk)) == (USBH_HcRhPortStatus_PES_Msk | USBH_HcRhPortStatus_CCS_Msk)))
            {
                bResetDone = 1;
                break;
            }
...
static int ohci_rh_polling(void)
{
    ...
    for (i = 0; i < 1; i++)
    {
        ...
        if (_ohci->HcRhPortStatus[i] & USBH_HcRhPortStatus_CCS_Msk)
        {
            ...
            while (1)
            {
                udev = ohci_find_device_by_port((int)_ohci_port);
            ...
            udev->parent = USBNULL;
            udev->port_num = (uint8_t)_ohci_port;
...
static void td_done(TD_T *td)
{
    ...
    {
        uint16_t     sf;
        uint32_t     iso_sf_delta;
        uint32_t     interval_shift;
        unsigned int idx;

        sf = (uint16_t)(info & 0xFFFFU);
        iso_sf_delta = (((uint32_t)sf + 0x10000U) - (uint32_t)utr->iso_sf) & 0xFFFFU;

        /* get_ohci_interval() only ever returns a power-of-two value, so divide
         * by it via the equivalent right-shift instead of a runtime division. */
        interval_shift = get_ohci_interval_shift((int)td->ed->bInterval);
        idx = iso_sf_delta >> interval_shift;
