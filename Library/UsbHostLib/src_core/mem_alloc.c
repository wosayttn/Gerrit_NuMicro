...
{
    int read_hw_mem_pool_used = _hw_mem_pool_used;
    int read_dma_mem_pool_used = _dma_men_pool_used;
    USB_debug("USB H/W memory: %d/%d, DMA memory: %d/%d\n", read_hw_mem_pool_used, HW_MEM_UNIT_NUM,
              read_dma_mem_pool_used, DMA_MEM_UNIT_NUM);
    return (uint32_t)read_dma_mem_pool_used;
}

static void  memory_counter(int size)
{
    int new_size = _usbh_dma_mem_used + size;
    int new_max_size = _usbh_dma_max_mem_used;
    _usbh_dma_mem_used = new_size;

    if (_usbh_dma_mem_used > new_max_size)
    {
...
void *usbh_alloc_mem(int size)
{
    ...
    (void)memset(&_dma_mem_pool[start], 0, (uint32_t)DMA_MEM_UNIT_SIZE * (uint32_t)wanted);
    memory_counter(size);
...
int usbh_free_mem(const void *p, int size)
{
    ...
    offset = (paddr - base) / (uint32_t)DMA_MEM_UNIT_SIZE;
    start = (int)offset;
...
/**
 * @brief Allocate a new USB device structure.
 * @return Pointer to the allocated UDEV_T structure, or USBNULL on failure.
 * @retval USBNULL if memory allocation fails.
 * @retval Pointer to the allocated UDEV_T structure on success.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> The allocator returns untyped storage that is owned and initialized
 *                       as a USB device object before the typed pointer is used.
 */
UDEV_T *alloc_device(void)
{
    ...
    udev = usbh_alloc_mem((int)sizeof(*udev));
...
void free_device(UDEV_T *udev)
{
    ...
    (void)usbh_free_mem(udev, (int)sizeof(*udev));
...
/// @endcond HIDDEN_SYMBOLS
/**
 * @brief Allocate a new UTR (USB Transfer Request) structure.
 * @param[in] udev Pointer to the USB device associated with the transfer.
 * @return Pointer to the allocated UTR_T structure, or USBNULL on failure.
 * @retval USBNULL if memory allocation fails.
 * @retval Pointer to the allocated UTR_T structure on success.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> The allocator returns untyped storage that is owned and initialized
 *                       as a USB transfer-request object before the typed pointer is used.
 */
UTR_T *alloc_utr(UDEV_T *udev)
{
    ...
    /* cppcheck-suppress misra-c2012-11.5 */
    utr = usbh_alloc_mem((int)sizeof(*utr));
...
void free_utr(const UTR_T *utr)
{
    ...
    (void)usbh_free_mem(utr, (int)sizeof(*utr));
    memory_counter(-(int)sizeof(*utr));
}
...
QH_T *alloc_ehci_QH(void)
{
    ...
    for (i = (_sidx + 1U) % (uint32_t)HW_MEM_UNIT_NUM; i != _sidx; i = (i + 1U) % (uint32_t)HW_MEM_UNIT_NUM)
    {
        if (_hw_unit_used[i] == 0U)
        {
            _hw_unit_used[i] = 1;
...
iTD_T *alloc_ehci_iTD(void)
{
    ...
    for (i = (_sidx + 1U) % (uint32_t)HW_MEM_UNIT_NUM; i != _sidx; i = (i + 1U) % (uint32_t)HW_MEM_UNIT_NUM)
    {
        if (i >= ((uint32_t)HW_MEM_UNIT_NUM - 1U))
        {
            continue;
        }

        if ((_hw_unit_used[i] == 0U) && (_hw_unit_used[i + 1U] == 0U))
        {
            _hw_unit_used[i] = 1U;
            _hw_unit_used[i + 1U] = 1U;
            _sidx = i + 1U;
            _hw_mem_pool_used += 2;
