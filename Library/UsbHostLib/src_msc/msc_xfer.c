...
static int  do_scsi_command(MSC_T *msc, uint8_t *buff, uint32_t data_len, int bIsDataIn, int timeout_ticks)
{
    ...
    if (data_len > 0U)
    {
        if (bIsDataIn)
        {
            ret = msc_bulk_transfer(msc, msc->ep_bulk_in, buff, (int)data_len, 500);
        }
        else
        {
            ret = msc_bulk_transfer(msc, msc->ep_bulk_out, buff, (int)data_len, 500);
        }
