...
static int msc_bulk_transfer(MSC_T *msc, EP_INFO_T *ep, uint8_t *data_buff, uint32_t data_len, int timeout_ticks)
{
    UTR_T     *utr;
    uint32_t  t0;
    int       ret;
...
{
    static uint32_t __tag = 0x10E24388U;
#if (NVT_DCACHE_ON == 1)
    ...
    if (data_len > 0U)
    {
        if (bIsDataIn)
        {
            ...
            SCB_InvalidateDCache_by_Addr((void *)msc_tmp_buffer, (int32_t)data_len);
        ...
        else
        {
            ...
            SCB_CleanDCache_by_Addr(msc_tmp_buffer, (int32_t)data_len);
