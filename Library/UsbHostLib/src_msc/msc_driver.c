...
static MSC_T  *g_msc_list;       /* Global list of Mass Storage Class device. A multi-lun device can have
                             several instances appeared with different lun. */

static volatile uint8_t  g_fat_drv_used[USBDRV_CNT];
static TCHAR    _path[3] = { '3', ':', 0 };

static MSC_T    _msc_pool[USBH_MSC_POOL_NUM];
static uint8_t  _msc_used[USBH_MSC_POOL_NUM];
...
{
    int drv_char = drv_no + (int)'0';

    _path[0] =  (TCHAR)drv_char;

    (void)f_mount(NULL, _path, 1);
...
int  usbh_umas_ioctl(int drv_no, int cmd, void *buff)
{
    ...
    switch(cmd)
    {
        ...
        case GET_SECTOR_COUNT:
            (void)memcpy(buff, &msc->uTotalSectorN, sizeof(msc->uTotalSectorN));
            return RES_OK;

        case GET_SECTOR_SIZE:
            (void)memcpy(buff, &msc->nSectorSize, sizeof(msc->nSectorSize));
            return RES_OK;
...
static MSC_T *msc_alloc(void)
{
    ...
    if(msc == NULL)
    {
        USB_error("msc_alloc failed! All %d MSC_T slots are in use. Raise USBH_MAX_FUNC_DEV in config.h to support more mass storage devices.\n", USBH_MSC_POOL_NUM);
        return NULL;
    }
    (void)memset(msc, 0, sizeof(*msc));
    return msc;
}

static void msc_free(const MSC_T *msc)
{
    uint32_t  primask;
    uint8_t   was_used = 0U;
    int       i;
    ...
    if(i >= USBH_MSC_POOL_NUM)
    {
        USB_error("msc_free: device does not belong to _msc_pool! Ignored.\n");
    }
    else if(was_used == 0U)
    {
        USB_error("msc_free: slot %d released twice! It may still be in use by another owner.\n", i);
    }
    else
    {
        /* no action required */
...
static int  umass_init_device(MSC_T *msc)
{
    ...
    for(lun = 0; lun <= (int)msc->max_lun; lun++)
    {
        ...
        if(disk_ok == 0)
        {
            continue;              /* no valid disk in this lun */
        }
        ...
        else
        {
            ...
            {
                int drv_char = (int)try_msc->drv_no + (int)'0';
                _path[0] =  (TCHAR)drv_char;
            }

            (void)f_mount(&try_msc->fatfs_vol, _path, 1);
            bHasMedia = 1;
...
static void msc_disconnect(const IFACE_T *iface)
{
    int    i;
    MSC_T  *msc_p;
    MSC_T  *msc;
...
int  usbh_umas_init(void)
{
    ...
    fatfs_drive_int();
    (void)memset(_msc_used, 0, sizeof(_msc_used));
    g_msc_list = NULL;
