...
{
    int32_t  drv_char = (int32_t)'0' + drv_no;
    _path[0] = (TCHAR)drv_char;
...
/**
 * @brief  Issue GET_MAX_LUN MSC class request to query the maximum logical unit number.
 *
 * @static_deviation
 ...
 *                       its return value to uint8_t* here is the established, project-wide
...
/**
  ...
  */
int  usbh_umas_read(int drv_no, uint32_t sec_no, uint16_t sec_cnt, uint8_t *buff)
{
    MSC_T   *msc;
    ...
    cmd_blk->CDB[1]  = msc->lun << 5;
    cmd_blk->CDB[2]  = (uint8_t)((sec_no >> 24) & 0xFFU);
    cmd_blk->CDB[3]  = (uint8_t)((sec_no >> 16) & 0xFFU);
    cmd_blk->CDB[4]  = (uint8_t)((sec_no >> 8) & 0xFFU);
...
/**
  ...
  */
int  usbh_umas_write(int drv_no, uint32_t sec_no, uint16_t sec_cnt, uint8_t *buff)
{
    MSC_T   *msc;
    struct bulk_cb_wrap  *cmd_blk;         /* MSC Bulk-only command block */
    int   ret;
...
/**
  ...
  * @retval      UMAS_ERR_DRIVE_NOT_FOUND   There's no mass storage device mounted to this volume.
  * @retval      UMAS_ERR_IVALID_PARM       Invalid ioctl command.
  *
  * @static_deviation
  * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
  * <b>Justification:</b> buff is the generic void* ioctl buffer defined by the FatFs diskio
  *                       interface (see ff.h). The command code selects the concrete data type
  *                       written to it (always a uint32_t for these commands), so this is a
  *                       standard, well-defined generic-interface cast pattern.
  */
...
/// @endcond HIDDEN_SYMBOLS
/**
 * @brief  Initialize a Mass Storage device: probe each logical unit for valid media and
 *         mount it to a free FatFs drive volume.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> usbh_alloc_mem() is the USB Host common generic memory pool allocator;
 *                       it returns void* by design so it can back any object type. Converting
 *                       its return value to MSC_T* here is the established, project-wide
 *                       allocator usage pattern.
 */
static int  umass_init_device(MSC_T *msc)
{
    ...
    for (lun = 0; lun <= (int)msc->max_lun; lun++)
    {
        ...
        try_msc->uTotalSectorN = ((uint32_t)try_msc->scsi_buff[0] << 24) | ((uint32_t)try_msc->scsi_buff[1] << 16) |
                                 ((uint32_t)try_msc->scsi_buff[2] << 8) | (uint32_t)try_msc->scsi_buff[3];
        try_msc->nSectorSize = ((uint32_t)try_msc->scsi_buff[4] << 24) | ((uint32_t)try_msc->scsi_buff[5] << 16) |
                               ((uint32_t)try_msc->scsi_buff[6] << 8) | (uint32_t)try_msc->scsi_buff[7];
        ...
        else
        {
            ...
            int32_t drv_char = (int32_t)'0' + try_msc->drv_no;
            _path[0] = (TCHAR)drv_char;
            /* Deviation: f_mount is declared in ff.h which is not parsed by cppcheck MISRA config */
            // cppcheck-suppress misra-c2012-17.3
...
/**
 * @brief  Probe callback: check if the given interface is a supported Mass Storage device.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> usbh_alloc_mem() is the USB Host common generic memory pool allocator;
