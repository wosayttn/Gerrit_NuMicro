...
#define CDC_ERRMSG(...)
#endif
#if defined(CDC_DEBUG) && (defined(ENABLE_ERROR_MSG) || defined(ENABLE_DEBUG_MSG))
#define CDC_DBGMSG      (void)usbh_printf
#else
#define CDC_DBGMSG(...)
#endif

#define CDC_STATUS_BUFF_SIZE    64U
#define CDC_RX_BUFF_SIZE        64U
...
#define CDC_IF_HDR_MANDATORY_LEN            3U      /* Mandatory prefix length of a CDC class-specific interface descriptor header */
...
extern int  cdc_config_parser(struct cdc_dev_t *cdev);
...
typedef struct cdc_dev_t
{
    ...
    uint32_t            sts_buff[CDC_STATUS_BUFF_SIZE/4U];
    uint32_t            rx_buff[CDC_RX_BUFF_SIZE/4U];
