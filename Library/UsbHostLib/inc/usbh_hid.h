...
#define CONFIG_HID_MAX_REPORT_PER_DEV 16U  /*!< Maximum parsed report metadata entries per HID device.               */
...
typedef struct rp_desc_info
{
    ...
    RP_INFO_T   *report;
    uint8_t     report_count;           /* Number of occupied entries in report_pool                  */
    RP_INFO_T   report_pool[CONFIG_HID_MAX_REPORT_PER_DEV];  /* CPU-only report metadata */
} RPD_T;
...
int32_t  usbh_hid_set_report_non_blocking(HID_DEV_T *hdev, int rtp_typ, int rtp_id, uint8_t *data, uint16_t len);
/// @endcond HIDDEN_SYMBOLS
