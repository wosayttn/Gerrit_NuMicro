...
static HID_DEV_T *find_hid_device_by_iface(const IFACE_T *iface)
{
    int     i;
...
static void  hid_disconnect(const IFACE_T *iface)
{
    HID_DEV_T   *hdev;
    HID_DEV_T   *p;
    RP_INFO_T   *rp;
    ...
    hdev = find_hid_device_by_iface(iface);
    if(hdev == NULL)
    {
        return;
    }
