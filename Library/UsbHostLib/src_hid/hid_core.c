 ...
 * <b>Justification:</b> USB Host Library stores the owning interface as an opaque context;
 *                       this conversion restores its validated IFACE_T type.
 */
...
/**
 * @brief Submit a non-blocking HID SET_REPORT request.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> USB Host Library stores the owning interface as an opaque context;
...
/**
 * @brief Complete a HID interrupt-in transfer.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> The transfer context is assigned as HID_DEV_T before submission
...
static void  hid_read_irq(UTR_T *utr)
{
    ...
    if (ret)
    {
        ...
        (void)usbh_free_mem(utr->buff, (int32_t)utr->data_len);
        free_utr(utr);
    }
}

/**
 * @brief Complete a HID interrupt-out transfer.
 * @param[in] utr Pointer to the completed UTR.
 * 
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> The transfer context is assigned as HID_DEV_T before submission
 ...
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> The interface context is restored to IFACE_T and USB Host Library
 *                       allocated storage is assigned to the typed transfer buffer.
 */
int32_t usbh_hid_start_int_read(HID_DEV_T *hdev, uint8_t ep_addr, HID_IR_FUNC *func)
{
    ...
    /* cppcheck-suppress misra-c2012-11.5 */
    utr->buff = usbh_alloc_mem((int32_t)ep->wMaxPacketSize);
...
int32_t usbh_hid_stop_int_read(HID_DEV_T *hdev, uint8_t ep_addr)
{
    ...
    (void)usbh_free_mem(utr->buff, (int32_t)utr->ep->wMaxPacketSize);
    free_utr(utr);
