 ...
 * <b>Justification:</b> HID_DEV_T::iface is a generic void* handle referencing the owning
 *                       IFACE_T interface object; the object is created and owned by the USB
 *                       Host core and is guaranteed valid whenever hdev is valid, so casting it
 *                       back to IFACE_T* here is a well-defined, established framework pattern.
 */
int32_t  usbh_hid_get_report_descriptor(HID_DEV_T *hdev, uint8_t *desc_buf, uint16_t buf_max_len)
{
    IFACE_T    *iface;
    uint32_t   xfer_len;
    int        ret;
    ...
    if ((hdev == USBNULL) || (hdev->iface == USBNULL))
    {
...
/**
 * @brief  Issue a HID class SET_REPORT request, non-blocking (interrupt-driven) version.
 * @param[in] hdev       HID device pointer
 * @param[in] rtp_typ    Report type.
 * @param[in] rtp_id     Report ID
...
/**
 * @brief  HID interrupt-in transfer complete callback.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> UTR_T.context is a generic void* handle used by the USB Host common
 *                       framework so that different class drivers can attach their own device
 *                       object; the framework's callback interface is fixed, so casting it back
 *                       to HID_DEV_T here is an architectural requirement. The object registered
 *                       to context is created and owned by this driver's own init flow and
 *                       remains valid for the callback's lifetime.
 */
static void  hid_read_irq(UTR_T *utr)
{
    ...
    if (hdev->bSubClassCode == (uint8_t)HID_SUBCLASS_BOOT_DEVICE)
    {
        ...
        {
            if (utr->xfer_len <= 0x7FFFFFFFU)
            {
                int parsed_len;

                parsed_len = (int)utr->xfer_len;
                (void)hid_parse_mouse_reports(hdev, utr->buff, parsed_len);
            }
        }
        ...
        if (hdev->bProtocolCode == (uint8_t)HID_PROTOCOL_KEYBOARD)
        {
            if (utr->xfer_len <= 0x7FFFFFFFU)
            {
                ...
                parsed_len = (int)utr->xfer_len;
                (void)hid_parse_keyboard_reports(hdev, utr->buff, parsed_len);
            }
        }
    }

    if ((hdev->read_func != USBNULL) && (utr->xfer_len != 0U))
    {
...
/**
 * @brief  HID interrupt-out transfer complete callback.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> UTR_T.context is a generic void* handle used by the USB Host common
...
static void  hid_write_irq(UTR_T *utr)
{
    ...
    if (hdev->write_func)
    {
        utr->data_len = (uint32_t)utr->ep->wMaxPacketSize;
        hdev->write_func(hdev, utr->ep->bEndpointAddress, utr->status, utr->buff, &(utr->data_len));
    }
 ...
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> usbh_alloc_mem() is a generic allocator returning void* by design;
 *                       assigning it directly to the object pointer utr->buff is the
 *                       established framework pattern used throughout the USB Host library.
 */
int32_t usbh_hid_start_int_read(HID_DEV_T *hdev, uint8_t ep_addr, HID_IR_FUNC *func)
{
    ...
    if ((iface == USBNULL) || (iface->udev == USBNULL))
    {
    ...
    utr->context = hdev;
    utr->ep = ep;
    utr->data_len = (uint32_t)ep->wMaxPacketSize;
    utr->xfer_len = 0;
