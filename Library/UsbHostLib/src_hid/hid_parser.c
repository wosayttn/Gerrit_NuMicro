 ...
 *                       mapping it onto DESC_CONF_T/DESC_IF_T/DESC_HID_T structs avoids
 *                       repetitive byte-by-byte field assembly and mirrors the USB
...
int hid_parse_report_descriptor(HID_DEV_T *hdev, IFACE_T *iface)
{
    ...
    {
        uint32_t  want_len = (uint32_t)hidd->wDescriptorLength + 8U;

        /* wDescriptorLength is attacker/device controlled; clamp instead of
         * letting the uint16_t narrowing below silently wrap to a tiny value. */
        if (want_len > 0xFFFFU)
        {
            want_len = 0xFFFFU;
        }

        desc_buff_len = (uint16_t)want_len;
    }
    ...
    if ((hdev->bSubClassCode == HID_SUBCLASS_BOOT_DEVICE) && (hdev->bProtocolCode == HID_PROTOCOL_KEYBOARD))
    {
        ...
        {
            if ((report->usage_page == UP_LEDS) && (report->report_size == 1U) && (report->status.variable != 0U))
            {
                uint8_t  i;
                uint8_t ret;
                uint8_t leds = 0;
                ...
                ret = (uint8_t)usbh_hid_set_report(hdev, RT_OUTPUT, 0, &leds, 1);
...
static int hid_add_report(HID_DEV_T *hdev, uint8_t type)
{
    ...
    if (hdev->rpd.report_count >= (uint8_t)CONFIG_HID_MAX_REPORT_PER_DEV)
    {
        HID_ERRMSG("hid_add_report metadata pool exhausted!!\n");
        return USBH_ERR_MEMORY_OUT;
    }

    report = &hdev->rpd.report_pool[hdev->rpd.report_count];
    hdev->rpd.report_count++;
...
static int hid_parse_item(HID_DEV_T *hdev, uint8_t *buff)
{
    ...
    switch (tag)
    {
        ...
        case TAG_INPUT:
            ...
            {
                int  report_count = (int)_rp_info.report_count;
                ...
                _rp_info.report_count = (uint8_t)report_count;
...
int hid_parse_keyboard_reports(HID_DEV_T *hdev, const uint8_t *data, int data_len)
{
    ...
    for (report = hdev->rpd.report; report != USBNULL; report = report->next)
    {
        if ((hdev->rpd.has_report_id != 0U) && (report->report_id != (uint8_t)report_id))
        {
            continue;
        }
        ...
        if ((report->usage_page == UP_KEYCODE) && (report->app_usage == USAGE_ID_KEYBOARD))
        {
            ...
            if (report->report_size == (uint8_t)1)
            {
                for (i = 0; i < (int)report->report_count; i++)
                {
                    ...
                    bit_val = (((uint32_t)data[byte_idx] >> ((uint32_t)bit_idx % 8U)) & 0x1U);
                    usage_val |= (bit_val << (uint32_t)i);

                    if (bit_idx < 8)            /* is in the first byte         */
                    {
                        _keyboard_event.modifier |= (uint8_t)usage_val;
                    }
                    ...
                    else
                    {
                        if (bit_idx < (8 * 8))
                        {
                            _keyboard_event.keycode[(bit_idx - 16) / 8] |= (uint8_t)usage_val;
                        }
                    }
    ...
    if ((has_kbd_event != 0) && (_ptr_keyboard_callback != USBNULL))
    {
        uint8_t   pressed_lock_keys = 0;
        char      update_LEDs = (char)0;
        ...
        for (i = 0; i < 3; i++)
        {
            if (((pressed_lock_keys & (1U << (uint32_t)i)) != 0U) && ((hdev->rpd.last_pressed_lock_keys & (1U << (uint32_t)i)) == 0U))
            {
                /*
...
int hid_parse_mouse_reports(HID_DEV_T *hdev, const uint8_t *data, int data_len)
{
    ...
    for (report = hdev->rpd.report; report != USBNULL; report = report->next)
    {
                ...
                ((report->app_usage == USAGE_ID_MOUSE) || (report->app_usage == USAGE_ID_POINTER)))
        {
            ...
            if (report->status.variable)
            {
                ...
                for (i = 0; i < (int)report->report_count; i++)
                {
                    ...
                    bit_val = (((uint32_t)data[byte_idx] >> ((uint32_t)bit_idx % 8U)) & 0x1U);
                    _mouse_event.button_map |= (bit_val << (uint32_t)i);
                  ...
                  (report->data_usage == USAGE_ID_WHEEL)))
        {
            ...
            if (report->data_usage == USAGE_ID_X)
            {
                ...
                _mouse_event.X_bits = (int)report->report_size;
                _mouse_event.axis_relative = (uint8_t)report->status.relative;
            ...
            else if (report->data_usage == USAGE_ID_Y)
            {
                ...
                _mouse_event.Y_bits = (int)report->report_size;
            }
            else if (report->data_usage == USAGE_ID_WHEEL)
            {
                _mouse_event.wheel = s_val;
                _mouse_event.wheel_raw = usage_val;
                _mouse_event.wheel_bits = (int)report->report_size;
                _mouse_event.wheel_relative = (uint8_t)report->status.relative;
    ...
    if ((has_mouse_event != 0) && (_ptr_mouse_callback != USBNULL))
    {
