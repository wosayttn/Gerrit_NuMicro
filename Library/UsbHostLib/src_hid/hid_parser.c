 ...
 *                       standard descriptor layouts defined by the USB specification.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> USB Host Library allocator storage is assigned to the typed report
 *                       descriptor buffer owned and released by this function.
 */
int hid_parse_report_descriptor(HID_DEV_T *hdev, IFACE_T *iface)
{
    ...
    desc_buff = (uint8_t *)usbh_alloc_mem((int32_t)desc_buff_len);

    remain_len = usbh_hid_get_report_descriptor(hdev, desc_buff, desc_buff_len);

    if (remain_len <= 0)
    {
        (void)usbh_free_mem(desc_buff, (int32_t)desc_buff_len);
        return remain_len;
    }
