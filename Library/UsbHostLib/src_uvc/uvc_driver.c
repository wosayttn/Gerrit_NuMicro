...
/**
 * @brief  Allocate a UVC device context.
 * @param[in] udev Pointer to the UDEV_T structure representing the USB device.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> The USB host allocator returns generic memory; the allocated
 *                       UVC_DEV_T-sized object must be accessed as a UVC device context.
 */
static UVC_DEV_T *alloc_uvc_device(UDEV_T const *udev)
{
    ...
    vdev = (UVC_DEV_T *)usbh_alloc_mem((int32_t)sizeof(UVC_DEV_T));
    ...
    if (vdev->in_buff == USBNULL)
    {
        ...
        (void)usbh_free_mem(vdev, (int32_t)sizeof(UVC_DEV_T));
        return USBNULL;
    }
...
/**
 * @brief  Probe a UVC interface.
 * @param[in] iface Pointer to the IFACE_T structure representing the USB interface.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
 * <b>Justification:</b> The standard interface descriptor is interpreted as a Video
 *                       Control IAD only after its UVC collection subclass is confirmed.
 */
...
/**
 * @brief  Disconnect a UVC interface.
 * @param[in] iface Pointer to the IFACE_T structure representing the USB interface.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> iface->context stores the UVC device context assigned during
 *                       probing and must be recovered as UVC_DEV_T during disconnect.
 */
