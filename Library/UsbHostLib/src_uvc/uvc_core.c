...
int usbh_uvc_probe_control(UVC_DEV_T *vdev, uint8_t req, UVC_CTRL_PARAM_T *param)
{
    ...
    ret = usbh_ctrl_xfer(vdev->udev, bmRequestType, req,
                         ...
                         (uint16_t)sizeof(UVC_CTRL_PARAM_T), /* wLength - Length of parameter block */
...
static int usbh_uvc_still_probe_control(UVC_DEV_T *vdev, uint8_t req, UVC_STILL_CTRL_PARAM_T *param)
{
    ...
    ret = usbh_ctrl_xfer(vdev->udev, bmRequestType, req,
                         ...
                         vdev->iface_stream->if_num,    /* wIndex - Zero and Interface       */
                         (uint16_t)sizeof(UVC_STILL_CTRL_PARAM_T), /* wLength - Length of parameter block */
                         (uint8_t *)param,              /* parameter block                   */
                         &xfer_len, UVC_REQ_TIMEOUT);
...
int  usbh_get_video_format(UVC_DEV_T *vdev, int index, IMAGE_FORMAT_E *format, int *width, int *height)
{
    ...
    *width  = (int)vc->width[index];
    *height = (int)vc->height[index];
    return 0;
}
...
int  usbh_set_video_format(UVC_DEV_T *vdev, IMAGE_FORMAT_E format, int width, int height)
{
    ...
    uint8_t          format_index_u8 = 0U;
    uint8_t          frame_index_u8 = 0U;
    uint8_t          format_found = 0U;
    ...
    for (i = 0; i < (int)vc->num_of_formats; i++)
    {
        ...
        {
            format_index_u8 = vc->format_idx[i];
            format_found = 1U;
            break;
        }
    }
...
int  usbh_get_video_still_format(UVC_DEV_T *vdev, int index, IMAGE_FORMAT_E *format, int *width, int *height)
{
    ...
    *width  = (int)vc->still_image_width[index];
    *height = (int)vc->still_image_height[index];
    return 0;
}
#if 0
...
static void  uvc_parse_streaming_data(UVC_DEV_T *vdev, uint8_t const *buff, int pkt_len)
{
    ...
    else
    {
        ...
        {
            if ((vdev->func_rx != USBNULL) && (vdev->img_size > 0))
            {
                (void)vdev->func_rx(vdev, vdev->img_buff, vdev->img_size);
            }
...
/**
 * @brief  Process a completed isochronous IN transfer.
 * @param[in] utr  Pointer to the completed UTR.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> utr->context stores the UVC device context assigned when the
 *                       transfer is created and must be recovered as UVC_DEV_T here.
 */
static void iso_in_irq(UTR_T *utr)
{
    ...
    if ((vdev == USBNULL) || (vdev->udev == USBNULL))
    {
        return;
    }
    ...
    for (uint32_t i = 0; i < (uint32_t)IF_PER_UTR; i++)
    {
        if (utr->iso_status[i] == 0)
        {
            ...
            {
                uvc_parse_streaming_data(vdev, utr->iso_buff[i], (int)utr->iso_xlen[i]);
            }
        }
        else
        {
