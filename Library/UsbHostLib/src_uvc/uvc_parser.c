/**************************************************************************//**
 * @file     uvc_parser.c
 * @version  V1.00
 * @brief    USB Host Video Class driver
 *
 * @note
 * SPDX-License-Identifier: Apache-2.0
 * Copyright (C) 2024 Nuvoton Technology Corp. All rights reserved.
*****************************************************************************/

#include <stdio.h>
#include <string.h>

#include "usbh_lib.h"
#include "usbh_uvc.h"

/** @addtogroup LIBRARY Library
  @{
*/

/** @addtogroup USBH_Library USBH Library
  @{
*/

/** @addtogroup USBH_EXPORTED_FUNCTIONS USBH Exported Functions
  @{
*/

static int uvc_validate_still_image_frame(const uint8_t *descriptor, int size,
        uint8_t *image_size_pattern_count, uint8_t *compression_pattern_count)
{
    int compression_count_offset;
    int required_length;

    if ((size < (int)UVC_STILL_DESC_BASE_LEN) ||
            ((int)descriptor[0] < (int)UVC_STILL_DESC_BASE_LEN) ||
            ((int)descriptor[0] > size))
    {
        return UVC_RET_PARSER;
    }

    *image_size_pattern_count = descriptor[4];
    compression_count_offset = (int)UVC_STILL_DESC_BASE_LEN +
                               ((int)*image_size_pattern_count * (int)sizeof(STILL_IMAGE_SIZE_T));

    if ((int)descriptor[0] <= compression_count_offset)
    {
        return UVC_RET_PARSER;
    }

    *compression_pattern_count = descriptor[compression_count_offset];
    required_length = compression_count_offset + 1 + (int)*compression_pattern_count;

    if ((int)descriptor[0] < required_length)
    {
        return UVC_RET_PARSER;
    }

    return UVC_RET_OK;
}

/**
 *  @brief  Parse and get Video Control interface information from descriptors.
 *  @param[in]  vdev    UVC device
 *  @param[in]  iface   Video Control interface
 *  @return   Success or failed.
 *  @retval   0        Success
 *  @retval   Otheriwse  Error occurred
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
 * <b>Justification:</b> USB configuration bytes are parsed through the applicable
 *                       descriptor layout after the descriptor type and subtype checks.
 */
int uvc_parse_control_interface(UVC_DEV_T *vdev, IFACE_T *iface)
{
    DESC_CONF_T    const *config;
    DESC_IF_T      const *ifd = USBNULL;
    DESC_VC_HDR_T  *ifd_vc_hdr;
    uint8_t        *cfg_buff;
    uint32_t       offset;
    int            size;

    UVC_DBGMSG("UVC parsing video control interface %d...\n", iface->if_num);

    vdev->iface_ctrl = iface;

    cfg_buff = vdev->udev->cfd_buff;
    /* cppcheck-suppress misra-c2012-11.3 */
    config = (DESC_CONF_T *)cfg_buff;

    /* step over configuration descritpor */
    offset = config->bLength;
    size = (int)config->wTotalLength - (int)config->bLength;

    /*------------------------------------------------------------------------------------*/
    /*  Find this Standard Video Control Interface Descriptor                             */
    /*------------------------------------------------------------------------------------*/
    while (size >= (int)sizeof(DESC_IF_T))
    {
        /* cppcheck-suppress misra-c2012-11.3 */
        ifd = (DESC_IF_T *)&cfg_buff[offset];

        if ((ifd->bDescriptorType == USB_DT_INTERFACE) && (ifd->bInterfaceNumber == iface->if_num))
        {
            break;
        }

        if (ifd->bLength == 0U)
        {
            return UVC_RET_PARSER;          /* prevent infinit loop                       */
        }

        offset += ifd->bLength;
        size -= (int)ifd->bLength;
    }

    if (size < (int)sizeof(DESC_IF_T))
    {
        /* cannot find the Standard VC descriptor     */
        UVC_ERRMSG("UVC_RET_PARSER! - Can't find the Standard Video Control Interface!\n");
        return UVC_RET_PARSER;
    }

    vdev->version = (char)ifd->bInterfaceProtocol;

    offset += ifd->bLength;

    /*------------------------------------------------------------------------------------*/
    /*  Parsing Video Control interface VC header descriptor                              */
    /*------------------------------------------------------------------------------------*/
    /* cppcheck-suppress misra-c2012-11.3 */
    ifd_vc_hdr = (DESC_VC_HDR_T *)&cfg_buff[offset];

    if ((ifd_vc_hdr->bDescriptorType != UVC_CS_INTERFACE) || (ifd_vc_hdr->bDescriptorSubType != VC_HEADER))
    {
        UVC_ERRMSG("UVC VC_HEADER not found!! 0x%x 0x%x\n", ifd_vc_hdr->bDescriptorType, ifd_vc_hdr->bDescriptorSubType);
        return UVC_RET_PARSER;
    }

    if (ifd_vc_hdr->bInCollection != (uint8_t)1U)
    {
        UVC_ERRMSG("UVC - number of streaming interface is not 1! Not supported!! %d\n", ifd_vc_hdr->bInCollection);
        return UVC_RET_DRV_NOT_SUPPORTED;
    }

    UVC_DBGMSG("VC Interface VC_HEADER\n");
    UVC_DBGMSG("    bcdUVC: 0%x\n", ifd_vc_hdr->bcdUVC);
    UVC_DBGMSG("    baInterfaceNr: 0%x\n", ifd_vc_hdr->baInterfaceNr);

    // ifnum_strm = ifd_vc_hdr->baInterfaceNr;
    offset += ifd_vc_hdr->bLength;
    size = (int)ifd_vc_hdr->wTotalLength - (int)ifd_vc_hdr->bLength;

    /*------------------------------------------------------------------------------------*/
    /*  Walk though Video Control interface descriptor group                              */
    /*------------------------------------------------------------------------------------*/
    while (size >= (int)sizeof(DESC_HDR_T))
    {
#ifdef UVC_DEBUG
        DESC_VC_IT_T  *ifd_it;
        DESC_VC_OT_T  *ifd_ot;
        DESC_VC_SU_T  *ifd_su;
        DESC_VC_PU_T  *ifd_pu;
        DESC_VC_EU_T  *ifd_eu;
        DESC_VC_ECU_T *ifd_ecu;
        int           i;
#endif

        /* cppcheck-suppress misra-c2012-11.3 */
        ifd = (DESC_IF_T *)&cfg_buff[offset];

        //UVC_DBGMSG("Parse VC - [%d] [0x%x] [0x%x]\n", ((CS_HDR_T *)&cfg_buff[offset])->bLength, ((CS_HDR_T *)&cfg_buff[offset])->bDescriptorType, ((CS_HDR_T *)&cfg_buff[offset])->bDescriptorSubtype);

        if (ifd->bDescriptorType == USB_DT_ENDPOINT)
        {
            break;
        }

        if (ifd->bDescriptorType != UVC_CS_INTERFACE)
        {
            UVC_DBGMSG("UVC VC descriptor parsing length error!! %d\n", size);
            return UVC_RET_PARSER;
        }

#ifdef UVC_DEBUG
        ifd_it = (DESC_VC_IT_T *)ifd;

        switch (ifd_it->bDescriptorSubType)
        {
            case VC_INPUT_TERMINAL:
                ifd_it = (DESC_VC_IT_T *)ifd;
                UVC_DBGMSG("VC Interface VC_INPUT_TERMINAL\n");
                UVC_DBGMSG("    bTerminalID:    0x%x\n", ifd_it->bTerminalID);
                UVC_DBGMSG("    wTerminalType:  0x%x\n", ifd_it->wTerminalType);
                UVC_DBGMSG("    bAssocTerminal: 0x%x\n", ifd_it->bAssocTerminal);
                break;

            case VC_OUTPUT_TERMINAL:
                ifd_ot = (DESC_VC_OT_T *)ifd;
                UVC_DBGMSG("VC Interface VC_OUTPUT_TERMINAL\n");
                UVC_DBGMSG("    bTerminalID:    0x%x\n", ifd_ot->bTerminalID);
                UVC_DBGMSG("    wTerminalType:  0x%x\n", ifd_ot->wTerminalType);
                UVC_DBGMSG("    bAssocTerminal: 0x%x\n", ifd_ot->bAssocTerminal);
                UVC_DBGMSG("    bSourceID:      0x%x\n", ifd_ot->bSourceID);

                if (ifd_ot->wTerminalType == TT_STREAMING)
                {
                    UVC_DBGMSG("UVC USB OT found.\n");
                }

                break;

            case VC_SELECTOR_UNIT:
                ifd_su = (DESC_VC_SU_T *)ifd;
                UVC_DBGMSG("VC Interface VC_SELECTOR_UNIT\n");
                UVC_DBGMSG("    bUnitID:    0x%x\n", ifd_su->bUnitID);
                UVC_DBGMSG("    bNrInPins:  0x%x\n", ifd_su->bNrInPins);

                for (i = 0; i < ifd_su->bNrInPins; i++)
                {
                    UVC_DBGMSG("    bSourceID:  0x%x\n", ifd_su->bSourceID[i]);
                }

                break;

            case VC_PROCESSING_UNIT:
                ifd_pu = (DESC_VC_PU_T *)ifd;
                UVC_DBGMSG("VC Interface VC_PROCESSING_UNIT\n");
                UVC_DBGMSG("    bUnitID:          0x%x\n", ifd_pu->bUnitID);
                UVC_DBGMSG("    wMaxMultiplier:   0x%x\n", ifd_pu->wMaxMultiplier);
                UVC_DBGMSG("    bControlSize:     0x%x\n", ifd_pu->bControlSize);
                UVC_DBGMSG("    bmControls:       0x%02x%02x%02x\n", ifd_pu->bmControls[2], ifd_pu->bmControls[1], ifd_pu->bmControls[0]);
                UVC_DBGMSG("    bmVideoStandards: 0x%x\n", ifd_pu->bmVideoStandards);
                break;

            case VC_EXTENSION_UNIT:
                ifd_eu = (DESC_VC_EU_T *)ifd;
                UVC_DBGMSG("VC Interface VC_EXTENSION_UNIT\n");
                UVC_DBGMSG("    bUnitID:      0x%x\n", ifd_eu->bUnitID);
                UVC_DBGMSG("    bNumControls: 0x%x\n", ifd_eu->bNumControls);
                UVC_DBGMSG("    bNrInPins:    0x%x\n", ifd_eu->bNrInPins);
                break;

            case VC_ENCODING_UNIT:
                ifd_ecu = (DESC_VC_ECU_T *)ifd;
                UVC_DBGMSG("VC Interface VC_ENCODING_UNIT\n");
                UVC_DBGMSG("    bUnitID:   0x%x\n", ifd_ecu->bUnitID);
                UVC_DBGMSG("    bSourceID: 0x%x\n", ifd_ecu->bSourceID);
                UVC_DBGMSG("    iEncoding: 0x%x\n", ifd_ecu->iEncoding);
                break;
        }

#endif

        if ((ifd->bLength < (uint8_t)sizeof(DESC_HDR_T)) || ((int)ifd->bLength > size))
        {
            UVC_ERRMSG("UVC_RET_PARSER! - VC descriptor length!\n");
            return UVC_RET_PARSER;
        }

        offset += ifd->bLength;
        size -= (int)ifd->bLength;
    }

    /*------------------------------------------------------------------------------------*/
    /*  Parsing Endpoint descriptors of Video Control interface                           */
    /*------------------------------------------------------------------------------------*/
    while (size >= (int)sizeof(DESC_HDR_T))
    {
        DESC_EP_T const *epd;

        /* cppcheck-suppress misra-c2012-11.3 */
        epd = (DESC_EP_T *)&cfg_buff[offset];

        if (epd->bDescriptorType != USB_DT_ENDPOINT)
        {
            UVC_DBGMSG("UVC VC interface endpoint parsing error, remain %d bytes!\n", size);
            break;
        }

        if ((epd->bLength < UVC_EP_DESC_MIN_LEN) || ((int)epd->bLength > size))
        {
            UVC_ERRMSG("UVC_RET_PARSER! - VC endpoint descriptor length!\n");
            return UVC_RET_PARSER;
        }

        if ((epd->bDescriptorType == USB_DT_ENDPOINT) && (epd->bmAttributes == 0x03U))
        {
            //vdev->ep_sts = usbh_iface_find_ep(iface, epd->bEndpointAddress, 0);
            //if (vdev->ep_sts == USBNULL)
            //  UVC_DBGMSG("UVC find interrupt in endpoint failed!\n");
            //else
            //    UVC_DBGMSG("UVC interrupt in endpoint 0x%x foudn.\n", epd->bEndpointAddress);
        }

        offset += epd->bLength;
        size -= (int)epd->bLength;
    }

    if (size != 0)
    {
        UVC_DBGMSG("Warning - VC interface parsing problem, remain %d bytes!\n", size);
    }

    return 0;
}

/**
 *  @brief  Parse and get Video Streaming interface information from descriptors.
 *  @param[in]  vdev    UVC device
 *  @param[in]  iface   Video Streaming interface
 *  @return   Success or failed.
 *  @retval   0        Success
 *  @retval   Otheriwse  Error occurred
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
 * <b>Justification:</b> USB configuration bytes are parsed through the applicable
 *                       descriptor layout after the descriptor type and subtype checks.
 */
int uvc_parse_streaming_interface(UVC_DEV_T *vdev, IFACE_T *iface)
{
    DESC_CONF_T    const *config;
    DESC_IF_T      const *ifd = USBNULL;
    DESC_EP_T      const *epd;
    DESC_VSI_HDR_T *ifd_vs_hdr;
    UVC_CTRL_T     *vc = &vdev->vc;
    UVC_STRM_T     *vs = &vdev->vs;
    uint8_t        *cfg_buff;
    uint32_t       offset;
    int            i;
    int            idx;
    int            size;

    UVC_DBGMSG("UVC parsing video streaming interface %d...\n", iface->if_num);

    vdev->iface_stream = iface;

    cfg_buff = vdev->udev->cfd_buff;
    /* cppcheck-suppress misra-c2012-11.3 */
    config = (DESC_CONF_T *)cfg_buff;

    /* step over configuration descritpor */
    offset = config->bLength;
    size = (int)config->wTotalLength - (int)config->bLength;

    /*------------------------------------------------------------------------------------*/
    /*  Find the starting of Standard Video Streaming Interface Descriptor                */
    /*------------------------------------------------------------------------------------*/
    while (size >= (int)sizeof(DESC_IF_T))
    {
        /* cppcheck-suppress misra-c2012-11.3 */
        ifd = (DESC_IF_T *)&cfg_buff[offset];

        if ((ifd->bDescriptorType == USB_DT_INTERFACE) && (ifd->bInterfaceNumber == iface->if_num) &&
                (ifd->bInterfaceClass == USB_CLASS_VIDEO) && (ifd->bInterfaceSubClass == UVC_SC_VIDEOSTREAMING))
        {
            break;
        }

        if (ifd->bLength == 0U)
        {
            return UVC_RET_PARSER;          /* prevent infinite loop                      */
        }

        offset += ifd->bLength;
        size -= (int)ifd->bLength;
    }

    if (size < (int)sizeof(DESC_IF_T))
    {
        UVC_ERRMSG("UVC_RET_PARSER! - Can't find the Standard Video Streaming Interface!\n");
        return UVC_RET_PARSER;
    }

    offset += ifd->bLength;
    size -= (int)ifd->bLength;

    /*------------------------------------------------------------------------------------*/
    /*  Parsing Video Streaming interface alternative setting 0                           */
    /*------------------------------------------------------------------------------------*/
    while (size >= (int)sizeof(DESC_IF_T))
    {
        DESC_STILL_IMAGE_FRAME_T   *vs_still_image_frame;
        DESC_VSU_FORMAT_T   const *vsu_format;
        DESC_VSU_FRAME_T    const *vsu_frame;
        DESC_MJPG_FORMAT_T  const *mjpg_format;
        DESC_MJPG_FRAME_T   const *mjpg_frame;
        uint8_t            descriptor_length;
        uint8_t            image_size_pattern_count;
        uint8_t            compression_pattern_count;
        uint8_t            current_format_slot;
        uint8_t            descriptor_advanced = 0U;

        /* cppcheck-suppress misra-c2012-11.3 */
        ifd = (DESC_IF_T *)&cfg_buff[offset];

        // sysprintf("[%d] 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n", size, cfg_buff[offset], cfg_buff[offset + 1U], cfg_buff[offset + 2U], cfg_buff[offset + 3U], cfg_buff[offset + 4U], cfg_buff[offset + 5U], cfg_buff[offset + 6U], cfg_buff[offset + 7U]);

        if (ifd->bDescriptorType != UVC_CS_INTERFACE)
        {
            break;
        }

        descriptor_length = ifd->bLength;

        if ((descriptor_length < UVC_CS_DESC_MIN_LEN) || ((int)descriptor_length > size))
        {
            UVC_ERRMSG("UVC_RET_PARSER! - VS class descriptor length!\n");
            return UVC_RET_PARSER;
        }

        /* cppcheck-suppress misra-c2012-11.3 */
        ifd_vs_hdr = (DESC_VSI_HDR_T *)&cfg_buff[offset];

        switch (ifd_vs_hdr->bDescriptorSubType)
        {
            case VS_INPUT_HEADER:
                if (descriptor_length < (uint8_t)sizeof(DESC_VSI_HDR_T))
                {
                    UVC_ERRMSG("UVC_RET_PARSER! - VS input header length!\n");
                    return UVC_RET_PARSER;
                }

                UVC_DBGMSG("VS Interface VS_INPUT_HEADER\n");
                UVC_DBGMSG("    bNumFormats:         0x%x\n", ifd_vs_hdr->bNumFormats);
                UVC_DBGMSG("    bEndpointAddress:    0x%x\n", ifd_vs_hdr->bEndpointAddress);
                UVC_DBGMSG("    bTerminalLink:       0x%x\n", ifd_vs_hdr->bTerminalLink);
                UVC_DBGMSG("    bStillCaptureMethod: 0x%x\n", ifd_vs_hdr->bStillCaptureMethod);
                UVC_DBGMSG("    bTriggerSupport:     0x%x\n", ifd_vs_hdr->bTriggerSupport);
                break;

            case VS_FORMAT_UNCOMPRESSED:
                if (descriptor_length < (uint8_t)sizeof(DESC_VSU_FORMAT_T))
                {
                    UVC_ERRMSG("UVC_RET_PARSER! - VS uncompressed format length!\n");
                    return UVC_RET_PARSER;
                }

                /* cppcheck-suppress misra-c2012-11.3 */
                vsu_format = (DESC_VSU_FORMAT_T *)ifd_vs_hdr;
                if (vc->num_of_formats >= (uint8_t)UVC_MAX_FORMAT)
                {
                    UVC_ERRMSG("UVC_RET_PARSER! - Too many VS formats!\n");
                    return UVC_RET_PARSER;
                }
                UVC_DBGMSG("VS Interface VS_FORMAT_UNCOMPRESSED\n");
                UVC_DBGMSG("    bFormatIndex:         0x%x\n", vsu_format->bFormatIndex);
                UVC_DBGMSG("    bNumFrameDescriptors: 0x%x\n", vsu_format->bNumFrameDescriptors);
                UVC_DBGMSG("    guidFormat:           0x%08x%08x%08x%08x\n", vsu_format->guidFormat[0], vsu_format->guidFormat[1], vsu_format->guidFormat[2], vsu_format->guidFormat[3]);
                UVC_DBGMSG("    bBitsPerPixel         0x%x\n", vsu_format->bBitsPerPixel);
                UVC_DBGMSG("    bDefaultFrameIndex:   0x%x\n", vsu_format->bDefaultFrameIndex);
                UVC_DBGMSG("    bAspectRatioX:        0x%x\n", vsu_format->bAspectRatioX);
                UVC_DBGMSG("    bAspectRatioY:        0x%x\n", vsu_format->bAspectRatioY);

                idx = (int)vc->num_of_formats;
                vc->format_idx[idx] = vsu_format->bFormatIndex;

                if (vsu_format->guidFormat[0] == (uint32_t)0x32595559UL)
                {
                    vc->format[idx] = UVC_FORMAT_YUY2;
                }
                else if (vsu_format->guidFormat[0] == (uint32_t)0x3231564EUL)
                {
                    vc->format[idx] = UVC_FORMAT_NV12;
                }
                else if (vsu_format->guidFormat[0] == (uint32_t)0x3032344DUL)
                {
                    vc->format[idx] = UVC_FORMAT_M420;
                }
                else if (vsu_format->guidFormat[0] == (uint32_t)0x30323449UL)
                {
                    vc->format[idx] = UVC_FORMAT_I420;
                }
                else
                {
                    /* No action */
                }

                vc->num_of_formats++;
                current_format_slot = vc->num_of_formats;
                current_format_slot--;

                /*------------------------------------------------------------------------*/
                /*  Parsing all frame descriptors under this format descriptor            */
                /*------------------------------------------------------------------------*/
                offset += vsu_format->bLength;
                size -= (int)vsu_format->bLength;

                for (i = 0; (i < (int)vsu_format->bNumFrameDescriptors) && (vc->num_of_frames < (uint8_t)UVC_MAX_FRAME); i++)
                {
                    if (size < (int)UVC_CS_DESC_MIN_LEN)
                    {
                        UVC_ERRMSG("UVC_RET_PARSER! - VS frame descriptor missing!\n");
                        return UVC_RET_PARSER;
                    }

                    /* cppcheck-suppress misra-c2012-11.3 */
                    vsu_frame = (DESC_VSU_FRAME_T *)&cfg_buff[offset];

                    if ((vsu_frame->bDescriptorType != UVC_CS_INTERFACE) ||
                            (vsu_frame->bDescriptorSubType != VS_FRAME_UNCOMPRESSED))
                    {
                        UVC_DBGMSG("Frame parsing error!!\n");
                        return UVC_RET_PARSER;
                    }

                    if ((vsu_frame->bLength < UVC_FRAME_DESC_MIN_LEN) || ((int)vsu_frame->bLength > size))
                    {
                        UVC_ERRMSG("UVC_RET_PARSER! - VS uncompressed frame length!\n");
                        return UVC_RET_PARSER;
                    }

                    UVC_DBGMSG("VS Interface VS_FRAME_UNCOMPRESSED\n");
                    UVC_DBGMSG("    bFormatIndex: 0x%x\n", vsu_frame->bFrameIndex);
                    UVC_DBGMSG("    wWidth:       0x%x\n", vsu_frame->wWidth);
                    UVC_DBGMSG("    wHeight       0x%x\n", vsu_frame->wHeight);
                    UVC_DBGMSG("    dwMinBitRate: 0x%x\n", vsu_frame->dwMinBitRate);
                    UVC_DBGMSG("    dwMaxBitRate: 0x%x\n", vsu_frame->dwMaxBitRate);
                    UVC_DBGMSG("    dwDefaultFrameInterval:  0x%x\n", vsu_frame->dwDefaultFrameInterval);

                    idx = (int)vc->num_of_frames;
                    vc->frame_idx[idx] = vsu_frame->bFrameIndex;
                    vc->frame_format[idx] = vc->format[current_format_slot];
                    vc->width[idx] = vsu_frame->wWidth;
                    vc->height[idx] = vsu_frame->wHeight;
                    vc->num_of_frames++;

                    offset += vsu_frame->bLength;
                    size -= (int)vsu_frame->bLength;
                }

                /*--------------------------------------------------------------------------*/
                /*  Parsing all still image frame descriptors under this format descriptor  */
                /*--------------------------------------------------------------------------*/
                if (size < (int)UVC_CS_DESC_MIN_LEN)
                {
                    descriptor_advanced = 1U;
                    break;
                }

                /* cppcheck-suppress misra-c2012-11.3 */
                vs_still_image_frame = (DESC_STILL_IMAGE_FRAME_T *)&cfg_buff[offset];

                if ((vs_still_image_frame->bDescriptorType != UVC_CS_INTERFACE) ||
                        (vs_still_image_frame->bDescriptorSubType != VS_STILL_IMAGE_FRAME))
                {
                    UVC_DBGMSG("No VS_STILL_IMAGE_FRAME found (optional), skipping.\n");
                    descriptor_advanced = 1U;
                    break;
                }

                if (uvc_validate_still_image_frame(&cfg_buff[offset], size,
                                                    &image_size_pattern_count,
                                                    &compression_pattern_count) != UVC_RET_OK)
                {
                    UVC_ERRMSG("UVC_RET_PARSER! - VS still image descriptor length!\n");
                    return UVC_RET_PARSER;
                }

                UVC_DBGMSG("VS Interface VS_STILL_IMAGE_FRAME\n");
                UVC_DBGMSG("    bEndpointAddress:        0x%x\n", vs_still_image_frame->bEndpointAddress);
                UVC_DBGMSG("    bNumImageSizePatterns:   0x%x\n", image_size_pattern_count);
                UVC_DBGMSG("    bNumCompressionPatterns: 0x%x\n", compression_pattern_count);

                for (i = 0; i < (int)image_size_pattern_count; i++)
                {
                    if (vc->num_of_still_frames >= (uint8_t)UVC_MAX_STILL_IMAGE)
                    {
                        UVC_ERRMSG("UVC_RET_PARSER! - Too many still image frames!\n");
                        return UVC_RET_PARSER;
                    }

                    idx = (int)vc->num_of_still_frames;

                    vc->still_image_width[idx] = vs_still_image_frame->wSize[i].wWidth;
                    vc->still_image_height[idx] = vs_still_image_frame->wSize[i].wHeight;
                    vc->still_image_format[idx] = vc->format[current_format_slot];

                    UVC_DBGMSG("    [%d]wWidth x wHeight:     0x%x x 0x%x\n", (i + 1), vs_still_image_frame->wSize[i].wWidth, vs_still_image_frame->wSize[i].wHeight);

                    vc->num_of_still_frames++;
                }

                offset += vs_still_image_frame->bLength;
                size -= (int)vs_still_image_frame->bLength;
                descriptor_advanced = 1U;
                break;

            case VS_FRAME_UNCOMPRESSED:
                if (descriptor_length < UVC_FRAME_DESC_MIN_LEN)
                {
                    UVC_ERRMSG("UVC_RET_PARSER! - VS uncompressed frame length!\n");
                    return UVC_RET_PARSER;
                }

                UVC_DBGMSG("VS Interface VS_FRAME_UNCOMPRESSED\n");
                UVC_DBGMSG("    bFormatIndex: 0x%x\n", ((DESC_VSU_FRAME_T *)ifd_vs_hdr)->bFrameIndex);
                UVC_DBGMSG("    wWidth:       0x%x\n", ((DESC_VSU_FRAME_T *)ifd_vs_hdr)->wWidth);
                UVC_DBGMSG("    wHeight       0x%x\n", ((DESC_VSU_FRAME_T *)ifd_vs_hdr)->wHeight);
                UVC_DBGMSG("    dwMinBitRate: 0x%x\n", ((DESC_VSU_FRAME_T *)ifd_vs_hdr)->dwMinBitRate);
                UVC_DBGMSG("    dwMaxBitRate: 0x%x\n", ((DESC_VSU_FRAME_T *)ifd_vs_hdr)->dwMaxBitRate);
                UVC_DBGMSG("    dwDefaultFrameInterval:  0x%x\n", ((DESC_VSU_FRAME_T *)ifd_vs_hdr)->dwDefaultFrameInterval);
                break;

            case VS_FORMAT_MJPEG:
                if (descriptor_length < (uint8_t)sizeof(DESC_MJPG_FORMAT_T))
                {
                    UVC_ERRMSG("UVC_RET_PARSER! - VS MJPEG format length!\n");
                    return UVC_RET_PARSER;
                }

                /* cppcheck-suppress misra-c2012-11.3 */
                mjpg_format = (DESC_MJPG_FORMAT_T *)ifd_vs_hdr;
                if (vc->num_of_formats >= (uint8_t)UVC_MAX_FORMAT)
                {
                    UVC_ERRMSG("UVC_RET_PARSER! - Too many VS formats!\n");
                    return UVC_RET_PARSER;
                }
                UVC_DBGMSG("VS Interface VS_FORMAT_MJPEG\n");
                UVC_DBGMSG("    bFormatIndex:         0x%x\n", mjpg_format->bFormatIndex);
                UVC_DBGMSG("    bNumFrameDescriptors: 0x%x\n", mjpg_format->bNumFrameDescriptors);
                UVC_DBGMSG("    bAspectRatioX:        0x%x\n", mjpg_format->bAspectRatioX);
                UVC_DBGMSG("    bAspectRatioY:        0x%x\n", mjpg_format->bAspectRatioY);

                idx = (int)vc->num_of_formats;
                vc->format_idx[idx] = mjpg_format->bFormatIndex;
                vc->format[idx] = UVC_FORMAT_MJPEG;
                vc->num_of_formats++;
                current_format_slot = vc->num_of_formats;
                current_format_slot--;

                /*------------------------------------------------------------------------*/
                /*  Parsing all frame descriptors under this format descriptor            */
                /*------------------------------------------------------------------------*/
                offset += mjpg_format->bLength;
                size -= (int)mjpg_format->bLength;

                for (i = 0; (i < (int)mjpg_format->bNumFrameDescriptors) && (vc->num_of_frames < (uint8_t)UVC_MAX_FRAME); i++)
                {
                    if (size < (int)UVC_CS_DESC_MIN_LEN)
                    {
                        UVC_ERRMSG("UVC_RET_PARSER! - VS frame descriptor missing!\n");
                        return UVC_RET_PARSER;
                    }

                    /* cppcheck-suppress misra-c2012-11.3 */
                    mjpg_frame = (DESC_MJPG_FRAME_T *)&cfg_buff[offset];

                    if ((mjpg_frame->bDescriptorType != UVC_CS_INTERFACE) ||
                            (mjpg_frame->bDescriptorSubType != VS_FRAME_MJPEG))
                    {
                        UVC_DBGMSG("Frame parsing error!!\n");
                        return UVC_RET_PARSER;
                    }

                    if ((mjpg_frame->bLength < UVC_FRAME_DESC_MIN_LEN) || ((int)mjpg_frame->bLength > size))
                    {
                        UVC_ERRMSG("UVC_RET_PARSER! - VS MJPEG frame length!\n");
                        return UVC_RET_PARSER;
                    }

                    UVC_DBGMSG("VS Interface VS_FRAME_MJPEG\n");
                    UVC_DBGMSG("    bFormatIndex: 0x%x\n", mjpg_frame->bFrameIndex);
                    UVC_DBGMSG("    wWidth:       0x%x\n", mjpg_frame->wWidth);
                    UVC_DBGMSG("    wHeight:      0x%x\n", mjpg_frame->wHeight);
                    UVC_DBGMSG("    dwMinBitRate: 0x%x\n", mjpg_frame->dwMinBitRate);
                    UVC_DBGMSG("    dwMaxBitRate: 0x%x\n", mjpg_frame->dwMaxBitRate);
                    UVC_DBGMSG("    dwDefaultFrameInterval:  0x%x\n", mjpg_frame->dwDefaultFrameInterval);

                    idx = (int)vc->num_of_frames;
                    vc->frame_idx[idx] = mjpg_frame->bFrameIndex;
                    vc->frame_format[idx] = vc->format[current_format_slot];
                    vc->width[idx] = mjpg_frame->wWidth;
                    vc->height[idx] = mjpg_frame->wHeight;
                    vc->num_of_frames++;

                    offset += mjpg_frame->bLength;
                    size -= (int)mjpg_frame->bLength;
                }

                /*--------------------------------------------------------------------------*/
                /*  Parsing all still image frame descriptors under this format descriptor  */
                /*--------------------------------------------------------------------------*/
                if (size < (int)UVC_CS_DESC_MIN_LEN)
                {
                    descriptor_advanced = 1U;
                    break;
                }

                /* cppcheck-suppress misra-c2012-11.3 */
                vs_still_image_frame = (DESC_STILL_IMAGE_FRAME_T *)&cfg_buff[offset];

                if ((vs_still_image_frame->bDescriptorType != UVC_CS_INTERFACE) ||
                        (vs_still_image_frame->bDescriptorSubType != VS_STILL_IMAGE_FRAME))
                {
                    UVC_DBGMSG("No VS_STILL_IMAGE_FRAME found (optional), skipping.\n");
                    descriptor_advanced = 1U;
                    break;
                }

                if (uvc_validate_still_image_frame(&cfg_buff[offset], size,
                                                    &image_size_pattern_count,
                                                    &compression_pattern_count) != UVC_RET_OK)
                {
                    UVC_ERRMSG("UVC_RET_PARSER! - VS still image descriptor length!\n");
                    return UVC_RET_PARSER;
                }

                UVC_DBGMSG("VS Interface VS_STILL_IMAGE_FRAME\n");
                UVC_DBGMSG("    bEndpointAddress:        0x%x\n", vs_still_image_frame->bEndpointAddress);
                UVC_DBGMSG("    bNumImageSizePatterns:   0x%x\n", image_size_pattern_count);
                UVC_DBGMSG("    bNumCompressionPatterns: 0x%x\n", compression_pattern_count);

                for (i = 0; i < (int)image_size_pattern_count; i++)
                {
                    if (vc->num_of_still_frames >= (uint8_t)UVC_MAX_STILL_IMAGE)
                    {
                        UVC_ERRMSG("UVC_RET_PARSER! - Too many still image frames!\n");
                        return UVC_RET_PARSER;
                    }

                    idx = (int)vc->num_of_still_frames;
                    vc->still_image_width[idx] = vs_still_image_frame->wSize[i].wWidth;
                    vc->still_image_height[idx] = vs_still_image_frame->wSize[i].wHeight;
                    vc->still_image_format[idx] = vc->format[current_format_slot];

                    UVC_DBGMSG("    [%d]wWidth x wHeight:     0x%x x 0x%x\n", (i + 1), vs_still_image_frame->wSize[i].wWidth, vs_still_image_frame->wSize[i].wHeight);

                    vc->num_of_still_frames++;
                }

                offset += vs_still_image_frame->bLength;
                size -= (int)vs_still_image_frame->bLength;
                descriptor_advanced = 1U;
                break;

            case VS_FRAME_MJPEG:
                if (descriptor_length < UVC_FRAME_DESC_MIN_LEN)
                {
                    UVC_ERRMSG("UVC_RET_PARSER! - VS MJPEG frame length!\n");
                    return UVC_RET_PARSER;
                }

                UVC_DBGMSG("VS Interface VS_FRAME_MJPEG\n");
                UVC_DBGMSG("    bFormatIndex: 0x%x\n", ((DESC_MJPG_FRAME_T *)ifd_vs_hdr)->bFrameIndex);
                UVC_DBGMSG("    wWidth:       0x%x\n", ((DESC_MJPG_FRAME_T *)ifd_vs_hdr)->wWidth);
                UVC_DBGMSG("    wHeight       0x%x\n", ((DESC_MJPG_FRAME_T *)ifd_vs_hdr)->wHeight);
                UVC_DBGMSG("    dwMinBitRate: 0x%x\n", ((DESC_MJPG_FRAME_T *)ifd_vs_hdr)->dwMinBitRate);
                UVC_DBGMSG("    dwMaxBitRate: 0x%x\n", ((DESC_MJPG_FRAME_T *)ifd_vs_hdr)->dwMaxBitRate);
                UVC_DBGMSG("    dwDefaultFrameInterval:  0x%x\n", ((DESC_MJPG_FRAME_T *)ifd_vs_hdr)->dwDefaultFrameInterval);
                break;

            case VS_STILL_IMAGE_FRAME:
                if (uvc_validate_still_image_frame(&cfg_buff[offset], size,
                                                    &image_size_pattern_count,
                                                    &compression_pattern_count) != UVC_RET_OK)
                {
                    UVC_ERRMSG("UVC_RET_PARSER! - VS still image descriptor length!\n");
                    return UVC_RET_PARSER;
                }

                UVC_DBGMSG("VS Interface VS_STILL_IMAGE_FRAME\n");
                UVC_DBGMSG("    bEndpointAddress:        0x%x\n", cfg_buff[offset + 3U]);
                UVC_DBGMSG("    bNumImageSizePatterns:   0x%x\n", image_size_pattern_count);
                UVC_DBGMSG("    bNumCompressionPatterns: 0x%x\n", compression_pattern_count);
                break;

            case VS_OUTPUT_HEADER:
            case VS_UNDEFINED:
            case VS_FORMAT_MPEG2TS:
            case VS_FORMAT_DV:
            case VS_COLORFORMAT:
            case VS_FORMAT_FRAME_BASED:
            case VS_FRAME_FRAME_BASED:
            case VS_FORMAT_STREAM_BASED:
            case VS_FORMAT_H264:
            case VS_FRAME_H264:
            case VS_FORMAT_H264_SIMULCAST:
            case VS_FORMAT_VP8:
            case VS_FRAME_VP8:
            case VS_FORMAT_VP8_SIMULCAST:
                UVC_DBGMSG("Unsupported VS class interface descriptor 0x%x! Skip.\n", ifd_vs_hdr->bDescriptorSubType);
                break;

            default:
                UVC_ERRMSG("Parsing error! Unknown VS class interface descriptor 0x%x!\n", ifd_vs_hdr->bDescriptorSubType);
                return UVC_RET_PARSER;
        }

        if (descriptor_advanced == 0U)
        {
            offset += descriptor_length;
            size -= (int)descriptor_length;
        }
    }

#if 0

    /*------------------------------------------------------------------------------------*/
    /*  Find the next alternative interface                                               */
    /*------------------------------------------------------------------------------------*/
    while (size >= sizeof(DESC_IF_T))
    {
        ifd = (DESC_IF_T *)&cfg_buff[offset];

        if ((ifd->bDescriptorType == USB_DT_INTERFACE) && (ifd->bInterfaceNumber == ifnum) &&
                (ifd->bAlternateSetting != 0))
        {
            break;                              /* done, not alternative setting 0        */
        }

        if (ifd->bLength == 0)
        {
            return UVC_RET_PARSER;          /* prevent infinite loop                      */
        }

        offset += ifd->bLength;
        size -= ifd->bLength;
    }

    if (size < sizeof(DESC_IF_T))
    {
        UVC_ERRMSG("UVC_RET_PARSER! - Can't find any ALT of VS interface!\n");
        return UVC_RET_PARSER;
    }

#endif

    /*------------------------------------------------------------------------------------*/
    /*  Find all alternative interface of Standard Video Streaming Interface              */
    /*------------------------------------------------------------------------------------*/
    while (size >= (int)sizeof(DESC_IF_T))
    {
        uint16_t  pksz;
        uint16_t  payload_size;
        uint16_t  transaction_count;

        /* cppcheck-suppress misra-c2012-11.3 */
        ifd = (DESC_IF_T *)&cfg_buff[offset];

        if ((ifd->bDescriptorType != USB_DT_INTERFACE) || (ifd->bInterfaceNumber != iface->if_num) ||
                (ifd->bInterfaceClass != USB_CLASS_VIDEO) || (ifd->bInterfaceSubClass != UVC_SC_VIDEOSTREAMING))
        {
            break;                          /* done, the following content not belong to this interface */
        }

        if (((int)ifd->bLength < (int)sizeof(DESC_IF_T)) || ((int)ifd->bLength > size))
        {
            UVC_ERRMSG("UVC_RET_PARSER! - VS interface descriptor length!\n");
            return UVC_RET_PARSER;
        }

        UVC_DBGMSG("Parsing VS if %d, alt %d...\n", iface->if_num, ifd->bAlternateSetting);

        if (ifd->bNumEndpoints != 1U)
        {
            UVC_DBGMSG("  [!] bNumEndpoints is not 1!\n");
        }

        offset += ifd->bLength;
        size -= (int)ifd->bLength;

        if (size < (int)UVC_EP_DESC_MIN_LEN)
        {
            UVC_ERRMSG("UVC_RET_PARSER! - VS endpoint descriptor missing!\n");
            return UVC_RET_PARSER;
        }

        /* cppcheck-suppress misra-c2012-11.3 */
        epd = (DESC_EP_T *)&cfg_buff[offset];

        if (epd->bDescriptorType != USB_DT_ENDPOINT)
        {
            UVC_DBGMSG("  [!] Endpoint descriptor not found!\n");
            return UVC_RET_PARSER;
        }

        if ((epd->bLength < UVC_EP_DESC_MIN_LEN) || ((int)epd->bLength > size))
        {
            UVC_ERRMSG("UVC_RET_PARSER! - VS endpoint descriptor length!\n");
            return UVC_RET_PARSER;
        }

        UVC_DBGMSG("  Endpoint wMaxPacketSize = %d\n", epd->wMaxPacketSize);

        if (vs->num_of_alt >= (uint8_t)UVC_MAX_ALT_IF)
        {
            UVC_ERRMSG("UVC_RET_PARSER! - Too many VS alternate interfaces!\n");
            return UVC_RET_PARSER;
        }

        vs->alt_no[vs->num_of_alt] = ifd->bAlternateSetting;
        pksz = epd->wMaxPacketSize;
        payload_size = (uint16_t)(pksz & 0x07FFU);
        transaction_count = (uint16_t)(1U + ((pksz >> 11U) & 0x3U));
        pksz = (uint16_t)(payload_size * transaction_count);
        vs->max_pktsz[vs->num_of_alt] = pksz;
        vs->num_of_alt++;

        offset += epd->bLength;
        size -= (int)epd->bLength;
    }

    UVC_DBGMSG("\n\n----------------------------------------------------------\n");
    UVC_DBGMSG("[Video Streaming interface parsing result dump]\n");

    for (i = 0; i < (int)vs->num_of_alt; i++)
    {
        UVC_DBGMSG("  Alt %d, wMaxPacketSize = %d\n", vs->alt_no[i], vs->max_pktsz[i]);
    }

    return 0;
}

/*! @}*/ /* end of group USBH_EXPORTED_FUNCTIONS */

/*! @}*/ /* end of group USBH_Library */

/*! @}*/ /* end of group LIBRARY */

