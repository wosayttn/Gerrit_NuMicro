...
{
    AC2_IF_HDR_T  *ac2_hdr;
    AC2_CLK_SRC_T *clk_src;
    AC2_IT_T      *ac2_itd;
    AC2_OT_T      *ac2_otd;
    ...
    switch (((CS_HDR_T *)bptr)->bDescriptorSubtype)
    {
        ...
        case UAC2_CLOCK_SOURCE:
            ...
            else if (uac->acif.mic_clk_src_id == 0)
            {
...
int uac_parse_control_interface(UAC_DEV_T *uac, IFACE_T *iface)
{
    ...
    /*------------------------------------------------------------------------------------*/
    if ((size > (int)sizeof(DESC_HDR_T)) && (((DESC_IF_T *)bptr)->bDescriptorType == CS_INTERFACE) &&
            (((CS_HDR_T *)bptr)->bDescriptorSubtype == HEADER))
    {
        /* bcdADC is at offset 3 in both AC_IF_HDR_T and AC2_IF_HDR_T */
        uac->acif.bcdADC = ((AC_IF_HDR_T *)bptr)->bcdADC;
    ...
    while (size > (int)sizeof(DESC_HDR_T))
    {
        ...
        if (uac->acif.bcdADC >= UAC_VERSION_2)
            ret = uac2_parse_ac_interface(uac, bptr);
        else
...
int uac_parse_streaming_interface(UAC_DEV_T *uac, IFACE_T *iface, uint8_t bAlternateSetting)
{
    ...
    while (size > (int)sizeof(DESC_HDR_T))
    {
        ...
        if (uac->acif.bcdADC >= UAC_VERSION_2)
            ret = uac2_parse_as_interface(&asif, bptr);
        else
    ...
    if (uac->acif.bcdADC >= UAC_VERSION_2)
    {
        UAC_DBGMSG("    as2_gen: %s\n", (asif.as2_gen == NULL) ? "Not Found" : "OK");
        UAC_DBGMSG("    it2: %s\n", (asif.it2 == NULL) ? "Not Found" : "OK");
        UAC_DBGMSG("    ot2: %s\n", (asif.ot2 == NULL) ? "Not Found" : "OK");
        UAC_DBGMSG("    ft2: %s\n", (asif.ft2 == NULL) ? "Not Found" : "OK");
        UAC_DBGMSG("    cs2_epd: %s\n", (asif.cs2_epd == NULL) ? "Not Found" : "OK");
    }
