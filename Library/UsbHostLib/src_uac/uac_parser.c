...
/**
 * @brief  Parse a Class-Specific Audio Control (AC) interface descriptor.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
 * <b>Justification:</b> bptr points at a raw descriptor byte stream; mapping it onto the
 *                       CS_HDR_T/AC_IT_T/AC_OT_T structs avoids repetitive byte-by-byte field
 *                       assembly and mirrors the USB Audio Class specification's descriptor
 *                       layout.
 */
static int  uac_parse_ac_interface(UAC_DEV_T *uac, uint8_t *bptr)
{
    ...
    switch (((CS_HDR_T *)bptr)->bDescriptorSubtype)
    {
        ...
        case INPUT_TERMINAL:
            ...
            if (ac_itd->wTerminalType == (uint16_t)UAC_TT_USB_STREAMING)
            {
        ...
        case OUTPUT_TERMINAL:
            ...
            if (ac_otd->wTerminalType == (uint16_t)UAC_TT_USB_STREAMING)
            {
                UAC_DBGMSG("USB streaming terminal found, ID=0x%x\n", ac_otd->bTerminalID);
            }
...
/**
 * @brief  Locate the microphone Feature Unit whose Source ID matches the mic Input Terminal.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
 * <b>Justification:</b> cfg_buff is the device's raw configuration descriptor byte buffer;
 *                       mapping it onto DESC_CONF_T/AC_FU_T structs avoids repetitive
 *                       byte-by-byte field assembly and mirrors the USB Audio Class
...
static int  uac_set_microphone_feature_unit(UAC_DEV_T *uac)
{
    ...
    while (size >= (int)sizeof(DESC_IF_T))
    {
        ...
        if ((hdr->bDescriptorType == (uint8_t)CS_INTERFACE) && (hdr->bDescriptorSubtype == (uint8_t)FEATURE_UNIT) &&
 ...
 * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
 * <b>Justification:</b> cfg_buff is the device's raw configuration descriptor byte buffer;
 *                       mapping it onto DESC_CONF_T/DESC_IF_T structs avoids repetitive
...
int uac_parse_control_interface(UAC_DEV_T *uac, IFACE_T *iface)
{
    ...
    if_num = (int)iface->if_num;                /* interface number of AC interface      */
    ...
    while (size >= (int)sizeof(DESC_IF_T))
    {
        ...
        if ((ifd->bDescriptorType == USB_DT_INTERFACE) && ((int)ifd->bInterfaceNumber == if_num) &&
                (ifd->bInterfaceClass == (uint8_t)USB_CLASS_AUDIO) && (ifd->bInterfaceSubClass == (uint8_t)SUBCLS_AUDIOCONTROL))
        {
            break;
        }
    ...
    while (size > (int)sizeof(DESC_HDR_T))
    {
        ...
        if (ifd->bDescriptorType != (uint8_t)CS_INTERFACE)
        {
            break;
        }
...
/**
 * @brief  Parse a Class-Specific / Endpoint descriptor of an Audio Streaming (AS) interface.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
 * <b>Justification:</b> bptr points at a raw descriptor byte stream; mapping it onto the
 *                       CS_HDR_T/DESC_EP_T/CS_EP_T/AS_GEN_T/AS_FT1_T structs avoids repetitive
 *                       byte-by-byte field assembly and mirrors the USB Audio Class
 *                       specification's descriptor layout.
 */
static int  uac_parse_as_interface(AS_IF_T *asif, uint8_t *bptr)
{
    ...
    /* cppcheck-suppress misra-c2012-11.3 */
    if (((CS_HDR_T *)bptr)->bDescriptorType == (uint8_t)CS_ENDPOINT)
    {
        /* cppcheck-suppress misra-c2012-11.3 */
...
/**
 * @brief  Locate an Input/Output Terminal descriptor in the device's configuration descriptor
 *         matching the given Terminal ID.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
 * <b>Justification:</b> cfg_buff is the device's raw configuration descriptor byte buffer;
 *                       mapping it onto DESC_CONF_T/AC_OT_T structs avoids repetitive
 *                       byte-by-byte field assembly and mirrors the USB Audio Class
 *                       specification's descriptor layout.
 */
static void *uac_find_terminal(UAC_DEV_T *uac, uint8_t bTerminalID)
{
    ...
    while (size >= (int)sizeof(DESC_IF_T))
    {
        ...
        if ((hdr->bDescriptorType == (uint8_t)CS_INTERFACE) &&
                ((hdr->bDescriptorSubtype == (uint8_t)INPUT_TERMINAL) || (hdr->bDescriptorSubtype == (uint8_t)OUTPUT_TERMINAL)) &&
 ...
 * <b>Justification:</b> uac_find_terminal() returns a generic void* handle so it can locate
 *                       either an Input or Output Terminal descriptor; converting its return
 *                       value to AC_OT_T* or AC_IT_T* here reflects the caller's already-known
 *                       terminal direction.
 */
int uac_parse_streaming_interface(UAC_DEV_T *uac, IFACE_T *iface, uint8_t bAlternateSetting)
{
    ...
    while (size >= (int)sizeof(DESC_IF_T))
    {
        ...
        if ((ifd->bDescriptorType == USB_DT_INTERFACE) &&
                ((int)ifd->bInterfaceNumber == if_num) && (ifd->bAlternateSetting == bAlternateSetting) &&
                (ifd->bInterfaceClass == (uint8_t)USB_CLASS_AUDIO) && (ifd->bInterfaceSubClass == (uint8_t)SUBCLS_AUDIOSTREAMING))
        {
            break;
        }
    ...
    while (size > (int)sizeof(DESC_HDR_T))
    {
        ...
        if ((ifd->bDescriptorType != (uint8_t)CS_INTERFACE) &&
                ...
                (ifd->bDescriptorType != (uint8_t)CS_ENDPOINT))
        {
            break;
        }
