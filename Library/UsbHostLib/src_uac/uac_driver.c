...
static int  uac_probe(IFACE_T *iface)
{
    ...
    {
        ret = usbh_uac_find_max_alt(iface, EP_ADDR_DIR_IN, EP_ATTR_TT_ISO, &bAlternateSetting);

        if (ret != 0)
        {
            ret = usbh_uac_find_max_alt(iface, EP_ADDR_DIR_OUT, EP_ATTR_TT_ISO, &bAlternateSetting);
        }
...
/**
 * @brief Disconnect a UAC interface.
 * @param[in] iface Pointer to the IFACE_T structure representing the USB interface.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> The interface context is assigned as UAC_DEV_T during probe and
 *                       is restored to that owning type during interface disconnect.
 */
