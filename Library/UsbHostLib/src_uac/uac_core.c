...
/// @endcond HIDDEN_SYMBOLS
/**
 * @brief  Isochronous-in transfer completion callback: dispatch received audio data.
 *
 * @static_deviation
 ...
 *                       object to a transfer request; the framework's callback signature is
 *                       fixed, so casting it back to UAC_DEV_T here is an architectural
 *                       requirement. The object registered to context is created and owned by
 *                       this driver's own start-audio flow and remains valid for the callback's
 *                       lifetime.
 */
...
/**
 * @brief  Isochronous-out transfer completion callback: dispatch audio data for transmission.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> UTR_T::context is a generic void* handle used by the USB Host common
