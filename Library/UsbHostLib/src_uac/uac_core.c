...
int  usbh_uac_get_channel_number(UAC_DEV_T *uac, uint8_t target)
{
    if (uac->acif.bcdADC >= UAC_VERSION_2)
    {
        ...
        return (int)as2_gen->bNrChannels;
    }
    else
    {
        AS_FT1_T   const *ft;
...
{
    return ((uint32_t)srate[2] << 16U) | ((uint32_t)srate[1] << 8U) | (uint32_t)srate[0];
}
                                ...
                                int max_cnt, uint8_t *type)
{
    ...
    if (uac->acif.bcdADC >= UAC_VERSION_2)
    {
        ...
        UAC2_FREQ_SUBRANGE_T  ranges[16] =
        {
            ...
            { 0U, 0U, 0U }, { 0U, 0U, 0U }, { 0U, 0U, 0U }, { 0U, 0U, 0U },
            { 0U, 0U, 0U }, { 0U, 0U, 0U }, { 0U, 0U, 0U }, { 0U, 0U, 0U }
        };                                         /* support up to 16 discrete rates       */
        uint32_t   dMax;
        uint32_t   dMin;
        uint16_t   num_ranges;
        int        cnt;
...
int  usbh_uac_sampling_rate_control(UAC_DEV_T *uac, uint8_t target, uint8_t req, uint32_t *srate)
{
    ...
    /*------------------------------------------------------------------------------------*/
    {
        ...
        tSampleFreq[1] = (uint8_t)((*srate >> 8U) & 0xFFU);
        tSampleFreq[2] = (uint8_t)((*srate >> 16U) & 0xFFU);
...
int usbh_uac_vol_control(UAC_DEV_T *uac, uint8_t target, uint8_t req, uint16_t chn, uint16_t *volume)
{
    ...
    if (uac->acif.bcdADC >= UAC_VERSION_2)
    {
        ...
        else if ((req == (uint8_t)UAC_GET_MIN) || (req == (uint8_t)UAC_GET_MAX) || (req == (uint8_t)UAC_GET_RES))
        {
            ...
            };                              /* 2 + 1 * 6 = 8 bytes for first sub-range   */
            uint32_t  u32ReqRequestType_tmp = (uint32_t)REQ_TYPE_IN | (uint32_t)REQ_TYPE_CLASS_DEV | (uint32_t)REQ_TYPE_TO_IFACE;
            bmRequestType = (uint8_t)u32ReqRequestType_tmp;
            ...
            {
                *volume = (uint16_t)range_buf[2] | ((uint16_t)range_buf[3] << 8U);
            }
            else if (req == (uint8_t)UAC_GET_MAX)
            {
                *volume = (uint16_t)range_buf[4] | ((uint16_t)range_buf[5] << 8U);
            }
            else /* UAC_GET_RES */
            {
                *volume = (uint16_t)range_buf[6] | ((uint16_t)range_buf[7] << 8U);
            }
...
int usbh_uac2_clock_set_freq(UAC_DEV_T *uac, uint8_t target, uint32_t freq)
{
    ...
    data[0] = (uint8_t)(freq & 0xFFU);
    data[1] = (uint8_t)((freq >> 8U) & 0xFFU);
    data[2] = (uint8_t)((freq >> 16U) & 0xFFU);
    data[3] = (uint8_t)((freq >> 24U) & 0xFFU);
...
int usbh_uac2_clock_get_freq(UAC_DEV_T *uac, uint8_t target, uint32_t *freq)
{
    ...
    if (xfer_len != 4U)
    {
        ...
        *freq = (uint32_t)data[0] | ((uint32_t)data[1] << 8U) |
                                   ...
                                   int max_cnt, uint16_t *num_ranges, uint8_t target)
{
    uint8_t     buff[(2 + (16 * 12))] =
    {
        ...
        0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U,
        0U, 0U
    };                                          /* 2-byte header and up to 16 sub-ranges    */
    uint32_t    xfer_len;
    uint16_t    wlen;
    int         cnt;
    int         i;
    ...
    if ((uint32_t)wlen > (uint32_t)sizeof(buff))
    {
        wlen = (uint16_t)sizeof(buff);
    }
    ...
    *num_ranges = (uint16_t)buff[0] | ((uint16_t)buff[1] << 8U);

    /* Limit to what we can store */
    cnt = ((int)*num_ranges > max_cnt) ? max_cnt : (int)*num_ranges;
    ...
    {
        uint32_t u32Received_cnt = (xfer_len - 2U) / 12U;
        cnt = (int)u32Received_cnt;
    }
    ...
    for (i = 0; i < cnt; i++)
    {
        ...
        ranges[i].dMIN = (uint32_t)p[0] | ((uint32_t)p[1] << 8U) |
                 ((uint32_t)p[2] << 16U) | ((uint32_t)p[3] << 24U);
        ranges[i].dMAX = (uint32_t)p[4] | ((uint32_t)p[5] << 8U) |
                 ((uint32_t)p[6] << 16U) | ((uint32_t)p[7] << 24U);
        ranges[i].dRES = (uint32_t)p[8] | ((uint32_t)p[9] << 8U) |
 ...
 * <b>Justification:</b> The validated USB configuration byte buffer is reinterpreted as
 *                       standard audio descriptor layouts defined by the USB specification.
 */
int  usbh_uac2_find_pcm_alt(UAC_DEV_T *uac, IFACE_T *iface, uint8_t dir, uint8_t attr, uint8_t max_bitres, uint8_t *bAlternateSetting)
{
    ...
    while (size >= (int)sizeof(DESC_IF_T))
    {
        ...
        if ((ifd->bDescriptorType == USB_DT_INTERFACE) &&
                ((int)ifd->bInterfaceNumber == if_num) &&
                ...
                (ifd->bNumEndpoints > 0U))
        {
            ...
            if (has_iso_ep == 0U)
            {
                parsed_len += (int)ifd->bLength;
                size -= (int)ifd->bLength;
                continue;
            }

            /* Scan the CS descriptors following this interface descriptor */
            inner_parsed_len = parsed_len + (int)ifd->bLength;
            inner_size = size - (int)ifd->bLength;
            ...
            while ((inner_size > (int)sizeof(DESC_HDR_T)) && (stop_scan == 0U))
            {
                ...
                if (stop_scan == 0U)
                {
                    inner_parsed_len += (int)cs->bLength;
                    inner_size -= (int)cs->bLength;
                }
            }

            if ((is_pcm != 0U) && (cur_bitres > best_bitres) &&
...
/**
 * @brief Complete an audio isochronous-in transfer.
 * @param[in] utr Pointer to the USB transfer request structure (UTR_T) that has completed.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> The transfer context is assigned as UAC_DEV_T before submission
 *                       and is restored to that owning type in this completion callback.
 */
 ...
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> USB Host Library allocator storage is assigned to the typed audio
 *                       transfer buffer and released by the corresponding stop operation.
 */
int usbh_uac_start_audio_in(UAC_DEV_T *uac, UAC_CB_FUNC *func)
{
    ...
    if (uac->acif.bcdADC >= UAC_VERSION_2)
    {
        ret = usbh_uac2_find_pcm_alt(uac, iface, EP_ADDR_DIR_IN, EP_ATTR_TT_ISO, 0, &bAlternateSetting);
    }
    else
    {
        ret = UAC_RET_FUNC_NOT_FOUND;
    }
    ...
    {
        /* cppcheck-suppress misra-c2012-11.5 */
        buff = (uint8_t *)usbh_alloc_mem((int32_t)ep->wMaxPacketSize *
                  (int32_t)IF_PER_UTR * (int32_t)NUM_UTR);
    ...
    if (ret < 0)
    {
        ...
        {
            (void)usbh_free_mem(asif->utr[0]->buff,
                                (int32_t)asif->utr[0]->data_len * (int32_t)NUM_UTR);
        }
...
/**
 * @brief Complete an audio isochronous-out transfer.
 * @param[in] utr Pointer to the USB transfer request structure (UTR_T) that has completed.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
...
int usbh_uac_start_audio_out(UAC_DEV_T *uac, UAC_CB_FUNC *func)
{
    ...
    /*------------------------------------------------------------------------------------*/
    {
        ...
        if (uac->acif.bcdADC >= UAC_VERSION_2)
        {
            ret = usbh_uac2_find_pcm_alt(uac, iface, EP_ADDR_DIR_OUT, EP_ATTR_TT_ISO, 0, &bAlternateSetting);
        }
        else
        {
            ret = UAC_RET_FUNC_NOT_FOUND;
...
int usbh_uac_open(UAC_DEV_T *uac)
{
    ...
    if (iface != USBNULL)
    {
        ...
        if (uac->acif.bcdADC >= UAC_VERSION_2)
        {
            ret = usbh_uac2_find_pcm_alt(uac, iface, EP_ADDR_DIR_OUT, EP_ATTR_TT_ISO, peer_bitres, &bAlternateSetting);
        }
        else
        {
            ret = UAC_RET_FUNC_NOT_FOUND;
