                                ...
                                int max_cnt, uint8_t *type)
{
    ...
    if (uac->acif.bcdADC >= UAC_VERSION_2)
    {
        ...
        if ((num_ranges == 1) && (ranges[0].dMIN != ranges[0].dMAX))
        {
            *type = 0;                      /* continuous */
        ...
        else
        {
            ...
            *type = (uint8_t)num_ranges;
            ...
            for (i = 0; (i < num_ranges) && (i < max_cnt); i++)
...
int usbh_uac_mute_control(UAC_DEV_T *uac, uint8_t target, uint8_t req, uint16_t chn, uint8_t *mute)
{
    ...
    if (uac->acif.bcdADC >= UAC_VERSION_2)
    {
        if ((req != UAC_SET_CUR) && (req != UAC_GET_CUR))
        ...
        ret = usbh_ctrl_xfer(uac->udev, bmRequestType, UAC2_CUR,
                             (UAC2_FU_MUTE_CONTROL << 8) | chn,
                             (bUnitID << 8) | (uac->acif.iface->if_num),
...
int usbh_uac_vol_control(UAC_DEV_T *uac, uint8_t target, uint8_t req, uint16_t chn, uint16_t *volume)
{
    ...
    if (uac->acif.bcdADC >= UAC_VERSION_2)
    {
        ...
        else if ((req == UAC_GET_MIN) || (req == UAC_GET_MAX) || (req == UAC_GET_RES))
        {
...
int usbh_uac2_get_clock_source_id(UAC_DEV_T *uac, uint8_t target)
{
    ...
    if (uac->acif.clk_src_id == 0)
        return UAC_RET_DEV_NOT_SUPPORTED;

    if (uac->acif.mic_clk_src_id == 0)
        return UAC_RET_DEV_NOT_SUPPORTED;
...
int usbh_uac2_clock_set_freq(UAC_DEV_T *uac, uint8_t target, uint32_t freq)
{
    ...
    ret = usbh_ctrl_xfer(uac->udev,
                         ...
                         UAC2_CUR,                                               /* bRequest */
                         (UAC2_CS_SAM_FREQ_CONTROL << 8),                        /* wValue */
                         ((target == UAC_SPEAKER ? uac->acif.clk_src_id : uac->acif.mic_clk_src_id) << 8) | (uac->acif.iface->if_num), /* wIndex */
                         4,                                                      /* wLength */
                         data,
                         &xfer_len, UAC_REQ_TIMEOUT);
                                   ...
                                   int max_cnt, uint16_t *num_ranges, uint8_t target)
{
    ...
    *num_ranges = buff[0] | (buff[1] << 8);
    ...
    if (xfer_len < (uint32_t)(2 + cnt * 12))
        cnt = (xfer_len - 2) / 12;

    for (i = 0; i < cnt; i++)
    {
        uint8_t *p = &buff[2 + i * 12];
    ...
    while (size >= (int)sizeof(DESC_IF_T))
    {
                ...
                (ifd->bNumEndpoints > 0))
        {
            ...
            /* Verify this alt has the right ISO endpoint direction */
            {
                ...
                {
                    if (iface->alt[j].ifd->bAlternateSetting == cur_alt)
                    {
                        uint8_t k;

                        for (k = 0; k < iface->alt[j].ifd->bNumEndpoints; k++)
                        {
                            EP_INFO_T *ep = &iface->alt[j].ep[k];
            ...
            inner = bptr + ifd->bLength;
            inner_size = size - ifd->bLength;

            while (inner_size > (int)sizeof(DESC_HDR_T))
            {
                CS_HDR_T *cs = (CS_HDR_T *)inner;

                if (cs->bLength == 0)
                    break;

                /* Stop at next interface descriptor */
                if (cs->bDescriptorType == USB_DT_INTERFACE)
                    break;

                if (cs->bDescriptorType == CS_INTERFACE)
                {
                    if (cs->bDescriptorSubtype == AS_GENERAL)
                    {
                        AS2_GEN_T *gen = (AS2_GEN_T *)inner;

                        /* bmFormats bit 0 = PCM.  Reject if only RAW (bit 31). */
                        if (gen->bmFormats & 0x01)
                            is_pcm = 1;
                    }
                    else if (cs->bDescriptorSubtype == FORMAT_TYPE)
                    {
                        AS2_FT1_T *ft = (AS2_FT1_T *)inner;
            ...
            if (is_pcm && (cur_bitres > best_bitres) &&
                    ((max_bitres == 0) || (cur_bitres <= max_bitres)))
            {
