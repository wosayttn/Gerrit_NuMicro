...
/**
 * @brief  CDC INT-in complete function.
 *
 * @static_deviation
...
static void  cdc_int_in_irq(UTR_T *utr)
{
    ...
    if ((cdev->sts_func != USBNULL) && (utr->xfer_len != 0U))
    {
...
int32_t usbh_cdc_start_polling_status(CDC_DEV_T *cdev, CDC_CB_FUNC *func)
{
    ...
    if ((func == USBNULL) || (cdev->utr_sts != USBNULL))
    {
        return USBH_ERR_INVALID_PARAM;
    }
...
/**
 * @brief  CDC BULK-in complete function.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> UTR_T.context is a generic void* handle used by the USB Host common
 ...
 * @retval   Otherwise   Failed
 */
int32_t usbh_cdc_send_data(CDC_DEV_T *cdev, uint8_t *buff, uint32_t buff_len)
{
    EP_INFO_T   *ep;
    UTR_T       *utr;
    uint32_t    t0;
