...
/**
 * @brief  Handle completion of a CDC interrupt-IN transfer.
 * @param[in] utr   The UTR of the interrupt-in transfer.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> The USB host callback context is stored as void *;
 *                       this callback restores the associated CDC device type.
 */
...
/**
 * @brief  Handle completion of a CDC bulk-IN transfer.
 * @param[in] utr   The UTR of the bulk-in transfer.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> The USB host callback context is stored as void *;
...
int32_t usbh_cdc_send_data(CDC_DEV_T *cdev, uint8_t *buff, int buff_len)
{
    ...
    utr->ep = ep;
    utr->buff = buff;
    utr->data_len = (uint32_t)buff_len;
    utr->xfer_len = 0;
...
int32_t usbh_dcache_cdc_send_data(CDC_DEV_T *cdev, uint8_t const *buff, int buff_len)
{
...
#endif
    int ret = 0;
    unsigned int temp_duff_len = (unsigned int)buff_len;
    ...
    while (temp_duff_len > 0U)
    {
        ...
        SCB_CleanDCache_by_Addr((void *)cdc_tmp_buffer, (int32_t)CDC_BLOCK_SIZE);

        ret = usbh_cdc_send_data(cdev, cdc_tmp_buffer, (int)CDC_BLOCK_SIZE);
