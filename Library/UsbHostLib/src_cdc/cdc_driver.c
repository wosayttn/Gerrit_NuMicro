...
/**
 * @brief  Allocate a CDC device object.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> usbh_alloc_mem() is the USB Host common layer's centralized generic
 *                       memory pool allocator and necessarily returns void* since it serves
 *                       many different object types; assigning its result directly to the
 *                       CDC_DEV_T handle at the allocation site is the established common-layer
 *                       allocation pattern, and the returned pointer is NULL-checked immediately
...
{
    CDC_DEV_T  *p;
    int        if_num = (int)iface_data->if_num;
...
static int  cdc_probe(IFACE_T *iface)
{
    ...
    if (ifd->bInterfaceClass == USB_CLASS_DATA)
    {
        ...
        if (cdev == USBNULL)
        {
            ...
            cdev->ifnum_data = (int)iface->if_num;
        }
...
/// @endcond HIDDEN_SYMBOLS
/**
 * @brief  Disconnect handler of the CDC device interface.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.5<br>
 * <b>Justification:</b> IFACE_T.context is a generic void* handle used by the USB Host common
 ...
 *                       to CDC_DEV_T here is an architectural requirement. The object registered
