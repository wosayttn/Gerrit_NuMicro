...
/**
 * @brief  Parse CDC class-specific (CS_INTERFACE) descriptors.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
 * <b>Justification:</b> The source data is a USB descriptor byte stream; mapping it onto
 *                       DESC_HDR_T / CDC_IF_HDR_T descriptor structs avoids repetitive byte-by-byte
 *                       field assembly and mirrors the USB CDC specification's descriptor layout.
 *                       bLength and descriptor-type fields are checked before any field access.
 */
static int  cdc_parse_cs_interface(CDC_DEV_T *cdev, uint8_t *buffer, int size)
{
    ...
    while (size_tmp > 0)
    {
        ...
        while (size_tmp >= (int)sizeof(DESC_HDR_T))
        {
            ...
            if (header->bDescriptorType != (uint8_t)CDC_CS_INTERFACE)
            {
                return parsed;
            }

            {
                ...
                switch (cifd->bDescriptorSubtype)
                {
                    ...
                    case CDC_DT_UNION:
                        ...
                        if (cifd->bLength >= 5U)
                        {
                            cdev->ifnum_data = (int)cifd->payload[1];
                        }
...
/**
 * @brief  Parse the CDC device's configuration descriptor to locate CDC interfaces.
 * @param[in,out] cdev  The CDC device object.
 * @return  0 on success, negative on failure.
 *
 * @static_deviation
 * <b>Rule:</b>          MISRA C:2012 Rule 11.3<br>
 * <b>Justification:</b> The source data is a USB descriptor byte stream; mapping it onto
 *                       DESC_CONF_T / DESC_HDR_T / DESC_IF_T descriptor structs avoids repetitive
 *                       byte-by-byte field assembly and mirrors the USB specification's descriptor
 *                       layout. bLength and remaining-buffer-length are checked before each cast.
 */
