...
static void rndis_notification_irq(UTR_T *utr)
{
    ...
    if ((session == USBNULL) || (session->notification_utr != utr))
    {
        return;
    }

    if ((session->stopping != 0U) || (session->notification_io_generation != session->io_generation) ||
...
int rndis_ctrl_allocate_notification(RNDIS_SESSION_T *session)
{
    ...
    /* cppcheck-suppress misra-c2012-11.5 */
    session->notification_buffer = (uint8_t *)usbh_alloc_mem(RNDIS_NOTIFICATION_SIZE);
...
void rndis_ctrl_free_notification(RNDIS_SESSION_T *session)
{
    ...
    {
        (void)usbh_free_mem(session->notification_buffer, RNDIS_NOTIFICATION_SIZE);
...
static int rndis_notification_prepare(RNDIS_SESSION_T *session)
{
    ...
    if ((session->notification_utr != USBNULL) && (session->notification_active == 0U))
    {
                                 ...
                                 uint32_t fixed_length, uint32_t *message_length)
{
    ...
    if ((rndis_get_le32(&session->ctrl_response[8]) != session->request_id) ||
            (rndis_get_le32(&session->ctrl_response[12]) != RNDIS_STATUS_SUCCESS))
    {
        return RNDIS_ERR_PROTOCOL;
    }
...
static int rndis_process_query(RNDIS_SESSION_T *session, uint8_t const *info, uint32_t info_length)
{
    ...
    switch (session->pending_oid)
    {
        case OID_GEN_HARDWARE_STATUS:
            if ((info_length != 4U) || (rndis_get_le32(info) != 0U))
            {
                return RNDIS_ERR_PROTOCOL;
            }
        ...
        case OID_GEN_MEDIA_IN_USE:
            if ((info_length != 4U) || (rndis_get_le32(info) != RNDIS_MEDIUM_802_3))
            {
                return RNDIS_ERR_PROTOCOL;
            }
        ...
        case OID_GEN_MEDIA_CONNECT_STATUS:
            if ((info_length != 4U) || (rndis_get_le32(info) != RNDIS_MEDIA_CONNECTED))
            {
                return RNDIS_ERR_PROTOCOL;
            }
...
static int rndis_process_response(RNDIS_SESSION_T *session, uint32_t actual)
{
    ...
    if (session->ctrl_state == RNDIS_CTRL_WAIT_INITIALIZE)
    {
        ...
        if (tx_max_transfer_size > RNDIS_SLOT_SIZE)
        {
            tx_max_transfer_size = RNDIS_SLOT_SIZE;
        }
