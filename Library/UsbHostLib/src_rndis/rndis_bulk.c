...
static void rndis_rx_irq(UTR_T *utr)
{
    ...
    for (index = 0U; index < RNDIS_RX_SLOT_COUNT; index++)
    {
        ...
        if (slot->utr == utr)
        {
            ...
            if (slot->io_generation != session->io_generation)
            {
                rndis_record_stale_completion(session);
                return;
            }
...
static int rndis_submit_rx(RNDIS_SESSION_T *session, RNDIS_SLOT_T *slot)
{
    ...
    slot->utr->buff = slot->buffer->data;
...
static int rndis_submit_tx(RNDIS_SESSION_T *session, RNDIS_SLOT_T *slot)
{
    ...
    if (slot->state != RNDIS_SLOT_READY)
    {
        return RNDIS_ERR_NOT_READY;
    }
...
int rndis_bulk_allocate(RNDIS_SESSION_T *session)
{
    ...
    /* cppcheck-suppress misra-c2012-11.5 */
    session->ctrl_command = (uint8_t *)usbh_alloc_mem((int32_t)RNDIS_CTRL_COMMAND_SIZE);
    /* cppcheck-suppress misra-c2012-11.5 */
    session->ctrl_response = (uint8_t *)usbh_alloc_mem((int32_t)RNDIS_CTRL_RESPONSE_SIZE);
    ...
    {
        /* cppcheck-suppress misra-c2012-11.5 */
        session->rx[index].buffer = (RNDIS_SLOT_BUFFER_T *)usbh_alloc_mem((int32_t)sizeof(RNDIS_SLOT_BUFFER_T));
    ...
    /* cppcheck-suppress misra-c2012-11.5 */
    session->tx[0].buffer = (RNDIS_SLOT_BUFFER_T *)usbh_alloc_mem((int32_t)sizeof(RNDIS_SLOT_BUFFER_T));
...
void rndis_bulk_free(RNDIS_SESSION_T *session)
{
    ...
    {
        (void)usbh_free_mem(session->ctrl_response, (int32_t)RNDIS_CTRL_RESPONSE_SIZE);
        session->ctrl_response = USBNULL;
    }

    if (session->ctrl_command != USBNULL)
    {
        (void)usbh_free_mem(session->ctrl_command, (int32_t)RNDIS_CTRL_COMMAND_SIZE);
...
static void rndis_process_rx(RNDIS_SESSION_T *session, RNDIS_SLOT_T *slot)
{
    ...
    message_length = rndis_get_le32(&slot->buffer->data[4]);
    data_offset = rndis_get_le32(&slot->buffer->data[8]);
    data_length = rndis_get_le32(&slot->buffer->data[12]);

    if (rndis_get_le32(&slot->buffer->data[0]) != RNDIS_MSG_PACKET)
    {
        session->stats.rx_drops++;
        rndis_publish_rx_diagnostic(session, RNDIS_RX_DIAG_MESSAGE_TYPE, slot, 1U,
                                    rndis_get_le32(&slot->buffer->data[0]), message_length, data_offset, data_length);
        return;
    }
    ...
    {
        session->rx_callback(session, &slot->buffer->data[data_start], (uint16_t)data_length);
    }
}
                        ...
                        uint32_t *submit_id)
{
    ...
    if ((frame_len < 14U) || (frame_len > session->max_frame) ||
            (message_length > packet_limit) ||
            ((uint32_t)frame_len > (RNDIS_SLOT_SIZE - 44U)))
    {
        session->stats.tx_drops++;
        return RNDIS_ERR_FRAME_SIZE;
    }
    ...
    (void)memset(slot->buffer->data, 0, 44U);
    rndis_put_le32(&slot->buffer->data[0], RNDIS_MSG_PACKET);
    rndis_put_le32(&slot->buffer->data[4], message_length);
    rndis_put_le32(&slot->buffer->data[8], 36U);
    rndis_put_le32(&slot->buffer->data[12], (uint32_t)frame_len);
    (void)memcpy(&slot->buffer->data[44], frame, frame_len);
