...
{
    if (bc12_ports[port].drv->set_switches != NULL)
    {
...
{
    if ((port < 0) || (bc12_ports[port].drv->ramp_allowed == NULL))
    {
        return 0;
    }

    return (bc12_ports[port].drv->ramp_allowed(supplier) != 0) ? 1 : 0;
}
...
{
    if ((port < 0) || (bc12_ports[port].drv->ramp_max == NULL))
    {
        return 0;
    }

    return (int)bc12_ports[port].drv->ramp_max(supplier, sup_curr);
}
