...
static inline atomic_val_t ATOMIC_OP(uint32_t operation, atomic_t *addr, atomic_val_t value)
{
    ...
    switch (operation)
    {
        ...
        default:
            /* No operation for unknown codes */
            break;
    }
...
static inline atomic_val_64_t ATOMIC_OP64(uint32_t operation, atomic_64_t *addr, atomic_val_64_t value)
{
    //    int was_masked = __disable_irq();        //We are not in RTOS environment
    switch (operation)
    {
        ...
        default:
            /* Only orr / bic are used with 64-bit atomics */
            break;
    }
...
static inline atomic_val_64_t atomic_or64(atomic_64_t *addr, atomic_val_64_t bits)
{
