...
{
    return i2c_xfer((int)tcpc_config[port].i2c_info.port,
...
#ifdef SW
    return i2c_xfer_unlocked((int)tcpc_config[port].i2c_info.port,
                             tcpc_config[port].i2c_info.addr_flags,
...
{
    return i2c_read_block((int)tcpc_config[port].i2c_info.port,
                          tcpc_config[port].i2c_info.addr_flags,
                          reg, in, size);
}
...
{
    return i2c_write_block((int)tcpc_config[port].i2c_info.port,
                           tcpc_config[port].i2c_info.addr_flags,
                           reg, out, size);
}
...
{
    return i2c_update8((int)tcpc_config[port].i2c_info.port,
                       tcpc_config[port].i2c_info.addr_flags,
                       reg, mask, action);
}
...
static inline int tcpc_update16(int port, int reg,
                                ...
                                enum mask_update_action action)
{
    return i2c_update16((int)tcpc_config[port].i2c_info.port,
                        tcpc_config[port].i2c_info.addr_flags,
                        reg, mask, action);
}
...
static inline int tcpc_write(int port, int reg, int val)
{
    return tcpc_addr_write(port,
                           (int)tcpc_config[port].i2c_info.addr_flags, reg, val);
}
...
{
    i2c_lock((int)tcpc_config[port].i2c_info.port, lock);
}
...
static inline int tcpm_init(int port)
{
    ...
    {
        int (* volatile post_init_fp)(int) = board_tcpc_post_init;

        if (post_init_fp != NULL)
        {
            rv = post_init_fp(port);
        }
    }
...
static inline int tcpm_set_vconn(int port, int enable)
{
    ...
    return tcpm_sop_prime_enable(port, (bool)(enable != 0));
}
...
static inline void tcpm_enable_auto_discharge_disconnect(int port, int enable)
{
    ...
    if (tcpc->tcpc_enable_auto_discharge_disconnect != NULL)
    {
...
{
    return (tcpc_config[port].drv->get_snk_ctrl != NULL) ?
           (bool)tcpc_config[port].drv->get_snk_ctrl(port) : false;
}
static inline int tcpm_set_snk_ctrl(int port, int enable)
{
    ...
    {
        return (int)EC_ERROR_UNIMPLEMENTED;
    }
}
...
static inline bool tcpm_get_src_ctrl(int port)
{
    ...
    return (tcpc_config[port].drv->get_src_ctrl != NULL) ?
           (bool)tcpc_config[port].drv->get_src_ctrl(port) : false;
}
...
{
    return (tcpc_config[port].drv->drp_toggle != NULL) ? 1 : 0;
}
...
{
    if (tcpc_config[port].drv->debug_accessory != NULL)
    {
...
{
    if (tcpc_config[port].drv->debug_detach != NULL)
    {
...
{
    if (tcpc_config[port].drv->get_chip_info != NULL)
    {
...
static inline enum ec_error_list tcpc_set_bist_test_mode(int port, bool enable)
{
    ...
    if (tcpc->set_bist_test_mode != NULL)
    {
