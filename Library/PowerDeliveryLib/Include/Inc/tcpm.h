...
//extern uint8_t I2C_WriteByteTwoRegs(I2C_T *i2c, uint8_t u8SlaveAddr, uint16_t u16DataAddr, uint8_t data);
//extern uint8_t I2C_ReadByteTwoRegs(I2C_T *i2c, uint8_t u8SlaveAddr, uint16_t u16DataAddr);
...
static inline int tcpc_addr_write(int port, int i2c_addr, int reg, int val)
{
  ...
  int rv = 0;
	if(port == 0)
			return (int)I2C_WriteByteTwoRegsProtect(I2C0, I2C_ADDR[port], reg, val);
	return rv; 
#endif 	
}
...
#else
	int rv = 0;
	uint8_t g_au8MstTxData[2];
	g_au8MstTxData[0] = val;
	g_au8MstTxData[1] = val>>8;
    if(port == 0)
	{
		uint8_t u8WriteByte = I2C_WriteMultiBytesTwoRegsProtect(I2C0, I2C_ADDR[port], reg, g_au8MstTxData, 2);
		if(u8WriteByte != 2)
		{
			rv = 1; //error 	
		}
	}
	return rv; //successful
#endif	
}
...
static inline int tcpc_addr_read(int port, int i2c_addr, int reg, int *val)
{
	...
	if(port == 0)
        *val = I2C_ReadByteTwoRegs(I2C0, I2C_ADDR[port], reg);		/**
																															* I2C_ReadByteTwoRegs();     	
																															* return     Read a byte data from Slav																													*
																															*/
	return rv;    	
	#else
	int rv = 0; 
	...
	{
			uint8_t u8ReadByte;
			uint8_t g_u8MstRxData[1];	
			u8ReadByte = I2C_ReadMultiBytesTwoRegsProtect(I2C0, I2C_ADDR[port], reg, g_u8MstRxData, 1);
			if(u8ReadByte != 1)
			{
					DBG_PRINTF("Err %d byte\n", u8ReadByte); 
					DBG_PRINTF("I2C SADDR = 0x%x\n", I2C_ADDR[port]);
					DBG_PRINTF("reg = 0x%x\n", reg); 
					rv = 1; //error 	
			}
			else
...
static inline int tcpc_addr_read16(int port, int i2c_addr, int reg, int *val)
{
  ...
  if(port == 0)
	{
			uint8_t u8ReadByte;
			uint8_t g_u8MstRxData[2];	
            u8ReadByte = I2C_ReadMultiBytesTwoRegsProtect(I2C0, I2C_ADDR[port], reg, g_u8MstRxData, 2);
			if(u8ReadByte != 2)
			{
//					DBG_PRINTF("Err %d byte\n", u8ReadByte); 
//					DBG_PRINTF("I2C SADDR = 0x%x\n", I2C_ADDR[port]);
//				  DBG_PRINTF("reg = 0x%x\n", reg); 
			...
			{
					*val = (g_u8MstRxData[1] <<8) | g_u8MstRxData[0];  //??endian??
			}
	}
	return rv; 
#endif	
...
static inline int tcpc_addr_read16_no_lpm_exit(int port, int i2c_addr,
					       int reg, int *val)
{
	return tcpc_addr_read16(port, I2C_ADDR[port], reg, val);
}
...
static inline int tcpc_read_block(int port, int reg, uint8_t *in, int size)
{
	...
	{
        uint8_t u8ReadByte;
        u8ReadByte = I2C_ReadMultiBytesTwoRegsProtect(I2C0, I2C_ADDR[port], reg, in, size);
	    if(u8ReadByte != size)
            rv = 1; //error 	
	}
	return rv; 
		...
		const uint8_t *out, int size)
{
	...
	{
        uint8_t u8WriteByte;
		u8WriteByte = I2C_WriteMultiBytesTwoRegsProtect(I2C0, I2C_ADDR[port], reg, (uint8_t *)out, size);
        if(u8WriteByte != size)
		    rv = 1; //error 	
	}
	return rv; 
			       ...
			       enum mask_update_action action)
{
	...
	return i2c_update8(tcpc_config[port].i2c_info.port,
			   ...
			   reg, mask, action);
#else
	return i2c_update8(port, I2C_ADDR[port], reg, mask, action);
#endif	
}
				...
				enum mask_update_action action)
{
	...
	return i2c_update16(tcpc_config[port].i2c_info.port,
			    ...
			    reg, mask, action);
#else
  return i2c_update16(port, I2C_ADDR[port], reg, mask, action);
#endif	
}
