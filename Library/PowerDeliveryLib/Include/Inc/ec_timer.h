...
{
    uint32_t diff = to_time - from_time;
    return (int32_t)diff;
}
...
{
    return (uint32_t)time_until(start.le.lo, get_time().le.lo);
}
...
{
    return (time_until(a, b) < 0) ? 1 : 0;
}

#endif  /* __CROS_EC_EC_TIMER_H */
