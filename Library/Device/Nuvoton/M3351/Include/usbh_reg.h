...
typedef struct
{
     ...
     * |[8]     |SIEPD     |SINTEN Pipeline Disable Bit
     * |        |          |When set, waits for all USB bus activity to complete prior to returning completion status to the List Processor
     * |        |          |This is a failsafe mechanism to avoid potential problems with the clk_dr transition between 1.5 MHz and 12 MHz.
     ...
     * @var USBH_T::VERSION
     * Offset: 0xFFC  USB Host Controller Version Number Register
     * ---------------------------------------------------------------------------------------------------
     * |Bits    |Field     |Descriptions
     * | :----: | :----:   | :---- |
     * |[15:0]  |MINOR     |USB Host Controller RTL Design MINOR Version Number
     * |        |          |Minor version number is dependent on ECO version control
     * |        |          |0x0000: (current Minor Version Number)
     * |[23:16] |SUB       |USB Host Controller RTL Design SUB Version Number
     * |        |          |Sub version number is relative to key feature
     * |        |          |0x01: (current Sub Version Number)
     * |[31:24] |MAJOR     |USB Host Controller RTL Design MAJOR Version Number
    ...
    __I  uint32_t RESERVE1[893];
    __I  uint32_t VERSION;               /*!< [0x0ffc] USB Host Controller Version Number Register                      */
...
#define USBH_VERSION_MINOR_Pos           (0)                                               /*!< USBH_T::VERSION: MINOR Position        */
#define USBH_VERSION_MINOR_Msk           (0xfffful << USBH_VERSION_MINOR_Pos)              /*!< USBH_T::VERSION: MINOR Mask            */

#define USBH_VERSION_SUB_Pos             (16)                                              /*!< USBH_T::VERSION: SUB Position          */
#define USBH_VERSION_SUB_Msk             (0xfful << USBH_VERSION_SUB_Pos)                  /*!< USBH_T::VERSION: SUB Mask              */

#define USBH_VERSION_MAJOR_Pos           (24)                                              /*!< USBH_T::VERSION: MAJOR Position        */
#define USBH_VERSION_MAJOR_Msk           (0xfful << USBH_VERSION_MAJOR_Pos)                /*!< USBH_T::VERSION: MAJOR Mask            */
