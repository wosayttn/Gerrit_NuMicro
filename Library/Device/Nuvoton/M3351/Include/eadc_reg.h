 ...
 * |        |          |Note 1: This function is workable only when any one of INTPOS (EADC_SCTLx[5]), x=0~23 is set.
 ...
 * |        |          |Others = Reserved.
 * |        |          |Note: the function is for internal used, it's not complete function
 ...
 * |        |          |8'b00000000 = ADC converts for 1 time.
 * |        |          |8'b00000001 = ADC converts for 2 times.
 * |        |          |8'b00000010 = ADC converts for 3 times.
 * |        |          |8'b00000011 = ADC converts for 4 times.
 * |        |          |8'b00000100 = ADC converts for 5 times.
 * |        |          |uFF1A
 * |        |          |uFF1A
 * |        |          |uFF1A
 * |        |          |8'b11111101 = ADC converts for 254 times.
 * |        |          |8'b11111110 = ADC converts for 255 times.
 ...
 * |        |          |0 = Calibrate fine comparator offset.
 * |        |          |1 = Calibrate MSB.
 * |        |          |2 = Calibrate MSB-1.
 * |        |          |3 = Calibrate MSB-2.
 * |        |          |4 = Calibrate MSB-3.
 * |        |          |5 = Calibrate MSB-4. (Default)
 * |        |          |6 = Calibrate MSB-5.
 * |        |          |7 = Reserved.
 * |        |          |Others = reserved.
 * |[20]    |CAL_CCO   |Calibration for coarse comparator offset
 * |        |          |0 = Coarse comparator offset calibration disabled.
 * |        |          |1 = Coarse comparator offset calibration enabled. (Default)
 * |        |          |Note: This bit needs to work with CALSEL. 
 ...
 * @var EADC_T::DAT19[8]
 * Offset: 0x200  EADC Data Register 19 for Sample Module 19
 * ---------------------------------------------------------------------------------------------------
 * |Bits    |Field     |Descriptions
 * | :----: | :----:   | :---- |
 * |[15:0]  |RESULT    |EADC Conversion Result
 ...
 * |        |          |1 = Data in RESULT[11:0] bits is valid.
 * @var EADC_T::SCTL19[8]
 * Offset: 0x230  EADC Sample Module 19 Control Register
 * ---------------------------------------------------------------------------------------------------
 * |Bits    |Field     |Descriptions
 * | :----: | :----:   | :---- |
 * |[4:0]   |CHSEL     |EADC Sample Module Channel Selection
 ...
 * Offset: 0x260  EADC Sample Module19 Control Register 1
 * ---------------------------------------------------------------------------------------------------
 * |Bits    |Field     |Descriptions
 * | :----: | :----:   | :---- |
 * |[0]     |ALIGN     |Alignment Selection
 ...
 * |        |          |11 = EADC_CLK/16.
 * @var EADC_T::TEST
 * Offset: 0xFF0  EADC Test Mode Control Register
 * ---------------------------------------------------------------------------------------------------
 * |Bits    |Field     |Descriptions
 * | :----: | :----:   | :---- |
 ...
 * |        |          |This register defined the pre-channel switch cycles (0~7 EADC_CLK).
 * |[8]     |DECADD    |EADC Decode Add One Cycle
 * |        |          |0 = Decode Keep Original Cycle.
 * |        |          |1 = Decode Add One Cycle.
 * |[9]     |MSBADD    |EADC Decode Add One Cycle
 * |        |          |0 = Decode Keep Original Cycle.
 * |        |          |1 = Decode Add One Cycle.
 * |        |          |Note: with DECADD
 * @var EADC_T::DEBUG
 * Offset: 0xFF4  EADC Debug Mode Control Register
 * ---------------------------------------------------------------------------------------------------
 * |Bits    |Field     |Descriptions
 * | :----: | :----:   | :---- |
 * |[0]     |ENFLO     |Extending Internal FLO Edge Enable Bit
 * |        |          |1 =extending internal FLO edge is enabled 0= extending internal FLO edge is disabled.
 * |        |          |Note: this bit is only for analog RD debug ADC macro
 * |[1]     |ENBOOST   |ADC Boost Mode Enable Bit
 * |        |          |1 = boost mode is enabled  0 = boost mode is diisabled.
 * |        |          |Note: this bit is only for analog RD debug ADC macro
 * |[4]     |SELAYNC   |Asynchronous Mode Selection AYNC Of Internal Comparator
 * |        |          |0 = internal comparator asynchronous mode SELAYNC is disabled.
 * |        |          |1 = internal comparator asynchronous mode SELAYNC is enabled..
 * |        |          |Note: this bit is only for analog RD debug ADC macro
 * |[5]     |SELASYF   |Asynchronous Mode Selection ASYF Of Internal Comparator
 * |        |          |0 = internal comparator asynchronous mode SELASYF is disabled.
 * |        |          |1 = internal comparator asynchronous mode SELASYF is enabled.
 * |        |          |Note: this bit is only for analog RD debug ADC macro
 * |[9:8]   |ASDELAYC  |Delay Trim Bits For Comparator Timing Control Bits 2'b00: delay time 0.2ns 2'b01: delay time 0.35ns 2'b10: delay time 2.4ns 2'b11: delay time 4.4ns
 * |        |          |Note: When ADC is enabled, these trim bits will load from ROMMAP
 * |        |          |Note: these bits is only for analog RD debug ADC macro
 * |[11:10] |ASDELAYF  |Delay Trim Bits For Comparator Timing Control Bits 2'b00: delay time 0.2ns  2'b01: delay time 0.35ns 2'b10: delay time 2.4ns 2'b11: delay time 4.4ns
 * |        |          |Note: When ADC is enabled, these trim bits will load from ROMMAP
 * |        |          |Note: these bits is only for analog RD debug ADC macro
 * |[23:16] |TRIMSMPL  |Trim Sample Cycle Selection Bits
 * |        |          |8'b00000000: trim sample 1 cycle
 * |        |          |8'b00000001: trim sample 2 cycles
 * |        |          |8'b00000010: trim sample 3 cycles
 * |        |          |....
 * |        |          |8'b11111111: trim sample 256 cycles
 * |        |          |Note: Trim sample cycle in normal mode default = 1 sample cycle, but cal mode TRIMSMPL = 4 cycle.
 * |        |          |Note: these bits is only for analog RD debug ADC macro
 * |[24]    |DEBUGCNTEN|Debug Count Enable Bit
 * |        |          |0 = Debug count is disabled.
 * |        |          |1 = Debug count is enabled.
 * |[29:25] |DEBUGCNT  |Debug Counts For ADC Conversion
 * |[31]    |DEBUGEN   |Debug Mode Enable Bit
 * |        |          |0 = debug mode disabled.
 * |        |          |1 = debuge mode enabled.
 * |        |          |Note: Enable DEBUGEN can perform calibration R/W operation
 * @var EADC_T::VREF
 * Offset: 0xFF8  EADC Reference Voltage Control Register
 * ---------------------------------------------------------------------------------------------------
 * |Bits    |Field     |Descriptions
 * | :----: | :----:   | :---- |
 * |[0]     |VREFSW    |Clamp MUX Of VREF Selection Bit
 * |        |          |0 = disabled (default).
 * |        |          |1 = enabled.
 * |        |          |Note: This register is useless in M460HD/LD because analog macro not support
 * |[1]     |REFSEL    |Positive Reference Voltage Source Selection
 * |        |          |0 = Positive Reference Voltage Source from AVDD.
 * |        |          |1 = Positive Reference Voltage Source from VREFP.
 * |        |          |Note: This register is useless in M460HD/LD because analog macro not support
 * @var EADC_T::VERSION
 ...
 */
    ...
    __I  uint32_t DAT19[8];             /*!< [0x0200-0x022c] EADC Data Register 19~26 for Sample Module 19~26           */                     
    __I  uint32_t RESERVE6[4];
    __IO uint32_t SCTL19[8];            /*!< [0x0230-0x025c] EADC Sample Module 19~26 Control Register                  */
    __I  uint32_t RESERVE7[4];
    __IO uint32_t M19CTL1[8];            /*!< [0x0260-0x028c] EADC Sample Module 19~30 Control Register 1               */
    __I  uint32_t RESERVE8[860];
    __IO uint32_t TEST;                  /*!< [0x0ff0] EADC Test Mode Control Register                                  */
    __IO uint32_t DEBUG;                 /*!< [0x0ff4] EADC Debug Mode Control Register                                 */
...
#define EADC_CALCTL_CAL_CCO_Pos          (20)                                              /*!< EADC_T::CALCTL: CAL_CCO Position       */
#define EADC_CALCTL_CAL_CCO_Msk          (0x1ul << EADC_CALCTL_CAL_CCO_Pos)                /*!< EADC_T::CALCTL: CAL_CCO Mask           */
...
#define EADC_TEST_MSBADD_Pos             (9)                                               /*!< EADC_T::TEST: MSBADD Position          */
#define EADC_TEST_MSBADD_Msk             (0x1ul << EADC_TEST_MSBADD_Pos)                   /*!< EADC_T::TEST: MSBADD Mask              */
...
#define EADC_DEBUG_DEBUGCNTEN_Msk        (0x1ul << EADC_DEBUG_DEBUGCNTEN_Pos)              /*!< EADC_T::DEBUG: DEBUGCNTEN Mask         */

#define EADC_DEBUG_DEBUGCNT_Pos          (25)                                              /*!< EADC_T::DEBUG: DEBUGCNT Position       */
#define EADC_DEBUG_DEBUGCNT_Msk          (0x1ful << EADC_DEBUG_DEBUGCNT_Pos)               /*!< EADC_T::DEBUG: DEBUGCNT Mask           */
...
#define EADC1_CMP_CMPDAT_Pos              (16)                                             /*!< EADC_T::CMP: CMPDAT Position          */
#define EADC1_CMP_CMPDAT_Msk              (0x3ffful << EADC1_CMP_CMPDAT_Pos)               /*!< EADC_T::CMP: CMPDAT Mask              */
...
//#define EADC_CURDAT_CURDAT_Pos           (0)                                               /*!< EADC_T::CURDAT: CURDAT Position        */
//#define EADC_CURDAT_CURDAT_Msk           (0x1fffful << EADC_CURDAT_CURDAT_Pos)             /*!< EADC_T::CURDAT: CURDAT Mask            */

//#define EADC_SWTRG_SWTRG_Pos             (0)                                               /*!< EADC_T::SWTRG: SWTRG Position          */
//#define EADC_SWTRG_SWTRG_Msk             (0x1fffful << EADC_SWTRG_SWTRG_Pos)               /*!< EADC_T::SWTRG: SWTRG Mask              */

//#define EADC_PENDSTS_STPF_Pos            (0)                                               /*!< EADC_T::PENDSTS: STPF Position         */
//#define EADC_PENDSTS_STPF_Msk            (0x1fffful << EADC_PENDSTS_STPF_Pos)              /*!< EADC_T::PENDSTS: STPF Mask             */

//#define EADC_OVSTS_SPOVF_Pos             (0)                                               /*!< EADC_T::OVSTS: SPOVF Position          */
//#define EADC_OVSTS_SPOVF_Msk             (0x1fffful << EADC_OVSTS_SPOVF_Pos)               /*!< EADC_T::OVSTS: SPOVF Mask              */

//#define EADC_STATUS3_CURSPL_Pos          (0)                                               /*!< EADC_T::STATUS3: CURSPL Position       */
//#define EADC_STATUS3_CURSPL_Msk          (0x1ful << EADC_STATUS3_CURSPL_Pos)               /*!< EADC_T::STATUS3: CURSPL Mask           */

//#define EADC_PDMACTL_PDMATEN_Pos         (0)                                               /*!< EADC_T::PDMACTL: PDMATEN Position      */
//#define EADC_PDMACTL_PDMATEN_Msk         (0x1fffful << EADC_PDMACTL_PDMATEN_Pos)           /*!< EADC_T::PDMACTL: PDMATEN Mask          */
