...
{
#ifdef PLDM_EMU // Todo: This should be removed for real chip
...
void FMC_NSCBA_Setup(void)
{
    ...
    printf("SCU->DFNSADDR: 0x%08X, 0x%08X\n", SCU->DFNSADDR, (uint32_t)&SCU->DFNSADDR);
    /* Check if NSCBA value with current active NSCBA */
...
static uint32_t s_u32VSrcIdx[32] =
{
    ...
    VSRC_NULL,  VSRC_NULL,  VSRC_CACHE
};

static uint32_t s_u32VAddrIdx[32] =
{
...
__WEAK void SCU_IRQHandler(void)
{
    ...
    if(u32Reg)
    {
        ...
        for(i = 0; i < (sizeof(s_astrSlaveName) / sizeof(s_astrSlaveName[0])); i++)
        {
            if(u32Reg & (1 << i))
            {
#if (CHECK_SCU_VIOLATION == 1)
                printf("[Info] Slave violation: %s access %s@0x%08X illegallly.\n", s_astrMasterName[M32(s_u32VSrcIdx[i])], s_astrSlaveName[i], M32(s_u32VAddrIdx[i]));
#endif
                SCU->SVINTSTS = (1 << i);
                //break;
            }
