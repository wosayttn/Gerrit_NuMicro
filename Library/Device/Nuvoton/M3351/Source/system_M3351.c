...
void FMC_NSCBA_Setup(void)
{
    ...
    uint32_t u32Config14, u32NSCBAConfig = ((DFMC_SECURE_END - DFMC_DATA_FLASH_BASE) / 0x1000UL) - 1UL;

    /* Check if NSCBA value with current active NSCBA */
    if ((SCU->DFNSADDR & 0xFUL) != u32NSCBAConfig)
    {
        /* Unlock Protected Register */
        SYS_UnlockReg();
        ...
        if ((u32Config14 & 0x6FUL) == ((DFMC_SECURE_EEPROM << 6UL) | ((DFMC_ENABLE_EEPROM == 0UL) ? 0x20UL : 0UL) | u32NSCBAConfig))
        {
            return ;
        }
        ...
        /* Set new base */
        FMC->ISPDAT = ~0x6FUL | ((DFMC_SECURE_EEPROM << 6UL) | ((DFMC_ENABLE_EEPROM == 0UL) ? 0x20UL : 0UL) | u32NSCBAConfig);
        FMC->ISPCMD = FMC_ISPCMD_PROGRAM;
        FMC->ISPTRG = FMC_ISPTRG_ISPGO_Msk;
