...
/* Static vector table */
const VECTOR_TABLE_Type __VECTOR_TABLE[FMC_VECMAP_SIZE / 4U] __VECTOR_TABLE_ATTRIBUTE =
{
...
__WEAK void Reset_Handler_PreInit(void)
{
    ...
    /* Set core clock */
    (void)CLK_SetCoreClock(FREQ_144MHZ);
...
void Reset_Handler(void)
{
    ...
    SCB->CPACR |= ((3UL << (10UL * 2UL)) |               /* set CP10 Full Access */
                   (3UL << (11UL * 2UL)));               /* set CP11 Full Access */
#endif
...
{
    uint32_t u32IRQ = 0;
    uint32_t *pu32StackFrame = (uint32_t *)NULL;
    ...
    */
    ...
    __ASM volatile("mrs %0, ipsr            \n" // Read IPSR (Exception number)
                   ...
                   "mrseq %1, msp           \n"
                   "mrsne %1, psp           \n"
                   : "=r"(u32IRQ), "=r"(pu32StackFrame));

    // Get the instruction caused the hardfault
    ProcessHardFault(pu32StackFrame);
}
...
__WEAK void MemManage_Handler(void)
{
    ...
    if (u32MMFSR & SCB_CFSR_DACCVIOL_Msk)
    {
        ...
        // Log or handle the error
        debug_printf("* Please set Default Processor mode for Thread execution to Privileged mode in RTX_Config.h !");
    }
