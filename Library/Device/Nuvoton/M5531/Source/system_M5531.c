...
static const char *sc_astrMasterName[] =
{
    ...
    "SDH1",  "EMAC",  "CRYPTO", "CRC",   "GDMA", NULL,     "LPPDMA", "CCAP",
    "SPIM0"
};
static const char *sc_astrMasterVioName[] =
{
    "GDMA",  "PDMA0", "PDMA1",  "USBH0", NULL,     "HSUSBH", "HSUSBD", "SDH0",
    "SDH1",  "EMAC",  "CRYPTO", "CRC",   "LPPDMA", "CCAP",   NULL,     NULL,
    "SPIM0"
};
 ...
 */
/** @static_deviation
 * <b>Rule:</b> cppcheck knownConditionTrueFalse<br>
 * <b>Justification:</b> `NonCacheable_start` and `NonCacheable_end` are linker-defined weak
 *                      symbols whose addresses evaluate to 0 if the non-cacheable region is
 *                      omitted by the linker script. Testing their presence triggers a false
 *                      positive constant condition warning in static analysis.
 */
__WEAK int32_t InitPreDefMPURegion(const ARM_MPU_Region_t *psMPURegion, uint32_t u32RegionCnt)
{
    ...
    if ((((uint32_t)&NonCacheable_start) != 0UL) && (((uint32_t)&NonCacheable_end) != 0UL))
    {
...
void FMC_NSCBA_Setup(void)
{
    ...
    /* Check if NSCBA value is consistent with current active NSCBA. */
 ...
 *
 * @return   MPC_SIE_ERR_NONE: Success
 *           Others: Failed
 */
static enum mpc_sie_error_t SetupMPC(
...
)
{
    ...
    {
        .base         = u32MemBaseAddr,
        .limit        = u32MemBaseAddr + u32MemByteSize - 1UL,
        .range_offset = 0,
    ...
    if (u32MemByteSize_NS > 0UL)
    {
        if ((eRetCode = mpc_sie_config_region(&mpc_dev, u32MemBaseAddr_NS, u32MemBaseAddr_NS + u32MemByteSize_NS - 1UL, MPC_SIE_SEC_ATTR_NONSECURE)) != MPC_SIE_ERR_NONE)
        {
            return eRetCode;
        }
    }
    ...
    if (u32MemByteSize_S > 0UL)
    {
        if ((eRetCode = mpc_sie_config_region(&mpc_dev, u32MemBaseAddr_S, u32MemBaseAddr_S + u32MemByteSize_S - 1UL, MPC_SIE_SEC_ATTR_SECURE)) != MPC_SIE_ERR_NONE)
        {
            return eRetCode;
        }
    }
...
#if defined (SETUP_SECURE_SPIFLASH_SIZE) && (SETUP_SECURE_SPIFLASH_SIZE == 1UL)
    {
