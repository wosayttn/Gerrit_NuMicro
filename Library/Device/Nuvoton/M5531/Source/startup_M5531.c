...
__NO_RETURN void Reset_Handler(void)
{
    ...
    if (SCU_IS_CPU_NS(SCU_NS) == 0UL)
    {
        ...
        for(;;)
        {
            if ((PMC->SYSRB1PC & PMC_SYSRB1PC_PCBUSY_Msk) == 0UL)
            {
                if ((PMC->SYSRB2PC & PMC_SYSRB2PC_PCBUSY_Msk) == 0UL)
                {
                    break;
                }
            }
    ...
    SCB->CPACR |= ((3UL << (10UL * 2UL)) |         /* Enable CP10 Full Access */
                   (3UL << (11UL * 2UL)));         /* Enable CP11 Full Access */
    ...
    /* PDEPU ON, Clock OFF */
    PWRMODCTL->CPDLPSTATE |= (0x1UL << PWRMODCTL_CPDLPSTATE_ELPSTATE_Pos);
#endif
