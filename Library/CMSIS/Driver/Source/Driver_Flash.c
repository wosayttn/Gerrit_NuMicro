...
{
    uint32_t u32ReadCnt, u32DataBytes;
    uint32_t *pu32DataBuf = (uint32_t *)data;
    uint32_t u32IsRegLocked = SYS_IsRegLocked();

    u32DataBytes = (1 << DriverCapabilities.data_width);

    if ((addr % u32DataBytes) || ((cnt * u32DataBytes) > RTE_FLASH_BYTE_SIZE) ||
            ((addr + (cnt * u32DataBytes)) > RTE_FLASH_BYTE_SIZE))
        return ARM_DRIVER_ERROR_PARAMETER;
    ...
    {
        *pu32DataBuf = FMC_Read(FMC_APROM_BASE + RTE_FLASH_START_OFFSET + addr + (u32ReadCnt * (1 << DriverCapabilities.data_width)));
...
static int32_t ARM_Flash_ProgramData(uint32_t addr, const void *data, uint32_t cnt)
{
    ...
    {
        FMC_Write8Bytes(FMC_APROM_BASE + RTE_FLASH_START_OFFSET + addr + (u32ProgCnt * FLASH_PROGRAM_UNIT), *pu32DataBuf, *(pu32DataBuf + 1));
        ...
        pu32DataBuf += (FLASH_PROGRAM_UNIT / FLASH_BYTES_PER_WORD);
    }
