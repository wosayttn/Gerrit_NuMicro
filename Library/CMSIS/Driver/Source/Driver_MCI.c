...
static int32_t MCIn_UnlockCard(MCI_RESOURCES *pMCIn)
{
    ...
    uint8_t *buf;
    uint32_t arg = 0;
    int32_t i32TimeoutCnt = SDH_TIMEOUT_CNT;
    ...
    while ((sdh->DMACTL & SDH_DMACTL_DMARST_Msk) && (--i32TimeoutCnt > 0));

    if (i32TimeoutCnt <= 0) return SDH_ERR_TIMEOUT;
    ...
    i32TimeoutCnt = SDH_TIMEOUT_CNT;

    while ((sdh->CTL & SDH_CTL_COEN_Msk) && (--i32TimeoutCnt > 0)) {}
