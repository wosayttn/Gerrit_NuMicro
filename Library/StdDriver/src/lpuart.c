...
void LPUART_ClearIntFlag(LPUART_T *lpuart, uint32_t u32InterruptFlag)
{
    ...
    if ((u32InterruptFlag & LPUART_INTSTS_RLSINT_Msk) != 0UL)  /* Clear Receive Line Status Interrupt */
    {
    ...
    if ((u32InterruptFlag & LPUART_INTSTS_MODEMINT_Msk) != 0UL)  /* Clear Modem Status Interrupt */
    {
        lpuart->MODEMSTS |= LPUART_MODEMSTS_CTSDETF_Msk;
    }

    if ((u32InterruptFlag & LPUART_INTSTS_BUFERRINT_Msk) != 0UL)  /* Clear Buffer Error Interrupt */
    {
        lpuart->FIFOSTS = LPUART_FIFOSTS_RXOVIF_Msk | LPUART_FIFOSTS_TXOVIF_Msk;
    }

    if ((u32InterruptFlag & LPUART_INTSTS_WKINT_Msk) != 0UL)  /* Clear Wake-up Interrupt */
    {
...
uint32_t LPUART_Read(const LPUART_T *lpuart, uint8_t pu8RxBuf[], uint32_t u32ReadBytes)
{
    ...
    for (u32Count = 0UL; u32Count < u32ReadBytes; u32Count++)
    {
        ...
        while ((lpuart->FIFOSTS & LPUART_FIFOSTS_RXEMPTY_Msk) != 0UL)  /* Check RX empty => failed */
        {
            u32delayno++;
...
uint32_t LPUART_Write(LPUART_T *lpuart, const uint8_t pu8TxBuf[], uint32_t u32WriteBytes)
{
    ...
    for (u32Count = 0UL; u32Count != u32WriteBytes; u32Count++)
    {
        ...
        while ((lpuart->FIFOSTS & LPUART_FIFOSTS_TXFULL_Msk) != 0UL)  /* Check Tx Full */
        {
            u32delayno++;
