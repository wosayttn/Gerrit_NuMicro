...
void PWM_EnableFaultBrake(PWM_T *pwm, uint32_t u32ChannelMask, uint32_t u32LevelMask, uint32_t u32BrakeSource)
{
    ...
    for (i = 0UL; i < PWM_CHANNEL_NUM; i ++)
    {
        if ((u32ChannelMask & (1UL << i)) != 0UL)
        {
            if ((u32BrakeSource & (PWM_BRKCTL0_1_SYSEEN_Msk | PWM_BRKCTL0_1_SYSLEN_Msk)) != 0UL)
            {
                *((__IO uint32_t *)((uint32_t) & ((pwm)->BRKCTL0_1) + (uint32_t)(i >> 1UL))) |= (u32BrakeSource & (PWM_BRKCTL0_1_SYSEEN_Msk | PWM_BRKCTL0_1_SYSLEN_Msk));
                (pwm)->FAILBRK |= (u32BrakeSource & 0xBUL);
            }
            else
            {
                *((__IO uint32_t *)((uint32_t) & ((pwm)->BRKCTL0_1) + (uint32_t)(i >> 1UL))) |= u32BrakeSource;
            }
        }
        ...
        if ((u32LevelMask & (1UL << i)) != 0UL)
        {
            if ((i % 2UL) == 0UL)
            {
                ...
                *((__IO uint32_t *)((uint32_t) & ((pwm)->BRKCTL0_1) + (uint32_t)(i >> 1UL))) &= ~PWM_BRKCTL0_1_BRKAEVEN_Msk;
                *((__IO uint32_t *)((uint32_t) & ((pwm)->BRKCTL0_1) + (uint32_t)(i >> 1UL))) |= (3UL << PWM_BRKCTL0_1_BRKAEVEN_Pos);
            }
            else
            {
                //set brake action as high level for odd channel
                *((__IO uint32_t *)((uint32_t) & ((pwm)->BRKCTL0_1) + (uint32_t)(i >> 1UL))) &= ~PWM_BRKCTL0_1_BRKAODD_Msk;
                *((__IO uint32_t *)((uint32_t) & ((pwm)->BRKCTL0_1) + (uint32_t)(i >> 1UL))) |= (3UL << PWM_BRKCTL0_1_BRKAODD_Pos);
            }
        }
        else
        {
            if ((i % 2UL) == 0UL)
            {
                ...
                *((__IO uint32_t *)((uint32_t) & ((pwm)->BRKCTL0_1) + (uint32_t)(i >> 1UL))) &= ~PWM_BRKCTL0_1_BRKAEVEN_Msk;
                *((__IO uint32_t *)((uint32_t) & ((pwm)->BRKCTL0_1) + (uint32_t)(i >> 1UL))) |= (2UL << PWM_BRKCTL0_1_BRKAEVEN_Pos);
            }
            else
            {
                //set brake action as low level for odd channel
                *((__IO uint32_t *)((uint32_t) & ((pwm)->BRKCTL0_1) + (uint32_t)(i >> 1UL))) &= ~PWM_BRKCTL0_1_BRKAODD_Msk;
                *((__IO uint32_t *)((uint32_t) & ((pwm)->BRKCTL0_1) + (uint32_t)(i >> 1UL))) |= (2UL << PWM_BRKCTL0_1_BRKAODD_Pos);
            }
        }
    }
...
void PWM_EnableDeadZone(PWM_T *pwm, uint32_t u32ChannelNum, uint32_t u32Duration)
{
    ...
    *((__IO uint32_t *)((uint32_t) & ((pwm)->DTCTL0_1) + (uint32_t)(u32ChannelNum >> 1UL))) &= ~PWM_DTCTL0_1_DTCNT_Msk;
    *((__IO uint32_t *)((uint32_t) & ((pwm)->DTCTL0_1) + (uint32_t)(u32ChannelNum >> 1UL))) |= (PWM_DTCTL0_1_DTEN_Msk | (u32Duration & PWM_DTCTL0_1_DTCNT_Msk));
}
...
void PWM_DisableDeadZone(PWM_T *pwm, uint32_t u32ChannelNum)
{
    ...
    *((__IO uint32_t *)((uint32_t) & ((pwm)->DTCTL0_1) + (uint32_t)(u32ChannelNum >> 1UL))) &= ~PWM_DTCTL0_1_DTEN_Msk;
}
...
{
    uint32_t u32CapFFlag = 0UL;
    uint32_t u32CapRFlag = 0UL;
...
{
    uint32_t u32Shift = (u32ChannelNum >> 1UL) * (uint32_t)PWM_CLKSRC_ECLKSRC2_Pos;

    (pwm)->CLKSRC = ((pwm)->CLKSRC & ~(PWM_CLKSRC_ECLKSRC0_Msk << u32Shift)) |
                    (u32ClkSrcSel << u32Shift);
}
...
{
    uint32_t u32Shift = u32BrakePinNum * (uint32_t)PWM_BNF_BRK1FEN_Pos;

    (pwm)->BNF = ((pwm)->BNF & ~((PWM_BNF_BRK0FCNT_Msk | PWM_BNF_BRK0FCS_Msk) << u32Shift)) |
                 (((u32ClkCnt << PWM_BNF_BRK0FCNT_Pos) | (u32ClkDivSel << PWM_BNF_BRK0FCS_Pos) | PWM_BNF_BRK0FEN_Msk) << u32Shift);
}
...
void PWM_DisableBrakeNoiseFilter(PWM_T *pwm, uint32_t u32BrakePinNum)
{
    ...
    (pwm)->BNF &= ~(PWM_BNF_BRK0FEN_Msk << u32Shift);
}
...
void PWM_EnableBrakePinInverse(PWM_T *pwm, uint32_t u32BrakePinNum)
{
    ...
    (pwm)->BNF |= (PWM_BNF_BRK0PINV_Msk << u32Shift);
}
...
{
    uint32_t u32Shift = u32BrakePinNum << 3UL;
