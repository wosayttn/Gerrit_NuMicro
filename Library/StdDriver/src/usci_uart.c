...
void UUART_ClearIntFlag(UUART_T *psUUART, uint32_t u32Mask)
{
    ...
    if ((u32Mask & UUART_ABR_INT_MASK) != 0U) /* Clear Auto-baud Rate Interrupt */
    {
        psUUART->PROTSTS = UUART_PROTSTS_ABRDETIF_Msk;
    }

    if ((u32Mask & UUART_RLS_INT_MASK) != 0U)  /* Clear Receive Line Status Interrupt */
    {
        psUUART->PROTSTS = (UUART_PROTSTS_BREAK_Msk | UUART_PROTSTS_FRMERR_Msk | UUART_PROTSTS_PARITYERR_Msk);
    }

    if ((u32Mask & UUART_BUF_RXOV_INT_MASK) != 0U)  /* Clear Receive Buffer Over-run Error Interrupt */
    {
        psUUART->BUFSTS = UUART_BUFSTS_RXOVIF_Msk;
    }

    if ((u32Mask & UUART_TXST_INT_MASK) != 0U)  /* Clear Transmit Start Interrupt */
    {
        psUUART->PROTSTS = UUART_PROTSTS_TXSTIF_Msk;
    }

    if ((u32Mask & UUART_TXEND_INT_MASK) != 0U)  /* Clear Transmit End Interrupt */
    {
        psUUART->PROTSTS = UUART_PROTSTS_TXENDIF_Msk;
    }

    if ((u32Mask & UUART_RXST_INT_MASK) != 0U)  /* Clear Receive Start Interrupt */
    {
        psUUART->PROTSTS = UUART_PROTSTS_RXSTIF_Msk;
    }

    if ((u32Mask & UUART_RXEND_INT_MASK) != 0U)  /* Clear Receive End Interrupt */
    {
...
uint32_t UUART_GetIntFlag(const UUART_T *psUUART, uint32_t u32Mask)
{
    ...
    if ((u32Mask & UUART_ABR_INT_MASK) != 0U)
    {
        if ((psUUART->PROTSTS & UUART_PROTSTS_ABRDETIF_Msk) != 0U)
        {
    ...
    if ((u32Mask & UUART_RLS_INT_MASK) != 0U)
    {
        if ((psUUART->PROTSTS & (UUART_PROTSTS_BREAK_Msk | UUART_PROTSTS_FRMERR_Msk | UUART_PROTSTS_PARITYERR_Msk)) != 0U)
        {
    ...
    if ((u32Mask & UUART_BUF_RXOV_INT_MASK) != 0U)
    {
        if ((psUUART->BUFSTS & UUART_BUFSTS_RXOVIF_Msk) != 0U)
        {
    ...
    if ((u32Mask & UUART_TXST_INT_MASK) != 0U)
    {
        if ((psUUART->PROTSTS & UUART_PROTSTS_TXSTIF_Msk) != 0U)
        {
    ...
    if ((u32Mask & UUART_TXEND_INT_MASK) != 0U)
    {
        if ((psUUART->PROTSTS & UUART_PROTSTS_TXENDIF_Msk) != 0U)
        {
    ...
    if ((u32Mask & UUART_RXST_INT_MASK) != 0U)
    {
        if ((psUUART->PROTSTS & UUART_PROTSTS_RXSTIF_Msk) != 0U)
        {
    ...
    if ((u32Mask & UUART_RXEND_INT_MASK) != 0U)
    {
        if ((psUUART->PROTSTS & UUART_PROTSTS_RXENDIF_Msk) != 0U)
        {
...
uint32_t UUART_Open(UUART_T *psUUART, uint32_t u32Baudrate)
{
    ...
    else
    {
        ...
        for (uint32_t u32DSCnt = 6UL; u32DSCnt <= 0x10UL; u32DSCnt++)  /* DSCNT could be 0x5~0xF */
        {
            ...
            {
                u32ClkDiv = 0x400UL;
                u32Tmp = (uint32_t)abs((int)u32Tmp4 - (int)u32Baudrate);
                u32Tmp2 = u32Tmp;
            }
            else    /* Avoid decimal of PDSCNT being discarded */
            {
                u32Tmp = (uint32_t)abs((int)u32Tmp4 - (int)u32Baudrate);
                u32Tmp2 = (uint32_t)abs((int)u32Tmp5 - (int)u32Baudrate);
            }
...
uint32_t UUART_Read(const UUART_T *psUUART, uint8_t *pu8RxBuf, uint32_t u32ReadBytes)
{
    ...
    for (u32Count = 0UL; u32Count < u32ReadBytes; u32Count++)
    {
        ...
        while ((psUUART->BUFSTS & UUART_BUFSTS_RXEMPTY_Msk) != 0U)   /* Check RX empty => failed */
        {
            u32Delayno++;
