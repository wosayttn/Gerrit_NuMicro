...
void UART_ClearIntFlag(UART_T *psUART, uint32_t u32InterruptFlag)
{
    ...
    if ((u32InterruptFlag & UART_INTSTS_RLSINT_Msk) != 0U)      /* Clear Receive Line Status Interrupt */
    {
        psUART->FIFOSTS = UART_FIFOSTS_BIF_Msk | UART_FIFOSTS_FEF_Msk |
                          UART_FIFOSTS_PEF_Msk | UART_FIFOSTS_ADDRDETF_Msk;
    }

    if ((u32InterruptFlag & UART_INTSTS_MODEMINT_Msk) != 0U)    /* Clear Modem Status Interrupt */
    {
        psUART->MODEMSTS |= UART_MODEMSTS_CTSDETF_Msk;
    }

    if ((u32InterruptFlag & UART_INTSTS_BUFERRINT_Msk) != 0U)   /* Clear Buffer Error Interrupt */
    {
        psUART->FIFOSTS = UART_FIFOSTS_RXOVIF_Msk | UART_FIFOSTS_TXOVIF_Msk;
    }

    if ((u32InterruptFlag & UART_INTSTS_WKINT_Msk) != 0U)       /* Clear Wake-up Interrupt */
    {
    ...
    if ((u32InterruptFlag & UART_INTSTS_LININT_Msk) != 0U)      /* Clear LIN Bus Interrupt */
    {
...
void UART_Open(UART_T *psUART, uint32_t u32Baudrate)
{
    ...
    uint8_t u8UartClkDivNum;
    uint32_t au32ClkTbl[4UL] = {__HXT, 0UL, __LXT, __HIRC_DIV2};

    /* Get UART clock source selection */
    u8UartClkSrcSel = (uint8_t)((CLK->CLKSEL1 & CLK_CLKSEL1_UARTSEL_Msk) >> CLK_CLKSEL1_UARTSEL_Pos);

    /* Get UART clock divider number */
    u8UartClkDivNum = (uint8_t)((CLK->CLKDIV0 & CLK_CLKDIV0_UARTDIV_Msk) >> CLK_CLKDIV0_UARTDIV_Pos);
    ...
    if (u32Baudrate != 0UL)
    {
        uint32_t u32BaudDiv = UART_BAUD_MODE2_DIVIDER((au32ClkTbl[u8UartClkSrcSel]) / (u8UartClkDivNum + 1UL), u32Baudrate);

        if (u32BaudDiv > 0xFFFFUL)
        {
            psUART->BAUD = (UART_BAUD_MODE0 | UART_BAUD_MODE0_DIVIDER((au32ClkTbl[u8UartClkSrcSel]) / (u8UartClkDivNum + 1UL), u32Baudrate));
        }
        else
        {
 ...
 */
uint32_t UART_Read(const UART_T *psUART, uint8_t *pu8RxBuf, uint32_t u32ReadBytes)
{
    uint32_t u32Count;
    ...
    for (u32Count = 0UL; u32Count < u32ReadBytes; u32Count++)
    {
        ...
        while ((psUART->FIFOSTS & UART_FIFOSTS_RXEMPTY_Msk) != 0U)   /* Check RX empty => failed */
        {
            u32Delayno++;
        ...
        pu8RxBuf[u32Count] = (uint8_t)psUART->DAT;    /* Get Data from UART RX  */
    }
...
void UART_SetLine_Config(UART_T *psUART, uint32_t u32Baudrate, uint32_t u32DataWidth, uint32_t u32Parity, uint32_t  u32StopBits)
{
    uint8_t u8UartClkSrcSel;
    uint8_t u8UartClkDivNum;
    uint32_t au32ClkTbl[4] = {__HXT, 0UL, __LXT, __HIRC_DIV2};
    ...
    if (u32Baudrate != 0UL)
    {
        ...
        u32Baud_Div = UART_BAUD_MODE2_DIVIDER((au32ClkTbl[u8UartClkSrcSel]) / (u8UartClkDivNum + 1UL), u32Baudrate);
...
void UART_SelectIrDAMode(UART_T *psUART, uint32_t u32Buadrate, uint32_t u32Direction)
{
    ...
    if (u32Buadrate != 0UL)
    {
        uint32_t u32Baud_Div = UART_BAUD_MODE0_DIVIDER((au32ClkTbl[u8UartClkSrcSel]) / (u8UartClkDivNum + 1UL), u32Buadrate);
 ...
 */
uint32_t UART_Write(UART_T *psUART, const uint8_t *pu8TxBuf, uint32_t u32WriteBytes)
{
    uint32_t  u32Count;
    ...
    for (u32Count = 0UL; u32Count != u32WriteBytes; u32Count++)
    {
        ...
        while ((psUART->FIFOSTS & UART_FIFOSTS_TXFULL_Msk) != 0U)  /* Check Tx Full */
        {
            u32Delayno++;
