...
void UART_ClearIntFlag(UART_T *uart, uint32_t u32InterruptFlag)
{
    ...
    if ((u32InterruptFlag & UART_INTSTS_SWBEINT_Msk) != 0U)   /* Clear Bit Error Detection Interrupt */
    {
...
void UART_Open(UART_T *uart, uint32_t u32BaudRate)
{
    ...
    uint32_t au32ClkTbl[8UL] = {__HXT, 0UL, __LXT, __HIRC, 0UL, __LIRC, 0UL, 0UL};
...
void UART_SetLine_Config(UART_T *uart, uint32_t u32BaudRate, uint32_t u32DataWidth, uint32_t u32Parity, uint32_t  u32StopBits)
{
    uint32_t u32UartClkSrcSel = 0UL;
    uint32_t u32UartClkDivNum = 0UL;
    uint32_t au32ClkTbl[8UL] = {__HXT, 0UL, __LXT, __HIRC, 0, __LIRC, 0UL, 0UL};
...
uint32_t UART_Write(UART_T *uart, const uint8_t pu8TxBuf[], uint32_t u32WriteBytes)
{
    ...
    for (u32Count = 0UL; u32Count != u32WriteBytes; u32Count++)
    {
        ...
        while ((uart->FIFOSTS & UART_FIFOSTS_TXFULL_Msk) != 0U)   /* Check Tx Full */
        {
            u32DelayNum++;
