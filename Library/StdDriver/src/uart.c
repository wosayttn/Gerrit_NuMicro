...
/**
 * @brief   Get UART module clock source and clock divider by UART base address.
 * @param[in]  uart              The pointer of the specified UART module.
 * @param[out] pu32ClkSrcSel     Pointer to store the clock source selection.
 * @param[out] pu32ClkDivNum     Pointer to store the clock divider number.
 * @retval  0   Success.
 * @retval -1   Invalid UART base address.
 */
static int32_t UART_GetClkSrcAndDiv(const UART_T *uart, uint32_t *pu32ClkSrcSel, uint32_t *pu32ClkDivNum)
{
    int32_t i32Ret = 0;
    ...
    {
        case UART0_BASE:
            u32Module = UART0_MODULE;
            break;

        case UART1_BASE:
            u32Module = UART1_MODULE;
            break;

        case UART2_BASE:
            u32Module = UART2_MODULE;
            break;

        case UART3_BASE:
            u32Module = UART3_MODULE;
            break;

        case UART4_BASE:
            u32Module = UART4_MODULE;
            break;

        case UART5_BASE:
            u32Module = UART5_MODULE;
            break;

        case UART6_BASE:
            u32Module = UART6_MODULE;
            break;

        case UART7_BASE:
            u32Module = UART7_MODULE;
            break;

        case UART8_BASE:
            u32Module = UART8_MODULE;
            break;

        case UART9_BASE:
            u32Module = UART9_MODULE;
            break;
    ...
    if (i32Ret == 0)
    {
        *pu32ClkSrcSel = CLK_GetModuleClockSource(u32Module);
        *pu32ClkDivNum = CLK_GetModuleClockDivider(u32Module);
    }
...
{
    if ((u32InterruptFlag & UART_INTSTS_SWBEINT_Msk) != 0U) /* Clear Single-wire Bit Error Detect Interrupt */
    {
...
void UART_Open(UART_T *uart, uint32_t u32baudrate)
{
    ...
    if (UART_GetClkSrcAndDiv(uart, &u32UartClkSrcSel, &u32UartClkDivNum) != 0)
    {
        return;
    }
...
uint32_t UART_Read(const UART_T *uart, uint8_t pu8RxBuf[], uint32_t u32ReadBytes)
{
    ...
    for (u32Count = 0UL; u32Count < u32ReadBytes; u32Count++)
    {
        ...
        while ((uart->FIFOSTS & UART_FIFOSTS_RXEMPTY_Msk) != 0U)  /* Check RX empty => failed */
        {
            u32delayno++;
...
void UART_SelectIrDAMode(UART_T *uart, uint32_t u32Buadrate, uint32_t u32Direction)
{
    ...
    while ((uart->FIFOSTS & UART_FIFOSTS_TXRXACT_Msk) != 0U) {};
...
uint32_t UART_Write(UART_T *uart, const uint8_t pu8TxBuf[], uint32_t u32WriteBytes)
{
    ...
    for (u32Count = 0UL; u32Count != u32WriteBytes; u32Count++)
    {
        ...
        while ((uart->FIFOSTS & UART_FIFOSTS_TXFULL_Msk) != 0U)  /* Wait Tx not full or Time-out manner */
        {
            u32delayno++;
