...
uint32_t USPI_SetBusClock(USPI_T *uspi, uint32_t u32BusClock)
{
    ...
    else if (u32BusClock >= u32MaxClk)
    {
        u32ClkDiv = 0UL;
        u32UspiClk = u32MaxClk;
    }
    else
    {
        uint32_t u32ClkDivMax = (USPI_BRGEN_CLKDIV_Msk >> USPI_BRGEN_CLKDIV_Pos);

        u32ClkDiv = (((((uint32_t)u32MaxClk * 10UL) / (uint32_t)u32BusClock) + 5UL) / 10UL) - 1UL;

        if (u32ClkDiv > u32ClkDivMax)
        {
            u32ClkDiv = u32ClkDivMax;
        }
    ...
    /* Set USCI_SPI bus clock */
    uspi->BRGEN = (uspi->BRGEN & ~USPI_BRGEN_CLKDIV_Msk) | (u32ClkDiv << USPI_BRGEN_CLKDIV_Pos);
