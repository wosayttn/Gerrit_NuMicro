...
uint32_t UI2C_Open(UI2C_T *psUI2C, uint32_t u32BusClock)
{
    ...
    psUI2C->LINECTL |= ((uint32_t)8U << UI2C_LINECTL_DWIDTH_Pos);
 ...
 */
uint32_t UI2C_GetBusClockFreq(const UI2C_T *psUI2C)
{
    uint32_t u32Divider;
    uint32_t u32Pclk;
 ...
 */
uint32_t UI2C_GetData(const UI2C_T *psUI2C)
{
    return (psUI2C->RXDAT);
}
...
uint8_t UI2C_WriteByte(UI2C_T *psUI2C, uint8_t u8SlaveAddr, const uint8_t u8Data)
{
    ...
    while (u8Xfering != 0U)
    {
        while ((UI2C_GET_PROT_STATUS(psUI2C) & 0x3F00U) == 0UL)
        {
        }
        ...
        switch (UI2C_GET_PROT_STATUS(psUI2C) & 0x3F00U)
        {
            case UI2C_PROTSTS_STARIF_Msk:
                ...
                UI2C_SET_DATA(psUI2C, ((uint32_t)u8SlaveAddr << 1U));                   /* Write SLA+W to Register UI2C_TXDAT */
            ...
            case UI2C_PROTSTS_ACKIF_Msk:
                ...
                {
                    UI2C_SET_DATA(psUI2C, u8Data);                           /* Write data to Register TXDAT */
...
uint8_t UI2C_WriteByteTwoRegs(UI2C_T *psUI2C, uint8_t u8SlaveAddr, uint16_t u16DataAddr, const uint8_t u8Data)
{
    ...
    uint8_t u8Err = 0U;
    uint32_t u8Ctrl = 0UL;
    const uint8_t u8DataAddrHi = (uint8_t)((u16DataAddr & 0xFF00U) >> 8U);
    uint32_t u32TXLen = 0U;
    ...
    while (u8Xfering != 0U)
    {
        ...
        switch (UI2C_GET_PROT_STATUS(psUI2C) & 0x3F00U)
        {
            ...
            case UI2C_PROTSTS_ACKIF_Msk:
                ...
                if (u32TXLen == 0U)
                {
                    UI2C_SET_DATA(psUI2C, u8DataAddrHi);                                /* Write Hi byte data address to UI2C_TXDAT */
                    u32TXLen++;
                }
                else if (u32TXLen == 1U)
                {
...
uint8_t UI2C_ReadByte(UI2C_T *psUI2C, uint8_t u8SlaveAddr)
{
    ...
    while (u8Xfering != 0U)
    {
        ...
        switch (UI2C_GET_PROT_STATUS(psUI2C) & 0x3F00U)
        {
            case UI2C_PROTSTS_STARIF_Msk:
                UI2C_CLR_PROT_INT_FLAG(psUI2C, UI2C_PROTSTS_STARIF_Msk);     /* Clear START INT Flag */
                UI2C_SET_DATA(psUI2C, (((uint32_t)u8SlaveAddr << 1U) | 0x01U));             /* Write SLA+R to Register UI2C_TXDAT */
    ...
    while (u8Xfering != 0U)
    {
        ...
        switch (UI2C_GET_PROT_STATUS(psUI2C) & 0x3F00U)
        {
            case UI2C_PROTSTS_STARIF_Msk:
                ...
                {
                    UI2C_SET_DATA(psUI2C, (((uint32_t)u8SlaveAddr << 1U) | 0x01U));        /* Write SLA+R to Register TXDAT */
                    eEvent = MASTER_SEND_H_RD_ADDRESS;
                }
                else
                {
    ...
    while (u8Xfering != 0U)
    {
        ...
        switch (UI2C_GET_PROT_STATUS(psUI2C) & 0x3F00U)
        {
            ...
            case UI2C_PROTSTS_ACKIF_Msk:
                ...
                if (eEvent == MASTER_SEND_ADDRESS)
                {
                    UI2C_SET_DATA(psUI2C, u8DataAddrHi);                                /* Write Hi byte address of register */
                    eEvent = MASTER_SEND_DATA;
                }
                else if (eEvent == MASTER_SEND_DATA)
                {
                ...
                {
                    u8Ctrl = UI2C_CTL_PTRG;                                  /* SLA+R ACK */
                    eEvent = MASTER_READ_DATA;
                }
