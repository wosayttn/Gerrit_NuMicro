...
uint32_t UI2C_GetIntFlag(const UI2C_T *ui2c, uint32_t u32Mask)
{
    ...
    if (((u32Mask & UI2C_TO_INT_MASK) != 0UL) && ((ui2c->PROTSTS & UI2C_PROTSTS_TOIF_Msk) != 0UL))
    {
        u32IntFlag |= UI2C_TO_INT_MASK;
    }

    /* Check Start Condition Received Interrupt Flag */
    if (((u32Mask & UI2C_STAR_INT_MASK) != 0UL) && ((ui2c->PROTSTS & UI2C_PROTSTS_STARIF_Msk) != 0UL))
    {
        u32IntFlag |= UI2C_STAR_INT_MASK;
    }

    /* Check Stop Condition Received Interrupt Flag */
    if (((u32Mask & UI2C_STOR_INT_MASK) != 0UL) && ((ui2c->PROTSTS & UI2C_PROTSTS_STORIF_Msk) != 0UL))
    {
        u32IntFlag |= UI2C_STOR_INT_MASK;
    }

    /* Check Non-Acknowledge Interrupt Flag */
    if (((u32Mask & UI2C_NACK_INT_MASK) != 0UL) && ((ui2c->PROTSTS & UI2C_PROTSTS_NACKIF_Msk) != 0UL))
    {
        u32IntFlag |= UI2C_NACK_INT_MASK;
    }

    /* Check Arbitration Lost Interrupt Flag */
    if (((u32Mask & UI2C_ARBLO_INT_MASK) != 0UL) && ((ui2c->PROTSTS & UI2C_PROTSTS_ARBLOIF_Msk) != 0UL))
    {
        u32IntFlag |= UI2C_ARBLO_INT_MASK;
    }

    /* Check Error Interrupt Flag */
    if (((u32Mask & UI2C_ERR_INT_MASK) != 0UL) && ((ui2c->PROTSTS & UI2C_PROTSTS_ERRIF_Msk) != 0UL))
    {
        u32IntFlag |= UI2C_ERR_INT_MASK;
    }

    /* Check Acknowledge Interrupt Flag */
    if (((u32Mask & UI2C_ACK_INT_MASK) != 0UL) && ((ui2c->PROTSTS & UI2C_PROTSTS_ACKIF_Msk) != 0UL))
    {
...
uint8_t UI2C_WriteByte(UI2C_T *ui2c, uint8_t u8SlaveAddr, const uint8_t u8Data)
{
    ...
    {
        while ((UI2C_GET_PROT_STATUS(ui2c) & 0x3F00U) == 0UL) {};                     /* Wait UI2C new status occur */
    ...
    while ((u8Xfering != 0U) && (u8Err == 0U))
    {
        ...
        switch (UI2C_GET_PROT_STATUS(ui2c) & 0x3F00U)
        {
            ...
            case UI2C_PROTSTS_ACKIF_Msk:
                ...
                {
                    if (u32txLen < (u32wLen + 1UL))                             /* TX length = u32wLen + 1(u8DataAddr)*/
                    {
                        UI2C_SET_DATA(ui2c, pu8Data[u32txLen]);    /* Write data to UI2C_TXDAT */
                        u32txLen++;
                    }
    ...
    while ((u8Xfering != 0U) && (u8Err == 0U))
    {
        ...
        switch (UI2C_GET_PROT_STATUS(ui2c) & 0x3F00U)
        {
            ...
            case UI2C_PROTSTS_ACKIF_Msk:
                ...
                if (u32txLen == 0UL)
                {
                    UI2C_SET_DATA(ui2c, ((u16DataAddr & 0xFF00U) >> 8U));  /* Write Hi byte data address to UI2C_TXDAT */
                    u32txLen++;
                }
                else if (u32txLen == 1UL)
                {
                    UI2C_SET_DATA(ui2c, (u16DataAddr & 0xFFU));         /* Write Lo byte data address to UI2C_TXDAT */
                    u32txLen++;
                }
                else if (u32txLen == 2UL)
                {
                    UI2C_SET_DATA(ui2c, u8Data);                                  /* Write data to UI2C_TXDAT */
                    u32txLen++;
                }
    ...
    while ((u8Xfering != 0U) && (u8Err == 0U))
    {
        ...
        switch (UI2C_GET_PROT_STATUS(ui2c) & 0x3F00U)
        {
            ...
            case UI2C_PROTSTS_NACKIF_Msk:
                ...
                {
                    u32Ctrl = (UI2C_CTL_PTRG | UI2C_CTL_STO);
                    u8Err = 1;
                }
                else
                {
...
/**
  ...
  * @param[in]  *ui2c           Point to UI2C peripheral
  * @param[in]  u8SlaveAddr     Access Slave address(7-bit)
  * @param[out] *pu8Data       Point to array to store data from Slave
  ...
  *
  */
uint32_t UI2C_ReadMultiBytes(UI2C_T *ui2c, uint8_t u8SlaveAddr, uint8_t *pu8Data, uint32_t u32rLen)
{
    uint8_t u8Xfering = 1U;
    uint8_t u8Err = 0U;
    uint32_t u32Ctrl = UI2C_CTL_PTRG;
    uint32_t u32rxLen = 0UL;
    enum UI2C_MASTER_EVENT eEvent = MASTER_SEND_START;
    UI2C_START(ui2c);                                                       /* Send START */
    ...
    while ((u8Xfering != 0U) && (u8Err == 0U))
    {
        ...
        switch (UI2C_GET_PROT_STATUS(ui2c) & 0x3F00U)
        {
            ...
            case UI2C_PROTSTS_ACKIF_Msk:
                ...
                {
                    pu8Data[u32rxLen] = (unsigned char) UI2C_GET_DATA(ui2c);    /* Receive Data */
                    u32rxLen++;
...
/**
  ...
  *
  */
uint32_t UI2C_ReadMultiBytesOneReg(UI2C_T *ui2c, uint8_t u8SlaveAddr, uint8_t u8DataAddr, uint8_t *pu8Data, uint32_t u32rLen)
{
    uint8_t u8Xfering = 1U;
    uint8_t u8Err = 0U;
    uint32_t u32Ctrl = UI2C_CTL_PTRG;
    ...
    while ((u8Xfering != 0U) && (u8Err == 0U))
    {
        ...
        switch (UI2C_GET_PROT_STATUS(ui2c) & 0x3F00U)
        {
            ...
            case UI2C_PROTSTS_ACKIF_Msk:
                ...
                {
                    pu8Data[u32rxLen] = (uint8_t) UI2C_GET_DATA(ui2c);      /* Receive Data */
                    u32rxLen++;
    ...
    while ((u8Xfering != 0U) && (u8Err == 0U))
    {
        ...
        switch (UI2C_GET_PROT_STATUS(ui2c) & 0x3F00U)
        {
            ...
            case UI2C_PROTSTS_ACKIF_Msk:
                ...
                if (eEvent == MASTER_SEND_ADDRESS)
                {
                    UI2C_SET_DATA(ui2c, ((u16DataAddr & 0xFF00U) >> 8U));  /* Write Hi byte address of register */
                    eEvent = MASTER_SEND_H_WR_ADDRESS;
                }
                else if (eEvent == MASTER_SEND_H_WR_ADDRESS)
                {
                    UI2C_SET_DATA(ui2c, (u16DataAddr & 0xFFU));       /* Write Lo byte address of register */
                    eEvent = MASTER_SEND_L_ADDRESS;
                }
...
/**
  ...
  *
  */
uint32_t UI2C_ReadMultiBytesTwoRegs(UI2C_T *ui2c, uint8_t u8SlaveAddr, uint16_t u16DataAddr, uint8_t *pu8Data, uint32_t u32rLen)
{
    uint8_t u8Xfering = 1U;
    uint8_t u8Err = 0U;
    uint32_t u32Ctrl = UI2C_CTL_PTRG;
