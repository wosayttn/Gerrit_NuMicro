...
/**
 * @brief      Get the frame bit rate factor according to the word width.
 *
...
/**
 * @brief      Check if the I2S module is valid.
 *
 * @param[in]  i2s is the base address of I2S module.
 *
 * @return     1 if the I2S module is valid; otherwise return 0.
 */
static uint32_t i2s_IsValidModule(const I2S_T *i2s)
{
...
uint32_t I2S_Open(I2S_T *i2s, uint32_t u32MasterSlave, uint32_t u32SampleRate, uint32_t u32WordWidth, uint32_t u32MonoData, uint32_t u32DataFormat)
{
    ...
    if ((i2s_IsValidModule(i2s) == 0UL) || (u32SampleRate == 0UL))
    {
        return 0UL;
    }
