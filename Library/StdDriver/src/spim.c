...
#define SPIM_GET_MLDOTCTL_MLDOPL0VT()   \
    (((inpw(SYS_BASE + 0xF54UL)) & SPIM_MLDOTCTL_MLDOPL0VT_Msk) >> SPIM_MLDOTCTL_MLDOPL0VT_Pos)

#define SPIM_SET_MLDOTCTL_MLDOPL0VT(x)                                                  \
    do{                                                                                 \
        uint32_t u32Value = ((inpw(SYS_BASE + 0xF54UL)) & ~SPIM_MLDOTCTL_MLDOPL0VT_Msk) | \
                            ((x) << SPIM_MLDOTCTL_MLDOPL0VT_Pos);                       \
        outpw((SYS_BASE + 0xF54UL), u32Value);                                            \
    }while(0)

#define SPIM_GET_MLDOTCTL_WRBUSY()  \
    (((inpw(SYS_BASE + 0xF54UL)) & SPIM_MLDOTCTL_WRBUSY_Msk) >> SPIM_MLDOTCTL_WRBUSY_Pos)

#define SPIM_GET_DLLTCTL_DLL0OLDOTRIM() \
    (((inpw(SYS_BASE + 0xF80UL)) & SPIM_DLLTCTL_DLL0OLDOTRIM_Msk) >> SPIM_DLLTCTL_DLL0OLDOTRIM_Pos)

#define SPIM_SET_DLLTCTL_DLL0OLDOTRIM(x)                                                    \
    do {                                                                                    \
        uint32_t u32Value = ((inpw(SYS_BASE + 0xF80UL)) & ~SPIM_DLLTCTL_DLL0OLDOTRIM_Msk) |   \
                            ((x) << SPIM_DLLTCTL_DLL0OLDOTRIM_Pos);                         \
        outpw((SYS_BASE + 0xF80UL), u32Value);                                                \
    } while (0)
...
static void SPIM_AdjustPowerLevelForDQS(void)
{
    ...
    uint32_t u32MLDOPL0;
    volatile int32_t i32TimeOutCnt = (SystemCoreClock >> 1U); /* 500ms time-out */
