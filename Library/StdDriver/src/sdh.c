...
/**
 * @brief      Get the DMA address of the buffer
 *
 * @param[in]  pu8Buf The pointer to the buffer
 *
 * @return     The DMA address of the buffer.
 */
static uint32_t SDH_GetDMAAddress(const uint8_t *pu8Buf)
{
    uintptr_t uAddr = (uintptr_t)pu8Buf;

    return (uint32_t)uAddr;
}

/**
 * @brief      Check if the SDH instance is valid
 *
 * @param[in]  sdh The pointer to the SDH instance
 *
 * @return     1 if the instance is valid, 0 otherwise.
 */
...
/**
 * @brief      Convert the SDH error code to int32_t
 *
 * @param[in]  u32ErrorCode The SDH error code
 *
 * @return     The converted int32_t error code.
 */
static int32_t SDH_ToInt32ErrorCode(uint32_t u32ErrorCode)
{
    return (int32_t)u32ErrorCode;
}

/**
 * @brief      Invalidate the SD card state
 *
 * @param[in]  pSD The pointer to the SDH_INFO_T structure
 *
 * @return     None.
 */
...
/**
 * @brief      Ensure the SD card is ready
 *
 * @param[in]  sdh The pointer to the SDH instance
 *
...
/**
 * @brief      Update the DataReadyFlag based on the SDH interrupt status
 *
 * @param[in]  sdh The pointer to the SDH instance
 * @param[in]  pSD The pointer to the SDH_INFO_T structure
 *
...
#if (NVT_DCACHE_ON == 1)
/**
 * @brief      Invalidate the D-Cache buffer for DMA operations
 *
 * @param[in]  pu8Buf  The pointer to the buffer
 ...
 * @return     None.
 */
static void SDH_InvalidateDCacheBuffer(const uint8_t *pu8Buf, uint32_t u32Size)
{
    uintptr_t u32StartAddr = (uintptr_t)pu8Buf;
    uintptr_t u32AlignedStart;
    uintptr_t u32AlignedEnd;
    uint8_t *pu8AlignedStart;
    ...
    pu8AlignedStart = (uint8_t *)u32AlignedStart;

    SCB_InvalidateDCache_by_Addr(pu8AlignedStart, u32CacheSize);
}

/**
 * @brief      Clean the D-Cache buffer for DMA operations
 *
 * @param[in]  pu8Buf  The pointer to the buffer
 * @param[in]  u32Size The size of the buffer
 *
 * @return     None.
 */
static void SDH_CleanDCacheBuffer(const uint8_t *pu8Buf, uint32_t u32Size)
{
    uintptr_t u32StartAddr = (uintptr_t)pu8Buf;
    uintptr_t u32AlignedStart;
    uintptr_t u32AlignedEnd;
    ...
    SCB_CleanDCache_by_Addr(pu8AlignedStart, i32CacheSize);
}
#endif

//------------------------------------------------------------------------------
/**
 * @brief      Get the SDH0 buffer address
 *
 * @param[in]  None
 *
 * @return     The pointer to the SDH0 buffer.
 */
...
/**
 * @brief      Get the SDH1 buffer address
 *
 * @param[in]  None
 *
 * @return     The pointer to the SDH1 buffer.
 */
...
/**
 * @brief      Get the SDH_INFO_T structure for the specified SDH instance
 *
 * @param[in]  sdh  The pointer to the SDH instance
 *
 * @return     The pointer to the SDH_INFO_T structure.
 */
...
int32_t SDH_CheckRB(SDH_T *sdh)
{
    ...
    while (u32Done == 0U)
    {
        ...
        while ((sdh->CTL & SDH_CTL_CLK8OEN_Msk) == SDH_CTL_CLK8OEN_Msk)
        {
            ...
            {
                pSD->i32ErrCode = SDH_ToInt32ErrorCode(SDH_ERR_TIMEOUT);
                break;
            }
        }
 ...
 *
 * @param[in]  sdh    The pointer to the SDH instance
 * @param[in]  u32Cmd The command index
 * @param[in]  u32Arg The command argument
 *
 * @return     The status of the operation.
 */
...
/**
 * @brief      Send a command to the SD card and get the response
 *
 * @param[in]  sdh           The pointer to the SDH instance
 * @param[in]  u32Cmd        The command index
 * @param[in]  u32Arg        The command argument
 * @param[in]  u32NtickCount The timeout count
 *
 * @return     The status of the operation.
 */
 ...
 *
 * @param[in]  u32Val The 32-bit value to swap
 *
...
/**
 * @brief       Send a command to the SD card and get a 16-byte response (CID or CSD)
 *
 * @param[in]   sdh       The pointer to the SDH instance
 * @param[in]   u32Cmd    The command index
 * @param[in]   u32Arg    The command argument
 * @param[out]  pu32R2ptr The pointer to the buffer to store the 16-byte response
 *
 * @return      The status of the operation.
 */
...
/**
 * @brief      Send a command to the SD card and get the response with data input
 *
 * @param[in]  sdh    The pointer to the SDH instance
 * @param[in]  u32Cmd The command index
 * @param[in]  u32Arg The command argument
...
/**
 * @brief      Set the SD card clock frequency
 *
 * @param[in]  sdh           The pointer to the SDH instance
 * @param[in]  u32SD_clk_khz The desired SD clock frequency in kHz
 *
 * @return     None.
 */
...
/**
 * @brief      Get the card detection status
 *
 * @param[in]  sdh The pointer to the SDH instance
 *
...
static uint32_t SDH_Init(SDH_T *sdh)
{
    volatile uint32_t u32i = 0U;
    uint32_t u32Resp;
    volatile uint32_t u32CmdTimeOut;
    SDH_INFO_T *pSD;
    int32_t i32TimeOutCount;
    ...
    {
        uint32_t u32Status;
        uint32_t au32CIDBuffer[4] = {0UL, 0UL, 0UL, 0UL};
...
/**
 * @brief      Switch the SD card to high-speed mode
 *
 * @param[in]  sdh The pointer to the SDH instance
 * @param[in]  pSD The pointer to the SD card information structure
 *
 * @return     The status of the operation.
 */
uint32_t SDH_SwitchToHighSpeed(SDH_T *sdh, SDH_INFO_T *pSD)
{
    volatile uint32_t u32Status = 0U;
    uint32_t u32CurrentConsumption = 0UL;
    uint32_t u32BusyStatus0 = 0UL;
    int32_t i32TimeOutCount = (int32_t)SDH_TIMEOUT_CNT;
    ...
    u32CurrentConsumption = ((uint32_t)pSD->dmabuf[0] << 8U) |
                            (uint32_t)pSD->dmabuf[1];

    if (u32CurrentConsumption == 0UL)
    {
        return Fail;
    }

    u32BusyStatus0 = ((uint32_t)pSD->dmabuf[28] << 8U) |
                     (uint32_t)pSD->dmabuf[29];

    if (u32BusyStatus0 == 0UL) /* function ready */
    {
        sdh->DMASA = SDH_GetDMAAddress(pSD->dmabuf);
...
uint32_t SDH_SelectCardType(SDH_T *sdh)
{
    ...
    u32Status = (uint32_t)SDH_CheckRB(sdh);
...
void SDH_Get_SD_info(SDH_T *sdh)
{
    ...
    uint32_t u32MULT;
    uint32_t u32Size;
    uint32_t au32Buffer[4] = {0UL, 0UL, 0UL, 0UL};
    //unsigned char *ptr;
    SDH_INFO_T *pSD;
    int32_t i32TimeOutCount = (int32_t)SDH_TIMEOUT_CNT;
...
/**
 * @brief      Close the SD card interface
 *
 * @param[in]  sdh The pointer to the SDH instance
 *
 * @return     None.
 ...
 *
 * @param[in]  sdh         The pointer to the SDH instance
 * @param[in]  pu8BufAddr  The buffer to store the read data
 * @param[in]  u32StartSec The starting sector to read from
 * @param[in]  u32SecCount The number of sectors to read
 *
 * @return     The status of the operation.
 */
static uint32_t _SDH_Read(SDH_T *sdh, const uint8_t *pu8BufAddr, uint32_t u32StartSec, uint32_t u32SecCount)
{
    uint32_t u32BIsSendCmd = FALSE;
    uint32_t u32Reg;
    uint32_t u32i = 0UL;
    uint32_t u32Loop = 0UL;
    ...
    if (SDH_SDCmdAndRsp(sdh, 12U, 0U, 0U) != Successful) /* stop command */
    {
        return SDH_CRC7_ERROR;
    }
...
#if (NVT_DCACHE_ON == 1)
        SDH_InvalidateDCacheBuffer(pu8BufAddr, u32SecCount * SDH_BLOCK_SIZE);
#endif
    }
 ...
 *
 * @param[in]  sdh         The pointer to the SDH instance
 * @param[in]  pu8BufAddr  The buffer containing the data to write
 * @param[in]  u32StartSec The starting sector to write to
 * @param[in]  u32SecCount The number of sectors to write
 *
 * @return     The status of the operation.
 */
...
uint32_t SDH_Write(SDH_T *sdh, const uint8_t *pu8BufAddr, uint32_t u32StartSec, uint32_t u32SecCount)
{
    ...
    {
#if (NVT_DCACHE_ON == 1)
        SDH_CleanDCacheBuffer(pu8BufAddr, u32SecCount * SDH_BLOCK_SIZE);
#endif
...
/**
 * @brief      Open the SD card disk
 *
 * @param[in]  sdh          The pointer to the SDH instance
 * @param[in]  u32CardDetSrc The card detection source
 *
 * @return     The status of the operation.
 */
