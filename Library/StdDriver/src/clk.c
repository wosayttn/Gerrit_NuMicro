...
uint32_t CLK_GetHXTFreq(void)
{
    ...
    if ((CLK->SRCCTL & CLK_SRCCTL_HXTEN_Msk) != 0UL)
    {
        u32Freq = __HXT;
    }
    else
...
uint32_t CLK_GetLXTFreq(void)
{
    ...
    if ((CLK->SRCCTL & CLK_SRCCTL_LXTEN_Msk) != 0UL)
    {
        u32Freq = __LXT;
    }
    else
...
uint32_t CLK_SetCoreClock(uint32_t u32Aclk)
{
	...
	while ((CLK->STATUS & CLK_STATUS_SCLKSWF_Msk) == 0UL)
	{
    ...
    if ((CLK->STATUS & CLK_STATUS_HXTSTB_Msk) != 0UL)
    {
...
void CLK_SetSCLK(uint32_t u32ClkSrc)
{
    ...
    while ((PMC->PLSTS & PMC_PLSTS_PLCBUSY_Msk) != 0UL)
    {
        if (u32TimeOutCount == 0UL)
        {
            break;
...
uint32_t CLK_EnableAPLL(uint32_t u32PllClkSrc, uint32_t u32PllFreq, uint32_t u32PllSelect)
{
    ...
    if (u32PllClkSrc == CLK_APLLCTL_APLLSRC_HXT)
    {
        ...
        /* Select APLL stable count for HXT source */
    #if (__HXT == FREQ_4MHZ)
        u32StableSel = CLK_APLLCTL_STBSEL_820;
    #elif (__HXT <= FREQ_12MHZ)
...
uint32_t CLK_WaitClockDisable(uint32_t u32ClkMask)
{
    ...
    while ((CLK->STATUS & u32ClkMask) != 0UL)
    {
        u32TimeOutCnt--;
        if (u32TimeOutCnt == 0UL)
        {
...
uint32_t CLK_GetAPLL0ClockFreq(void)
{
    ...
    if ((CLK->SRCCTL & CLK_SRCCTL_APLL0EN_Msk) == 0UL)
    {
    ...
    else                        /* PLL is in normal mode */
    {
        ...
        if ((u32PllReg & CLK_APLL0CTL_BP_Msk) != 0UL)
        {
...
uint32_t CLK_GetAPLL1ClockFreq(void)
{
    ...
    if ((CLK->SRCCTL & CLK_SRCCTL_APLL1EN_Msk) == 0UL)
    {
    ...
    else /* PLL1 is in normal mode */
    {
        ...
        if ((u32PllReg & CLK_APLL1CTL_BP_Msk) != 0UL)
        {
...
uint32_t CLK_SystemClockUpdate(void)
{
    ...
    uint32_t u32ClkSrc;
    uint32_t u32SclkDiv;
    const uint32_t gau32ClkSrcTbl[] = {__HIRC, 0UL, __HIRC48M, __HXT, 0UL, 0UL, 0UL, 0UL};
...
uint32_t CLK_GetModuleClockSource(uint64_t u64ModuleIdx)
{
    ...
    u32TmpVal = (M32(u32TmpAddr) & (uint32_t)((uint32_t)MODULE_CLKSEL_Msk(u64ModuleIdx) << (uint32_t)MODULE_CLKSEL_Pos(u64ModuleIdx))) >> (uint32_t)MODULE_CLKSEL_Pos(u64ModuleIdx);
...
uint32_t CLK_GetModuleClockDivider(uint64_t u64ModuleIdx)
{
    ...
    u32DivVal = (M32(u32DivAddr) & (uint32_t)((uint32_t)MODULE_CLKDIV_Msk(u64ModuleIdx) << (uint32_t)MODULE_CLKDIV_Pos(u64ModuleIdx))) >> (uint32_t)MODULE_CLKDIV_Pos(u64ModuleIdx);
