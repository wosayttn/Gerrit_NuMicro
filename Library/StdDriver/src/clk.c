...
uint32_t CLK_EnablePLL(uint32_t u32PllClkSrc, uint32_t u32PllFreq)
{
    ...
    if((u32PllFreq <= FREQ_320MHZ) && (u32PllFreq >= FREQ_30MHZ))
    {
        /* Select "NO" according to request frequency */
        if((u32PllFreq < FREQ_80MHZ) && (u32PllFreq >= FREQ_30MHZ))
        {
            u32NO = 3UL;
            u32PllFreq = u32PllFreq << 2;
        }
        else if((u32PllFreq < FREQ_160MHZ) && (u32PllFreq >= FREQ_80MHZ))
        {
            u32NO = 1UL;
