...
static uint32_t GetPLLClockFreq(void)
{
    ...
    if ((u32PllReg & (CLK_PLLCTL_PD_Msk | CLK_PLLCTL_OE_Msk)) != 0UL)
    {
    ...
    else                        /* PLL is in normal mode */
    {
        ...
        /* PLL source clock */
        if ((u32PllReg & CLK_PLLCTL_PLLSRC_Msk) != 0UL)
        {
        ...
        if ((u32PllReg & CLK_PLLCTL_BP_Msk) != 0UL)
        {
        ...
        else
        {
            ...
            /* Avoiding overflow. (a*b)/d is decomposed into a*(b/d) + (a*(b%d))/d. */
            u32Div = u32NR * u32NO;
            u32Mul = (u32NF << 1U);
            u32Q = u32Mul / u32Div;
            u32R = u32Mul % u32Div;
            u32PllTmp = ((u32FIN >> 2U) * u32Q) + (((u32FIN >> 2U) * u32R) / u32Div);

            if (u32PllTmp <= (FREQ_320MHZ >> 2U))
            {
                u32PllFreq = (u32PllTmp << 2U);
            }
            else
            {
                u32PllFreq = 0UL;
...
uint32_t CLK_GetLXTFreq(void)
{
    ...
    if ((CLK->PWRCTL & CLK_PWRCTL_LXTEN_Msk) != 0UL)
    {
        u32Freq = __LXT;
    }
    else
    ...
    else /* Others clock source configuration */
    {
        ...
        switch (u32ModuleIdx)
        {
            ...
            default:
            {
                ...
                {
                    const uint32_t au32DivTbl[4] = {0x0UL, 0x4UL, 0x8UL, 0x0UL};                /* CLK_CLKDIV0~2 */
                    uint32_t u32Div = 0UL;
...
void CLK_EnableSPDWKPin(uint32_t u32Port, uint32_t u32Pin, uint32_t u32TriggerType, uint32_t u32DebounceEn)
{
    ...
    u32tmpVal = M32(u32tmpAddr);

    if ((u32TriggerType & (CLK_SPDWKPIN0_RISING | CLK_SPDWKPIN0_FALLING)) != 0UL)
    {
    ...
    if ((u32TriggerType & (CLK_SPDWKPIN1_RISING | CLK_SPDWKPIN1_FALLING)) != 0UL)
    {
...
uint32_t CLK_GetModuleClockSource(uint32_t u32ModuleIdx)
{
    ...
    else if (MODULE_CLKSEL_Msk(u32ModuleIdx) != MODULE_NoMsk)
    {
        ...
        /* Get clock source selection setting */
        u32TmpVal = ((M32(u32TmpAddr) & (MODULE_CLKSEL_Msk(u32ModuleIdx) << MODULE_CLKSEL_Pos(u32ModuleIdx))) >> MODULE_CLKSEL_Pos(u32ModuleIdx));
    }
    else
    {
        return MODULE_NoMsk;
...
uint32_t CLK_GetModuleClockDivider(uint32_t u32ModuleIdx)
{
    ...
    switch (u32ModuleIdx)
    {
        ...
        default:
        {
            ...
            u32DivVal = ((M32(u32DivAddr) & (MODULE_CLKDIV_Msk(u32ModuleIdx) << MODULE_CLKDIV_Pos(u32ModuleIdx))) >> MODULE_CLKDIV_Pos(u32ModuleIdx));
			break;
        }
    }
