...
void CLK_SetHCLK(uint32_t u32ClkSrc, uint32_t u32ClkDiv)
{
    ...
    if (CLK_WaitClockReady(CLK_STATUS_HIRCSTB_Msk) == 0UL)
    {
...
{
    if ((uint32_t)MODULE_CLKDIV_Msk(u32ModuleIdx) != (uint32_t)MODULE_NoMsk)
    {
        uint32_t u32Div;
        const uint32_t au32DivTbl[] = {0x0UL, 0x4UL, 0xCUL, 0x10UL};
        uint32_t u32ClkDivMask;
        uint32_t u32ClkDivPos;

        /* Get clock divider control register address */
        u32Div = (uint32_t)&CLK->CLKDIV0 + au32DivTbl[(uint32_t)MODULE_CLKDIV(u32ModuleIdx)];
        u32ClkDivPos = (uint32_t)MODULE_CLKDIV_Pos(u32ModuleIdx);
        u32ClkDivMask = ((uint32_t)MODULE_CLKDIV_Msk(u32ModuleIdx) << u32ClkDivPos);
        /* Apply new divider */
        M32(u32Div) = (M32(u32Div) & (~u32ClkDivMask)) | u32ClkDiv;
    }

    if ((uint32_t)MODULE_CLKSEL_Msk(u32ModuleIdx) != (uint32_t)MODULE_NoMsk)
    {
        uint32_t u32Sel;
        const uint32_t au32SelTbl[] = {0x0UL, 0x4UL, 0x8UL, 0xCUL};
        uint32_t u32ClkSelMask;
        uint32_t u32ClkSelPos;

        /* Get clock select control register address */
        u32Sel = (uint32_t)&CLK->CLKSEL0 + au32SelTbl[(uint32_t)MODULE_CLKSEL(u32ModuleIdx)];
        u32ClkSelPos = (uint32_t)MODULE_CLKSEL_Pos(u32ModuleIdx);
        u32ClkSelMask = ((uint32_t)MODULE_CLKSEL_Msk(u32ModuleIdx) << u32ClkSelPos);
        /* Set new clock selection setting */
        M32(u32Sel) = (M32(u32Sel) & (~u32ClkSelMask)) | u32ClkSrc;
    }
}
...
void CLK_EnableModuleClock(uint32_t u32ModuleIdx)
{
    ...
    pu32ClkReg = (volatile uint32_t *)((uint32_t)&CLK->AHBCLK + au32ClkTbl[(uint32_t)MODULE_APBCLK(u32ModuleIdx)]);
    u32ModuleMask = (1UL << (uint32_t)MODULE_IP_EN_Pos(u32ModuleIdx));
    *pu32ClkReg |= u32ModuleMask;
}
...
uint32_t CLK_GetModuleClockSource(uint32_t u32ModuleIdx)
{
    ...
    if ((uint32_t)MODULE_CLKSEL_Msk(u32ModuleIdx) != (uint32_t)MODULE_NoMsk)
    {
        ...
        /* Get clock select control register address */
        u32TmpAddr = (uint32_t)&CLK->CLKSEL0 + au32SelTbl[(uint32_t)MODULE_CLKSEL(u32ModuleIdx)];
        u32ClkSelPos = (uint32_t)MODULE_CLKSEL_Pos(u32ModuleIdx);
        u32ClkSelMask = ((uint32_t)MODULE_CLKSEL_Msk(u32ModuleIdx) << u32ClkSelPos);

        /* Get clock source selection setting */
        u32TmpVal = ((inpw((uint32_t *)u32TmpAddr) & u32ClkSelMask) >> u32ClkSelPos);
    }
...
uint32_t CLK_GetModuleClockDivider(uint32_t u32ModuleIdx)
{
    ...
    if ((uint32_t)MODULE_CLKDIV_Msk(u32ModuleIdx) != (uint32_t)MODULE_NoMsk)
    {
        ...
        /* Get clock divider control register address */
        u32TmpAddr = (uint32_t)&CLK->CLKDIV0 + au32DivTbl[(uint32_t)MODULE_CLKDIV(u32ModuleIdx)];
        u32ClkDivPos = (uint32_t)MODULE_CLKDIV_Pos(u32ModuleIdx);
        u32ClkDivMask = ((uint32_t)MODULE_CLKDIV_Msk(u32ModuleIdx) << u32ClkDivPos);
        /* Get clock divider number setting */
        u32TmpVal = ((inpw((uint32_t *)u32TmpAddr) & u32ClkDivMask) >> u32ClkDivPos);
    }
...
{
    uint32_t u32Delay;
    uint32_t u32RemainUsec;

    /* It should <= 349525us for each delay loop */
    u32Delay = 349525UL;
    u32RemainUsec = u32USec;

    do
    {
        if (u32RemainUsec > u32Delay)
        {
            u32RemainUsec -= u32Delay;
        }
        else
        {
            u32Delay = u32RemainUsec;
            u32RemainUsec = 0UL;
        }
