...
int32_t I3C_ConfigRxDMA(I3C_DEVICE_T *dev, uint32_t u32Src, uint32_t u32Dest, uint32_t u32ByteCnts)
{
    ...
    PDMA_SetTransferMode(pdma, ch, PDMA_I3C0_RX, 0, 0);
    pdma->CHCTL |= (1 << ch);
    return 0;
}
...
int32_t I3C_ConfigTxDMA(I3C_DEVICE_T *dev, uint32_t u32Src, uint32_t u32Dest, uint32_t u32ByteCnts)
{
    ...
    pdma->DSCT[ch].SA = u32Src;
    pdma->DSCT[ch].DA = u32Dest;
    PDMA_SetTransferMode(pdma, ch, PDMA_I3C0_TX, 0, 0);
    pdma->CHCTL |= (1 << ch);
...
static int32_t I3C_DEFTGTS(I3C_DEVICE_T *dev)
{
    ...
    {
        I3C_DrvMsg("\n[ DRV ] [ DEFTGTS, PASS ] (RESP: 0x%08x)\n", dev->cmd_response);
        return I3C_STS_NO_ERR;
    }
    else
    {
        I3C_DrvMsg("\n[ DRV ] [ DEFTGTS, error code %d ] (RESP: 0x%08x)\n",
        ...
        if (ibi_len == 0)
        {
            ...
            else
            {
                ...
                if (dev->ibi_id != 0)
                {
                    ...
                    if (i >= 7)
                    {
                        return -1;    /* No Target's DA matched */
                    }

                    if (ibi_id == dev->main_controller_da)
                    {
                        I3C_DrvMsg("\t[ DRV ] Direct return to Controller\n");
                    }
                    else
                    {
                        if (I3C_DEFTGTS(dev) != I3C_STS_NO_ERR)
                        {
                            return -1;    /* I3C_DEFTGTS error */
                        }
                    }
