...
#define U32_TO_U8(u32_val) ((uint8_t)((u32_val) & 0xFFU))

#define U32_TO_I32(u32_val) \
    ( (int32_t)(union { uint32_t u; int32_t i; }){ .u = (u32_val) }.i )

#define VOLATILE_U32_COPY(dst, src) \
    do {                            \
        uint32_t tmp_ = (src);      \
        (dst) = tmp_;               \
    } while (0)
...
/**
  * @brief      Initialize I3C Device Type
  *
  * @param[in]  dev     The pointer of the specified I3C device
  *
  * @retval     I3C_STS_NO_ERR          No error
  * @retval     I3C_STS_INVALID_INPUT   Invalid input parameter
  *
  * @details    This function is used to initial I3C device type.
  * @note       This function must be called before I3C communication.
  *
  */
int32_t I3C_DeviceInit(I3C_DEVICE_T *dev)
{
    ...
    if (dev->device_role == I3C_CONTROLLER)
    {
        ...
        dev->port->DEVADDR = (I3C_DEVADDR_DAVALID_Msk | ((dev->main_controller_da & 0x7FUL) << I3C_DEVADDR_DA_Pos));
        /* Set as Controller role */
        dev->port->DEVCTLE = dev->device_role;
        /* Set Controller includes I3C Broadcast Address */
    ...
    else if (dev->device_role == I3C_TARGET)
    {
        ...
        {
            /* Program Static Address for Target */
            dev->port->DEVADDR |= (I3C_DEVADDR_SAVALID_Msk | ((dev->main_target_sa & 0x7FUL) << I3C_DEVADDR_SA_Pos));
    ...
    if (dev->device_role == I3C_CONTROLLER)
    {
        if ((dev->port->DEVCTL & I3C_DEVCTL_ENABLE_Msk) == 0UL)
        {
            return I3C_STS_INVALID_STATE;
        }
    }

    if (u32IsRegLocked != 0UL)
    {
        SYS_UnlockReg();
    }
...
/**
  ...
  * @param[in]  u8DAddr         7Bits Device Dynamic Address
...
int32_t I3C_SetDeviceAddr(I3C_T *i3c, uint8_t u8DevIndex, uint8_t u8DevType, uint8_t u8DAddr, uint8_t u8SAddr)
{
    uint32_t i;
    uint32_t count = 0;
    uint32_t u32Device = 0;

    if (((u8DAddr & 0x80U) != 0U) || ((u8SAddr & 0x80U) != 0U))
    {
        return I3C_STS_INVALID_INPUT;
    }
...
/**
  ...
  * @param[in]  dev     The pointer of the specified I3C device
  *
  * @details    This function is used to configure I3C bus timing.
  *
  */
void I3C_BusClkConfig(I3C_DEVICE_T *dev)
{
    ...
    fcount = ((float)200UL * (float)dev->engclk) / (float)1000000UL;
    ...
    fcount = ((float)1UL * (float)dev->engclk) / (float)1000000UL;
    count = (uint32_t)fcount;
    ...
    fcount = ((float)1300UL * (float)dev->engclk) / (float)1000000000UL;
    count = (uint32_t)fcount;
...
/**
  ...
  * @param[in]  dev     The pointer of the specified I3C device
  *
  * @details    This function is used to show present state information of I3C device.
...
void I3C_PresentStateInfo(I3C_DEVICE_T *dev)
{
    ...
    else
    {
        if (role == 0UL)
        {
            if ((reg_val[0] & I3C_PRESENTS_CTRIDLES_Msk) != 0UL)
            {
...
/**
  ...
  *
  * @param[in]  dev         The pointer of the specified I3C device
  * @param[in]  u32Src      The source address of DMA transfer
  * @param[in]  u32Dest     The destination address of DMA transfer
  * @param[in]  u32ByteCnts The number of bytes to transfer
  *
  * @retval     0           Success
  * @retval     -1          Failure
  *
  * @details    This function is used to configure I3C Rx DMA transfer on PDMA channel.
  *
  */
...
/**
  ...
  * @retval     -1          Failure
  *
  * @details    This function is used to configure I3C Tx DMA transfer on PDMA channel.
  */
...
/**
  ...
  * @retval     I3C_STS_INVALID_INPUT   Invalid input parameter
  *
  * @details    This function is used to perform Dynamic Address Assignment by ENTDAA and SETDASA CCC.
  *
  */
int32_t I3C_CtrDAA(I3C_DEVICE_T *dev)
{
    ...
    {
        // Check if the target index is valid (0 to 6)
    ...
    {
        // Check if the target count is valid (1 to 7)
        // Note: The maximum number of targets that can be assigned dynamic addresses is 7 (0 to 6)
    ...
    val |= ((count << I3C_CMDQUE_DEVCOUNT_Pos)
            ...
            | ((dev->tx_id & 0xFUL) << I3C_CMDQUE_TID_Pos)
            | ((dev->ccc_code & 0xFFUL) << I3C_CMDQUE_CMD_Pos)
...
/**
  ...
  * @retval     I3C_STS_INVALID_INPUT   Invalid input parameter
  *
  * @details    This function is used to perform Write CCC operation by using SDAP or Transfer Argument.
  * @note       This function is used for Controller to send CCC command to Target device.
  *
  */
int32_t I3C_CtrCCCSet(I3C_DEVICE_T *dev)
{
    ...
    if ((dev->tx_len > 0UL) && (dev->tx_len <= I3C_SDAP_MAX_SIZE))
    {
        ...
        switch (dev->tx_len)
        {
            case 3:
                ...
                val |= ((dev->tx_buf[2] & 0xFFUL) << I3C_CMDQUE_DATBYTE2_Pos);
                val |= (1UL << (I3C_CMDQUE_BYTESTRB_Pos + 1UL));
                val |= ((dev->tx_buf[1] & 0xFFUL) << I3C_CMDQUE_DATBYTE1_Pos);
                val |= (1UL << (I3C_CMDQUE_BYTESTRB_Pos + 0UL));
                val |= ((dev->tx_buf[0] & 0xFFUL) << I3C_CMDQUE_DATBYTE0_Pos);
                break;
    ...
    val |= (I3C_CMDQUE_CP_Msk
            | ((dev->target_index & 0x1FUL) << I3C_CMDQUE_DEVINDX_Pos)
            | ((dev->tx_id & 0xFUL) << I3C_CMDQUE_TID_Pos)
            | ((dev->ccc_code & 0xFFUL) << I3C_CMDQUE_CMD_Pos)
            | I3C_CMDATTR_TRANSFER_CMD);

    if (dev->is_last_cmd != 0U)
    {
        /* Stop and Response Status on Transfer Completion */
        val |= (I3C_CMDQUE_TOC_Msk | I3C_CMDQUE_ROC_Msk);
    }
...
/**
  ...
  * @retval     I3C_STS_INVALID_INPUT   Invalid input parameter
  *
  * @details    This function is used to perform Read CCC operation by using Transfer Argument.
  * @note       This function is used for Controller to send CCC command to Target device.
  *
  */
int32_t I3C_CtrCCCGet(I3C_DEVICE_T *dev)
{
    ...
    if (dev->is_DB != 0U)
    {
        val |= ((dev->DB & 0xFFUL) << I3C_CMDQUE_DB_Pos);
    }
...
/**
  * @brief      Perform Private/HDR-DDR Write Operation in Controller
  *
  * @param[in]  dev     The pointer of the specified I3C device
  *
  * @retval     I3C_STS_NO_ERR          No error
  * @retval     I3C_STS_INVALID_INPUT   Invalid input parameter
  *
  * @details    This function is used to perform Private/HDR-DDR Write operation in Controller.
  * @note       This function is used for Controller to send Private/HDR-DDR command to Target device.
  *
  */
int32_t I3C_CtrWrite(I3C_DEVICE_T *dev)
{
    ...
    /* Program HDR command */
    if (dev->is_HDR_cmd != 0U)
    {
        val |= (I3C_CMDQUE_CP_Msk | ((dev->ccc_code & 0xFFUL) << I3C_CMDQUE_CMD_Pos));
    }
...
/**
  * @brief      Perform Private/HDR-DDR Read Operation in Controller
  *
  * @param[in]  dev     The pointer of the specified I3C device
  *
  * @retval     I3C_STS_NO_ERR          No error
  * @retval     I3C_STS_INVALID_INPUT   Invalid input parameter
  *
  * @details    This function is used to perform Private/HDR-DDR Read operation in Controller.
  * @note       This function is used for Controller to send Private/HDR-DDR command to Target device and receive data from Target device.
  *
  */
int32_t I3C_CtrRead(I3C_DEVICE_T *dev)
{
    ...
    if (dev->is_HDR_cmd != 0U)
    {
        val |= (I3C_CMDQUE_CP_Msk | ((dev->HDR_cmd & 0xFFUL) << I3C_CMDQUE_CMD_Pos));
    }
...
/**
  * @brief      Perform HDR-BT Write Operation in Controller
  *
  * @param[in]  dev     The pointer of the specified I3C device
  *
  * @retval     I3C_STS_NO_ERR          No error
  * @retval     I3C_STS_INVALID_INPUT   Invalid input parameter
  *
  * @details    This function is used to perform HDR-BT Write operation in Controller.
  * @note       This function is used for Controller to send HDR-BT command to Target device and send data to Target device.
  *
  */
...
/**
  * @brief      Perform HDR-BT Read Operation in Controller
  *
  * @param[in]  dev     The pointer of the specified I3C device
  *
  * @retval     I3C_STS_NO_ERR          No error
  * @retval     I3C_STS_INVALID_INPUT   Invalid input parameter
  *
  * @details    This function is used to perform HDR-BT Read operation in Controller.
  * @note       This function is used for Controller to send HDR-BT command to Target device and receive data from Target device.
  *
  */
...
/**
  ...
  * @retval     I3C_STS_INVALID_INPUT   Invalid input parameter
  *
  * @details    This function is used to perform DEFTGTS CCC to Secondary Controller (Target).
  * @note       This function is used for Controller to send DEFTGTS CCC command to Secondary Controller (Target) device.
  *
  */
int32_t I3C_CtrDEFTGTS(I3C_DEVICE_T *dev)
{
    ...
    dev->tx_buf[2] = U32_TO_U8(((dev->port->SLVCHAR & I3C_SLVCHAR_DCR_Msk) >> I3C_SLVCHAR_DCR_Pos)); // DCR;
    dev->tx_buf[3] = U32_TO_U8((dev->port->SLVCHAR & 0xFFU)); // BCR;
    dev->tx_buf[4] = 0x0; // SA
    ...
    for (i = 0U; i < dev->tx_buf[0]; i++)
    {
        ...
        dev->tx_buf[(j * 4U) + 5U + 0U] = U32_TO_U8((dev->port->TGTCHAR[i].DADDR << 1U)); // DA
        dev->tx_buf[(j * 4U) + 5U + 1U] = U32_TO_U8((dev->port->TGTCHAR[i].BCRDCR & 0xFFU)); // DCR;
        dev->tx_buf[(j * 4U) + 5U + 2U] = U32_TO_U8(((dev->port->TGTCHAR[i].BCRDCR & 0xFF00U) >> 8U)); // BCR;
    ...
    dev->is_last_cmd  = 1;
    dev->ccc_code     = U32_TO_U8(I3C_CCC_DEFTGTS); // Not support IRQ
    ...
    {
        uint32_t arg0 = (uint32_t)((dev->cmd_response & I3C_CTRRESP_ERRSTS_Msk) >> I3C_CTRRESP_ERRSTS_Pos);
        uint32_t arg1 = dev->cmd_response;
...
/**
  * @brief      Perform GETACCCR CCC for Activating the Secondary Controller
  *
  * @param[in]  dev     The pointer of the specified I3C device
  *
  * @retval     I3C_STS_NO_ERR          No error
  * @retval     I3C_STS_INVALID_INPUT   Invalid input parameter
  *
  * @details    This function is used to perform GETACCCR CCC for Activating the Secondary Controller.
  * @note       This function is used for Controller to send GETACCCR CCC command to Secondary Controller (Target) device.
  *
  */
int32_t I3C_CtrGETACCCR(I3C_DEVICE_T *dev)
{
    ...
    dev->rx_len       = 1;
    dev->is_last_cmd  = 1;
    dev->ccc_code     = U32_TO_U8(I3C_CCC_GETACCCR); // Not support IRQ
...
/**
  ...
  * @retval     I3C_STS_INVALID_INPUT   Invalid input parameter
  *
  * @details    This function is used to get In-Band Interrupt Event in Controller.
  * @note       This function is used for Controller to get In-Band Interrupt Event from Target device.
  *
  */
int32_t I3C_CtrGetIBI(I3C_DEVICE_T *dev)
{
    if ((dev->ibi_status & BIT31) != 0UL)
    {
    ...
    else
    {
        ...
        if (ibi_len == 0UL)
        {
            ...
            if (ibi_id == 0x2UL)
            {
                ...
                dev->ibi_id   = U32_TO_U8(ibi_id);
                dev->ibi_len  = U32_TO_U8(ibi_len);
            }
            else
            {
                ...
                if (dev->ibi_id != 0U)
                {
                    // Check if Target's DA matched and send GETACCCR CCC. Maximum 7 Target's DA can be stored in Controller.
                    for (i = 0UL; i < I3C_CTR_TGT_TABLE_CNT; i++)
                    {
                    ...
                    {
                        return -5;    /* I3C_GETACCCR error */
                    }
                }

                // monitor bus owner status
                if ((dev->port->INTSTS & I3C_INTSTS_BUSOWNER_UPDATED) != 0UL)
                {
        ...
        else
        {
            ...
            {
                VOLATILE_U32_COPY(p32Buf[i], dev->port->IBIQUE);
            }
            ...
            dev->ibi_id      = U32_TO_U8(ibi_id);
            dev->ibi_len     = U32_TO_U8(ibi_len - 1UL);
...
/**
  ...
  * @retval     None
  *
  * @details    This function is used to parse error condition and recovery in Controller.
  * @note       This function is used for Controller to parse error condition and recovery.
  *
  */
void I3C_CtrHandleTransErr(I3C_DEVICE_T *dev)
{
    ...
    switch (err_status)
    {
        ...
        case I3C_CTRRESP_BRD_ADDR_NACK_ERR:
            ...
            {
                dev->target_count = dev->target_count - (uint8_t)LEN;
            }
...
/**
  * @brief      Reset and Resume Target
  *
  * @param[in]  dev         The pointer of the specified I3C device
  * @param[in]  ExtCmdIdx   The index of the extended command to reset
  *
  * @retval     None
  *
  * @details    This function is used to reset and resume the Target.
  *
  */
static void I3C_TgtResetAndResume(I3C_DEVICE_T *dev, uint8_t ExtCmdIdx)
{
    ...
    {
        dev->port->EXTDBRST = (1UL << (ExtCmdIdx & 0x7UL));
    }
...
/**
  ...
  * @retval     I3C_STS_INVALID_INPUT   Invalid input parameter
  *
  * @details    This function is used to get Target Response Result in Controller Write Operation.
  * @note       This function is used for Target to get Response Result.
  *
  */
int32_t I3C_TgtRecv(I3C_DEVICE_T *dev)
{
    ...
    if ((dev->port->INTSTS & I3C_INTSTS_RESPRDY_Msk) == 0UL)
    {
    ...
    if ((u32RespQ & I3C_TGTRESP_FIRST_Msk) == 0UL)
    {
    ...
    if ((int32_t)u8ErrSts != I3C_STS_NO_ERR)
    {
        ...
        return U32_TO_I32(u32RespQ & I3C_TGTRESP_ERRSTS_Msk);
    }
    ...
    */
    ...
    else
    {
        if (u8CmdSize != 0U)
        {
            VOLATILE_U32_COPY(pu32RxBuf[RxBufIdx], dev->port->TXRXDAT);
            I3C_DrvMsg("[ DRV ] CCC/CMD code: 0x%08x\n", pu32RxBuf[RxBufIdx]);
            RxBufIdx++;
        }
    ...
    dev->tgtRespQ[0].RxBufAddr = (uint32_t)dev->rx_buf;
    ...
    if (u8CmdSize != 0U)
    {
        ...
        else if (u8CmdSize == 4U)
        {
            ...
            dev->tgtRespQ[0].CmdCCC = U32_TO_U8(I3C_CCC_HDRBT);
        }
        else
        {
        ...
        dev->tgtRespQ[0].RxBufAddr += 4UL; // the First Data entry in the RX FIFO for the transfer's encoded command word
    }

    dev->tgtRespQ[0].RxBufLen = u8DataLen;

    if ((u32RespQ & I3C_TGTRESP_LAST_Msk) != 0UL)
    {
        return I3C_STS_NO_ERR;
    }
...
/**
  ...
  * @retval     I3C_STS_INVALID_INPUT   Invalid input parameter
  *
  * @details    This function is used to prepare Target Extended Transmit Command Data for Controller Read Operation.
  *
  */
        ...
        else
        {
            ...
            dev->port->EXTCMD[cmd_idx].WORD2 =
                ((txlen << I3C_TGTCMDW2_DATALEN_Pos) |
                 ...
                 ((ccc_code & 0xFFUL) << I3C_TGTCMDW2_CCCHEAD_Pos));
            dev->port->EXTCMD[cmd_idx].WORD3 = 0; // For HDR-BT Transfer Read command
        }
...
/**
  ...
  *
  * @details    This function is used to get Target Extended Transmit Command Result in Controller Read Operation.
  * @note       This function is used for Target to get Extended Transmit Command Result.
  *
  */
int32_t I3C_TgtGetSendResult(I3C_DEVICE_T *dev)
{
    uint32_t i;
    uint32_t u32CMDDoneBitMsk;
    uint32_t CmdRegVal[3];
    I3C_DrvMsg("\n");

    if ((dev->port->INTSTS & I3C_INTSTS_EXTFINS_Msk) == 0UL)
    {
        return I3C_STS_INVALID_STATE;
    }
    ...
    // Check which Extended Command has finished and parse the result
    for (i = 0UL; i < I3C_DEVICE_EXTCMD_CNT; i++)
    {
        if ((u32CMDDoneBitMsk & (1UL << i)) != 0UL)
        {
            uint32_t err_sts;
            uint32_t dat_len;
            ...
            if (err_sts == I3C_TGTCMDSTS_NO_ERR)
            {
                ...
                if ((CmdRegVal[0] & I3C_TGTCMDW1_BTDLEN_Msk) != 0UL)
                {
                    I3C_DrvMsg("\t[ HDR-BT transfer ]\n");
                    dat_len = ((CmdRegVal[2] & I3C_TGTCMDW3_BTDL_Msk) >> I3C_TGTCMDW3_BTDL_Pos);
                }

                if ((CmdRegVal[0] & I3C_TGTCMDW1_FINITEDL_Msk) != 0UL)   //Finite length transfer
                {
            ...
            else
            {
                ...
                switch (err_sts)
                {
                    ...
                    case I3C_TGTCMDSTS_CONTROLLER_TERMINATE_ERR:
                        I3C_DrvMsg("\t# Controller early termination Error \n");
                        break;
                ...
                if (dev_sts != 0UL)
                {
                    if ((dev_sts & I3C_CCCDEVS_PROTERR_Msk) != 0UL)
                    {
                        I3C_DrvMsg("\t# Protocol Error \n");
                    }

                    if ((dev_sts & I3C_CCCDEVS_UDFERR_Msk) != 0UL)
                    {
                        I3C_DrvMsg("\t# Underflow Error \n");
                    }

                    if ((dev_sts & I3C_CCCDEVS_OVFERR_Msk) != 0UL)
                    {
                        I3C_DrvMsg("\t# Overflow Error \n");
                    }

                    if ((dev_sts & I3C_CCCDEVS_DATNRDY_Msk) != 0UL)
                    {
                        I3C_DrvMsg("\t# Data Not Ready \n");
                    }

                    if ((dev_sts & I3C_CCCDEVS_BFNAVAIL_Msk) != 0UL)
                    {
                        I3C_DrvMsg("\t# Buffer Not Available \n");
                    }

                    if ((dev_sts & I3C_CCCDEVS_FRAMEERR_Msk) != 0UL)
                    {
                        I3C_DrvMsg("\t# Frame Error \n");
                    }

                    if ((dev_sts & I3C_CCCDEVS_SLVBUSY_Msk) != 0UL)
                    {
                        I3C_DrvMsg("\t# Target Busy \n");
                        I3C_TgtResetAndResume(dev, (uint8_t)i);
                    }
                }
...
/**
  ...
  * @retval     I3C_STS_INVALID_INPUT   Invalid input parameter
  *
  * @details    This function is used to issue In-Band Interrupt Event in Target.
  *
  */
int32_t I3C_TgtIssueIBI(I3C_DEVICE_T *dev)
{
    ...
    if (dev->target_index > (I3C_MAIN_TGT_CNT + I3C_VIRTUAL_TGT_CNT - 1U))
    {
    ...
    switch (dev->ibi_type)
    {
        ...
        case I3C_IBI_TYPE_CR:
            ...
            dev->port->SIR = (((dev->target_index & 0xFUL) << I3C_SIR_TGTIDX_Pos) | (0UL << I3C_SIR_CTL_Pos));
            /* Trigger MR request */
    ...
    {
        dev->port->SIR |= (((dev->target_index & 0xFUL) << I3C_SIR_TGTIDX_Pos) | I3C_SIR_EN_Msk | I3C_SIR_TS_Msk);
    }
    else
    {
        dev->port->SIR |= (((dev->target_index & 0xFUL) << I3C_SIR_TGTIDX_Pos) | I3C_SIR_EN_Msk);
    }
...
/**
  ...
  * @retval     None
  *
  * @details    This function is used to parse Error Condition and Recovery in Target.
  *
  */
void I3C_TgtHandleTransErr(I3C_DEVICE_T *dev)
{
    ...
    if (dev_status != 0UL)
    {
        ...
        if ((dev_status & I3C_CCCDEVS_SLVBUSY_Msk) != 0UL)
        {
            I3C_DrvMsg("\t#Target busy status\n");
            resume = TRUE;
        }

        if ((dev_status & I3C_CCCDEVS_FRAMEERR_Msk) != 0UL)
        {
            I3C_DrvMsg("\t# Dev frame error\n");
        }

        if ((dev_status & I3C_CCCDEVS_BFNAVAIL_Msk) != 0UL)
        {
            I3C_DrvMsg("\t# Dev buffer not available\n");
        }

        if ((dev_status & I3C_CCCDEVS_DATNRDY_Msk) != 0UL)
        {
            I3C_DrvMsg("\t# Dev data not ready\n");
        }

        if ((dev_status & I3C_CCCDEVS_OVFERR_Msk) != 0UL)
        {
            I3C_DrvMsg("\t# Dev overflow error\n");
        }

        if ((dev_status & I3C_CCCDEVS_UDFERR_Msk) != 0UL)
        {
            I3C_DrvMsg("\t# Dev underflow error\n");
        }

        if ((dev_status & I3C_CCCDEVS_PROTERR_Msk) != 0UL)
        {
    ...
    switch (err_status)
    {
        ...
        case I3C_TGTRESP_CONTROLLER_TERMINATE_ERR:
            I3C_DrvMsg("\t# Transfer Error: Controller early termination Error \n");
            break;
...
/**
  ...
  * @retval     None
  *
  * @details    This function is used to handle Target Interrupt Status.
  *
  */
void I3C_TgtHandleIntSts(I3C_DEVICE_T *dev)
{
    ...
    if ((dev->intsts & I3C_INTSTS_DA_ASSIGNED) != 0UL)
    {
        /* Main Target Address */
        if ((dev->port->DEVADDR & I3C_DEVADDR_DAVALID_Msk) != 0UL)
        {
        ...
        for (i = 0UL; i < I3C_VIRTUAL_TGT_CNT; i++)
        {
            idx = i + I3C_MAIN_TGT_CNT;

            if ((dev->port->VTGTCFG[i].ADDR & I3C_VTGTADDR_DAVALID_Msk) != 0UL)
            {
    ...
    if ((dev->intsts & I3C_INTSTS_GRPDA_ASSIGNED) != 0UL)
    {
        /* Group Address 0~3 */
        for (i = 0UL; i < I3C_GROUP_TGT_CNT; i++)
        {
            idx = i + I3C_MAIN_TGT_CNT + I3C_VIRTUAL_TGT_CNT;
    ...
    if ((dev->intsts & I3C_INTSTS_CCC_UPDATED) != 0UL)
    {
        if ((dev->port->SLVEVNTS & I3C_SLVEVNTS_MWLUPD_Msk) != 0UL)
        {
        ...
        else if ((dev->port->SLVEVNTS & I3C_SLVEVNTS_MRLUPD_Msk) != 0UL)
        {
            uint32_t arg0 = (uint32_t)((dev->port->SLVMXLEN & I3C_SLVMXLEN_MRL_Msk) >> I3C_SLVMXLEN_MRL_Pos);
            uint32_t arg1 = (uint32_t)((dev->port->DEVCTL & I3C_DEVCTL_IBIPSIZE_Msk) >> I3C_DEVCTL_IBIPSIZE_Pos);
