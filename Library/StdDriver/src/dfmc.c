...
int32_t DFMC_Erase(uint32_t u32PageAddr)
{
    ...
    u32TimeOutCnt = DFMC_TIMEOUT_ERASE;

    while ((DFMC->ISPTRG & DFMC_ISPTRG_ISPGO_Msk) != 0UL)
    {
    ...
    if ((DFMC->ISPCTL & DFMC_ISPCTL_ISPFF_Msk) != 0UL)
    {
...
uint32_t DFMC_Read(uint32_t u32Addr)
{
    ...
    u32TimeOutCnt = DFMC_TIMEOUT_READ;

    while ((DFMC->ISPSTS & DFMC_ISPSTS_ISPBUSY_Msk) != 0UL)
    {
        if (u32TimeOutCnt == 0UL)
        {
    ...
    *pi32ErrBit = -1;
    ...
    if (DFMC->ECCSTS != 0UL)
    {
        ...
        if (DFMC->MPDATE != 0UL)
        {
            ...
            for (i = 0UL; i < (sizeof(au8DFMC_ECC_ErrorBit) / sizeof(au8DFMC_ECC_ErrorBit[0])); i++)
            {
                if (au8DFMC_ECC_ErrorBit[i] == DFMC->MPDATE)
                {
                    *pi32ErrBit = (int32_t)i;
                    break;
                }
            }
        }
...
int32_t DFMC_Write(uint32_t u32Addr, uint32_t u32Data)
{
    ...
    u32TimeOutCnt = DFMC_TIMEOUT_WRITE;
...
uint32_t  DFMC_GetChkSum(uint32_t u32Addr, uint32_t u32Count)
{
    ...
    if (((u32Addr % DFMC_FLASH_PAGE_SIZE) != 0UL) || ((u32Count % DFMC_FLASH_PAGE_SIZE) != 0UL))
    {
    ...
    else
    {
        ...
        u32TimeOutCnt = DFMC_TIMEOUT_CHKSUM;
...
uint32_t  DFMC_CheckAllOne(uint32_t u32Addr, uint32_t u32Count)
{
    ...
    u32TimeOutCnt0 = DFMC_TIMEOUT_CHKALLONE;
    ...
    {
        uint32_t u32TimeOutCnt1 = DFMC_TIMEOUT_CHKALLONE;
...
int32_t DFMC_EEPROM_Init(void)
{
    ...
    u32TimeOutCnt = DFMC_TIMEOUT_EEPROM_INIT;
    ...
    while ((DFMC->ISPSTS & DFMC_ISPSTS_EEP_INIT_Msk) != 0UL)
    {
        if (u32TimeOutCnt == 0UL)
        {
            g_DFMC_i32ErrCode = DFMC_ERR_TIMEOUT;
    ...
    if ((DFMC->ISPSTS & DFMC_ISPSTS_ISPFF_Msk) != 0UL)
    {
...
uint32_t DFMC_EEPROM_Read(uint32_t u32Addr)
{
    ...
    while ((DFMC->ISPSTS & DFMC_ISPSTS_EEP_PGFULL_Msk) != 0UL)
    {
        if (u32TimeOutCnt == 0UL)
        {
            g_DFMC_i32ErrCode = DFMC_ERR_TIMEOUT;
