...
/**
  * @brief Execute DFMC_ISPCMD_PAGE_ERASE command to erase a flash page. The page size is DFMC_FLASH_PAGE_SIZE bytes.
...
/**
  ...
  */
int32_t DFMC_Mass_Erase(uint32_t u32PageAddr)
{
    int32_t i32Ret = 0;
    int32_t i32TimeOutCnt;
...
/**
  ...
  * @param[in]  u32Addr Address of the flash location to be read.
  *             It must be a word aligned address.
  *             pi32ErrBit -1:    No ECC Error, 
  *                        Other: Error bit location if ECC single error is detected.
  * @return The word data read from specified flash address.
  *         Return 0xFFFFFFFF if read failed.
  * @note   Global error code g_DFMC_i32ErrCode
...
/**
  ...
  *
  * @note     Global error code g_DFMC_i32ErrCode
  *           DFMC_ERR_TIMEOUT   Run/Read check sum time-out failed
  *           DFMC_ERR_FAIL      Run/Read check sum failed
  *           DFMC_ERR_NON_ALIGN u32Addr or u32Count must be aligned with DFMC_FLASH_PAGE_SIZE
  */
...
/**
  ...
  *
  * @note     Global error code g_DFMC_i32ErrCode
  *           DFMC_ERR_TIMEOUT          RUN_ALL_ONE or CHECK_ALL_ONE commands time-out
  *           DFMC_READ_ALLONE_CMD_FAIL CHECK_ALL_ONE command failed
  */
uint32_t  DFMC_CheckAllOne(uint32_t u32Addr, uint32_t u32Count)
{
    ...
    if(g_DFMC_i32ErrCode == DFMC_OK)
    {
        if(DFMC->ISPDAT == DFMC_READ_ALLONE_YES)
            u32Ret = DFMC_READ_ALLONE_YES;
        else if(DFMC->ISPDAT == DFMC_READ_ALLONE_NOT)
            u32Ret = DFMC_READ_ALLONE_NOT;
        else
            g_DFMC_i32ErrCode = DFMC_READ_ALLONE_CMD_FAIL;
    }
...
#if (CHCKE_PAGE_FULL_PROC == 1)
    i32TimeOutCnt = DFMC_TIMEOUT_ERASE;
    while(DFMC->ISPSTS & DFMC_ISPSTS_EEP_PF_Msk)
    {
        if(i32TimeOutCnt-- <= 0)
        {
            g_DFMC_i32ErrCode = DFMC_ERR_TIMEOUT;
...
#if (CHCKE_PAGE_FULL_PROC == 1)
    i32TimeOutCnt = DFMC_TIMEOUT_ERASE;
    while(psDFMC->ISPSTS & DFMC_ISPSTS_EEP_PF_Msk)
    {
        if(i32TimeOutCnt-- <= 0)
        {
            g_DFMC_i32ErrCode = DFMC_ERR_TIMEOUT;
