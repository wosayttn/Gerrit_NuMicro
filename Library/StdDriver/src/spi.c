...
#define SPI_FIFO_TXTH_MAX       (SPI_FIFOCTL_TXTH_Msk >> SPI_FIFOCTL_TXTH_Pos)
#define SPI_FIFO_RXTH_MAX       (SPI_FIFOCTL_RXTH_Msk >> SPI_FIFOCTL_RXTH_Pos)

static uint32_t SPII2S_GetSourceClockFreq(const SPI_T *i2s);

static int32_t SPI_IsValidModule(const SPI_T *spi)
{
    return ((spi == SPI0) || (spi == SPI1)) ? 1 : 0;
}
...
{
    uint32_t u32WasEnabled = 0UL;
    uint32_t u32TxLevel = (u32TxThreshold & SPI_FIFO_TXTH_MAX);
    uint32_t u32RxLevel = (u32RxThreshold & SPI_FIFO_RXTH_MAX);
    ...
    spi->FIFOCTL = (spi->FIFOCTL & ~(SPI_FIFOCTL_TXTH_Msk | SPI_FIFOCTL_RXTH_Msk)) |
                   (u32TxLevel << SPI_FIFOCTL_TXTH_Pos) |
                   (u32RxLevel << SPI_FIFOCTL_RXTH_Pos);
