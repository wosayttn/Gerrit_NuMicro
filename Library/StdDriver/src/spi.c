...
/**
 * @brief      Check if the SPI module is valid.
 *
 * @param[in]  spi is the base address of SPI module.
 *
 * @return     1 if the SPI module is valid; otherwise return 0.
 */
static uint32_t SPI_IsValidModule(const SPI_T *spi)
{
    return (((spi == SPI0) || (spi == SPI1) ||
             (spi == SPI2) || (spi == SPI3)) ? 1UL : 0UL);
}

/**
 * @brief      Check if the SPI module is operating in master mode or in slave mode.
 *
 * @param[in]  u32MasterSlave is the operating mode of SPI module. Valid values are:
 *                            - \ref SPI_MASTER
 *                            - \ref SPI_SLAVE
 *
 * @return     1 if the SPI module is operating in master mode or in slave mode; otherwise return 0.
 */
...
/**
 * @brief      Check if the SPI module is operating in SPI_MODE_0, SPI_MODE_1, SPI_MODE_2 or SPI_MODE_3.
 *
 * @param[in]  u32SPIMode is the transfer timing of SPI module. Valid values are:
 *                        - \ref SPI_MODE_0
 *                        - \ref SPI_MODE_1
 *                        - \ref SPI_MODE_2
 *                        - \ref SPI_MODE_3
 *
 * @return     1 if the SPI module is operating in SPI_MODE_0, SPI_MODE_1, SPI_MODE_2 or SPI_MODE_3; otherwise return 0.
 */
...
/**
 * @brief      Wait until the SPI module is ready to be disabled.
 *
 * @param[in]  spi is the base address of SPI module.
 *
 * @return     SPI_OK if the SPI module is ready to be disabled; otherwise return SPI_ERR_TIMEOUT.
 */
...
/**
 * @brief       Enter SPI configuration mode.
 *
 * @param[in]   spi            is the base address of SPI module.
 * @param[out]  pu32WasEnabled Indicates whether the SPI module was enabled before entering configuration mode.
 *
 * @return      SPI_OK if the SPI module successfully entered configuration mode; otherwise return SPI_ERR_FAIL or SPI_ERR_TIMEOUT.
 */
...
/**
 * @brief      Exit SPI configuration mode.
 *
 * @param[in]  spi           is the base address of SPI module.
 * @param[in]  u32WasEnabled Indicates whether the SPI module was enabled before entering configuration mode.
 *
 * @return     None.
 */
...
uint32_t SPII2S_EnableMCLK(SPI_T *i2s, uint32_t u32BusClock)
{
    ...
    if ((SPI_IsValidModule(i2s) == 0UL) || (u32BusClock == 0UL))
    {
        return 0UL;
    }
