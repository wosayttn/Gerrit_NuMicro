...
uint32_t SPI_Open(SPI_T *spi, uint32_t u32MasterSlave, uint32_t u32SPIMode, uint32_t u32DataWidth, uint32_t u32BusClock)
{
    ...
    if (SPI_EnterConfigMode(spi, &u32Ctl) != SPI_OK)
    {
        return 0UL;
    }

    u32Ctl = ((u32DataWidthTmp << SPI_CTL_DWIDTH_Pos) | u32SPIMode | u32MasterSlave);
    ...
    else     /* For slave mode, force the SPI peripheral clock rate to equal APB clock rate. */
    {
        ...
        /* Get the PCLK clock frequency of SPI. */
...
void SPI_SetFIFO(SPI_T *spi, uint32_t u32TxThreshold, uint32_t u32RxThreshold)
{
    ...
    spi->FIFOCTL = (spi->FIFOCTL & ~(SPI_FIFOCTL_TXTH_Msk | SPI_FIFOCTL_RXTH_Msk)) |
                   ((u32TxThreshold & 0x7UL) << SPI_FIFOCTL_TXTH_Pos) |
                   ((u32RxThreshold & 0x7UL) << SPI_FIFOCTL_RXTH_Pos);
...
void SPI_ClearIntFlag(SPI_T *spi, uint32_t u32Mask)
{
    ...
    {
        u32ClearMask |= SPI_STATUS_UNITIF_Msk;
    }

    if (u32Mask & SPI_SSACT_INT_MASK)
    {
        u32ClearMask |= SPI_STATUS_SSACTIF_Msk;
    }

    if (u32Mask & SPI_SSINACT_INT_MASK)
    {
        u32ClearMask |= SPI_STATUS_SSINAIF_Msk;
    }

    if (u32Mask & SPI_SLVUR_INT_MASK)
    {
        u32ClearMask |= SPI_STATUS_SLVURIF_Msk;
    }

    if (u32Mask & SPI_SLVBE_INT_MASK)
    {
        u32ClearMask |= SPI_STATUS_SLVBEIF_Msk;
    }

    if (u32Mask & SPI_TXUF_INT_MASK)
    {
        u32ClearMask |= SPI_STATUS_TXUFIF_Msk;
    }

    if (u32Mask & SPI_FIFO_RXOV_INT_MASK)
    {
        u32ClearMask |= SPI_STATUS_RXOVIF_Msk;
    }

    if (u32Mask & SPI_FIFO_RXTO_INT_MASK)
    {
        u32ClearMask |= SPI_STATUS_RXTOIF_Msk;
    }
