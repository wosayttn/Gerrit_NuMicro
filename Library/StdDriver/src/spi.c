...
static int32_t SPI_IsValidModule(const SPI_T *spi)
{
    return (spi == SPI0) ? 1 : 0;
}
...
{
    if (SPI_IsValidModule(spi) && (u32WasEnabled != 0UL))
    {
        spi->CTL |= SPI_CTL_SPIEN_Msk;
    }
}
...
/**
  ...
  *          In Master mode, u32BusClock shall be non-zero. In Slave mode, u32BusClock is ignored and the SPI clock divider setting will be 0.
...
/**
  ...
  * @param[in]  spi The pointer of the specified SPI module.
    * @param[in]  u32TxThreshold Decides the TX FIFO threshold. Valid range is 0 ~ 7.
    * @param[in]  u32RxThreshold Decides the RX FIFO threshold. Valid range is 0 ~ 7.
    * @details Set TX FIFO threshold and RX FIFO threshold configurations.
    *          Values outside the field width are clipped to the hardware threshold bits.
  */
