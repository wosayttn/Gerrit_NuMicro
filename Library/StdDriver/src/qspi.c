...
#define QSPI_MODE_MASK              (QSPI_CTL_CLKPOL_Msk | QSPI_CTL_RXNEG_Msk | QSPI_CTL_TXNEG_Msk)
#define QSPI_FIFO_TXTH_MAX          (QSPI_FIFOCTL_TXTH_Msk >> QSPI_FIFOCTL_TXTH_Pos)
#define QSPI_FIFO_RXTH_MAX          (QSPI_FIFOCTL_RXTH_Msk >> QSPI_FIFOCTL_RXTH_Pos)

static int32_t QSPI_IsValidModule(const QSPI_T *qspi)
{
    return (qspi == QSPI0) ? 1 : 0;
}
...
{
    uint32_t u32WasEnabled = 0UL;
    uint32_t u32PinMask = (u32SSPinMask & QSPI_SSCTL_SS_Msk);
    uint32_t u32ActLevel = (u32ActiveLevel & QSPI_SSCTL_SSACTPOL_Msk);
    ...
    qspi->SSCTL = (qspi->SSCTL & (~(QSPI_SSCTL_AUTOSS_Msk | QSPI_SSCTL_SSACTPOL_Msk | QSPI_SSCTL_SS_Msk))) |
                  (u32PinMask | u32ActLevel | QSPI_SSCTL_AUTOSS_Msk);
