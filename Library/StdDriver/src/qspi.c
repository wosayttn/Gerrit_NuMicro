...
uint32_t QSPI_GetStatus(const QSPI_T *qspi, uint32_t u32Mask)
{
    ...
    /* Check RX empty flag */
    if (((u32Mask & QSPI_RX_EMPTY_MASK) != 0U) &&
            ((u32Status & QSPI_STATUS_RXEMPTY_Msk) != 0U))
    {
        u32Flag |= QSPI_RX_EMPTY_MASK;
    }

    /* Check RX full flag */
    if (((u32Mask & QSPI_RX_FULL_MASK) != 0U) &&
            ((u32Status & QSPI_STATUS_RXFULL_Msk) != 0U))
    {
        u32Flag |= QSPI_RX_FULL_MASK;
    }

    /* Check TX empty flag */
    if (((u32Mask & QSPI_TX_EMPTY_MASK) != 0U) &&
            ((u32Status & QSPI_STATUS_TXEMPTY_Msk) != 0U))
    {
        u32Flag |= QSPI_TX_EMPTY_MASK;
    }

    /* Check TX full flag */
    if (((u32Mask & QSPI_TX_FULL_MASK) != 0U) &&
            ((u32Status & QSPI_STATUS_TXFULL_Msk) != 0U))
    {
        u32Flag |= QSPI_TX_FULL_MASK;
    }

    /* Check TX/RX reset flag */
    if (((u32Mask & QSPI_TXRX_RESET_MASK) != 0U) &&
            ((u32Status & QSPI_STATUS_TXRXRST_Msk) != 0U))
    {
        u32Flag |= QSPI_TXRX_RESET_MASK;
    }

    /* Check QSPIEN flag */
    if (((u32Mask & QSPI_QSPIEN_STS_MASK) != 0U) &&
            ((u32Status & QSPI_STATUS_QSPIENSTS_Msk) != 0U))
    {
        u32Flag |= QSPI_QSPIEN_STS_MASK;
    }

    /* Check QSPIx_SS line status */
    if (((u32Mask & QSPI_SSLINE_STS_MASK) != 0U) &&
            ((u32Status & QSPI_STATUS_SSLINE_Msk) != 0U))
    {
