...
uint32_t QSPI_SetBusClock(QSPI_T *qspi, uint32_t u32BusClock)
{
    ...
    else
    {
        ...
        u32Div = (uint32_t)(((((uint32_t)u32ClkSrc * 10ULL) / ((uint32_t)u32BusClock + 5ULL)) / 10ULL) - 1ULL); /* Round to the nearest integer */
...
void QSPI_SetFIFO(QSPI_T *qspi, uint32_t u32TxThreshold, uint32_t u32RxThreshold)
{
    ...
    qspi->FIFOCTL = (qspi->FIFOCTL & ~(QSPI_FIFOCTL_TXTH_Msk | QSPI_FIFOCTL_RXTH_Msk)) |
                    ((u32TxThreshold & 0x7UL) << QSPI_FIFOCTL_TXTH_Pos) |
                    ((u32RxThreshold & 0x7UL) << QSPI_FIFOCTL_RXTH_Pos);
...
void QSPI_ClearIntFlag(QSPI_T *qspi, uint32_t u32Mask)
{
    ...
    {
        u32ClearMask |= QSPI_STATUS_UNITIF_Msk;
    }

    if (u32Mask & QSPI_SSACT_INT_MASK)
    {
        u32ClearMask |= QSPI_STATUS_SSACTIF_Msk;
    }

    if (u32Mask & QSPI_SSINACT_INT_MASK)
    {
        u32ClearMask |= QSPI_STATUS_SSINAIF_Msk;
    }

    if (u32Mask & QSPI_SLVTO_INT_MASK)
    {
        u32ClearMask |= QSPI_STATUS_SLVTOIF_Msk;
    }

    if (u32Mask & QSPI_SLVUR_INT_MASK)
    {
        u32ClearMask |= QSPI_STATUS_SLVURIF_Msk;
    }

    if (u32Mask & QSPI_SLVBE_INT_MASK)
    {
        u32ClearMask |= QSPI_STATUS_SLVBEIF_Msk;
    }

    if (u32Mask & QSPI_TXUF_INT_MASK)
    {
        u32ClearMask |= QSPI_STATUS_TXUFIF_Msk;
    }

    if (u32Mask & QSPI_FIFO_RXOV_INT_MASK)
    {
        u32ClearMask |= QSPI_STATUS_RXOVIF_Msk;
    }

    if (u32Mask & QSPI_FIFO_RXTO_INT_MASK)
    {
        u32ClearMask |= QSPI_STATUS_RXTOIF_Msk;
    }
