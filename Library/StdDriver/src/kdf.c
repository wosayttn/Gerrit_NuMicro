...
int32_t  g_KDF_i32ErrCode = (int32_t)eKDF_ERRCODE_SUCCESS;   /*!< KDF global error code */
...
int32_t KDF_GetKeyBitSize(uint32_t u32KeySizeSel)
{
    ...
    {
        return (int32_t)eKDF_ERRCODE_INVALID_PARAM;
    }
...
int32_t KDF_DeriveKey(E_KDF_MODE eMode, uint32_t u32DeriveKeyParam, uint32_t u32KeyBitSize, uint32_t *pu32KeyOut)
{
    ...
    while (i32LeftKeyBitSize > 0)
    {
        ...
        while (KDF->STS & KDF_STS_HMACBUSY_Msk)
        {
            ...
            {
                return (int32_t)eKDF_ERRCODE_TIMEOUT;
            }
            u32TimeOutCount--;
        }
        ...
        {
            return (int32_t)eKDF_ERRCODE_FAIL;
        }
    ...
    return (int32_t)eKDF_ERRCODE_SUCCESS;
}
