...
{
    const uint16_t au16CntTbl[21] = { 4, 6, 6, 7, 8, 8, 8, 9, 12, 13, 16, 17, 18, 0, 0, 0, 32, 48, 64, 96, 128 };
    uint32_t u32SizeIdx = (u32Meta & KS_METADATA_SIZE_Msk) >> KS_METADATA_SIZE_Pos;

    if (u32SizeIdx >= (sizeof(au16CntTbl) / sizeof(au16CntTbl[0])))
    {
        return 0xFFFFFFFFUL;
    }

    return au16CntTbl[u32SizeIdx];
}
