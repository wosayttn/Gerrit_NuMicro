...
void ACMP_Calibration(ACMP_T *acmp)
{
    ...
    if (((acmp->CALSTS & ACMP_CALSTS_DONE0_Msk) == 0) || ((acmp->CALSTS & ACMP_CALSTS_DONE1_Msk) == 0))
    {
        uint32_t u32Delay = 0;
        ...
        while ((acmp->CALSTS & ACMP_CALSTS_DONE0_Msk) == 0)   /* Wait calibration finish */
        {
            if (--u32Delay == 0)
            {
        ...
        while ((acmp->CALSTS & ACMP_CALSTS_DONE1_Msk) == 0)  /* Wait calibration finish */
        {
            if (--u32Delay == 0)
            {
                g_ACMP_i32ErrCode = ACMP_TIMEOUT_ERR;
