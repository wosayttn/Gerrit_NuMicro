...
/**
  ...
  * @param[in]  u32Addr   Address of the flash location to be read.
  *             It must be a double-word aligned address.
  * @param[out] au32Data[0]  holds data read from flash address u32Addr.
  * @param[out] au32Data[1]  holds data read from flash address u32Addr + 4.
  * @param[out] au32Data[2]  If ECC error happened, au32Data[2] holds the bit number of error bit.
  * @return   FMC_OK:          Success
  * @return   FMC_ERR_TIMEOUT: Read time-out
  * @return   FMC_ERR_FAIL:    Read failed
    ...
    else
    {
        for (i = 0UL; i < u32Count; i++)
        {
            if (i < FMC_CONT_CONFIG_CNT)
                ...
                au32Config[i] = FMC_Read(FMC_USER_CONFIG_16 + ((i - FMC_CONT_CONFIG_CNT) << 2));
