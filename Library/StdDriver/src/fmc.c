...
int32_t FMC_GetXOMState(uint32_t u32XomNum)
{
    ...
    {
        i32RetCode = (int32_t)(((FMC->XOMSTS & 0xFUL) & (1UL << u32XomNum)) >> u32XomNum);
    }
...
int32_t FMC_WriteConfig(uint32_t u32ConfigAddr, uint32_t u32ConfigVal)
{
    ...
    if ((i32RetCode = FMC_EraseConfig(u32ConfigAddr)) != FMC_OK)
    {
        g_FMC_i32ErrCode = i32RetCode;
    ...
    if((i32RetCode = FMC_Write(u32ConfigAddr, u32ConfigVal)) != FMC_OK)
    {
        g_FMC_i32ErrCode = i32RetCode;
        goto Exit_FMC_WriteConfig;
    }
