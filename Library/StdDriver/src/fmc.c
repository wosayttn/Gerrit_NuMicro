...
{
    uint32_t u32Addr;
    int32_t i32Active, i32Ret = FMC_OK;
    int32_t i32TimeOutCnt;
...
int32_t FMC_GetXOMState(uint32_t u32XomNum)
{
    ...
    {
        i32Ret = (int32_t)(((FMC->XOMSTS & 0xFUL) & (1UL << u32XomNum)) >> u32XomNum);
    }
...
int32_t FMC_ConfigXOM(uint32_t u32XomNum, uint32_t u32XomBase, uint8_t u8XomPage)
{
    ...
    if (FMC_GetXOMState(u32XomNum) == XOMR_ACTIVE)
    {
        return XOMR_ACTIVE;
    }
...
/**
  ...
  * @param[out] pu32Data0  Place holder of word read from flash address u32Addr.
  * @param[out] pu32Data1  Place holder of word read from flash address u32Addr+4.
...
int32_t FMC_Read_64_ECC(uint32_t u32Addr, uint32_t au32Data[])
{
    ...
    else
    {
        ...
        if (FMC->ECCSTS != 0)
        {
            g_FMC_i32ErrCode = FMC_ERR_ECC;
            i32Ret = FMC_ERR_ECC;

            if (FMC->MPDATE != 0)   // Return ECC correction bit
            {
                uint32_t i;
...
/**
  ...
  * @param[out]  pu32Config A word array.
  *              pu32Config[0] holds CONFIG0, while pu32Config[1] holds CONFIG1.
  ...
  *           -2  Invalid parameter
  */
int32_t FMC_ReadConfig(uint32_t pu32Config[], uint32_t u32Count)
{
    int32_t  i32Ret = FMC_OK;
    uint32_t i;
    ...
    else
    {
        ...
        {
            if (i < FMC_CONT_CONFIG_CNT)
                pu32Config[i] = FMC_Read(FMC_CONFIG_BASE + (i << 2));
            else
                pu32Config[i] = FMC_Read(FMC_USER_CONFIG_16 + ((i - 16) << 2));
...
int32_t FMC_WriteConfig(uint32_t u32ConfigAddr, uint32_t u32ConfigVal)
{
    ...
    if ((i32Ret = FMC_EraseConfig(u32ConfigAddr)) != FMC_OK)
    {
        g_FMC_i32ErrCode = i32Ret;
        goto Exit_FMC_WriteConfig;
    }
    ...
    if((i32Ret = FMC_Write(u32ConfigAddr, u32ConfigVal)) != FMC_OK)
    {
        g_FMC_i32ErrCode = i32Ret;
        goto Exit_FMC_WriteConfig;
    }
