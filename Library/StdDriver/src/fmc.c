...
static uint32_t FMC_GetAPROMSize(void)
{
    ...
    {
        0x20000UL,    /* DID bits[7:4] N1 field: 0x5 */
        0x40000UL,    /* DID bits[7:4] N1 field: 0x7 */
        0x80000UL,    /* DID bits[7:4] N1 field: 0x9 */
        0x100000UL    /* DID bits[7:4] N1 field: 0xB */
    };

    u32DID = FMC_ReadDID();

    if (u32DID == 0xFFFFFFFFUL)
    {
        /* Read DID failed, cannot determine APROM size. */
        return 0UL;
    }

    u32DID_N1 = (u32DID & 0x000000F0UL) >> 4UL;

    if ((u32DID_N1 < 0x5UL) || (u32DID_N1 > 0xBUL) || ((u32DID_N1 & 0x1UL) == 0UL))
    {
        /* Invalid DID N1 value, cannot determine APROM size. */
        return 0UL;
    }

    u32Idx = (u32DID_N1 - 5UL) / 2UL;
...
int32_t FMC_EraseXOM(uint32_t u32XomNum)
{
    ...
    if (i32Active != 0)
    {
        ...
        {
            case 0U:
                u32Addr = (FMC->XOMR0STS & 0xFFFFFF00UL) >> 8UL;
                break;

            default:
                u32Addr = (FMC->XOMR1STS & 0xFFFFFF00UL) >> 8UL;
                break;
        }
...
int32_t FMC_GetXOMState(uint32_t u32XomNum)
{
    ...
    {
        if (((FMC->XOMSTS & 0xFUL) & (1UL << u32XomNum)) == (1UL << u32XomNum))
        {
            return (int32_t)XOMR_ACTIVE;
        }
        else
        {
            return (int32_t)XOMR_INACTIVE;
        }
    }
...
int32_t FMC_ConfigXOM(uint32_t u32XomNum, uint32_t u32XomBase, uint8_t u8XomPage)
{
    ...
    FMC->ISPADDR = FMC_XOM_BASE + (u32XomNum * 0x10UL);
    ...
    {
        FMC->ISPCMD  = FMC_ISPCMD_PROGRAM;
        FMC->ISPADDR = FMC_XOM_BASE + ((u32XomNum * 0x10UL) + 0x04UL);
    ...
    {
        FMC->ISPCMD  = FMC_ISPCMD_PROGRAM;
        FMC->ISPADDR = FMC_XOM_BASE + ((u32XomNum * 0x10UL) + 0x08UL);
        FMC->ISPDAT  = 0u;
        FMC->ISPTRG  = FMC_ISPTRG_ISPGO_Msk;
        u32TimeOutCnt = FMC_TIMEOUT_WRITE;
...
int32_t FMC_Read_64_ECC(uint32_t u32Addr, uint32_t au32Data[])
{
    ...
    else
    {
        ...
        if (FMC->ECCSTS != 0UL)
        {
            ...
            if (FMC->MPDATE != 0UL)     // Return ECC correction bit
            {
                uint32_t i;
...
int32_t FMC_WriteConfig(uint32_t u32ConfigAddr, uint32_t u32ConfigVal)
{
    ...
    {
        if ((FMC_Read(u32ConfigAddr) == 0xFFFFFFFFUL) && (g_FMC_i32ErrCode == FMC_OK))
        {
...
uint32_t  FMC_GetChkSum(uint32_t u32Addr, uint32_t u32Count)
{
    ...
    if (((u32Addr % FMC_FLASH_PAGE_SIZE) != 0UL) || ((u32Count % FMC_FLASH_PAGE_SIZE) != 0UL))
    {
        g_FMC_i32ErrCode = FMC_ERR_INVALID_PARAM;
        return 0xFFFFFFFFUL;
    }
...
uint32_t  FMC_CheckAllOne(uint32_t u32Addr, uint32_t u32Count)
{
    ...
    u32TimeOutCnt0 = FMC_TIMEOUT_CHKALLONE;

    while ((FMC->ISPSTS & FMC_ISPSTS_ISPBUSY_Msk) != 0UL)
    {
        if (u32TimeOutCnt0 == 0UL)
        {
            g_FMC_i32ErrCode = FMC_ERR_TIMEOUT;
            break;
        }
    ...
    if (g_FMC_i32ErrCode == FMC_OK)
    {
        uint32_t u32TimeOutCnt1 = FMC_TIMEOUT_CHKALLONE;
...
int32_t FMC_EraseBank(uint32_t u32BankAddr)
{
    ...
    if ((u32BankAddr != FMC_APROM_BASE) && (u32BankAddr != (u32APROMSize / 2UL)))
    {
        g_FMC_i32ErrCode = FMC_ERR_INVALID_PARAM;
        return FMC_ERR_INVALID_PARAM;
    }
    ...
    for (u32Addr = u32BankAddr; u32Addr < (u32BankAddr + (u32APROMSize / 2UL)); u32Addr += FMC_FLASH_PAGE_SIZE)
    {
        uint32_t  u32TimeOutCnt;
