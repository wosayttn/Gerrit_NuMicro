...
NVT_ITCM void KPI_IRQHandler(void)
{
    ...
    if ((status & KPI_STATUS_KIF_Msk) != 0UL)
    {
        uint32_t i;
        uint32_t j;
        uint32_t idx;
        ...
        for (i = 0; i < row; i++)
        {
            for (j = 0; j < col; j++)
            {
                ...
                /* Key Release */
                if ((status & KPI_STATUS_KRIF_Msk) != 0UL)
                {
                    if ((u32KeyRelease[idx] & mask) != 0UL)
                    {
                        /* Clean event */
                        KPI->KRF[idx] = mask;
                        /* Record the key */
        ...
        for (i = 0; i < row; i++)
        {
            for (j = 0; j < col; j++)
            {
                ...
                /* Key Press */
                if ((status & KPI_STATUS_KPIF_Msk) != 0UL)
                {
                    if ((u32KeyPress[idx] & mask) != 0UL)
                    {
                        /* Clean event */
                        KPI->KPF[idx] = mask;
                        /* Record the key */
...
{
    uint32_t u32FirstKey = s_u32FirstKey;

    if (u32FirstKey != s_u32LastKey)
    {
        return 1;
    }
...
{
    uint32_t u32LastKey = s_u32LastKey;
    KPI_KEY_T key = {0xff, 0xff, 0xffff};

    /* Check if queue is empty */
    if (s_u32FirstKey != u32LastKey)
    {
        uint32_t u32MaxKeyCnt = s_u32MaxKeyCnt;
        /* Pop the key from queue */
        key = s_pKeyQueue[s_u32FirstKey];
        s_u32FirstKey++;

        /* Wrap around check */
        if (s_u32FirstKey >= u32MaxKeyCnt)
        {
            s_u32FirstKey = 0;
        }
    }
