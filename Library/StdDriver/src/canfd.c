...
void CANFD_InitTxDBuf(CANFD_T *psCanfd, const CANFD_RAM_PART_T *psRamConfig, const CANFD_ELEM_SIZE_T *psElemSize, E_CANFD_DATA_FIELD_SIZE eTxBufSize);
...
uint32_t CANFD_GetNominalBitRate(const CANFD_T *psCanfd)
{
    ...
    if (psCanfd == (CANFD_T *)CANFD0)
    {
        u32CanfdDiv = ((CLK->CLKDIV4 & CLK_CLKDIV4_CANFD0DIV_Msk) >> CLK_CLKDIV4_CANFD0DIV_Pos) + 1UL;
    }
    else
    {
        return 0UL;
...
static void CANFD_CalculateRamAddress(CANFD_RAM_PART_T *psConfigAddr, const CANFD_ELEM_SIZE_T *psConfigSize)
{
    ...
    if (psConfigSize->u32SIDFC > 0U)
    {
        psConfigAddr->u32SIDFC_FLSSA = 0;
        u32RamAddrOffset += (psConfigSize->u32SIDFC * (uint32_t)sizeof(CANFD_STD_FILTER_T));
    }

    /* Get the Extend Message ID Filter element address */
    if (psConfigSize->u32XIDFC > 0U)
    {
        psConfigAddr->u32XIDFC_FLESA = u32RamAddrOffset;
        u32RamAddrOffset += (psConfigSize->u32XIDFC * (uint32_t)sizeof(CANFD_EXT_FILTER_T));
    }

    /* Get the Tx Buffer element address */
    if (psConfigSize->u32TxBuf > 0U)
    {
        psConfigAddr->u32TXBC_TBSA = u32RamAddrOffset;
        u32RamAddrOffset += (psConfigSize->u32TxBuf * (uint32_t)sizeof(CANFD_BUF_T));
    }

    /* Get the Rx Buffer element address */
    if (psConfigSize->u32RxBuf > 0U)
    {
        psConfigAddr->u32RXBC_RBSA = u32RamAddrOffset;
        u32RamAddrOffset += (psConfigSize->u32RxBuf * (uint32_t)sizeof(CANFD_BUF_T));
    }

    /* Get the Rx FIFO0 element address */
    if (psConfigSize->u32RxFifo0 > 0U)
    {
        psConfigAddr->u32RXF0C_F0SA = u32RamAddrOffset;
        u32RamAddrOffset += (psConfigSize->u32RxFifo0 * (uint32_t)sizeof(CANFD_BUF_T));
    }

    /* Get the Rx FIFO1 element address */
    if (psConfigSize->u32RxFifo1 > 0U)
    {
        psConfigAddr->u32RXF1C_F1SA = u32RamAddrOffset;
        u32RamAddrOffset += (psConfigSize->u32RxFifo1 * (uint32_t)sizeof(CANFD_BUF_T));
    }

    /* Get the Rx FIFO1 element address */
    if (psConfigSize->u32TxEventFifo > 0U)
    {
...
static void CANFD_SetTimingConfig(CANFD_T *psCanfd, const CANFD_TIMEING_CONFIG_T *psConfig)
{
    ...
    if (psCanfd == (CANFD_T *)CANFD0)
    {
        uint32_t u32Canfd0PreDivider = (uint32_t)psConfig->u8PreDivider;

        /* Get CANF D0 clock divider number */
        CLK->CLKDIV4 = (CLK->CLKDIV4 & ~CLK_CLKDIV4_CANFD0DIV_Msk) | ((u32Canfd0PreDivider - 1UL) << CLK_CLKDIV4_CANFD0DIV_Pos);
    }

    /* nominal bit rate */
    psCanfd->NBTP = ((((uint32_t)psConfig->u8NominalRJumpwidth & 0x7FUL) - 1UL) << 25U) |
                    ((((uint32_t)psConfig->u16NominalPrescaler & 0x1FFUL) - 1UL) << 16U) |
                    (((((uint32_t)psConfig->u8NominalPhaseSeg1 + (uint32_t)psConfig->u8NominalPropSeg) & 0xFFUL) - 1UL) << 8U) |
                    ((((uint32_t)psConfig->u8NominalPhaseSeg2 & 0x7FUL) - 1UL) << 0U);
    ...
    if (psCanfd->CCCR & CANFD_CCCR_FDOE_Msk)
    {
        ...
        psCanfd->DBTP = ((((uint32_t)psConfig->u8DataPrescaler & 0x1FUL) - 1UL) << 16U) |
                        (((((uint32_t)psConfig->u8DataPhaseSeg1 + (uint32_t)psConfig->u8DataPropSeg) & 0x1FUL) - 1UL) << 8U) |
                        ((((uint32_t)psConfig->u8DataPhaseSeg2 & 0xFUL) - 1UL) << 4U) |
                        ((((uint32_t)psConfig->u8DataRJumpwidth & 0xFUL) - 1UL) << 0U);
...
static void CANFD_GetSegments(uint32_t u32NominalBaudRate, uint32_t u32DataBaudRate, uint32_t u32Ntq, uint32_t u32Dtq, CANFD_TIMEING_CONFIG_T *psConfig)
{
    ...
    psConfig->u8NominalPhaseSeg1 = (uint8_t)(u32P1 - 1U);
    psConfig->u8NominalPhaseSeg2 = (uint8_t)(u32Ntq - u32P1);
    /* sjw is 20% of total TQ, rounded to nearest int */
    psConfig->u8NominalRJumpwidth = (uint8_t)((u32Ntq + (5U - 1U)) / 5U);
    ...
    if (u32Dtq > 0U)
    {
        ...
        /* subtract one TQ for sync seg */
        psConfig->u8DataPhaseSeg1 = (uint8_t)(u32P1 - 1U);
        psConfig->u8DataPhaseSeg2 = (uint8_t)(u32Dtq - u32P1);
        /* sjw is 20% of total TQ, rounded to nearest int */
        psConfig->u8DataRJumpwidth = (uint8_t)((u32Dtq + (5U - 1U)) / 5U);
    }
    else
    {
        psConfig->u8DataPropSeg = 0U;
...
static uint32_t CANFD_CalculateTimingValues(const CANFD_T *psCanfd, uint32_t u32NominalBaudRate, uint32_t u32DataBaudRate, uint32_t u32SourceClock_Hz, CANFD_TIMEING_CONFIG_T *psConfig)
{
    uint32_t u32Ntq;
    uint32_t u32Dtq;
    uint32_t u32NominalBaudRateCalc = u32NominalBaudRate;

    /* observe baud rate maximums */
    if (u32NominalBaudRateCalc > MAX_NOMINAL_BAUDRATE)
    {
        u32NominalBaudRateCalc = MAX_NOMINAL_BAUDRATE;
    }

    for (u32Ntq = MAX_TIME_QUANTA; u32Ntq >= MIN_TIME_QUANTA; u32Ntq--)
    {
        uint32_t u32Nclk = u32NominalBaudRateCalc * u32Ntq;
        uint32_t u32NominalPrescaler;

        for (u32NominalPrescaler = 1U; u32NominalPrescaler <= 0x400U; u32NominalPrescaler++)
        {
            uint32_t u32Nclk2 = u32Nclk * u32NominalPrescaler;

            if (((u32SourceClock_Hz / u32Nclk2) <= 4U) && ((u32SourceClock_Hz % u32Nclk2) == 0U))
            {
                ...
                if ((psCanfd->CCCR & CANFD_CCCR_FDOE_Msk) != 0UL)
                {
                    ...
                    if (u32DataBaudRate == 0U)
                    {
                        u32Dtq = 0U;
                        psConfig->u8DataPrescaler = 0U;
                        CANFD_GetSegments(u32NominalBaudRateCalc, u32DataBaudRate, u32Ntq, u32Dtq, psConfig);
                        return TRUE;
                    }

                    /* if baudrates are the same and the solution for nominal will work for
                    data, then use the nominal settings for both */
                    if ((u32DataBaudRate == u32NominalBaudRateCalc) && (u32NominalPrescaler <= 0x20U))
                    {
                        u32Dtq = u32Ntq;
                    ...
                    for (u32Dtq = MAX_TIME_QUANTA; u32Dtq >= MIN_TIME_QUANTA; u32Dtq--)
                    {
                        uint32_t u32Dclk = u32DataBaudRate * u32Dtq;
                        uint32_t u32DataPrescaler;

                        for (u32DataPrescaler = 1U; u32DataPrescaler <= 0x20U; u32DataPrescaler++)
                        {
                            uint32_t u32Dclk2 = u32Dclk * u32DataPrescaler;

                            if (u32SourceClock_Hz == (u32Dclk2 * (uint32_t)psConfig->u8PreDivider))
                            {
                ...
                {
                    CANFD_GetSegments(u32NominalBaudRateCalc, 0U, u32Ntq, 0U, psConfig);
                    return TRUE;
                }
...
void CANFD_Open(CANFD_T *psCanfd, CANFD_FD_T *psCanfdStr)
{
    ...
    if ((SYS->PDID == 0x00F250E0UL) || (SYS->PDID == 0x00F25000UL) || (SYS->PDID == 0x00F25001UL))
    {
        g_u32Workaround = 1UL;
    }
    ...
    if (psCanfdStr->sElemSize.u32TxEventFifo != 0U)
    {
...
uint32_t CANFD_TransmitDMsg(CANFD_T *psCanfd, uint32_t u32TxBufIdx, CANFD_FD_MSG_T *psTxMsg)
{
    ...
    if (CANFD_ReadReg(&(psCanfd->TXBRP)) & (1UL << u32TxBufIdx))
    {
        return 0;
    }

    psTxBuffer = (CANFD_BUF_T *)(CANFD_SRAM_BASE_ADDR(psCanfd) + (CANFD_ReadReg(&(psCanfd->TXBC)) & 0xFFFFUL) + (u32TxBufIdx * (uint32_t)sizeof(CANFD_BUF_T)));
    ...
    {
        psTxBuffer->u32Id = (psTxMsg->u32Id & 0x7FFUL) << 18U;
    }
    ...
    psTxBuffer->u32Config = ((uint32_t)CANFD_EncodeDLC(psTxMsg->u32DLC) << 16U);
    ...
    for (u32Idx = 0U; u32Idx < ((psTxMsg->u32DLC + (4UL - 1UL)) / 4UL); u32Idx++)
    {
    ...
    switch (u32RxFifoNum)
    {
        case 0:
            if (psElemSize-> u32RxFifo0)
            {
                ...
                (void)memset((uint32_t *)(u32Address), 0x00, (u32Size * 4U * psElemSize->u32RxFifo0));
            }
            else
            {
                psCanfd->RXF0C = 0;
        ...
        case 1:
            if (psElemSize-> u32RxFifo1)
            {
                ...
                /*Clear the RX FIFO 0 Memory*/
                (void)memset((uint32_t *)(u32Address), 0x00, (u32Size * 4U * psElemSize->u32RxFifo1));
            }
            else
            {
                psCanfd->RXF1C = 0;
...
void CANFD_InitTxDBuf(CANFD_T *psCanfd, const CANFD_RAM_PART_T *psRamConfig, const CANFD_ELEM_SIZE_T *psElemSize, E_CANFD_DATA_FIELD_SIZE eTxBufSize)
{
    ...
    psCanfd->TXBC = (((psElemSize->u32TxBuf & 0x3FUL) << CANFD_TXBC_NDTB_Pos) | (psRamConfig->u32TXBC_TBSA & CANFD_TXBC_TBSA_Msk));
    ...
    (void)memset((uint32_t *)(u32Address), 0x00, (u32Size * 4U * psElemSize->u32TxBuf));
}
...
void CANFD_InitRxDBuf(CANFD_T *psCanfd, const CANFD_RAM_PART_T *psRamConfig, const CANFD_ELEM_SIZE_T *psElemSize, E_CANFD_DATA_FIELD_SIZE eRxBufSize)
{
    ...
    (void)memset((uint32_t *)(u32Address), 0x00, (u32Size * 4U * psElemSize->u32RxBuf));
}
...
void CANFD_ConfigSIDFC(CANFD_T *psCanfd, const CANFD_RAM_PART_T *psRamConfig, const CANFD_ELEM_SIZE_T *psElemSize)
{
    ...
    (void)memset((uint32_t *)(u32Address), 0x00, (psElemSize->u32SIDFC * 4U));
}
...
void CANFD_ConfigXIDFC(CANFD_T *psCanfd, const CANFD_RAM_PART_T *psRamConfig, const CANFD_ELEM_SIZE_T *psElemSize)
{
    ...
    /*Clear the Filter List Memory*/
    (void)memset((uint32_t *)(u32Address), 0x00, (psElemSize->u32XIDFC * 8U));
}
...
uint32_t CANFD_ReadRxBufMsg(CANFD_T *psCanfd, uint8_t u8MbIdx, CANFD_FD_MSG_T *psMsgBuf)
{
    ...
    if (u8MbIdx < CANFD_MAX_RX_BUF_ELEMS)
    {
        ...
        if (u8MbIdx < 32U)
        {
            newData = (CANFD_ReadReg(&psCanfd->NDAT1) >> u8MbIdx) & 1U;
        }
        else
        {
            newData = (CANFD_ReadReg(&psCanfd->NDAT2) >> (u8MbIdx - 32U)) & 1U;
        }
        ...
        if (newData)
        {
            ...
            psRxBuffer = (CANFD_BUF_T *)(CANFD_SRAM_BASE_ADDR(psCanfd) + (CANFD_ReadReg(&psCanfd->RXBC) & 0xFFFFUL) + ((uint32_t)u8MbIdx * (uint32_t)sizeof(CANFD_BUF_T)));
            ...
            {
                uint32_t u32MbIdxOffset = (uint32_t)u8MbIdx - 32UL;

                psCanfd->NDAT2 = CANFD_ReadReg(&psCanfd->NDAT2) | (1UL << u32MbIdxOffset);
            }
...
uint32_t CANFD_ReadRxFifoMsg(CANFD_T *psCanfd, uint8_t u8FifoIdx, CANFD_FD_MSG_T *psMsgBuf)
{
    ...
    if (u8FifoIdx < CANFD_NUM_RX_FIFOS)
    {
        ...
        if ((CANFD_ReadReg(pRXFS) & 0x7FUL) > 0U)
        {
            uint8_t GetIndex;
            CANFD_BUF_T *pRxBuffer;

            GetIndex = (uint8_t)((CANFD_ReadReg(pRXFS) >> 8U) & 0x3FUL);
            pRxBuffer = (CANFD_BUF_T *)(CANFD_SRAM_BASE_ADDR(psCanfd) + (CANFD_ReadReg(pRXFC) & 0xFFFFUL) + ((uint32_t)GetIndex * (uint32_t)sizeof(CANFD_BUF_T)));
...
void CANFD_CopyDBufToMsgBuf(CANFD_BUF_T *psRxBuf, CANFD_FD_MSG_T *psMsgBuf)
{
    ...
    {
        psMsgBuf->u32Id = (psRxBuf->u32Id  >> 18U) & 0x7FFUL;
...
void CANFD_CopyTxEvntFifoToUsrBuf(CANFD_T *psCanfd, uint32_t u32TxEvntNum, CANFD_TX_EVNT_ELEM_T *psTxEvntElem)
{
    ...
    if ((pu32TxEvnt[0] & TX_FIFO_E0_EVENT_XTD_Msk) > 0U)
    {
