...
{
    uint32_t u32NominalTseg1;
    uint32_t u32DataTseg1;
    ...
    u32NominalTseg1 = (((uint32_t)psConfig->u8NominalPhaseSeg1 + (uint32_t)psConfig->u8NominalPropSeg) & 0x000000FFUL);
    u32DataTseg1 = (((uint32_t)psConfig->u8DataPhaseSeg1 + (uint32_t)psConfig->u8DataPropSeg) & 0x0000001FUL);

    /* Nominal bit rate */
    psCanfd->NBTP = ((((uint32_t)psConfig->u8NominalRJumpwidth & 0x0000007FUL) - 1UL) << 25) |
                    ((((uint32_t)psConfig->u16NominalPrescaler & 0x000001FFUL) - 1UL) << 16) |
                    (((u32NominalTseg1 - 1UL) & 0x000000FFUL) << 8) |
                    ((((uint32_t)psConfig->u8NominalPhaseSeg2 & 0x0000007FUL) - 1UL) << 0);

    /* Data bit rate */
    if ((psCanfd->CCCR & CANFD_CCCR_FDOE_Msk) != 0UL)
    {
        psCanfd->DBTP = ((((uint32_t)psConfig->u8DataPrescaler & 0x0000001FUL) - 1UL) << 16) |
                        (((u32DataTseg1 - 1UL) & 0x0000001FUL) << 8) |
                        ((((uint32_t)psConfig->u8DataPhaseSeg2 & 0x0000000FUL) - 1UL) << 4) |
                        ((((uint32_t)psConfig->u8DataRJumpwidth & 0x0000000FUL) - 1UL) << 0);
    }
}
...
static uint32_t CANFD_CalculateTimingValues(const CANFD_T *psCanfd, uint32_t u32NominalBaudRate, uint32_t u32DataBaudRate, uint32_t u32SourceClock_Hz, CANFD_TIMEING_CONFIG_T *psConfig)
{
    ...
    for (u32Ntq = (uint32_t)MAX_TIME_QUANTA; u32Ntq >= (uint32_t)MIN_TIME_QUANTA; u32Ntq--)
    {
        ...
        for (u16NominalPrescaler = (uint16_t)1U; u16NominalPrescaler <= (uint16_t)0x400U; u16NominalPrescaler++)
        {
            uint32_t u32Nclk2;

            psConfig->u16NominalPrescaler = u16NominalPrescaler;
            ...
            if (((u32SourceClock_Hz / u32Nclk2) <= 5UL) && ((u32SourceClock_Hz % u32Nclk2) == 0UL))
            {
                ...
                if ((psCanfd->CCCR & CANFD_CCCR_FDOE_Msk) != 0UL)
                {
                    ...
                    for (u32Dtq = (uint32_t)MAX_TIME_QUANTA; u32Dtq >= (uint32_t)MIN_TIME_QUANTA; u32Dtq--)
                    {
                        ...
                        for (u8DataPrescaler = (uint8_t)1U; u8DataPrescaler <= (uint8_t)0x20U; u8DataPrescaler++)
                        {
                            uint32_t u32Dclk2;

                            psConfig->u8DataPrescaler = u8DataPrescaler;
...
void CANFD_Open(CANFD_T *psCanfd, CANFD_FD_T *psCanfdStr)
{
    ...
    if (psCanfdStr->sBtConfig.bEnableLoopBack != 0U)
    {
...
uint32_t CANFD_ReadRxFifoMsg(CANFD_T *psCanfd, uint8_t u8FifoIdx, CANFD_FD_MSG_T *psMsgBuf)
{
    ...
    if (u8FifoIdx < (uint8_t)CANFD_NUM_RX_FIFOS)
    {
        ...
        if ((*pRXFS & 0x7FUL) > 0UL)
        {
            ...
            pRxBuffer = (CANFD_BUF_T *)(CANFD_GetSramBaseAddress(psCanfd) + (*pRXFC & 0xFFFFUL) + (GetIndex * sizeof(CANFD_BUF_T)));
