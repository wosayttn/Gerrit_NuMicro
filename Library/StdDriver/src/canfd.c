...
/**
    * @brief      Returns CAN FD message RAM base address.
    *
    * @param[in]  psCanfd      The pointer of CAN FD module.
    *
    * @return     Message RAM base address.
    */
static uint32_t CANFD_GetSramBaseAddress(const CANFD_T *psCanfd)
{
	uintptr_t uRamBaseAddr;

	uRamBaseAddr = (uintptr_t)psCanfd;
	uRamBaseAddr += (uintptr_t)CANFD_SRAM_OFFSET;
	return (uint32_t)uRamBaseAddr;
}
...
void CANFD_GetDefaultConfig(CANFD_FD_T *psConfig, uint8_t u8OpMode)
{
    ...
    if (u8OpMode == (uint8_t)CANFD_OP_CAN_MODE)
    {
        ...
        psConfig->sBtConfig.bFDEn = (uint8_t)FALSE;
        psConfig->sBtConfig.bBitRateSwitch = (uint8_t)FALSE;
    }
    else
    {
        psConfig->sBtConfig.sDataBitRate.u32BitRate = 1000000;
        psConfig->sBtConfig.bFDEn = (uint8_t)TRUE;
        psConfig->sBtConfig.bBitRateSwitch = (uint8_t)TRUE;
    }

    /*Disable the Internal Loopback mode */
    psConfig->sBtConfig.bEnableLoopBack = (uint8_t)FALSE;
...
static void CANFD_SetTimingConfig(CANFD_T *psCanfd, const CANFD_TIMEING_CONFIG_T *psConfig)
{
    ...
    psCanfd->NBTP = (((psConfig->u8NominalRJumpwidth & 0x7FUL) - 1UL) << 25) +
                    ...
                    (((((uint32_t)psConfig->u8NominalPhaseSeg1 + (uint32_t)psConfig->u8NominalPropSeg) & 0xFFUL) - 1UL) << 8) +
    ...
    if ((psCanfd->CCCR & CANFD_CCCR_FDOE_Msk) != 0UL)
    {
        ...
        psCanfd->DBTP = (((psConfig->u8DataPrescaler & 0x1FUL) - 1UL) << 16) +
                (((((uint32_t)psConfig->u8DataPhaseSeg1 + (uint32_t)psConfig->u8DataPropSeg) & 0x1FUL) - 1UL) << 8) +
...
static void CANFD_GetSegments(uint32_t u32NominalBaudRate, uint32_t u32DataBaudRate, uint32_t u32Ntq, uint32_t u32Dtq, CANFD_TIMEING_CONFIG_T *psConfig)
{
    ...
    psConfig->u8NominalRJumpwidth = (uint8_t)((u32Ntq + 4UL) / 5UL);
    ...
    if (u32Dtq > 0UL)
    {
        ...
        /* Distribute time quanta */
        u32P1 = (u32Dtq * u32IdealSpPermille) / 1000UL;
        /* CAN controller doesn't separate prop seg and phase seg 1 */
        psConfig->u8DataPropSeg = 0U;
        /* Subtract one TQ for sync seg */
        psConfig->u8DataPhaseSeg1 = (uint8_t)(u32P1 - 1UL);
        psConfig->u8DataPhaseSeg2 = (uint8_t)(u32Dtq - u32P1);
        /* sjw is 20% of total TQ, rounded to nearest int */
        psConfig->u8DataRJumpwidth = (uint8_t)((u32Dtq + 4UL) / 5UL);
    }
    else
    {
        psConfig->u8DataPropSeg = 0U;
        psConfig->u8DataPhaseSeg1 = 0U;
        psConfig->u8DataPhaseSeg2 = 0U;
        psConfig->u8DataRJumpwidth = 0U;
    }
}
...
static uint32_t CANFD_CalculateTimingValues(const CANFD_T *psCanfd, uint32_t u32NominalBaudRate, uint32_t u32DataBaudRate, uint32_t u32SourceClock_Hz, CANFD_TIMEING_CONFIG_T *psConfig)
{
    uint32_t u32Ntq;
    uint32_t u32Dtq;
    uint32_t u32NominalPrescaler;
    uint32_t u32DataPrescaler;
    ...
    for (u32Ntq = (uint32_t)MAX_TIME_QUANTA; u32Ntq >= (uint32_t)MIN_TIME_QUANTA; u32Ntq--)
    {
        ...
        for (u32NominalPrescaler = 0x0001U; u32NominalPrescaler <= 0x400U; u32NominalPrescaler++)
        {
            uint32_t u32Nclk2;

            psConfig->u16NominalPrescaler = (uint16_t)u32NominalPrescaler;

            u32Nclk2 = u32Nclk * u32NominalPrescaler;

            if ((u32Nclk2 != 0UL) && ((u32SourceClock_Hz / u32Nclk2) <= 256UL) && ((u32SourceClock_Hz % u32Nclk2) == 0UL))
            {
                psConfig->u8PreDivider = (uint8_t)(u32SourceClock_Hz / u32Nclk2);

                /*FD Operation enabled*/
                if ((psCanfd->CCCR & CANFD_CCCR_FDOE_Msk) != 0UL)
                {
                    /* if not using baudrate switch then we are done */
                    if (u32DataBaudRate == 0UL)
                    {
                        u32Dtq = 0;
                    ...
                    if ((u32DataBaudRate == u32NominalBaudRateTemp) && (u32NominalPrescaler <= 0x20U))
                    {
                        u32Dtq = u32Ntq;
                        psConfig->u8DataPrescaler = (uint8_t)u32NominalPrescaler;
                        CANFD_GetSegments(u32NominalBaudRateTemp, u32DataBaudRate, u32Ntq, u32Dtq, psConfig);
                        return TRUE;
                    }
                    ...
                    for (u32Dtq = (uint32_t)MAX_TIME_QUANTA; u32Dtq >= (uint32_t)MIN_TIME_QUANTA; u32Dtq--)
                    {
                        ...
                        for (u32DataPrescaler = 0x01U; u32DataPrescaler <= 0x20U; u32DataPrescaler++)
                        {
                            uint32_t u32Dclk2;

                            psConfig->u8DataPrescaler = (uint8_t)u32DataPrescaler;

                            u32Dclk2 = u32Dclk * u32DataPrescaler;
...
void CANFD_Open(CANFD_T *psCanfd, CANFD_FD_T *psCanfdStr)
{
    ...
    if (psCanfdStr->sBtConfig.bBitRateSwitch != 0UL)
    {
        /* enable FD and baud-rate switching */
        psCanfd->CCCR |= CANFD_CCCR_BRSE_Msk;
    }

    if (psCanfdStr->sBtConfig.bFDEn != 0UL)
    {
        /*FD Operation enabled*/
    ...
    if (CANFD_CalculateTimingValues(psCanfd, psCanfdStr->sBtConfig.sNormBitRate.u32BitRate, psCanfdStr->sBtConfig.sDataBitRate.u32BitRate,
                                    u32CanFdClock, &psCanfdStr->sBtConfig.sConfigBitTing) != 0UL)
    {
    ...
    if (psCanfdStr->sBtConfig.bEnableLoopBack != 0UL)
    {
...
uint32_t CANFD_TransmitTxMsg(CANFD_T *psCanfd, uint32_t u32TxBufIdx, CANFD_FD_MSG_T *psTxMsg)
{
    ...
    {
        /* wait for completion */
        while ((psCanfd->TXBRP & (1UL << u32TxBufIdx)) != 0UL)
        {
...
void CANFD_CopyDataToTransmitBuffer(const CANFD_T *psCanfd, uint32_t u32TxBufIdx, CANFD_FD_MSG_T *psTxMsg)
{
    ...
    psTxBuffer = (CANFD_BUF_T *)(CANFD_GetSramBaseAddress(psCanfd) + (psCanfd->TXBC & 0xFFFFUL) + (u32TxBufIdx * sizeof(CANFD_BUF_T)));
    ...
    psTxBuffer->u32Config = ((uint32_t)CANFD_EncodeDLC((uint8_t)psTxMsg->u32DLC) << TX_BUFFER_T1_ELEM_DLC_Pos);
    ...
    if (psTxMsg->bEvntFifoCon != 0U)
    {
        psTxBuffer->u32Config = (psTxBuffer->u32Config & (~TX_BUFFER_T1_ELEM_MM1_Msk));
        psTxBuffer->u32Config |= ((uint32_t)psTxMsg->u8MsgMarker << TX_BUFFER_T1_ELEM_MM1_Pos);
 ...
 */
void CANFD_InitRxFifo(CANFD_T *psCanfd, uint32_t u32RxFifoNum, const CANFD_RAM_PART_T *psRamConfig, const CANFD_ELEM_SIZE_T *psElemSize, uint32_t u32FifoWM, E_CANFD_DATA_FIELD_SIZE eFifoSize)
{
    uint32_t u32Address;
    uint32_t u32Size;
    ...
    switch (u32RxFifoNum)
    {
        case 0:
            if (psElemSize-> u32RxFifo0 != 0UL)
            {
                ...
                u32Address = CANFD_GetSramBaseAddress(psCanfd) + (psRamConfig->u32RXF0C_F0SA & CANFD_RXF0C_F0SA_Msk);
                u32Size = (uint32_t)eFifoSize;
        ...
        case 1:
            if (psElemSize-> u32RxFifo1 != 0UL)
            {
                ...
                u32Address = CANFD_GetSramBaseAddress(psCanfd) + (psRamConfig->u32RXF1C_F1SA & CANFD_RXF1C_F1SA_Msk);
...
void CANFD_InitTxDBuf(CANFD_T *psCanfd, const CANFD_RAM_PART_T *psRamConfig, CANFD_ELEM_SIZE_T *psElemSize, E_CANFD_DATA_FIELD_SIZE eTxBufSize)
{
    ...
    u32Address = CANFD_GetSramBaseAddress(psCanfd) + (psRamConfig->u32TXBC_TBSA & CANFD_TXBC_TBSA_Msk);
...
void CANFD_InitRxDBuf(CANFD_T *psCanfd, const CANFD_RAM_PART_T *psRamConfig, const CANFD_ELEM_SIZE_T *psElemSize, E_CANFD_DATA_FIELD_SIZE eRxBufSize)
{
    ...
    u32Address = CANFD_GetSramBaseAddress(psCanfd) + (psRamConfig->u32RXBC_RBSA & CANFD_RXBC_RBSA_Msk);
 ...
 * @details     Function configures the data structures used by a dedicated Rx Buffer.
 */
void CANFD_ConfigSIDFC(CANFD_T *psCanfd, const CANFD_RAM_PART_T *psRamConfig, const CANFD_ELEM_SIZE_T *psElemSize)
{
    uint32_t u32Address;
    ...
    u32Address = CANFD_GetSramBaseAddress(psCanfd) + (psRamConfig->u32SIDFC_FLSSA & CANFD_SIDFC_FLSSA_Msk);
 ...
 */
void CANFD_ConfigXIDFC(CANFD_T *psCanfd, const CANFD_RAM_PART_T *psRamConfig, const CANFD_ELEM_SIZE_T *psElemSize)
{
    uint32_t u32Address;
    ...
    /*Get the Filter List Standard Start Address in the RAM*/
    u32Address = CANFD_GetSramBaseAddress(psCanfd) + (psRamConfig->u32XIDFC_FLESA & CANFD_XIDFC_FLESA_Msk);
    /*Clear the Filter List Memory*/
...
void CANFD_SetSIDFltr(const CANFD_T *psCanfd, uint32_t u32FltrIdx, uint32_t u32Filter)
{
    ...
    psFilter = (CANFD_STD_FILTER_T *)(CANFD_GetSramBaseAddress(psCanfd) + (psCanfd->SIDFC & CANFD_SIDFC_FLSSA_Msk) + (u32FltrIdx * sizeof(CANFD_STD_FILTER_T)));
...
void CANFD_SetXIDFltr(const CANFD_T *psCanfd, uint32_t u32FltrIdx, uint32_t u32FilterLow, uint32_t u32FilterHigh)
{
    ...
    psFilter = (CANFD_EXT_FILTER_T *)(CANFD_GetSramBaseAddress(psCanfd) + (psCanfd->XIDFC & CANFD_XIDFC_FLESA_Msk) + (u32FltrIdx * sizeof(CANFD_EXT_FILTER_T)));
...
uint32_t CANFD_ReadRxBufMsg(CANFD_T *psCanfd, uint8_t u8MbIdx, CANFD_FD_MSG_T *psMsgBuf)
{
    ...
    if (u8MbIdx < CANFD_MAX_RX_BUF_ELEMS)
    {
        ...
        if (newData != 0UL)
        {
            ...
            psRxBuffer = (CANFD_BUF_T *)(CANFD_GetSramBaseAddress(psCanfd) + (psCanfd->RXBC & 0xFFFFUL) + (u8MbIdx * sizeof(CANFD_BUF_T)));
            /* read the message */
...
uint32_t CANFD_ReadRxFifoMsg(CANFD_T *psCanfd, uint8_t u8FifoIdx, CANFD_FD_MSG_T *psMsgBuf)
{
    ...
    if (u8FifoIdx < (uint8_t)CANFD_NUM_RX_FIFOS)
    {
        ...
        if ((*pRXFS & 0x7FUL) > 0UL)
        {
            ...
            pRxBuffer = (CANFD_BUF_T *)(CANFD_GetSramBaseAddress(psCanfd) + (*pRXFC & 0xFFFFUL) + ((uint32_t)GetIndex * sizeof(CANFD_BUF_T)));
...
void CANFD_CopyDBufToMsgBuf(const CANFD_T *psCanfd, CANFD_BUF_T *psRxBuf, CANFD_FD_MSG_T *psMsgBuf)
{
    ...
    if ((psRxBuf->u32Id & RX_BUFFER_AND_FIFO_R0_ELEM_ESI_Msk) != 0UL)
    {
        psMsgBuf->bErrStaInd = (uint8_t)TRUE;
    }
    else
    {
        psMsgBuf->bErrStaInd = (uint8_t)FALSE;
    }

    /* if 29-bit ID */
    if ((psRxBuf->u32Id & RX_BUFFER_AND_FIFO_R0_ELEM_XTD_Msk) != 0UL)
    {
    ...
    if ((psRxBuf->u32Id  & RX_BUFFER_AND_FIFO_R0_ELEM_RTR_Msk) != 0UL)
    {
    ...
    if ((psRxBuf->u32Config &  RX_BUFFER_AND_FIFO_R1_ELEM_ANMF_Msk) != 0UL)
    {
        psMsgBuf->bNonMatchingFrame = (uint8_t)TRUE;
    }
    else
    {
        psMsgBuf->bNonMatchingFrame = (uint8_t)FALSE;
        psMsgBuf->u8FilterIndex = (uint8_t)((psRxBuf->u32Config & RX_BUFFER_AND_FIFO_R1_ELEM_FIDX_Msk) >> RX_BUFFER_AND_FIFO_R1_ELEM_FIDX_Pos);
    }

    if ((psRxBuf->u32Config &  RX_BUFFER_AND_FIFO_R1_ELEM_FDF_Msk) != 0UL)
    {
        psMsgBuf->bFDFormat = (uint8_t)TRUE;
    }
    else
    {
        psMsgBuf->bFDFormat = (uint8_t)FALSE;
    }

    if ((psRxBuf->u32Config &  RX_BUFFER_AND_FIFO_R1_ELEM_BSR_Msk) != 0UL)
    {
        psMsgBuf->bBitRateSwitch = (uint8_t)TRUE;
    }
    else
    {
        psMsgBuf->bBitRateSwitch = (uint8_t)FALSE;
    }

    if ((psCanfd->TSCC & CANFD_TSCC_TSS_Msk) != 0UL)
    {
        psMsgBuf->bTimestampCaptured = (uint8_t)FALSE;
        psMsgBuf->u8RxTimestampPointer = 0;
        psMsgBuf->u16RxTimestamp = (uint16_t)(psRxBuf->u32Config & RX_BUFFER_AND_FIFO_R1_ELEM_RXTS_Msk);
    }

    psMsgBuf->u32DLC = CANFD_DecodeDLC((uint8_t)((psRxBuf->u32Config & RX_BUFFER_AND_FIFO_R1_ELEM_DLC_Msk) >> RX_BUFFER_AND_FIFO_R1_ELEM_DLC_Pos));
...
uint32_t CANFD_ReadTxFifoEventMsg(CANFD_T *psCanfd, uint8_t u8TxEvntIdx, CANFD_TX_EVNT_ELEM_T *psEventFifoMsgBuf)
{
    ...
    if (u8TxEvntIdx < CANFD_MAX_TX_EVNT_FIFO_ELEMS)
    {
        if ((psCanfd->TXEFS & 0x3FUL) > 0UL)
        {
            ...
            /* check for overflow */
            if ((psCanfd->TXEFS & CANFD_TXEFS_TEFL_Msk) != 0UL)
            {
                /* clear overflow flag */
...
void CANFD_CopyTxEvntFifoToUsrBuf(const CANFD_T *psCanfd, uint32_t u32TxEvntNum, CANFD_TX_EVNT_ELEM_T *psTxEvntElem)
{
    ...
    pu32TxEvnt = (uint32_t *)(CANFD_GetSramBaseAddress(psCanfd) + CANFD_GetTxEventFifoElementAddress(psCanfd, u32TxEvntNum));

    /*Get the Error State Indicator*/
    if ((pu32TxEvnt[0] & TX_FIFO_E0_EVENT_ESI_Msk) > 0UL)
    {
        psTxEvntElem->bErrStaInd = (uint8_t)TRUE; //Transmitting node is error passive
    }
    else
    {
        psTxEvntElem->bErrStaInd = (uint8_t)FALSE;//Transmitting node is error active
    }
    ...
    {
        psTxEvntElem->bRemote = (uint8_t)TRUE; //Remote frame
    }
    else
    {
        psTxEvntElem->bRemote = (uint8_t)FALSE; //Data frame
    }

    /*Get the FD Format type*/
    if ((pu32TxEvnt[1] & TX_FIFO_E1_EVENT_FDF_Msk) > 0UL)
    {
        psTxEvntElem->bFDFormat = (uint8_t)TRUE; //CAN FD frame format
    }
    else
    {
        psTxEvntElem->bFDFormat = (uint8_t)FALSE; //Classical CAN frame format
    }

    /*Get the Bit Rate Switch type*/
    if ((pu32TxEvnt[1] & TX_FIFO_E1_EVENT_BRS_Msk) > 0UL)
    {
        psTxEvntElem->bBitRateSwitch = (uint8_t)TRUE;  //Frame transmitted with bit rate switching
    }
    else
    {
        psTxEvntElem->bBitRateSwitch = (uint8_t)FALSE; //Frame transmitted without bit rate switching
    }
...
void CANFD_RunToNormal(CANFD_T *psCanfd, uint8_t u8Enable)
{
    ...
    if (u8Enable != 0UL)
    {
        ...
        while ((psCanfd->CCCR & CANFD_CCCR_INIT_Msk) != 0UL)
        {
            if (u32TimeOutCount == 0UL)
            {
                break;
