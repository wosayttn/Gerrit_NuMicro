...
int32_t HSUSBD_Open(S_HSUSBD_INFO_T *param, HSUSBD_CLASS_REQ pfnClassReq, HSUSBD_SET_INTERFACE_REQ pfnSetInterface)
{
    ...
    while ((HSUSBD->PHYCTL & HSUSBD_PHYCTL_PHYCLKSTB_Msk) == 0U)
    {
        if (--u32TimeOutCnt == 0U)
        {
        ...
        case DESC_CONFIG:
        {
            ...
            if ((HSUSBD->OPER & 0x04UL) == 0x04UL)
            {
                ...
                if (u32Len > u32TotalLen)
                {
                    ...
                    if ((g_hsusbd_CtrlMaxPktSize != 0U) && ((u32Len % g_hsusbd_CtrlMaxPktSize) == 0UL))
                    {
        ...
        case DESC_OTHERSPEED:
        {
            ...
            else
            {
                ...
                if (u32Len > u32TotalLen)
                {
                    ...
                    if ((g_hsusbd_CtrlMaxPktSize != 0UL) && ((u32Len % g_hsusbd_CtrlMaxPktSize) == 0UL))
                    {
                        g_hsusbd_CtrlZero = (uint8_t)1UL;
                    }
                }
        ...
        switch (gUsbCmd.bRequest)
        {
            ...
            case GET_STATUS:
            {
                ...
                else if (gUsbCmd.bmRequestType == 0x82UL)
                {
                    ...
                    buf[0] = (HSUSBD_GetStall((uint32_t)ep) != 0U) ? (uint8_t)1 : (uint8_t)0;
                }
                else
                {
                    /* Not Supported */
...
void HSUSBD_UpdateDeviceState(void)
{
    switch (gUsbCmd.bRequest)
    {
        ...
        case SET_FEATURE:
        {
            ...
            else if (g_hsusbd_EnableTestMode != 0U)
            {
...
void HSUSBD_CtrlIn(void)
{
    ...
    if (g_hsusbd_CtrlInSize >= g_hsusbd_CtrlMaxPktSize)
    {
        ...
        for (i = 0UL; i < cnt; i++)
        {
            ...
            (void)memcpy((uint8_t *)&u32CtrlInData, g_hsusbd_CtrlInPointer, 4UL);
            HSUSBD->CEPDAT = u32CtrlInData;
            g_hsusbd_CtrlInPointer = &g_hsusbd_CtrlInPointer[4];
        }
    ...
    else
    {
        ...
        for (i = 0UL; i < cnt; i++)
        {
            uint8_t u8Value = g_hsusbd_CtrlInPointer[i];
...
/**
  ...
  * @retval None.
  */
void HSUSBD_SetEpBufAddr(uint32_t u32Ep, uint32_t u32Base, uint32_t u32Len)
{
    if (u32Ep == CEP)
    {
...
/**
  ...
  */
void HSUSBD_SetEpStall(uint32_t u32Ep)
{
    if (u32Ep == CEP)
    {
...
/**
  ...
  */
void HSUSBD_ClearEpStall(uint32_t u32Ep)
{
...
/**
  ...
  */
uint32_t HSUSBD_GetEpStall(uint32_t u32Ep)
{
