...
uint32_t TPWM_ConfigOutputFreqAndDuty(TIMER_T *timer, uint32_t u32Frequency, uint32_t u32DutyCycle)
{
    ...
    if (u32Prescaler == 0UL || u32Period == 0UL)
    {
        return 0UL;
    }
...
{
    return (((timer->PWMINTSTS1 & (0x3UL << u32IntSource)) != 0UL) ? 1UL : 0UL);
}
...
{
    return (((timer)->PWMAINTSTS & TIMER_PWMAINTSTS_IFAIF_Msk) != 0UL) ? 1UL : 0UL;
}
