/**************************************************************************//**
 * @file     retarget.c
 * @version  V1.00
 * @brief    Debug port and semihost setting source file
 *
 * SPDX-License-Identifier: Apache-2.0
 * @copyright (C) 2025 Nuvoton Technology Corp. All rights reserved.
 ******************************************************************************/
#include <string.h>
#include "NuMicro.h"
#include "debug_printf.h"

#if defined (_RTE_) || __has_include("RTE_Components.h")
    #include "RTE_Components.h"
#endif

// -------- <<< Use Configuration Wizard in Context Menu >>> --------
#ifndef STDIN_ECHO
    // <q> Echo STDIN to STDOUT
    // <i> Enable echoing of characters typed on STDIN back to STDOUT
    #define STDIN_ECHO          0
#endif
// -------- <<< End of Configuration Wizard Context Menu >>> --------

#ifndef _RTE_
    // Include CMSIS-Compiler toolchain-specific retarget source files when RTE is disabled.

    #if defined (__GNUC__) && !defined(__ARMCC_VERSION)
        #ifndef NVT_ISP_FUNC
            #include "../../../../CMSIS/Compiler/source/core/gcc/retarget_syscalls.c"
        #else
            // retarget_syscalls.c is omitted to reduce code size for ISP samples.
        #endif
    #endif

    #if defined (__ARMCC_VERSION)
        #include "../../../../CMSIS/Compiler/source/core/armcc/retarget_io.c"
    #endif

    #if defined (__ICCARM__)
        /*cstat !MISRAC2012-Dir-4.10: retarget_io.c is provided by Arm CMSIS.*/
        #include "../../../../CMSIS/Compiler/source/core/iar/retarget_io.c"
    #endif

#endif  // _RTE_

int stdout_putchar(int ch);
int stdin_getchar(void);
int stderr_putchar(int ch);
void ProcessHardFault(uint32_t *pu32StackFrame);

#if defined(DEBUG_ENABLE_SEMIHOST)

static volatile int32_t g_ICE_Connected = 1;
static uint8_t g_buf_len = 0;

/* The static buffer is used to speed up the semihost */
static char g_buf[16];

int32_t SH_Return(int32_t n32In_R0, int32_t n32In_R1, int32_t *pn32Out_R0);
int32_t SH_DoCommand(int32_t n32In_R0, int32_t n32In_R1, int32_t *pn32Out_R0);

/**
 * @brief      The function to process semihosted command
 *
 * @param[in]  n32In_R0  : semihost register 0
 * @param[in]  n32In_R1  : semihost register 1
 * @param[out] pn32Out_R0: semihost register 0
 *
 * @retval     0: No ICE debug
 * @retval     1: ICE debug
 */
int32_t SH_Return(int32_t n32In_R0, int32_t n32In_R1, int32_t *pn32Out_R0)
{
    (void)n32In_R1;

    if (g_ICE_Connected != 0)
    {
        if (pn32Out_R0 != (int32_t *)NULL)
        {
            *pn32Out_R0 = n32In_R0;
        }

        return 1;
    }

    return 0;
}

/**
 * @brief      The function to process semihosted command
 *
 * @param[in]  n32In_R0  : semihost register 0
 * @param[in]  n32In_R1  : semihost register 1
 * @param[out] pn32Out_R0: semihost register 0
 *
 * @retval     0: No ICE debug
 * @retval     1: ICE debug
 */
#if !defined (__ARMCC_VERSION)
int32_t SH_DoCommand(int32_t n32In_R0, int32_t n32In_R1, int32_t *pn32Out_R0)
{
    __ASM volatile("BKPT   0xAB\n");     // Wait ICE or HardFault
    return SH_Return(n32In_R0, n32In_R1, pn32Out_R0);
}
#endif
#endif

/**
 * @brief     Send a character data to UART debug port.
 *
 * @param[in] ch: A character data writes to debug port
 *
 * @returns   None
 */
#ifndef NONBLOCK_PRINTF
static void SendChar_ToUART(int ch)
{
    if ((char)ch == '\n')
    {
        while ((DEBUG_PORT->FIFOSTS & UART_FIFOSTS_TXFULL_Msk) != 0UL) {}

        DEBUG_PORT->DAT = (uint32_t)'\r';
    }

    while ((DEBUG_PORT->FIFOSTS & UART_FIFOSTS_TXFULL_Msk) != 0UL) {}

    DEBUG_PORT->DAT = (uint32_t)ch;
}

#else
/* Non-block implement of send char */
# define BUF_SIZE    512
static void SendChar_ToUART(int ch)
{
    static uint8_t u8Buf[BUF_SIZE] = {0};
    static int32_t i32Head = 0;
    static int32_t i32Tail = 0;
    int32_t i32Tmp;

    // Only flush the data in buffer to UART when ch == 0
    if (ch)
    {
        if (ch == '\n')
        {
            i32Tmp = i32Head + 1;

            if (i32Tmp > BUF_SIZE)
            {
                i32Tmp = 0;
            }

            if (i32Tmp != i32Tail)
            {
                u8Buf[i32Head] = '\r';
                i32Head = i32Tmp;
            }
        }

        // Push char
        i32Tmp = i32Head + 1;

        if (i32Tmp > BUF_SIZE)
        {
            i32Tmp = 0;
        }

        if (i32Tmp != i32Tail)
        {
            u8Buf[i32Head] = ch;
            i32Head = i32Tmp;
        }
    }
    else
    {
        if (i32Tail == i32Head)
        {
            return;
        }
    }

    // pop char
    do
    {
        i32Tmp = i32Tail + 1;

        if (i32Tmp > BUF_SIZE)
        {
            i32Tmp = 0;
        }

        if ((DEBUG_PORT->FIFOSTS & UART_FIFOSTS_TXFULL_Msk) == 0UL)
        {
            DEBUG_PORT->DAT = u8Buf[i32Tail];
            i32Tail = i32Tmp;
        }
        else
        {
            break; // FIFO full
        }
    } while (i32Tail != i32Head);
}
#endif

/**
 * @brief     Send a character data to UART debug port or semihost.
 *
 * @param[in] ch: A character data writes to debug port
 *
 * @returns   None
 */
static void SendChar(int ch)
{
#if defined(DEBUG_ENABLE_SEMIHOST)
    g_buf[g_buf_len++] = (char)ch;
    g_buf[g_buf_len] = '\0';

    if ((g_buf_len >= (sizeof(g_buf) - 1U)) || ((char)ch == '\n') || ((char)ch == '\0'))
    {
        // Send the char
        if (g_ICE_Connected != 0)
        {
            if (SH_DoCommand(0x04, (int)g_buf, NULL) != 0)
            {
                g_buf_len = 0;
                return;
            }
        }
        else
        {
# if (DEBUG_ENABLE_SEMIHOST == 1) // Re-direct to UART Debug Port only when DEBUG_ENABLE_SEMIHOST=1
            uint8_t i;

            for (i = 0U; i < g_buf_len; i++)
            {
                SendChar_ToUART((int)g_buf[i]);
            }

            g_buf_len = 0;
# endif
        }
    }

#else
    SendChar_ToUART(ch);
#endif
}

/**
 * @brief    Wait UART debug port or semihost to input a char.
 *
 * @param    None
 *
 * @returns  Get character data from UART debug port or semihost
 */
static char GetChar(void)
{
#ifdef DEBUG_ENABLE_SEMIHOST

    if (g_ICE_Connected != 0)
    {
        int nRet;

#if defined (__ICCARM__)

        while (SH_DoCommand(0x7, 0, &nRet) != 0)
        {
            if (nRet != 0)
            {
                return (char)nRet;
            }
        }

#else

        while (SH_DoCommand(0x101, 0, &nRet) != 0)
        {
            if (nRet != 0)
            {
                SH_DoCommand(0x07, 0, &nRet);
                return (char)nRet;
            }
        }

#endif
    }
    else
    {
# if (DEBUG_ENABLE_SEMIHOST == 1) // Re-direct to UART Debug Port only when DEBUG_ENABLE_SEMIHOST=1

        // Use debug port when ICE is not connected at semihost mode
        while (g_ICE_Connected == 0)
        {
            if ((DEBUG_PORT->FIFOSTS & UART_FIFOSTS_RXEMPTY_Msk) == 0UL)
            {
                return ((char)DEBUG_PORT->DAT);
            }
        }

# endif
    }

    return (char)(0);

#else

    for (;;)
    {
        if ((DEBUG_PORT->FIFOSTS & UART_FIFOSTS_RXEMPTY_Msk) == 0UL)
        {
            return ((char)DEBUG_PORT->DAT);
        }
    }

#endif
}

#if defined(TRACK_HARDFAULT) && (TRACK_HARDFAULT == 1)
static void usage_fault_track(uint32_t u32CFSR)
{
    /* Usage Fault Status Register */
    debug_printf("usage fault: ");

    if ((u32CFSR & SCB_CFSR_UNDEFINSTR_Msk) != 0UL)
    {
        debug_printf("UNDEFINSTR ");
    }

    if ((u32CFSR & SCB_CFSR_INVSTATE_Msk) != 0UL)
    {
        debug_printf("INVSTATE ");
    }

    if ((u32CFSR & SCB_CFSR_INVPC_Msk) != 0UL)
    {
        debug_printf("INVPC ");
    }

    if ((u32CFSR & SCB_CFSR_NOCP_Msk) != 0UL)
    {
        debug_printf("NOCP ");
    }

    if ((u32CFSR & SCB_CFSR_UNALIGNED_Msk) != 0UL)
    {
        debug_printf("UNALIGNED ");
    }

    if ((u32CFSR & SCB_CFSR_DIVBYZERO_Msk) != 0UL)
    {
        debug_printf("DIVBYZERO ");
    }

    debug_printf("\n");
}

static void bus_fault_track(uint32_t u32CFSR)
{
    /* Bus Fault Status Register */
    debug_printf("bus fault: ");

    if ((u32CFSR & SCB_CFSR_IBUSERR_Msk) != 0UL)
    {
        debug_printf("IBUSERR ");
    }

    if ((u32CFSR & SCB_CFSR_PRECISERR_Msk) != 0UL)
    {
        debug_printf("PRECISERR ");
    }

    if ((u32CFSR & SCB_CFSR_IMPRECISERR_Msk) != 0UL)
    {
        debug_printf("IMPRECISERR ");
    }

    if ((u32CFSR & SCB_CFSR_UNSTKERR_Msk) != 0UL)
    {
        debug_printf("UNSTKERR ");
    }

    if ((u32CFSR & SCB_CFSR_STKERR_Msk) != 0UL)
    {
        debug_printf("STKERR ");
    }

    if ((u32CFSR & SCB_CFSR_BFARVALID_Msk) != 0UL)
    {
        debug_printf("SCB->BFAR:%08X ", SCB->BFAR);
    }

    debug_printf("\n");
}

static void mem_manage_fault_track(uint32_t u32CFSR)
{
    /* Memory Fault Status Register */
    debug_printf("mem manage fault: ");

    if ((u32CFSR & SCB_CFSR_IACCVIOL_Msk) != 0UL)
    {
        debug_printf("IACCVIOL ");
    }

    if ((u32CFSR & SCB_CFSR_DACCVIOL_Msk) != 0UL)
    {
        debug_printf("DACCVIOL ");
    }

    if ((u32CFSR & SCB_CFSR_MUNSTKERR_Msk) != 0UL)
    {
        debug_printf("MUNSTKERR ");
    }

    if ((u32CFSR & SCB_CFSR_MSTKERR_Msk) != 0UL)
    {
        debug_printf("MSTKERR ");
    }

    if ((u32CFSR & SCB_CFSR_MLSPERR_Msk) != 0UL)
    {
        debug_printf("MLSPERR ");
    }

    if ((u32CFSR & SCB_CFSR_MMARVALID_Msk) != 0UL)
    {
        debug_printf("SCB->MMFAR:%08X ", SCB->MMFAR);
    }

    debug_printf("\n");

}

static void TrackHardFault(void)
{
    if ((SCB->HFSR & SCB_HFSR_VECTTBL_Msk) != 0UL)
    {
        debug_printf("failed vector fetch\n");
    }

    if ((SCB->HFSR & SCB_HFSR_FORCED_Msk) != 0UL)
    {
        uint32_t u32SCB_CFSR = SCB->CFSR;

        if ((u32SCB_CFSR & SCB_CFSR_BUSFAULTSR_Msk) != 0UL)
        {
            bus_fault_track(u32SCB_CFSR);
        }

        if ((u32SCB_CFSR & SCB_CFSR_MEMFAULTSR_Msk) != 0UL)
        {
            mem_manage_fault_track(u32SCB_CFSR);
        }

        if ((u32SCB_CFSR & SCB_CFSR_USGFAULTSR_Msk) != 0UL)
        {
            usage_fault_track(u32SCB_CFSR);
        }
    }

    if ((SCB->HFSR & SCB_HFSR_DEBUGEVT_Msk) == SCB_HFSR_DEBUGEVT_Msk)
    {
        debug_printf("debug event\n");
    }
}
#endif

__WEAK void ProcessHardFault(uint32_t *pu32StackFrame)
{
    uint32_t inst;
    uint32_t addr;
    /* Enable hardfault tracking when TRACK_HARDFAULT is set to 1. */
#if defined(TRACK_HARDFAULT) && (TRACK_HARDFAULT == 1)
    uint32_t taddr;
    uint32_t tdata;
    uint32_t rm;
    uint32_t rn;
    uint32_t rt;
    uint32_t imm5;
    uint32_t imm8;
    uint32_t u32CFSR;
    uint32_t u32BFAR;
#endif

    // It is caused by hardfault. Just process the hard fault.
    /*
        r0  = pu32StackFrame[0]
        r1  = pu32StackFrame[1]
        r2  = pu32StackFrame[2]
        r3  = pu32StackFrame[3]
        r12 = pu32StackFrame[4]
        lr  = pu32StackFrame[5]
        pc  = pu32StackFrame[6]
        psr = pu32StackFrame[7]
    */

    if (NULL == pu32StackFrame)
    {
        return ;
    }

    // Get the instruction caused the hardfault
    addr = pu32StackFrame[6];
    inst = M16(addr);

    if (inst == 0xBEABU)
    {
        /*
            If the instruction is 0xBEAB, it means it is caused by BKPT without ICE connected.
            We still return for output/input message to UART.
        */
#if defined(DEBUG_ENABLE_SEMIHOST)
        g_ICE_Connected = 0;        // Set a flag for ICE offline
#endif
        pu32StackFrame[6] += 2U;    // Return to next instruction
        return ;
    }

    /* Enable hardfault tracking when TRACK_HARDFAULT is set to 1. */
#if defined(TRACK_HARDFAULT) && (TRACK_HARDFAULT == 1)
    /* Read volatile registers into temporary variables to fix IAR [Pa082] the order of volatile accesses is undefined */
    u32CFSR = SCB->CFSR;
    u32BFAR = SCB->BFAR;

    /* It is casued by hardfault (Not semihost). Just process the hard fault here. */
    debug_printf("\nHardFault Analysis:\n");
    debug_printf("    PC  : 0x%08X, LR  : 0x%08X, XPSR : 0x%08X\n",
           pu32StackFrame[5], pu32StackFrame[6], pu32StackFrame[7]);
    debug_printf("    CFSR: 0x%08X, BFAR: 0x%08X, MMFAR: 0x%08X\n", u32CFSR, u32BFAR, SCB->MMFAR);
    debug_printf("Instruction code = %x\n", inst);

    if ((inst >> 12U) == 5U)
    {
        // 0101xx Load/store (register offset) on page C2-327 of Armv8-M ref
        rm = (inst >> 6U) & 0x7U;
        rn = (inst >> 3U) & 0x7U;
        rt = inst & 0x7U;

        debug_printf("LDR/STR rt=%x rm=%x rn=%x\n", rt, rm, rn);
        taddr = pu32StackFrame[rn] + pu32StackFrame[rm];
        tdata = pu32StackFrame[rt];
        debug_printf("[0x%08x] 0x%04x %s 0x%x [0x%x]\n", addr, inst,
               ((inst & BIT11) != 0UL) ? "LDR" : "STR", tdata, taddr);
    }
    else if ((inst >> 13U) == 3U)
    {
        // 011xxx    Load/store word/byte (immediate offset) on page C2-327 of Armv8-M ref
        imm5 = (inst >> 6U) & 0x1fU;
        rn = (inst >> 3U) & 0x7U;
        rt = inst & 0x7U;

        debug_printf("LDR/STR rt=%x rn=%x imm5=%x\n", rt, rn, imm5);
        taddr = pu32StackFrame[rn] + imm5;
        tdata = pu32StackFrame[rt];
        debug_printf("[0x%08x] 0x%04x %s 0x%x [0x%x]\n", addr, inst,
               ((inst & BIT11) != 0UL) ? "LDR" : "STR", tdata, taddr);
    }
    else if ((inst >> 12U) == 8U)
    {
        // 1000xx    Load/store halfword (immediate offset) on page C2-328
        imm5 = (inst >> 6U) & 0x1fU;
        rn = (inst >> 3U) & 0x7U;
        rt = inst & 0x7U;

        debug_printf("LDRH/STRH rt=%x rn=%x imm5=%x\n", rt, rn, imm5);
        taddr = pu32StackFrame[rn] + imm5;
        tdata = pu32StackFrame[rt];
        debug_printf("[0x%08x] 0x%04x %s 0x%x [0x%x]\n", addr, inst,
               ((inst & BIT11) != 0UL) ? "LDR" : "STR", tdata, taddr);
    }
    else if ((inst >> 12U) == 9U)
    {
        // 1001xx    Load/store (SP-relative) on page C2-328
        imm8 = inst & 0xffU;
        rt = (inst >> 8U) & 0x7U;

        debug_printf("LDRH/STRH rt=%x imm8=%x\n", rt, imm8);
        taddr = pu32StackFrame[6] + imm8;
        tdata = pu32StackFrame[rt];
        debug_printf("[0x%08x] 0x%04x %s 0x%x [0x%x]\n", addr, inst,
               ((inst & BIT11) != 0UL) ? "LDR" : "STR", tdata, taddr);
    }
    else
    {
        debug_printf("Unexpected instruction\n");
    }

    TrackHardFault();
#endif

#if (CHECK_SCU_VIOLATION == 1)
    // If CHECK_SCU_VIOLATION is enabled, the code continues execution to check the SCU violation status.
    return ;
#else

    // Halt here
    for (;;) {}

#endif
}

// RTE_CMSIS_Compiler_STDOUT
int stdout_putchar(int ch)
{
    SendChar(ch);
    return 0;
}

// RTE_CMSIS_Compiler_STDIN
int stdin_getchar(void)
{
    return (int)GetChar();
}

// RTE_CMSIS_Compiler_STDERR
int stderr_putchar(int ch)
{
    SendChar(ch);
    return 0;
}

