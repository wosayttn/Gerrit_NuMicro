                             ...
                             uint32_t u32EnableInt)
{
    (void) u32DropCount;
    (void) u32Timeout;
    TIMER_T *ptimerTmp = timer;    /* store the timer base to configure compare value */
    uint32_t u32PairValid = 0UL;
    ...
    if (u32PairValid != 0UL)
    {
        ...
        if (u32EnableInt != 0UL)
        {
            ptimerTmp->EXTCTL = TIMER_EXTCTL_CAPIEN_Msk;
        }
        else
        {
            ptimerTmp->EXTCTL = 0UL;
        }
