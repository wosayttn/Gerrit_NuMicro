...
void CCAP_OpenPipes(uint32_t u32SensorSetting, uint32_t u32SensorDataFmt, uint32_t u32OutPacketFmt, uint32_t u32OutPlanarFmt)
{
    ...
    CCAP->PARM  = ((((u32SensorDataFmt & BIT1) != 0UL) ? 1UL : 0UL) << CCAP_PARM_INFMTH_Pos);
    CCAP->PARM  = CCAP->PARM | ((((u32SensorDataFmt & BIT1) != 0UL) ? 1UL : 0UL) << CCAP_PARM_INDATORDH_Pos);
