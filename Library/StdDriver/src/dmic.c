...
void DMIC_Open(DMIC_T *dmic)
{
    ...
    while ((dmic->DIV & DMIC_DIV_FCLR_Msk) == DMIC_DIV_FCLR_Msk)
    {
        if (u32Delay == 0UL)
        {
            break;
...
void DMIC_SetDSPGainVolume(DMIC_T *dmic, uint32_t u32ChMsk, int16_t i16ChVolume)
{
    ...
    uint32_t u32TmpChVolume = (uint32_t)i32TmpChVolume;

    if (i32TmpChVolume > 0L)
    {
        u32SetChVolume = u32TmpChVolume * 128UL;
    }
    else
    {
        u32SetChVolume = (u32TmpChVolume * 128UL) | BIT15;
    }

    u32SetChVolume = (uint32_t)((uint16_t)u32SetChVolume);
...
void DMIC_ClearFIFO(DMIC_T *dmic)
{
    ...
    while ((DMIC_IS_FIFOEMPTY(dmic) == 0UL) && (u32Delay > 0UL))
    {
        --u32Delay;
    }
}
