    ...
    for(u16Prescale = 1U; u16Prescale <= 0x1000U; u16Prescale++)
    {
        ...
        if(u32NearestUnitTimeNsec < u32UnitTimeNsec)
        {
            if((u16Prescale == 0x1000U) ||   /* limit to the maximum unit time(nano second) */
               (!((1000000U * (u16Prescale + 1U) > (u32NearestUnitTimeNsec * u32EPWMClockSrc)))))
            {
                u32Exit = 1U;
            }
            else
    ...
    for(i = 0U; i < EPWM_CHANNEL_NUM; i ++)
    {
        ...
        else
        {
            if((i & 0x1U) == 0U)
            {
                ...
                (epwm)->BRKCTL[i >> 1U] &= ~EPWM_BRKCTL0_1_BRKAEVEN_Msk;
                (epwm)->BRKCTL[i >> 1U] |= (EPWM_BRKCTL0_1_BRKAEVEN_Msk & ~(EPWM_BRKCTL0_1_BRKAEVEN_Msk >> 1U));
            }
            else
            {
                /* set brake action as low level for odd channel */
                (epwm)->BRKCTL[i >> 1U] &= ~EPWM_BRKCTL0_1_BRKAODD_Msk;
                (epwm)->BRKCTL[i >> 1U] |= (EPWM_BRKCTL0_1_BRKAODD_Msk & ~(EPWM_BRKCTL0_1_BRKAODD_Msk >> 1U));
            }
        }
    }
}
...
{
    uint32_t u32CapFLIF = (((epwm)->CAPIF & (EPWM_CAPIF_CFLIF0_Msk << u32ChannelNum)) != 0U) ? 1UL : 0UL;
    uint32_t u32CapRLIF = (((epwm)->CAPIF & (EPWM_CAPIF_CRLIF0_Msk << u32ChannelNum)) != 0U) ? 1UL : 0UL;
    return ((u32CapFLIF << 1U) | u32CapRLIF);
}
/**
...
{
    (epwm)->INTEN0 &= ~(((uint32_t)EPWM_DUTY_INT_DOWN_COUNT_MATCH_CMP | (uint32_t)EPWM_DUTY_INT_UP_COUNT_MATCH_CMP) << u32ChannelNum);
}
...
{
    /* every two channels shares the same setting */
    uint32_t u32ChPair = u32ChannelNum >> 1U;
    (epwm)->SYNC = (((epwm)->SYNC & ~(((3UL << EPWM_SYNC_SINSRC0_Pos) << (u32ChPair << 1U)) | ((1UL << EPWM_SYNC_PHSDIR0_Pos) << u32ChPair))) | \
                    (u32Direction << EPWM_SYNC_PHSDIR0_Pos << u32ChPair) | ((u32SyncSrc << EPWM_SYNC_SINSRC0_Pos) << (u32ChPair << 1U)));
    (epwm)->PHS[(u32ChPair)] = u32StartPhase;
}
