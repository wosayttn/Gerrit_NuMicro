...
#define SYS_SET_BOD_LPM() \
    do{ \
        while((SYS->BODCTL & SYS_BODCTL_WRBUSY_Msk) != 0UL) \
        { \
        } \
...
/**
  ...
  * \hideinitializer
  */
#define SYS_DISABLE_POR()               (SYS->PORCTL = 0x5AA55AA5UL)
...
/**
  ...
  * \hideinitializer
  */
#define SYS_ENABLE_POR()                (SYS->PORCTL = 0UL)
...
#define SYS_WAIT_VREFCTL_WRBUSY() \
    do{ \
        uint32_t u32TimeOutCnt = SYS_TIMEOUT; \
        while((SYS->VREFCTL & SYS_VREFCTL_WRBUSY_Msk) != 0UL) \
        { \
            u32TimeOutCnt--; \
            if(u32TimeOutCnt == 0UL) \
            { \
...
__STATIC_INLINE uint32_t SYS_IsRegLocked(void)
{
    return ((SYS->REGLCTL & 1UL) != 0UL) ? 0UL : 1UL;
}
