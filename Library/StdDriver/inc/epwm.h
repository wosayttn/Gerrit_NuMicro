 ...
 * \hideinitializer
 */
#define  EPWM_DISABLE_TIMER_SYNC(epwm, u32ChannelMask) ((epwm)->SSCTL &= ~((u32ChannelMask) & 0x3FUL))
...
__STATIC_INLINE void EPWM_SET_ALIGNED_TYPE(EPWM_T *epwm, uint32_t u32ChannelMask, uint32_t u32AlignedType)
{
    uint32_t const u32ValidMask = u32ChannelMask & 0x3FUL;
    ...
    do
    {
        ...
        for (i = 0UL; i < 6UL; i++)
        {
            uint32_t const u32BitMask = 1UL << i;

            if ((u32ValidMask & u32BitMask) != 0UL)
            {
...
__STATIC_INLINE void EPWM_SET_DEADZONE_CLK_SRC(EPWM_T *epwm, uint32_t u32ChannelNum, uint32_t u32AfterPrescaler)
{
    ...
    u32DTCLKSrc = ((u32AfterPrescaler & 0x1UL) << EPWM_DTCTL_DTCKSEL0_Pos) << (u32ChannelNum >> 1U);
