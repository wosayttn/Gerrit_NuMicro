...
/**
  ...
  * @param[in]  uspi The pointer of the specified USCI_SPI module.
  * @param[in]  u32SuspCycle Decide the length of suspend interval. Valid range is 0 ~ 15; out-of-range values are clipped.
  * \hideinitializer
  */
#define USPI_SET_SUSPEND_CYCLE(uspi, u32SuspCycle) ((uspi)->PROTCTL = ((uspi)->PROTCTL & ~USPI_PROTCTL_SUSPITV_Msk) | (((u32SuspCycle) & (USPI_PROTCTL_SUSPITV_Msk >> USPI_PROTCTL_SUSPITV_Pos)) << USPI_PROTCTL_SUSPITV_Pos))
...
#define USPI_SET_DATA_WIDTH(uspi, u32Width) \
    do{ \
        uint32_t u32Width_ = (u32Width); \
        if ((u32Width_ < 4UL) && (u32Width_ > 0UL)) { \
            u32Width_ = 4UL; \
        } else if (u32Width_ >= 16UL) { \
            u32Width_ = 0UL; \
        } else { \
        } \
        if (u32Width_ == 0UL) { \
            (uspi)->LINECTL = ((uspi)->LINECTL & ~USPI_LINECTL_DWIDTH_Msk) | (0UL << USPI_LINECTL_DWIDTH_Pos); \
        } else { \
            (uspi)->LINECTL = ((uspi)->LINECTL & ~USPI_LINECTL_DWIDTH_Msk) | (u32Width_ << USPI_LINECTL_DWIDTH_Pos); \
        } \
    }while(0)
