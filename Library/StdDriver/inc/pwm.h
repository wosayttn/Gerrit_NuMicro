...
#define PWM_SSCTL_SSRC_BPWM0                     (2UL<<PWM_SSCTL_SSRC_Pos)    /*!< Synchronous start source comes from PWM0 */
#define PWM_SSCTL_SSRC_BPWM1                     (3UL<<PWM_SSCTL_SSRC_Pos)    /*!< Synchronous start source comes from PWM0 */
...
#define PWM_FB_EDGE_ADC1RM                       (PWM_BRKCTL0_1_EADC1EBEN_Msk)     /*!< ADC1 Result Monitor (ADCRM) as edge-detect fault brake source */
...
#define PWM_FB_LEVEL_ADC1RM                      (PWM_BRKCTL0_1_EADC1LBEN_Msk)     /*!< ADC1 Result Monitor (ADCRM) as level-detect fault brake source */
 ...
 * \hideinitializer
 */
#define PWM_SET_PRESCALER(pwm, u32ChannelNum, u32Prescaler) (*(__IO uint32_t *) (&((pwm)->CLKPSC[0]) + ((u32ChannelNum) >> 1)) = (u32Prescaler))
...
#define PWM_SET_DEADZONE_CLK_SRC(pwm, u32ChannelNum, u32AfterPrescaler) \
    (*(__IO uint32_t *) (&((pwm)->DTCTL[0]) + ((u32ChannelNum) >> 1)) = (*(__IO uint32_t *) (&((pwm)->DTCTL[0]) + ((u32ChannelNum) >> 1)) & ~PWM_DTCTL0_1_DTCKSEL_Msk) | \
