...
#define LPSPI_OK                            (0)                               /*!< LPSPI operation succeeded */
#define LPSPI_ERR_FAIL                      (-1)                              /*!< LPSPI operation failed */
#define LPSPI_ERR_TIMEOUT                   (-2)                              /*!< LPSPI operation timed out */
...
/**
  ...
  * \hideinitializer
  */
#define LPSPI_DISABLE_3WIRE_MODE(lpspi)   ( (lpspi)->SSCTL &= ~LPSPI_SSCTL_SLV3WIRE_Msk )
...
#define LPSPI_SET_SS_HIGH(lpspi)                                        \
    do {                                                                \
        uint32_t u32SSCtl = (lpspi)->SSCTL & ~LPSPI_SSCTL_AUTOSS_Msk;   \
        uint32_t u32ActivePol = u32SSCtl & LPSPI_SSCTL_SSACTPOL_Msk;    \
        if (u32ActivePol != 0UL)                                        \
        {                                                               \
            (lpspi)->SSCTL = u32SSCtl | LPSPI_SSCTL_SS_Msk;             \
        }                                                               \
        else                                                            \
        {                                                               \
            (lpspi)->SSCTL = u32SSCtl & ~LPSPI_SSCTL_SS_Msk;            \
...
#define LPSPI_SET_SUSPEND_CYCLE(lpspi, u32SuspCycle)        \
    ((lpspi)->CTL = ((lpspi)->CTL & ~(LPSPI_CTL_SUSPITV_Msk)) | \
                    (((uint32_t)(u32SuspCycle) << LPSPI_CTL_SUSPITV_Pos) & LPSPI_CTL_SUSPITV_Msk))
...
#define LPSPI_SET_DATA_WIDTH(lpspi, u32Width)   \
    LPSPI_SetDataWidth((lpspi), (u32Width))
...
#define LPSPI_SET_AUTO_RX_TCNT(lpspi, u32TCNT)  \
    ((lpspi)->AUTOCTL = ((lpspi)->AUTOCTL & ~LPSPI_AUTOCTL_TCNT_Msk) | \
                        ((u32TCNT) << LPSPI_AUTOCTL_TCNT_Pos) & LPSPI_AUTOCTL_TCNT_Msk)
