...
/**
  ...
  * \hideinitializer
  */
#define LPTMR_IS_ACTIVE(lptmr)                      (((lptmr)->CTL & LPTMR_CTL_ACTSTS_Msk) != 0UL ? 1UL : 0UL)
...
__STATIC_INLINE uint32_t LPTMR_GetIntFlag(const LPTMR_T *lptmr)
{
    return (((lptmr->INTSTS & LPTMR_INTSTS_TIF_Msk) != 0UL) ? 1UL : 0UL);
}
...
__STATIC_INLINE uint32_t LPTMR_GetCaptureIntFlag(const LPTMR_T *lptmr)
{
    return (((lptmr->EINTSTS & LPTMR_EINTSTS_CAPIF_Msk) != 0UL) ? 1UL : 0UL);
}
...
__STATIC_INLINE uint32_t LPTMR_GetCaptureIntFlagOV(const LPTMR_T *lptmr)
{
    return (((lptmr->EINTSTS & LPTMR_EINTSTS_CAPIFOV_Msk) != 0UL) ? 1UL : 0UL);
}
