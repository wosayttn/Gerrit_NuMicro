...
{
    uint32_t u32TimeOutCnt = DFMC_TIMEOUT_READ;

    g_DFMC_i32ErrCode = DFMC_OK;
    DFMC->ISPCMD  = DFMC_ISPCMD_READ_CID;           /* Set ISP Command Code */
    DFMC->ISPADDR = 0x0UL;                          /* Must keep 0x0 when read CID */
    ...
    while (u32TimeOutCnt > 0UL)
    {
        if ((DFMC->ISPTRG & DFMC_ISPTRG_ISPGO_Msk) == 0UL)   /* Waiting for ISP Done */
        {
            return DFMC->ISPDAT;
        }
        u32TimeOutCnt--;
...
__STATIC_INLINE uint32_t DFMC_ReadPID(void)
{
    ...
    DFMC->ISPADDR = 0x04UL;                         /* Must keep 0x4 when read PID */
    DFMC->ISPTRG  = DFMC_ISPTRG_ISPGO_Msk;          /* Trigger to start ISP procedure */
