...
{
    uint32_t u32TimeOutCnt = FMC_TIMEOUT_READ;
    ...
    while ((FMC->ISPTRG & FMC_ISPTRG_ISPGO_Msk) != 0UL) /* Waiting for ISP Done */
    {
 ...
 *              FMC_ERR_TIMEOUT: Read timed out
 */
__STATIC_INLINE uint32_t FMC_ReadUID(uint32_t u32Index)
{
    uint32_t u32TimeOutCnt = FMC_TIMEOUT_READ;
    ...
    FMC->ISPADDR = u32Index << 2UL;  /* The UID is at offset 0x00, 0x04, and 0x08 with word alignment. */
    FMC->ISPDAT  = 0UL;
...
__STATIC_INLINE uint32_t FMC_ReadDID(void)
{
    ...
    FMC->ISPCMD  = FMC_ISPCMD_READ_DID;         /* Set ISP Command Code */
    FMC->ISPADDR = 0x0UL;                       /* Must keep 0x0 when read DID */
    FMC->ISPTRG  = FMC_ISPTRG_ISPGO_Msk;        /* Trigger to start ISP procedure */
...
{
    uint32_t u32TimeOutCnt = FMC_TIMEOUT_WRITE;
