...
#define FMC_TIMEOUT_READ        ((((uint32_t)SystemCoreClock) / 10UL) * 2UL) /*!< Read command time-out 100 ms         \hideinitializer */
#define FMC_TIMEOUT_WRITE       ((((uint32_t)SystemCoreClock) / 10UL) * 2UL) /*!< Write command time-out 100 ms        \hideinitializer */
#define FMC_TIMEOUT_ERASE       ((((uint32_t)SystemCoreClock) / 10UL) * 4UL) /*!< Erase command time-out 200 ms        \hideinitializer */
#define FMC_TIMEOUT_CHKSUM      (((uint32_t)SystemCoreClock) * 2UL)          /*!< Get checksum command time-out 2 s    \hideinitializer */
#define FMC_TIMEOUT_CHKALLONE   (((uint32_t)SystemCoreClock) * 2UL)          /*!< Check-all-one command time-out 2 s   \hideinitializer */
