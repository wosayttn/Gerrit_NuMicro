...
#define FMC_MULTI_WORD_PROG_LEN 256             /*!< The maximum length of a multi-word program \hideinitializer */
...
#define FMC_TIMEOUT_READ        (SystemCoreClock/5UL) /*!< Read command time-out 100 ms         \hideinitializer */
#define FMC_TIMEOUT_WRITE       (SystemCoreClock/5UL) /*!< Write command time-out 100 ms        \hideinitializer */
#define FMC_TIMEOUT_ERASE       (SystemCoreClock/2UL) /*!< Erase command time-out 200 ms        \hideinitializer */
...
__STATIC_INLINE uint32_t FMC_ReadCID(void)
{
    ...
    FMC->ISPADDR = 0x0UL;                         /* Must keep 0x0 when read CID */
...
__STATIC_INLINE uint32_t FMC_ReadPID(void)
{
    ...
    FMC->ISPADDR = 0x04UL;                       /* Must keep 0x4 when read PID */
    FMC->ISPTRG = FMC_ISPTRG_ISPGO_Msk;         /* Trigger to start ISP procedure */
#ifdef ISBEN
#if (ISBEN != 0)
    __ISB();
...
__STATIC_INLINE int32_t FMC_SetVectorPageAddr(uint32_t u32PageAddr)
{
    ...
    FMC->ISPTRG = 0x1UL;               /* Trigger to start ISP procedure */
#ifdef ISBEN
#if (ISBEN != 0)
    __ISB();
#endif
