...
{
    (psio)->SCCT[(u32SC)].SCCTL =
        ((psio)->SCCT[(u32SC)].SCCTL & ~(PSIO_SCCT_SCCTL_INISLOT_Msk |
    ...
    (psio)->SCCT[(u32SC)].SCCTL = ((psio)->SCCT[(u32SC)].SCCTL & ~(PSIO_SCCT_SCCTL_REPEAT_Msk)) |
                                  (((u32Repeat) == PSIO_REPEAT_ENABLE) ? PSIO_SCCT_SCCTL_REPEAT_Msk : 0UL);
}
...
{
    (psio)->GNCT[u32Pin].GENCTL =
        ((psio)->GNCT[(u32Pin)].GENCTL & ~(PSIO_GNCT_GENCTL_SCSEL_Msk |
