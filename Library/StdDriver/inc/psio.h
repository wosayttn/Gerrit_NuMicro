 ...
 * \hideinitializer
 */
#define PSIO_SET_PDMA_INPUT(psio, u32SC, u32InPin)                        \
    do {                                                                  \
        (psio)->PDMACTL = ((psio)->PDMACTL & ~PSIO_PDMACTL_INSCSEL_Msk) | \
                          ((u32SC) << PSIO_PDMACTL_INSCSEL_Pos) |         \
                          (u32InPin);                                     \
    } while (0)
 ...
 * \hideinitializer
 */
#define PSIO_SET_PDMA_OUTPUT(psio, u32SC, u32OutPin)                       \
    do {                                                                   \
        (psio)->PDMACTL = ((psio)->PDMACTL & ~PSIO_PDMACTL_OUTSCSEL_Msk) | \
                          ((u32SC) << PSIO_PDMACTL_OUTSCSEL_Pos) |         \
                          (u32OutPin);                                     \
    } while (0)
 ...
 * \hideinitializer
 */
#define PSIO_SET_TRIGSRC(psio, u32SC, u32SrcType)                                    \
    do {                                                                             \
        (psio)->SCCT[(u32SC)].SCCTL = ((psio)->SCCT[(u32SC)].SCCTL &                 \
                                       ~PSIO_SCCT_SCCTL_TRIGSRC_Msk) | (u32SrcType); \
    } while (0)
...
#define PSIO_SCSLOT_SET_SLOT(psio, u32SC, u32Slot, u32Cnt)                 \
    do {                                                                   \
        ...
        {                                                                  \
            (psio)->SCCT[(u32SC)].SCSLOT = ((psio)->SCCT[(u32SC)].SCSLOT &                             \
 ...
 * \hideinitializer
 */
#define PSIO_SCSLOT_SET_ALL_SLOT(psio, u32SC, u32Cnt)                                          \
    do {                                                                                       \
        (psio)->SCCT[(u32SC)].SCSLOT = (((u32Cnt) & 0xFUL) << PSIO_SCCT_SCSLOT_SLOT0CNT_Pos) | \
                                       (((u32Cnt) & 0xFUL) << PSIO_SCCT_SCSLOT_SLOT1CNT_Pos) | \
                                       (((u32Cnt) & 0xFUL) << PSIO_SCCT_SCSLOT_SLOT2CNT_Pos) | \
                                       (((u32Cnt) & 0xFUL) << PSIO_SCCT_SCSLOT_SLOT3CNT_Pos) | \
                                       (((u32Cnt) & 0xFUL) << PSIO_SCCT_SCSLOT_SLOT4CNT_Pos) | \
                                       (((u32Cnt) & 0xFUL) << PSIO_SCCT_SCSLOT_SLOT5CNT_Pos) | \
                                       (((u32Cnt) & 0xFUL) << PSIO_SCCT_SCSLOT_SLOT6CNT_Pos) | \
                                       (((u32Cnt) & 0xFUL) << PSIO_SCCT_SCSLOT_SLOT7CNT_Pos);  \
    } while (0)
 ...
 * \hideinitializer
 */
#define PSIO_ENABLE_PIN(psio, u32Pin)                                            \
    do {                                                                         \
        (psio)->GNCT[(u32Pin)].GENCTL = ((psio)->GNCT[(u32Pin)].GENCTL & ~PSIO_GNCT_GENCTL_PINEN_Msk) |     \
                                        PSIO_GNCT_GENCTL_PINEN_Msk;                                        \
    } while (0)
 ...
 * \hideinitializer
 */
#define PSIO_DISABLE_PIN(psio, u32Pin)                                         \
    do {                                                                         \
        (psio)->GNCT[(u32Pin)].GENCTL = (psio)->GNCT[(u32Pin)].GENCTL & ~PSIO_GNCT_GENCTL_PINEN_Msk;    \
    } while (0)
 ...
 * \hideinitializer
 */
#define PSIO_SET_WIDTH(psio, u32Pin, u32InWidth, u32OutWidth)                       \
    do {                                                                             \
        (psio)->GNCT[(u32Pin)].DATCTL = ((psio)->GNCT[(u32Pin)].DATCTL & ~PSIO_GNCT_DATCTL_INDATWD_Msk &       \
                                         ~PSIO_GNCT_DATCTL_OUTDATWD_Msk) |                                     \
                                        (((u32InWidth) == 0 ? 0 : ((u32InWidth) - 1)) <<                       \
                                         PSIO_GNCT_DATCTL_INDATWD_Pos) |                                       \
                                        (((u32OutWidth) == 0 ? 0 : ((u32OutWidth) - 1)) <<                     \
                                         PSIO_GNCT_DATCTL_OUTDATWD_Pos);                                       \
    } while (0)
 ...
 * \hideinitializer
 */
#define PSIO_SET_ORDER(psio, u32Pin, u32Order)                                      \
    do {                                                                             \
        (psio)->GNCT[(u32Pin)].DATCTL = ((psio)->GNCT[(u32Pin)].DATCTL & ~PSIO_GNCT_DATCTL_ORDER_Msk) |        \
                                        (u32Order);                                                             \
    } while (0)
 ...
 * \hideinitializer
 */
#define PSIO_SET_OUTPUT_DEPTH(psio, u32Pin, u32Depth)                                \
    do {                                                                              \
        (psio)->GNCT[(u32Pin)].DATCTL = ((psio)->GNCT[(u32Pin)].DATCTL & ~PSIO_GNCT_DATCTL_OUTDEPTH_Msk) |      \
                                        ((u32Depth) << PSIO_GNCT_DATCTL_OUTDEPTH_Pos);                          \
    } while (0)
 ...
 * \hideinitializer
 */
#define PSIO_SET_INPUT_DEPTH(psio, u32Pin, u32Depth)                                \
    do {                                                                             \
        (psio)->GNCT[(u32Pin)].DATCTL = ((psio)->GNCT[(u32Pin)].DATCTL & ~PSIO_GNCT_DATCTL_INDEPTH_Msk) |      \
                                        ((u32Depth) << PSIO_GNCT_DATCTL_INDEPTH_Pos);                          \
    } while (0)
...
#define PSIO_SET_CHECKPOINT(psio, u32Pin, u32CheckPoint, u32Slot)                        \
    do {                                                                                   \
        (psio)->GNCT[(u32Pin)].CPCTL0 = ((psio)->GNCT[(u32Pin)].CPCTL0 &                                              \
                                         ~(PSIO_GNCT_CPCTL0_CKPT0_Msk <<                                              \
...
#define PSIO_SET_ACTION(psio, u32Pin, u32CheckPoint, u32Action)                                  \
    do {                                                                                         \
        (psio)->GNCT[(u32Pin)].CPCTL1 = ((psio)->GNCT[(u32Pin)].CPCTL1 &                         \
                                         ~(PSIO_GNCT_CPCTL1_CKPT0ACT_Msk <<                      \
                                           ((u32CheckPoint) * PSIO_GNCT_CPCTL1_CKPT1ACT_Pos))) | \
                                        (((u32Action) & 0x7UL) <<                                \
                                         ((u32CheckPoint) * PSIO_GNCT_CPCTL1_CKPT1ACT_Pos));     \
    } while (0)
...
{
    uint32_t u32SlotOffset;
    uint32_t u32SCOffset;
    uint32_t u32SlotMask;
    uint32_t u32SCMask;
    uint32_t u32IntSel;

    u32IntSel = (u32Int & PSIO_INT1);
    u32SlotOffset = u32IntSel * ((uint32_t)PSIO_INTCTL_CONI1SS_Pos -
                                 (uint32_t)PSIO_INTCTL_CONI0SS_Pos);
    u32SCOffset = u32IntSel * ((uint32_t)PSIO_INTCTL_CONI1SCS_Pos -
                               (uint32_t)PSIO_INTCTL_CONI0SCS_Pos);
    u32SlotMask = (PSIO_INTCTL_CONI0SS_Msk << u32SlotOffset);
    u32SCMask = (PSIO_INTCTL_CONI0SCS_Msk << u32SCOffset);

    (psio)->INTCTL = ((psio)->INTCTL & ~(u32SlotMask | u32SCMask)) |
                     (((u32SC) << ((uint32_t)PSIO_INTCTL_CONI0SCS_Pos + u32SCOffset)) & u32SCMask) |
                     (((u32Slot) << ((uint32_t)PSIO_INTCTL_CONI0SS_Pos + u32SlotOffset)) & u32SlotMask);
}
...
static inline void PSIO_CLEAR_INTCTL(PSIO_T *psio, uint32_t u32Int)
{
    ...
    u32SCOffset = u32IntSel * ((uint32_t)PSIO_INTCTL_CONI1SCS_Pos -
                               (uint32_t)PSIO_INTCTL_CONI0SCS_Pos);
    u32IntMask = (PSIO_INTCTL_CONI0SS_Msk << u32SlotOffset) |
                 (PSIO_INTCTL_CONI0SCS_Msk << u32SCOffset);
    (psio)->INTCTL &= ~u32IntMask;
}
...
static inline void PSIO_SET_SCCTL(PSIO_T *psio, uint32_t u32SC, uint32_t u32InitSlot, uint32_t u32EndSlot, uint32_t u32LoopCnt, uint32_t u32Repeat)
{
    ...
    u32SCIndex = (u32SC & PSIO_SC3);
    (psio)->SCCT[u32SCIndex].SCCTL =
        ((psio)->SCCT[u32SCIndex].SCCTL & ~(PSIO_SCCT_SCCTL_INISLOT_Msk |
                                            PSIO_SCCT_SCCTL_ENDSLOT_Msk |
                                            PSIO_SCCT_SCCTL_SPLCNT_Msk |
                                            PSIO_SCCT_SCCTL_REPEAT_Msk)) |
        (((u32InitSlot) << PSIO_SCCT_SCCTL_INISLOT_Pos) & PSIO_SCCT_SCCTL_INISLOT_Msk) |
        (((u32EndSlot) << PSIO_SCCT_SCCTL_ENDSLOT_Pos) & PSIO_SCCT_SCCTL_ENDSLOT_Msk) |
        (((u32LoopCnt) << PSIO_SCCT_SCCTL_SPLCNT_Pos) & PSIO_SCCT_SCCTL_SPLCNT_Msk) |
        (((u32Repeat) << PSIO_SCCT_SCCTL_REPEAT_Pos) & PSIO_SCCT_SCCTL_REPEAT_Msk);
}
...
static inline void PSIO_SET_GENCTL(PSIO_T *psio, uint32_t u32Pin, uint32_t u32PinEn, uint32_t u32SC, uint32_t u32IOMode, uint32_t u32PinInit, uint32_t u32PinInterval)
{
    ...
    u32PinIndex = (u32Pin & PSIO_PIN7);
    (psio)->GNCT[u32PinIndex].GENCTL =
        ((psio)->GNCT[u32PinIndex].GENCTL & ~(PSIO_GNCT_GENCTL_SCSEL_Msk |
                                              PSIO_GNCT_GENCTL_IOMODE_Msk |
                                              PSIO_GNCT_GENCTL_INITIAL_Msk |
                                              PSIO_GNCT_GENCTL_INTERVAL_Msk |
                                              PSIO_GNCT_GENCTL_PINEN_Msk)) |
        (((u32SC) << PSIO_GNCT_GENCTL_SCSEL_Pos) & PSIO_GNCT_GENCTL_SCSEL_Msk) |
        (((u32IOMode) << PSIO_GNCT_GENCTL_IOMODE_Pos) & PSIO_GNCT_GENCTL_IOMODE_Msk) |
        (((u32PinInit) << PSIO_GNCT_GENCTL_INITIAL_Pos) & PSIO_GNCT_GENCTL_INITIAL_Msk) |
        (((u32PinInterval) << PSIO_GNCT_GENCTL_INTERVAL_Pos) & PSIO_GNCT_GENCTL_INTERVAL_Msk) |
        (((u32PinEn) << PSIO_GNCT_GENCTL_PINEN_Pos) & PSIO_GNCT_GENCTL_PINEN_Msk);
}
...
{
    uint32_t u32PinIndex;
    uint32_t u32ModeOffset;
    uint32_t u32CpOffset;
    uint32_t u32ModeMask;
    uint32_t u32CpMask;
    uint32_t u32SwPointIndex;

    u32PinIndex = (u32Pin & PSIO_PIN7);
    u32SwPointIndex = (u32SwPoint & PSIO_SWITCH_P1);
    u32ModeOffset = u32SwPointIndex * ((uint32_t)PSIO_GNCT_GENCTL_MODESW1_Pos -
                                       (uint32_t)PSIO_GNCT_GENCTL_MODESW0_Pos);
    u32CpOffset = u32SwPointIndex * ((uint32_t)PSIO_GNCT_GENCTL_SW1CP_Pos -
                                     (uint32_t)PSIO_GNCT_GENCTL_SW0CP_Pos);
    u32ModeMask = (PSIO_GNCT_GENCTL_MODESW0_Msk << u32ModeOffset);
    u32CpMask = (PSIO_GNCT_GENCTL_SW0CP_Msk << u32CpOffset);

    (psio)->GNCT[u32PinIndex].GENCTL =
        ((psio)->GNCT[u32PinIndex].GENCTL & ~(u32ModeMask | u32CpMask)) |
        (((u32SwMode) << ((uint32_t)PSIO_GNCT_GENCTL_MODESW0_Pos + u32ModeOffset)) & u32ModeMask) |
        ((((u32SwCP & PSIO_CP7) + 1UL) <<
          ((uint32_t)PSIO_GNCT_GENCTL_SW0CP_Pos + u32CpOffset)) & u32CpMask);
}
...
static inline void PSIO_SET_CP_CONFIG(PSIO_T *psio, uint32_t u32Pin, const S_PSIO_CP_CONFIG *sConfig)
{
    ...
    (psio)->GNCT[u32PinIndex].CPCTL0 = u32Cpctl0;
    (psio)->GNCT[u32PinIndex].CPCTL1 = u32Cpctl1;
}
