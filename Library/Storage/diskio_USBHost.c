...
DSTATUS disk_initialize(BYTE pdrv)        /* Physical drive number (0..) */
{
    ...
    if (usbh_umas_disk_status((int)pdrv) == UMAS_ERR_NO_DEVICE)
    {
        return STA_NODISK;
    }
...
)
{
    ...
    ret = usbh_umas_read((int)pdrv, (uint32_t)sector, (uint16_t)count, buff);

    if (ret != UMAS_OK)
    {
        (void)usbh_umas_reset_disk((int)pdrv);
        ret = usbh_umas_read((int)pdrv, (uint32_t)sector, (uint16_t)count, buff);
    }
...
)
{
    ...
    ret = usbh_umas_write((int)pdrv, (uint32_t)sector, (uint16_t)count, (uint8_t *)buff);
...
)
{
    ...
    ret = usbh_umas_ioctl((int)pdrv, (int)cmd, buff);
