...
)
{
    ...
    return (drv_ret == Successful) ? RES_OK : RES_ERROR;
}
...
)
{
	...
	/* cppcheck-suppress misra-c2012-11.8 ; 
	   DEVIATION: FatFs disk_write() supplies a const input buffer, 
	   while the vendor SDH_Write() API is not const-correct. 
	   SDH_Write() is used here as a read-only consumer of buff. 
	 */
...
)
{
    ...
    switch (cmd)
    {
        ...
        case GET_SECTOR_SIZE:
            *(WORD *)buff = (WORD)sdh_info->sectorSize;
            res = RES_OK;
            break;
