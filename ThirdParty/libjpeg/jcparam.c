...
#ifdef WITH_JPEGACC
  /* Allocate reciprocal quantization table space. */
  qrecptblptr = & cinfo->quant_recp_tbl_ptrs[which_tbl];
  if (*qrecptblptr == NULL)
    *qrecptblptr = jpeg_alloc_quant_recp_table((j_common_ptr) cinfo);

  /* Compute reciprocal metadata for each quant entry. */
  for (i = 0; i < DCTSIZE2; i++) {
    u16temp = (*qtblptr)->quantval[i];
    res = compute_reciprocal(u16temp,
                             (DCTELEM *)(&((*qrecptblptr)->quantval[i])));
  }

  /* Keep int16 copies to reduce load overhead in accelerated quantization. */
