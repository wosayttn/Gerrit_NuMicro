...
#endif
/* Headers for my_aes transparent driver */
#if defined(MBEDTLS_PSA_MY_AES_DRIVER_ENABLED)
#include "../drivers/my-aes/my_aes_driver_entrypoints.h"

#endif
/* Headers for nvt_cipher transparent driver */
#if defined(MBEDTLS_PSA_NVT_CIPHER_DRIVER_ENABLED)
#include "../drivers/nvt-drivers/nvt_cipher_driver_entrypoints.h"

#endif
/* Headers for nvt_aead transparent driver */
#if defined(MBEDTLS_PSA_NVT_AEAD_DRIVER_ENABLED)
#include "../drivers/nvt-drivers/nvt_aead_driver_entrypoints.h"

#endif
/* Headers for nvt_hash transparent driver */
#if defined(MBEDTLS_PSA_NVT_HASH_DRIVER_ENABLED)
#include "../drivers/nvt-drivers/nvt_hash_driver_entrypoints.h"
...
#define MY_AES_TRANSPARENT_DRIVER_ID (5)
#define NVT_CIPHER_TRANSPARENT_DRIVER_ID (6)
#define NVT_HASH_TRANSPARENT_DRIVER_ID (7)
#define NVT_AEAD_TRANSPARENT_DRIVER_ID (8)
...
#if defined(MBEDTLS_PSA_MY_AES_DRIVER_ENABLED)
    status = my_aes_driver_init( );
    if( status != PSA_SUCCESS )
        return( status );
#endif

#if defined(MBEDTLS_PSA_NVT_CIPHER_DRIVER_ENABLED)
    status = nvt_cipher_driver_init( );
    if( status != PSA_SUCCESS )
        return( status );
#endif

#if defined(MBEDTLS_PSA_NVT_AEAD_DRIVER_ENABLED)
    status = nvt_aead_driver_init( );
    if( status != PSA_SUCCESS )
        return( status );
#endif

#if defined(MBEDTLS_PSA_NVT_HASH_DRIVER_ENABLED)
    status = nvt_hash_driver_init( );
    if( status != PSA_SUCCESS )
        return( status );
#endif
...
#if defined(MBEDTLS_PSA_NVT_CIPHER_DRIVER_ENABLED)
    nvt_cipher_driver_free( );
#endif

#if defined(MBEDTLS_PSA_NVT_AEAD_DRIVER_ENABLED)
    nvt_aead_driver_free( );
#endif

#if defined(MBEDTLS_PSA_NVT_HASH_DRIVER_ENABLED)
    nvt_hash_driver_free( );
#endif
}
    ...
    size_t *output_length )
{
    ...
    switch( location )
    {
        case PSA_KEY_LOCATION_LOCAL_STORAGE:
...
#if defined(MBEDTLS_PSA_MY_AES_DRIVER_ENABLED)
            status = my_aes_transparent_cipher_encrypt( attributes,
                                                        key_buffer,
                                                        key_buffer_size,
                                                        alg,
                                                        iv,
            ...
            if( status != PSA_ERROR_NOT_SUPPORTED )
                return( status );
#endif /* MBEDTLS_PSA_MY_AES_DRIVER_ENABLED */
#if defined(MBEDTLS_PSA_NVT_CIPHER_DRIVER_ENABLED)
            status = nvt_cipher_transparent_cipher_encrypt( attributes,
                                                            key_buffer,
                                                            key_buffer_size,
                                                            alg,
                                                            iv,
            ...
            if( status != PSA_ERROR_NOT_SUPPORTED )
                return( status );
#endif /* MBEDTLS_PSA_NVT_CIPHER_DRIVER_ENABLED */
...
#if defined(MBEDTLS_PSA_MY_AES_DRIVER_ENABLED)
            status = my_aes_transparent_cipher_decrypt( attributes,
                                                        key_buffer,
                                                        key_buffer_size,
                                                        alg,
                                                        input,
...
#endif /* MBEDTLS_PSA_MY_AES_DRIVER_ENABLED */
#if defined(MBEDTLS_PSA_NVT_CIPHER_DRIVER_ENABLED)
            status = nvt_cipher_transparent_cipher_decrypt( attributes,
                                                            key_buffer,
                                                            key_buffer_size,
                                                            alg,
                                                            input,
    ...
    psa_algorithm_t alg )
{
    ...
    switch( location )
    {
        case PSA_KEY_LOCATION_LOCAL_STORAGE:
...
#endif /* PSA_CRYPTO_ACCELERATOR_DRIVER_PRESENT */
#if defined(MBEDTLS_PSA_MY_AES_DRIVER_ENABLED)
            status = my_aes_transparent_cipher_encrypt_setup(
                &operation->ctx.mbedtls_ctx,
                attributes,
                key_buffer,
                key_buffer_size,
            ...
            /* Declared with fallback == true */
            if( status == PSA_SUCCESS )
                operation->id = MY_AES_TRANSPARENT_DRIVER_ID;

            if( status != PSA_ERROR_NOT_SUPPORTED )
                return( status );
#endif /* MBEDTLS_PSA_MY_AES_DRIVER_ENABLED */
#if defined(MBEDTLS_PSA_NVT_CIPHER_DRIVER_ENABLED)
            status = nvt_cipher_transparent_cipher_encrypt_setup(
                &operation->ctx.nvt_cipher_ctx,
                attributes,
                key_buffer,
                key_buffer_size,
                alg );
            /* Declared with fallback == true */
            if( status == PSA_SUCCESS )
                operation->id = NVT_CIPHER_TRANSPARENT_DRIVER_ID;
...
#endif /* PSA_CRYPTO_ACCELERATOR_DRIVER_PRESENT */
#if defined(MBEDTLS_PSA_MY_AES_DRIVER_ENABLED)
            status = my_aes_transparent_cipher_decrypt_setup(
                &operation->ctx.mbedtls_ctx,
                attributes,
                key_buffer,
                key_buffer_size,
...
#endif /* MBEDTLS_PSA_MY_AES_DRIVER_ENABLED */
#if defined(MBEDTLS_PSA_NVT_CIPHER_DRIVER_ENABLED)
            status = nvt_cipher_transparent_cipher_decrypt_setup(
                &operation->ctx.nvt_cipher_ctx,
                attributes,
                key_buffer,
                key_buffer_size,
...
#if defined(MBEDTLS_PSA_MY_AES_DRIVER_ENABLED)
        case MY_AES_TRANSPARENT_DRIVER_ID:
            return( my_aes_transparent_cipher_set_iv(
                        &operation->ctx.mbedtls_ctx,
                        iv, iv_length ) );
#endif /* MBEDTLS_PSA_MY_AES_DRIVER_ENABLED */
#if defined(MBEDTLS_PSA_NVT_CIPHER_DRIVER_ENABLED)
        case NVT_CIPHER_TRANSPARENT_DRIVER_ID:
            return( nvt_cipher_transparent_cipher_set_iv(
                        &operation->ctx.nvt_cipher_ctx,
                        iv, iv_length ) );
#endif /* MBEDTLS_PSA_NVT_CIPHER_DRIVER_ENABLED */
    }
...
#if defined(MBEDTLS_PSA_MY_AES_DRIVER_ENABLED)
        case MY_AES_TRANSPARENT_DRIVER_ID:
            return( my_aes_transparent_cipher_update(
                        &operation->ctx.mbedtls_ctx,
                        input, input_length,
                        output, output_size, output_length ) );
#endif /* MBEDTLS_PSA_MY_AES_DRIVER_ENABLED */
#if defined(MBEDTLS_PSA_NVT_CIPHER_DRIVER_ENABLED)
        case NVT_CIPHER_TRANSPARENT_DRIVER_ID:
            return( nvt_cipher_transparent_cipher_update(
                        &operation->ctx.nvt_cipher_ctx,
                        input, input_length,
                        output, output_size, output_length ) );
#endif /* MBEDTLS_PSA_NVT_CIPHER_DRIVER_ENABLED */
...
#if defined(MBEDTLS_PSA_MY_AES_DRIVER_ENABLED)
        case MY_AES_TRANSPARENT_DRIVER_ID:
            return( my_aes_transparent_cipher_finish(
                        &operation->ctx.mbedtls_ctx,
                        output, output_size, output_length ) );
#endif /* MBEDTLS_PSA_MY_AES_DRIVER_ENABLED */
#if defined(MBEDTLS_PSA_NVT_CIPHER_DRIVER_ENABLED)
        case NVT_CIPHER_TRANSPARENT_DRIVER_ID:
            return( nvt_cipher_transparent_cipher_finish(
                        &operation->ctx.nvt_cipher_ctx,
                        output, output_size, output_length ) );
#endif /* MBEDTLS_PSA_NVT_CIPHER_DRIVER_ENABLED */
    }
...
#if defined(MBEDTLS_PSA_MY_AES_DRIVER_ENABLED)
        case MY_AES_TRANSPARENT_DRIVER_ID:
            status = my_aes_transparent_cipher_abort( &operation->ctx.mbedtls_ctx );
            mbedtls_platform_zeroize( &operation->ctx.mbedtls_ctx,
                                      sizeof( operation->ctx.mbedtls_ctx ) );
            return( status );
#endif /* MBEDTLS_PSA_MY_AES_DRIVER_ENABLED */
#if defined(MBEDTLS_PSA_NVT_CIPHER_DRIVER_ENABLED)
        case NVT_CIPHER_TRANSPARENT_DRIVER_ID:
            status = nvt_cipher_transparent_cipher_abort(
                         &operation->ctx.nvt_cipher_ctx );
            mbedtls_platform_zeroize( &operation->ctx.nvt_cipher_ctx,
                                      sizeof( operation->ctx.nvt_cipher_ctx ) );
            return( status );
#endif /* MBEDTLS_PSA_NVT_CIPHER_DRIVER_ENABLED */
    }
...
#if defined(MBEDTLS_PSA_NVT_HASH_DRIVER_ENABLED)
    status = nvt_hash_transparent_hash_compute(
...
#if defined(MBEDTLS_PSA_NVT_HASH_DRIVER_ENABLED)
    status = nvt_hash_transparent_hash_setup(
                &operation->ctx.nvt_hash_ctx, alg );
    if( status == PSA_SUCCESS )
        operation->id = NVT_HASH_TRANSPARENT_DRIVER_ID;
    ...
    psa_hash_operation_t *target_operation )
{
    ...
    {
#if defined(MBEDTLS_PSA_NVT_HASH_DRIVER_ENABLED)
        case NVT_HASH_TRANSPARENT_DRIVER_ID:
            target_operation->id = NVT_HASH_TRANSPARENT_DRIVER_ID;
            return( nvt_hash_transparent_hash_clone(
                        &source_operation->ctx.nvt_hash_ctx,
                        &target_operation->ctx.nvt_hash_ctx ) );
#endif
#if defined(MBEDTLS_PSA_BUILTIN_HASH)
        case PSA_CRYPTO_MBED_TLS_DRIVER_ID:
...
#if defined(MBEDTLS_PSA_NVT_HASH_DRIVER_ENABLED)
        case NVT_HASH_TRANSPARENT_DRIVER_ID:
            return( nvt_hash_transparent_hash_update(
                        &operation->ctx.nvt_hash_ctx,
                        input, input_length ) );
#endif
#if defined(MBEDTLS_PSA_BUILTIN_HASH)
        case PSA_CRYPTO_MBED_TLS_DRIVER_ID:
...
#if defined(MBEDTLS_PSA_NVT_HASH_DRIVER_ENABLED)
        case NVT_HASH_TRANSPARENT_DRIVER_ID:
            return( nvt_hash_transparent_hash_finish(
                        &operation->ctx.nvt_hash_ctx,
                        hash, hash_size, hash_length ) );
#endif
#if defined(MBEDTLS_PSA_BUILTIN_HASH)
...
#if defined(MBEDTLS_PSA_NVT_HASH_DRIVER_ENABLED)
        case NVT_HASH_TRANSPARENT_DRIVER_ID:
            return( nvt_hash_transparent_hash_abort(
                        &operation->ctx.nvt_hash_ctx ) );
#endif
#if defined(MBEDTLS_PSA_BUILTIN_HASH)
        case PSA_CRYPTO_MBED_TLS_DRIVER_ID:
    ...
    uint8_t *ciphertext, size_t ciphertext_size, size_t *ciphertext_length )
{
    ...
    switch( location )
    {
        case PSA_KEY_LOCATION_LOCAL_STORAGE:
...
#endif /* PSA_CRYPTO_DRIVER_TEST */
#if defined(MBEDTLS_PSA_NVT_AEAD_DRIVER_ENABLED)
            status = nvt_aead_transparent_encrypt(
                         attributes, key_buffer, key_buffer_size,
                         alg,
                         nonce, nonce_length,
                         additional_data, additional_data_length,
            ...
            if( status != PSA_ERROR_NOT_SUPPORTED )
                return( status );
#endif /* MBEDTLS_PSA_NVT_AEAD_DRIVER_ENABLED */
#endif /* PSA_CRYPTO_ACCELERATOR_DRIVER_PRESENT */
...
#endif /* PSA_CRYPTO_DRIVER_TEST */
#if defined(MBEDTLS_PSA_NVT_AEAD_DRIVER_ENABLED)
            status = nvt_aead_transparent_decrypt(
                        attributes, key_buffer, key_buffer_size,
                        alg,
                        nonce, nonce_length,
                        additional_data, additional_data_length,
...
#endif /* PSA_CRYPTO_DRIVER_TEST */
#if defined(MBEDTLS_PSA_NVT_AEAD_DRIVER_ENABLED)
            operation->id = NVT_AEAD_TRANSPARENT_DRIVER_ID;
            status = nvt_aead_transparent_encrypt_setup(
                        &operation->ctx.nvt_aead_ctx,
                        attributes, key_buffer, key_buffer_size,
                        alg );
            if( status != PSA_ERROR_NOT_SUPPORTED )
                return( status );
...
#if defined(MBEDTLS_PSA_NVT_AEAD_DRIVER_ENABLED)
            operation->id = NVT_AEAD_TRANSPARENT_DRIVER_ID;
            status = nvt_aead_transparent_decrypt_setup(
                        &operation->ctx.nvt_aead_ctx,
                        attributes,
                        key_buffer, key_buffer_size,
                        alg );
...
#endif /* PSA_CRYPTO_DRIVER_TEST */
#if defined(MBEDTLS_PSA_NVT_AEAD_DRIVER_ENABLED)
        case NVT_AEAD_TRANSPARENT_DRIVER_ID:
            return( nvt_aead_transparent_set_nonce(
                         &operation->ctx.nvt_aead_ctx,
                         nonce, nonce_length ) );
...
#if defined(MBEDTLS_PSA_NVT_AEAD_DRIVER_ENABLED)
        case NVT_AEAD_TRANSPARENT_DRIVER_ID:
            return( nvt_aead_transparent_set_lengths(
                        &operation->ctx.nvt_aead_ctx,
                        ad_length, plaintext_length ) );
...
#if defined(MBEDTLS_PSA_NVT_AEAD_DRIVER_ENABLED)
        case NVT_AEAD_TRANSPARENT_DRIVER_ID:
            return( nvt_aead_transparent_update_ad(
                        &operation->ctx.nvt_aead_ctx,
                        input, input_length ) );
...
#if defined(MBEDTLS_PSA_NVT_AEAD_DRIVER_ENABLED)
        case NVT_AEAD_TRANSPARENT_DRIVER_ID:
            return( nvt_aead_transparent_update(
                        &operation->ctx.nvt_aead_ctx,
                        input, input_length, output, output_size,
                        output_length ) );
...
#if defined(MBEDTLS_PSA_NVT_AEAD_DRIVER_ENABLED)
        case NVT_AEAD_TRANSPARENT_DRIVER_ID:
            return( nvt_aead_transparent_finish(
                        &operation->ctx.nvt_aead_ctx,
                        ciphertext, ciphertext_size,
                        ciphertext_length, tag, tag_size, tag_length ) );
...
#if defined(MBEDTLS_PSA_NVT_AEAD_DRIVER_ENABLED)
        case NVT_AEAD_TRANSPARENT_DRIVER_ID:
            return( nvt_aead_transparent_verify(
                        &operation->ctx.nvt_aead_ctx,
                        plaintext, plaintext_size,
                        plaintext_length, tag, tag_length ) );
...
#if defined(MBEDTLS_PSA_NVT_AEAD_DRIVER_ENABLED)
        case NVT_AEAD_TRANSPARENT_DRIVER_ID:
            return( nvt_aead_transparent_abort(
               &operation->ctx.nvt_aead_ctx ) );
