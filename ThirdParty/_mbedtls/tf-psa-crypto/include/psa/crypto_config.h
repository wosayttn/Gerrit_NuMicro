...
/**
 * \def MBEDTLS_PSA_MY_AES_DRIVER_ENABLED
 *
 * Enable the MY AES transparent driver integrated as a PSA accelerator.
 *
 * This driver is a transparent-mode driver (it operates on plaintext key
 * buffers provided by the caller) and implements AES cipher entrypoints
 * for PSA. To enable this driver, uncomment the following define. When
 * enabled the driver entrypoints' implementation will be compiled and the
 * dispatch layer may select the driver for supported AES operations.
 */
//#define MBEDTLS_PSA_MY_AES_DRIVER_ENABLED
